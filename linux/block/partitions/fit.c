...
int parse_fit_partitions(struct parsed_partitions *state, u64 fit_start_sector, u64 sectors, int *slot, int add_remain)
{
	...
	size_t bytes_left, bytes_to_copy, label_min;
	pgoff_t f_index = 0;

	if (fit_start_sector % (1<<(PAGE_SHIFT - SECTOR_SHIFT)))
		return -ERANGE;

	f_index = fit_start_sector >> (PAGE_SHIFT - SECTOR_SHIFT);
	page = read_mapping_page(mapping, f_index++, NULL);
	if (!page)
		return -ENOMEM;
	...
	printk("FIT: FDT structure size: %u bytes\n", size);
	...
	fdt_for_each_subnode(node, fit, images) {
		...
		printk("FIT: %16s sub-image 0x%08x - 0x%08x '%s' %s%s%s\n",
		...
		if (image_len & ((1 << PAGE_SHIFT)-1)) {
			...
			printk(KERN_ERR "FIT: sub-image %s end not aligned to page boundaries, padding to page boundaries\n", image_name);
			image_len += (PAGE_SIZE - (image_len % PAGE_SIZE));
			//continue;
		}
