...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_XPON_HAL_API_EXT)	
	//set GPIO share scheme reg for gpio mode
	tmp = regRead32(0xbfb00860);
	regWrite32(0xbfb00860, (tmp & (~((1<<19) | (1<<20)))));
	
    /*config GPIO PIN 12 output_mode*/
	...
	unsigned long count, void *data)
{
	...
	if(reset_rc != -1){
		...
		spin_lock_irqsave(&pcie_esd_lock, flags);	
		ret = pcie_reset_handler(reset_rc);
		spin_unlock_irqrestore(&pcie_esd_lock, flags);	
...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_XPON_HAL_API_EXT)
			if(!isFPGA && ! isMT7520 && !isMT7520G && !isMT7520S)
