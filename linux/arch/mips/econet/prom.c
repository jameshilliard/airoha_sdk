...
#ifdef CONFIG_MIPS_TC3262
char pkg_name[128];
#define END_PACKAGE_ID	(0xFFFFFFFF)

enum {
	ECNT_IC_PKG_ID = 0,
	ECNT_IC_PKG_STR_IDX,

	/* all new IDX should be added above this line */
	ECNT_IC_IDX_MAX,
};

typedef struct {
	unsigned int ecnt_ic_pkg_id;
	char *ecnt_ic_pkg_str;
} ECNT_IC_PKG_INFO;

static int get_ecnt_ic_pkg_info(ECNT_IC_PKG_INFO *ic_info)
{
	int i;
	
	ECNT_IC_PKG_INFO ecnt_ic_info[] = {
		/* Add newest package id of IC at here */
		
		/* EN7528 */
		{isEN7528HU,			"EcoNet EN7528HU SOC"},
		{isEN7528DU,			"EcoNet EN7528DU SOC"},
		{isEN7561DU,			"EcoNet EN7561DU SOC"},
		{isEN7561HU,			"EcoNet EN7561HU SOC"},
		{isEN7526FH_EN7528DU,	"EcoNet EN7526FH SOC"},
		{isEN7521G_EN7528DU,	"EcoNet EN7521G SOC"},

		/* EN7580 */
		{isEN7580GT,			"EcoNet EN7580GT SOC"},
		{isEN7580ST,			"EcoNet EN7580ST SOC"},
		{isEN7580GAT,			"EcoNet EN7580GAT SOC"},
		{isEN7565,				"EcoNet EN7565 SOC"},

		/* EN7516 */
		{isEN7516G,				"EcoNet EN7516G SOC"},

		/* EN7527 */
		{isEN7527G,				"EcoNet EN7527G SOC"},
		{isEN7561G,				"EcoNet EN7561G SOC"},
		
		/* EN7512 */
		{isEN7512,				"EcoNet EN7512 SOC"},
		{isEN7513,				"EcoNet EN7513 SOC"},
		{isEN7513G,				"EcoNet EN7513G SOC"},
		
		/* EN7521, EN7521FC */
		{isEN7521FCUD,			"EcoNet EN7521FCUD SOC"},
		{isEN7521F,				"EcoNet EN7521F SOC"},
		{isEN7521S,				"EcoNet EN7521S SOC"},
		{isEN7526D,				"EcoNet EN7526D SOC"},
		{isEN7526F,				"EcoNet EN7526F SOC"},
		{isEN7526G,				"EcoNet EN7526G SOC"},
		{isEN7526FT,			"EcoNet EN7526FT SOC"},
		{isEN7526FP,			"EcoNet EN7526FP SOC"},
		{isEN7526FT_C,			"EcoNet EN7526FT_C SOC"},

		/*must last element*/
	    {END_PACKAGE_ID,		""}
	};

	for(i = 0; ecnt_ic_info[i].ecnt_ic_pkg_id != END_PACKAGE_ID; i++) {
		if(ecnt_ic_info[i].ecnt_ic_pkg_id) {
			memcpy(ic_info, &ecnt_ic_info[i], sizeof(ECNT_IC_PKG_INFO));
			return 1;
		}
	}
...
void init_pkg_name(void)
{
#ifdef CONFIG_MIPS_TC3262
	ECNT_IC_PKG_INFO ic_info;

	if(get_ecnt_ic_pkg_info(&ic_info)) {
		strncpy(pkg_name, ic_info.ecnt_ic_pkg_str, sizeof(pkg_name) - 1);
		io_swap_noneed = 1;
	} else if (isTC3182)
		strncpy(pkg_name, "TrendChip TC3182 SOC", sizeof(pkg_name) - 1);
	else if (isRT65168)
		strncpy(pkg_name, "Ralink RT65168 SOC", sizeof(pkg_name) - 1);
	else if (isRT63165){
		io_swap_noneed = 1;
		strncpy(pkg_name, "Ralink RT63165 SOC", sizeof(pkg_name) - 1);
	} else if (isRT63365) {
		io_swap_noneed = 1;
...
#endif		
		strncpy(pkg_name, "Ralink RT63365 SOC", sizeof(pkg_name) - 1);
	}else if (isMT751020){
		io_swap_noneed = 1;
		strncpy(pkg_name, "Ralink MT751020 SOC", sizeof(pkg_name) - 1);
	}else if (isMT7505){
		io_swap_noneed = 1;
		strncpy(pkg_name, "Ralink MT7505 SOC", sizeof(pkg_name) - 1);
	}
	else
		strncpy(pkg_name, "TrendChip TC3169 SOC", sizeof(pkg_name) - 1);
#else
	if (isRT63260)	
		strncpy(pkg_name, "Ralink RT63260 SOC", sizeof(pkg_name) - 1);
	else if (isTC3162U)
		strncpy(pkg_name, "TrendChip TC3162U SOC", sizeof(pkg_name) - 1);
	else if (isTC3162L5P5)
		strncpy(pkg_name, "TrendChip TC3162L5/P5 SOC", sizeof(pkg_name) - 1);
	else if (isTC3162L4P4)
		strncpy(pkg_name, "TrendChip TC3162L4/P4 SOC", sizeof(pkg_name) - 1);
	else if (isTC3162L3P3)
		strncpy(pkg_name, "TrendChip TC3162L2F/P2F", sizeof(pkg_name) - 1);
	else if (isTC3162L2P2)
		strncpy(pkg_name, "TrendChip TC3162L2/P2", sizeof(pkg_name) - 1);
	else 
		strncpy(pkg_name, "TrendChip TC3162", sizeof(pkg_name) - 1);
#endif
}
...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CF_JOYMEV2_PON)
#define	ROOT_FLAG_ADDR		0x80001fff
...
void fh_prom(void)
{
  ...
  if(rootflagvalue == tmp[0])
#else/*TCSUPPORT_COMPILE*/ 
  char tmp[8] = {0};
  const char rootflagvalue = 1;

  //strcat(arcs_cmdline, "rootfstype=jffs2 ro init=/etc/preinit.sh");
  char *bufaddr = (char*)ROOT_FLAG_ADDR;
  memcpy(tmp,(char*)bufaddr,sizeof(char));
  printk("\n ******************** \n");
  printk("rootflag=%d \n", tmp[0]);
  printk("*bufaddr=%d \n", *bufaddr);
  printk("bufaddr=%x \n", bufaddr);
  printk("ROOT_FLAG_ADDR=%x \n", ROOT_FLAG_ADDR);
  printk("\n ******************** \n");
  if(rootflagvalue == tmp[0])
#endif/*TCSUPPORT_COMPILE*/  	
  {
	  strcat(arcs_cmdline,"root=ubi0:Rootfs rootfstype=ubifs ubi.mtd=6 rw init=/etc/preinit.sh");
	}
	else
	{
	  strcat(arcs_cmdline,"root=ubi0:Rootfs rootfstype=ubifs ubi.mtd=3 rw init=/etc/preinit.sh");
	}
}
#endif/*TCSUPPORT_COMPILE*/
	...
	if(isMT751020){
		...
		if(isFPGA)
			...
			if(VPint(CR_AHB_HWCONF) & (1<<10))
			{
				...
				else if(((VPint(CR_AHB_HWCONF) >> 11) & 0x3) == 0x1)
					memsize = 32 * 0x100000;
				else if(((VPint(CR_AHB_HWCONF) >> 11) & 0x3) == 0x2)
...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CF_JOYMEV2_PON)
static struct timer_list powerled_timer;
int powerLedONFlag = 0;
EXPORT_SYMBOL(powerLedONFlag);

void PowerLed_Timer(void)
{
	...
	{	
		if (isEN7526F)
			VPint(CR_GPIO_DATA) &= ~(1<<2);
		else if(isEN7526G)
			VPint(CR_GPIO_DATA) &= ~(1<<3);
		flag = 0;
	}
	else
	{
	...
	if (powerLedONFlag) 
	{
		...
		mod_timer(&powerled_timer, jiffies + (HZ * 0.25));

}

void PowerLedTimer_init(void)
{
	if(isEN7526F || isEN7526G)
	{
		init_timer(&powerled_timer);
		powerled_timer.expires = jiffies + (HZ * 1);
		powerled_timer.function = PowerLed_Timer;
		powerled_timer.data = 0x0;
		add_timer(&powerled_timer);
	}
