...
#define DUMMY_REG_3 0xf2c //PTPSPARE1 debug 0x1efb d000

#define INT_STATUS 0x810 //TEMPMONINTSTS
#define ptp_RightShift_bits 4	//according to 0x1efbd888 setting

#define EFUSE_THERMAL_VALID_LSB 39
#define EFUSE_BGP_ZERO_MINUS_D1 745
#define EFUSE_GBE_ZERO_MINUS_D1 777
#define EFUSE_CPU_ZERO_MINUS_D1 809

#define ECNT_THERMAL_VERSION "Ver 1.1.240605"
/************************************************************************
*                  M A C R O S
*************************************************************************
*/
...
{
	if (get_phy_efuse(EFUSE_THERMAL_VALID_LSB,1) == 1)
		return get_phy_efuse(EFUSE_BGP_ZERO_MINUS_D1,16);
	return 0;
}
...
{
	if (get_phy_efuse(EFUSE_THERMAL_VALID_LSB,1) == 1)
		return get_phy_efuse(EFUSE_GBE_ZERO_MINUS_D1,16);
	return 0;
}
...
{
	if (get_phy_efuse(EFUSE_THERMAL_VALID_LSB,1) == 1)
		return get_phy_efuse(EFUSE_CPU_ZERO_MINUS_D1,16);
	return 0;
}
...
{	
	return readl(thermal_phy->ptp_base + (DUMMY_REG_3));
}
...
static int thermal_phy_drv_probe(struct platform_device *pdev)
{
	...
	printk("7583 Thermal PHY probe Start %s\n",ECNT_THERMAL_VERSION);
	...
	printk("7583 Thermal Sensor address probe done\n");
