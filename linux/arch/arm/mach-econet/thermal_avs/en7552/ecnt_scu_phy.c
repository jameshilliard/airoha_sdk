...
//TADC_CPU
#define MUX_TADC_CPU       0x2f4
#define DOUT_TADC_CPU      0x2fc
...
static u16 ADC_MUX_SEL_RES0_DIO1_BJT2 = 0;
...
void thermal_TADC_mode(u16 tadc0_tcpu1,u16 res0_diode1_BJT2,u16 delayCnt);
...
void thermal_init(void)
{
  u32 temp=0;
  ADC_SEL_AVS0_CPU1 = 1;
  ADC_MUX_SEL_RES0_DIO1_BJT2 = 0;
  printk("thermal SCU version: 7552.2.20231016\n");
  ...
  //set TADC mode
  rw_chip_scu_data(MUX_TADC,0x33BE,TADC_SET_MASK,0);

  //TADC_CPU
  rw_chip_scu_data(MUX_TADC_CPU,0,1,TADC_RST_OFST);
  rw_chip_scu_data(MUX_TADC_CPU,1,1,TADC_RST_OFST);
  //set TADC mode
  //default diode,0x33fe for diode
  rw_chip_scu_data(MUX_TADC_CPU,0x33BE,TADC_SET_MASK,0);
...
{   
  ...
  if(res0_diode1_BJT2==1)
    rw_chip_scu_data(addr,0x7,TADC_MUX_MASK,TADC_MUX_OFST);
  else if(res0_diode1_BJT2==2)
    rw_chip_scu_data(addr,0x6,TADC_MUX_MASK,TADC_MUX_OFST);
  else
    rw_chip_scu_data(addr,0x0,TADC_MUX_MASK,TADC_MUX_OFST);
  ADC_SEL_AVS0_CPU1 = tadc0_tcpu1;
  ADC_MUX_SEL_RES0_DIO1_BJT2 = res0_diode1_BJT2;
...
u32 GetTADC_Sel(void) //H: ADC_SEL_AVS0_CPU1 // L:ADC_MUX_SEL_RES0_DIO1_BJT2
{
	u32 ret = ADC_SEL_AVS0_CPU1;
	
	return (ret<<16)+ADC_MUX_SEL_RES0_DIO1_BJT2;
}
