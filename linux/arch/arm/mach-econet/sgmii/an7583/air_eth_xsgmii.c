...
static void usxgmii_init(void)
{
	...
	if(dbg_print) printk("1fa74100 %x\n",RG_R_PL(_pxSGMII->multi_sgmii_base,USXGMII_MULTI_SGMII_BASE_OFFSET, (RgAddr)0x100));

	/******************************** AN7583 Add *********************************************/
    //5C20
	rg_usxgmii_an_control_7.dat.value = RG_R_PL(_pxSGMII->pcs1_base,USXGMII_PCS1_BASE_OFFSET,(RgAddr) _rg_usxgmii_an_control_7);
	rg_usxgmii_an_control_7.hal.rg_xfi_mode_tx_sel = 1;
    rg_usxgmii_an_control_7.hal.rg_xfi_mode_rx_sel = 1;
	RG_W_PL(_pxSGMII->pcs1_base,USXGMII_PCS1_BASE_OFFSET,(RgAddr) _rg_usxgmii_an_control_7,rg_usxgmii_an_control_7.dat.value); 
    if(dbg_print) printk("0x1fa75%x 0x%x\n",_rg_usxgmii_an_control_7,rg_usxgmii_an_control_7.dat.value);
	
	rg.value = 0;
    //Check AN Speed
    ETH_usxgmii_an_control_6_toggle();  //toggle to trigger
    ro_usxgmii_an_stats_0.dat.value = RG_R_PL(_pxSGMII->pcs1_base,USXGMII_PCS1_BASE_OFFSET,(RgAddr) _ro_usxgmii_an_stats_0);
    if(dbg_print)
	{	
		printk("0x1fa75%x  0x%x\n",_ro_usxgmii_an_stats_0,ro_usxgmii_an_stats_0.dat.value);
		printk("ro_cur_usxgmii_mode = %x\n", ro_usxgmii_an_stats_0.hal.ro_cur_usxgmii_mode);
	}
    if((ro_usxgmii_an_stats_0.hal.ro_cur_usxgmii_mode == 1) || (ro_usxgmii_an_stats_0.hal.ro_cur_usxgmii_mode == 3))//Upper 5G, Lower 1G
    {
        rg.value = RG_R_PL(_pxSGMII->multi_sgmii_base,USXGMII_MULTI_SGMII_BASE_OFFSET,(RgAddr)FPGA_STS_CTRL); 
        rg.bit.b28 = 0;
        rg.bit.b24 = 0;
		if(dbg_print) printk("0x1fa40%x  0x%x\n",FPGA_STS_CTRL,rg.value);
        RG_W_PL(_pxSGMII->multi_sgmii_base,USXGMII_MULTI_SGMII_BASE_OFFSET,(RgAddr)FPGA_STS_CTRL,rg.value);                  
    }
    else if((ro_usxgmii_an_stats_0.hal.ro_cur_usxgmii_mode == 2) || (ro_usxgmii_an_stats_0.hal.ro_cur_usxgmii_mode == 4))//Upper2.5G, Lower 100M
    {
        rg.value = RG_R_PL(_pxSGMII->multi_sgmii_base,USXGMII_MULTI_SGMII_BASE_OFFSET,(RgAddr)FPGA_STS_CTRL); 
        rg.bit.b28 = 1;
        rg.bit.b24 = 0;
...
static void xsgmii_force_data(u8 xsgmii,u8 mod,u8 an, u8 rate){
	...
	switch(xsgmii){
		case USXGMII:
			switch(rate){
				case 0:
					if(dbg_print)printk("USXGMII_force_data\n");
					rg_user_define_sel.dat.value = RG_R_PL(_pxSGMII->pcs1_base,USXGMII_PCS1_BASE_OFFSET,(RgAddr)_rg_user_define_sel);
					if(an) rg_user_define_sel.hal.rg_user_define_sel = 0b1;
					else rg_user_define_sel.hal.rg_user_define_sel = 0b0;
					...
					if(rg_user_define_xgmii_data_lsb.hal.rg_user_define_rxd_lsb != 0xfd){
						rg_user_define_xgmii_data_lsb.hal.rg_user_define_rxd_lsb = 0xfd;
						...
						rg_user_define_xgmii_control.hal.rg_user_define_rxc = 0b1;				
					...
					break;
				case 1:					
					if(dbg_print)printk("SXGMII_force_data\n");
					rg_user_define_sel.dat.value = RG_R_PL(_pxSGMII->pcs1_base,USXGMII_PCS1_BASE_OFFSET,(RgAddr)_rg_user_define_sel);
					if(an) rg_user_define_sel.hal.rg_user_define_sel = 0b1;
					else rg_user_define_sel.hal.rg_user_define_sel = 0b0;
					RG_W_PL(_pxSGMII->pcs1_base,USXGMII_PCS1_BASE_OFFSET,(RgAddr)_rg_user_define_sel,rg_user_define_sel.dat.value);
...
static irqreturn_t xsgmii_interrupt(int irq, void *dev_id){
	if(dbg_print) printk("eth xsgmii:%x, irq=%d\n",xsgmii_bk,irq);
	switch(xsgmii_bk){
		case SGMII :
			sgmii_isr();
			break;	
...
static void xsgmii_interrupt_init(u8 en){
	rg_type_t(HAL_rg_hsgmii_mode_interrupt) rg_hsgmii_mode_interrupt;		
	...
	if(dbg_print)printk("xsgmii: int_init enable = %x\n",en);
	if(_pxSGMII->irq < 0){
		printk("ETH IRQ NG\n");
		return _pxSGMII->irq;
	}	
	if(USXGMII == xsgmii_bk){
		printk("USXGMII DISABLE INT\n");
		return _pxSGMII->irq;	
	}
	rg_hsgmii_mode_interrupt.dat.value = RG_R_PL(_pxSGMII->pcs2_base,USXGMII_PCS2_BASE_OFFSET,(RgAddr) _rg_hsgmii_mode_interrupt);
...
u32 ETH_xSGMII_API(u8 xsgmii,u8 mod,u8 an, u8 rate){
	u32 data = 0x00;
	if(dbg_print) printk("ETH_xSGMII_API xsgmii=%x,mod = %x,an=%x,rate=%x\n",xsgmii,mod,an,rate);
	...
	if (mod < 2)
	{
		...
		xsgmii_init(xsgmii);
		//xsgmii_interrupt_init(XSGMII_INT_EN);
	}	
	...
	switch(mod){
		...
		case 3 ://update
			...
			if(dbg_print)printk("sgmii_link_sts cur mod %x,value %x\n",an,data);
			break;
		case 4 ://update
			...
			if(dbg_print)printk("sgmii_link_sts sel %x,value %x\n",an,data);
			break;
		case 5: //update
			xSGMII_PD(an);
			break;
