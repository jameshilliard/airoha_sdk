...
static void usxgmii_init(u8 Lane){
	...
	//for (Lane=0;Lane<2;Lane++)
	{
		...
		if(dbg_print) printk("1fc74100 %x\n",RG_R_PL(_pxSGMII->multi_sgmii_base[Lane],USXGMII_MULTI_SGMII_BASE_OFFSET, (RgAddr)msg_rx_ctrl_0));
	}
...
{
	RG_W_PL(_pxSGMII->ra_base[0],USXGMII_RA_BASE_OFFSET,(RgAddr) _rg_rate_adapt_ctrl_11,0x1);
	RG_W_PL(_pxSGMII->ra_base[0],USXGMII_RA_BASE_OFFSET,(RgAddr) _rg_rate_adapt_ctrl_11,0x0);
...
u32 PCIE0_xSGMII_API(u8 xsgmii,u8 mod,u8 an, u8 rate){	
	u8 Lane = 0;
	u32 data = 0;
	if(dbg_print) printk("PCIE0_xSGMII_API xsgmii=%x,mod = %x,an=%x,rate=%x\n",xsgmii,mod,an,rate);		
