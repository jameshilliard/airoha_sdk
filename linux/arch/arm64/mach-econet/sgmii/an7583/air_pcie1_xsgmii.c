...
#include "H/air_pcie1_xsgmii_config.h"
...
EXPORT_SYMBOL(PCIE1_xSGMII_Ver);
...
void PCIE1_xSGMII_Disconnect(u8 mod)
{
  ...
  if(dbg_print) printk("sgmii_p1 is %s\n",mod==1?"disconnect":"connect");
  #endif
}
...
static void hsgmii_init(void){
  ...
  SET_WAN_CONF((GET_WAN_CONF()&(~0xff))|0x11);
  SET_NP_SCU_SERDES_WIFI0_SEL(1);
  
  #ifdef FPGA_PLATFORM
  rg_type_t(HAL_FPGA_STS_CTRL) rg_FPGA_STS_CTRL;
  rg_FPGA_STS_CTRL.dat.value = RG_R_PL(_pxSGMII->multi_sgmii_base,SGMII_MULTI_SGMII_BASE_OFFSET,(RgAddr)FPGA_STS_CTRL);
  rg_FPGA_STS_CTRL.dat.bit.b0=1;
  RG_W_PL(_pxSGMII->multi_sgmii_base,SGMII_MULTI_SGMII_BASE_OFFSET,(RgAddr)FPGA_STS_CTRL,rg_FPGA_STS_CTRL.dat.value);
  rg_FPGA_STS_CTRL.dat.bit.b0=0;
  RG_W_PL(_pxSGMII->multi_sgmii_base,SGMII_MULTI_SGMII_BASE_OFFSET,(RgAddr)FPGA_STS_CTRL,rg_FPGA_STS_CTRL.dat.value);  
  #endif
...
static void sgmii_init(void){
  ...
  SET_WAN_CONF((GET_WAN_CONF()&(~0xff))|0x10);
  SET_NP_SCU_SERDES_WIFI0_SEL(1);
...
static void HGMII_2p5G(void){
  ...
  /*
  rg_type_t(HAL_RATE_ADP_P0_CTRL_0) ra_p0_ctl_0;
  ra_p0_ctl_0.dat.value = RG_R_PL(_pxSGMII->ra_base,SGMII_RA_BASE_OFFSET,(RgAddr)_rg_rate_adapt_ctrl_0);
  ra_p0_ctl_0.hal.rg_p0_dis_mii_mode = 0
  RG_W_PL(_pxSGMII->ra_base,SGMII_RA_BASE_OFFSET,(RgAddr)_rg_rate_adapt_ctrl_0,ra_p0_ctl_0.dat.value);
  */
  ...
  //john add---phy_reset
	...
	//sgmii_reg_an0.hal.sgmii_an_enable = 0;
	RG_W_PL(_pxSGMII->an_base,SGMII_AN_BASE_OFFSET,(RgAddr) _sgmii_reg_an0,sgmii_reg_an0.dat.value);
  //john add^^^
...
void xSGMII_Dbg_API(u8 xsgmii,u8 mod,u8 an, u8 rate)
{
  switch(an)
  {
    case 0:
...
static irqreturn_t xsgmii_interrupt(int irq, void *dev_id){
	...
	if(dbg_print) {
		printk("pcie1 xsgmii: an_int called\n");
		printk("pcie1 xsgmii: irq=%d\n",irq);
	}	
...
u32 PCIE1_xSGMII_API(u8 xsgmii,u8 mod,u8 an, u8 rate){		
	u32 data=0;
	if(dbg_print) printk("PCIE1_xSGMII_API xsgmii=%x,mod = %x,an=%x,rate=%x\n",xsgmii,mod,an,rate);
...
static int xsgmii_drv_probe(struct platform_device *pdev)
{
	...
	#if 1
	//printk("pcie1_xsgmii->pcs1_base= %lx\n", (unsigned long)_pxSGMII->pcs1_base);
	printk("pcie1_xsgmii->pcs2_base= %pS\n", _pxSGMII->pcs2_base);
	printk("pcie1_xsgmii->an_base=   %pS\n", _pxSGMII->an_base);
	printk("pcie1_xsgmii->ra_base=   %pS\n", _pxSGMII->ra_base);
	printk("pcie1_xsgmii->phya_base=   %pS\n", _pxSGMII->phya_base);
	printk("pcie1_xsgmii->multi_sgmii_base=   %pS\n", _pxSGMII->multi_sgmii_base);
	#endif


	if(_pxSGMII == NULL)printk("PCIE1 xsgmii drv probe ng\n");
