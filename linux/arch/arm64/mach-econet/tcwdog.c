...
/* 
	timercnt = WDOG_RELOAD_VAL *  WDOG_TIMETICK_1S * SYS_HCLK * 500
	7581's SYS_HCLK(300) is too high that would cause timer cnt overflow
	thus reduce the WDOG_RELOAD_VAL to aoivd overflow issue. 
*/
#if defined(TCSUPPORT_CPU_EN7581) || defined(TCSUPPORT_CPU_AN7552)
#define WDOG_RELOAD_VAL     28 
#else
...
static int cpu_wg_ignore[CPU_NUM] = {0};
...
int ignore_cpu_wg(int cpuindex,int val)
{
	if (cpuindex >= CPU_NUM) {
        	printk("ignore_cpu_wg parameter error. cpuindex = %d > CPU_NUM\n", cpuindex);
		return -1;
        }

	cpu_wg_ignore[cpuindex] = val;

	return 0;
}
EXPORT_SYMBOL(ignore_cpu_wg);
...
static ssize_t wdog_write(struct file *file, const char *data, size_t len, loff_t *ppos)
{
	...
	/* ignore the cpu1 watch dog time out when we disable cpu1 */
	for (i=0 ; i< CPU_NUM; i++)
		if ((dead_cpu== i) && cpu_wg_ignore[i]) {
			return len;
    }
...
static int wdogs_enable_write_proc(struct file *file, const char *buffer,
	...
	if(isFPGA && isEN7581)
	{
		if(cpu_nr == 1)
		{
			sscanf(val_string, "%d", &wdogCnts[0].wdog_enabled);
		}
		else if(cpu_nr == 2) 
		{
			sscanf(val_string, "%d %d", &wdogCnts[0].wdog_enabled, &wdogCnts[1].wdog_enabled);
		}
		else if(cpu_nr == 4) 
		{
 	...
 	else if (cpu_nr==2)	/*7523 has only two arm CPUs*/
    	sscanf(val_string, "%d %d", &wdogCnts[0].wdog_enabled, &wdogCnts[1].wdog_enabled) ;
    else {
...
void ecnt_wdt_isr(void)
{
	...
	syscore_shutdown();	
#ifdef TCSUPPORT_CPU_AN7583
#define DBG_7583_BOOT_MAGIC 0x3 /*same  in en7523_def.h in bootloader*/
	tmp = GET_DRAMC_CONF();
	tmp &= ~(1<<2);
	SET_DRAMC_CONF(tmp);
	SET_SCREG_WF0(GET_SCREG_WF0()|DBG_7583_BOOT_MAGIC);
#endif
	BUG();
}
