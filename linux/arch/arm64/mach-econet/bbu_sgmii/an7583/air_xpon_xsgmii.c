...
static int spd_sel_bk = Sync_XEPON;
static u8 xPON_BASER_Mode = 0;
...
extern int get_pon_phy_irq(void);
...
static u8 xsgmii_chg(u8 xsgmii){
	...
	//if((xsgmii_bk = 0xff)||(xsgmii != xsgmii_bk))
	{
        ...
        switch (xsgmii_bk)
        {
            case USXGMII:   spd_sel_bk = xPON_BASER_Mode? Eth_Ser_5GBaseR :  Sync_XEPON;break;
            case HSGMII:    spd_sel_bk = Eth_Ser_HSGMII;break;
            case SGMII:     spd_sel_bk = Sync_EPON_1;break;
            default : spd_sel_bk = Sync_XEPON;break;
        } 
...
/* //AN7583 Remove by DE suggestion
	RGDATA_t rg={0};
...
static void xPON_BASER_RA_Setting(u8 enable)
{
...
void XPON_usxgmii_an_control_6_toggle(void)
{
    if(dbg_print) printk("0x1fa85c1c toggle\n");
...
static u32 xSGMII_Link_STS(u8 xsgmii){
	...
	switch (xsgmii){
		case USXGMII :
            ...
            {
               sts.bit.b0 = XFI_PON_RX_SigDet_OUT_Read();
...
static u8 XFI_PON_RX_SigDet_Flag_Out(void)
{
	u8 i,cnt = 0;		
	for (i=0;i<=5;i++)
    {            
		cnt += XFI_PON_RX_SigDet_OUT_Read();
	}
	if(dbg_print)printk("RX_SigDet_Flag, cnt %x\n",cnt);
...
static void TMR_SGMII_LINKDN_CB(void){
	...
	link_status = XFI_PON_RX_SigDet_Flag_Out()& rg_hsgmii_pcs_state_2.hal.ro_rx_sync & rg_msg_rx_sts_15.hal.ro_lnk_sts_p0;
    dbg_print = dbg_print_temp;
    //if(aux.bit.b29) printk("PON PMA Sig = %d, ro_rx_sync = %d, ro_lnk_sts_p0 = %d\n",XFI_PON_RX_SigDet_Flag_Out(),rg_hsgmii_pcs_state_2.hal.ro_rx_sync,rg_msg_rx_sts_15.hal.ro_lnk_sts_p0);
	if(!link_status && !TMR_sgmii_link_dn_cb){
		...
		#if 1//ndef TEST
		//RX_RDY(0,0);
		XFI_PON_RX_CDR_LFP_L2D(1,0);			
		XFI_PON_SigDet_Int_Init(1);		
		#endif
		if(dbg_print)printk("---------------TMR_SGMII_LINKDN_CB &&& ---------------\n");
	}
...
static u8 SigDet_isr(void){	
	rg_type_t(HAL_XPON_INT_STA_3) XPON_INT_STA_3;		
	u8 sd_int = 0;
	if(XFI_PON_SigDet_IntEn_sta()){
		XPON_INT_STA_3.dat.value = XFI_PON_SigDet_Int_sta3_read();	
		...
		{			
			...
			if(XFI_PON_RX_CDR_LFP_L2D_sta()== 1) XFI_PON_RX_CDR_RST();
			if(XFI_PON_RX_RDY_Sta()== 0) XFI_PON_RX_RDY(1,0);
			XFI_PON_SigDet_Int_Init(0);
			if(dbg_print)printk("---------------rx_sigdet_int isr &&&---------------\n");				
		}
		
		XFI_PON_SigDet_Int_sta3_write(XPON_INT_STA_3.dat.value);
		if(dbg_print){
            XPON_INT_STA_3.dat.value = XFI_PON_SigDet_Int_sta3_read(); 
...
static void xSGMII_Wait_LSSignaling(u8 xsgmii){
    ...
    sync =(u16) RG_R_PL(_pxSGMII->pcs1_base,USXGMII_PCS1_BASE_OFFSET,(RgAddr) _ro_base_r_10gb_t_pcs_stus1);
   	signal = XFI_PON_RX_SigDet_OUT_Read();//XSGMII_SigDet_A_EN? XFI_ETH_RX_SigDet_Flag(): RX_SigDet_Flag_D();	
...
void XFI_PON_RX_CDR_RST(void)
{
#ifndef FPGA_PLATFORM
#if AN7583_COMMON_PHYA == 1

	XFI_PON_RX_CDR_LFP_L2D(1,0);
	XFI_PON_RX_CDR_LPF_RSTB(1,0);
	udelay(700);
	XFI_PON_RX_CDR_LPF_RSTB(1,1);
	udelay(100);
	XFI_PON_RX_CDR_LFP_L2D(1,1);
	
	//switch to auto
	XFI_PON_RX_CDR_LPF_RSTB(0,1);
	XFI_PON_RX_CDR_LFP_L2D(0,1);
#endif
#endif
...
{	
	...
	rg_type_t(HAL_XPON_INT_STA_3) XPON_INT_STA_3;		
#if AN7583_COMMON_PHYA == 1
	u8 signal = XSGMII_SigDet_A_EN? XFI_PON_RX_SigDet_Flag(): XFI_PON_RX_SigDet_Flag_D();
...
//Linkup int	
	...
	if(rg_xfi_pcs_int_sta_3.hal.link_up_st_int)	{
        ...
        for(i=0;i<100;i++)
        {
            sig_det_out = XFI_PON_RX_SigDet_OUT_Read();
...
static void xsgmii_interrupt_init(u8 xsgmii){
	...
	switch (xsgmii)
    {
        ...
        case HSGMII:
            en = H_SGMII_HOTPLUG;
            if(dbg_print) printk("PON HSGMII_Interrupt_Init %s\n",en? "Enable":"Disable");
            if(dbg_print) printk("PON TMR_ON = %d, H_SGMII_HOTPLUG = %d\n",TMR_ON,H_SGMII_HOTPLUG);
            if(!TMR_ON && H_SGMII_HOTPLUG)TMR_INI();
            break;
        case SGMII:     
            en = XSGMII_SGMII_INT_EN;
            if(dbg_print)printk("PON SGMII_Interrupt_Init %s\n",en? "Enable" : "Disable");
...
#endif
static void xPON_Int_Status(u8 sta_num)
{
...
void xPON_xSGMII_Dbg_Status(u8 xsgmii)
{
...
void xPON_xSGMII_Dbg_RA_Cnt(void)
{
    rg_type_t(HAL_rg_rate_adapt_ctrl_11)ra_ctl_11;
	ra_ctl_11.dat.value = RG_R_PL(_pxSGMII->ra_base,USXGMII_RA_BASE_OFFSET,(RgAddr)_rg_rate_adapt_ctrl_11);	
...
void xPON_xSGMII_Dbg_RA_Cnt_Clr(void)
{
	RG_W_PL(_pxSGMII->ra_base,USXGMII_RA_BASE_OFFSET,(RgAddr) _rg_rate_adapt_ctrl_2,0x3333);
    udelay(200);
	RG_W_PL(_pxSGMII->ra_base,USXGMII_RA_BASE_OFFSET,(RgAddr) _rg_rate_adapt_ctrl_2,0x0);
...
static void xPON_xSGMII_Dbg_PCS_Cnt(void)
{
...
void xPON_xSGMII_Dbg(u8 xsgmii,u8 mod,u8 an, u8 rate)
{
	switch(an)
	{
		case 0:
		...
		case 3:
            if(rate == 2)
            {
                ...
                xPON_xSGMII_Dbg_Status(xsgmii);
                xPON_xSGMII_Dbg_RA_Cnt();
                xPON_xSGMII_Dbg_RA_Cnt_Clr();
                if(xsgmii != USXGMII)
                {
                    xPON_xSGMII_Dbg_PCS_Cnt();
                }
			...
			break;
		case 4:
			xPON_Int_Status(rate);
			break;
        case 5 : 
            printk("Pon_EO_Scan, spd_sel_bk = %x\n\n",spd_sel_bk);
            Pon_EO_Scan(spd_sel_bk,0,7,rate);
            break;
		case 255://no use
...
u32 XPON_xSGMII_API(u8 xsgmii,u8 mod,u8 an, u8 rate){	
	...
	if(MAC_Polling_Print_EN) printk("ETH_xSGMII_API xsgmii=%x,mod = %x,an=%x,rate=%x\n",xsgmii,mod,an,rate);
        ...
        if((xsgmii == USXGMII)&&(mod==0)&&(an==0)&&(rate==1))
        {
            xPON_BASER_Mode = 1;
            rate = 0;
            printk("5G-BASER Mode Enable, rate=%d\n",rate);
		...
		if (mod < 2)
		{
            ...
            switch (xsgmii)
            {
                case USXGMII:   spd_sel = xPON_BASER_Mode? Eth_Ser_5GBaseR :  Sync_XEPON;break;
                case HSGMII:    spd_sel = Eth_Ser_HSGMII;break;
                case SGMII:     spd_sel = Sync_EPON_1;break;
                default : spd_sel = Sync_XEPON;break;
            } 
            printk("PON %s %s Mode PHY Init\n",(xsgmii==2)?"1G":(xsgmii? "2.5G":"10G"),(mod==1)?"AN":((an==1)? "Force AN":"Force"));
            xpon_init (spd_sel);
            fiber_plug_reset (PON_XFI_FIRST_PLUG_IN , spd_sel);
    ...
    #endif
#endif   
            XFI_PON_PCS_Reset_Release(spd_sel);
            xSGMII_Solution(0,xsgmii,mod,an,rate);//10.0.12
			xsgmii_init(xsgmii);
#ifndef FPGA_PLATFORM
#ifdef Enable_IRQ      
	...
	switch(mod){
		case 0: 
			...
			if((xsgmii == USXGMII) && (xPON_BASER_Mode == 0))
                usxgmii_pcs_an_ctrl7(an,rate); //update 5C20
            else if(xPON_BASER_Mode==1)
                xPON_BASER_RA_Setting(1);
        ...
        case 8:
            ...
            break;
        case 9:         
            XFI_PON_TXFIR_Set(0,an,rate,0);
            break;
        case 10:    
            XFI_PON_TXFIR_Set(1,an,rate,0);
            break;
            
        case 247 :
            xpon_linkup_wrapper_delay_time = rate * 20;
            printk("PON Total Delay Time = %d msec\n",rate*20);
        case 248 :
            if(rate == 0)
            {
                XSI_MAC_LOGIC_RESET(XPON_INDEX);
                ...
                if(USX_FORCE_USR_DATA) xsgmii_force_data(0,8,0,0);
                xpon_linkup_wrapper_wa_flag = 0;
                if(dbg_print) printk("PON Linkup Wrapper Finished\n");
            }
            break;

        case 249 :
            phy_eyescan_test(spd_sel_bk, an, rate);  
            break;        
        case 250:    
            XFI_PON_TXFIR_Get();
            break;        
        case 251:    
            XFI_PON_TXFIR_Bringup_Setting(0,an,rate,0,dbg_print);
            break;            
        case 252:    
            XFI_PON_TXFIR_Bringup_Setting(1,an,rate,0,dbg_print);
            break;
        ...
        case 253:
            ...
            break;
        case 254:
            XPON_xSGMII_Disconnect(an,xsgmii);
            break; 

        case 255:
            if(dbg_print) printk("xSGMII_Dbg %x,%x,%x,%x\n",xsgmii, mod, an, rate);
            xPON_xSGMII_Dbg( xsgmii, mod, an, rate);
            break;
...
static void xsgmii_int_init(void)
{
	...
	RG_W_PL(_pxSGMII->pcs1_base,USXGMII_PCS1_BASE_OFFSET,(RgAddr) _rg_xfi_pcs_int_ctrl_3,rg_xfi_pcs_int_ctrl_3.dat.value);
#endif
	if(dbg_print) printk("xsgmii_int_init\n");
}
...
static int xsgmii_drv_probe(struct platform_device *pdev)
{
	...
	printk("PON Serdes xsgmii_get_base \n");//AN7583 move to here to avoid COMMON PHYA block
	_pxSGMII = _pxSGMII_Tmp;	
#if AN7583_COMMON_PHYA == 0
	xsgmii_get_base(_pxSGMII_Tmp);
#else
    //Force to get multi_sgmii_base while in AN7583 common phya mode
    _pxSGMII->multi_sgmii_base = Get_Base(0x1fa84000);
    if (_pxSGMII->multi_sgmii_base == 0) _pxSGMII->multi_sgmii_base = NULL;
    //printk("XPON Serdes Multi SGMII Get Base = %x\n",0x1fa84000);
#endif	
