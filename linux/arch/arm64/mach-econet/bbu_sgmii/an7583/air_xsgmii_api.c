...
u8 dbg_print = 0;
u8 interrupt_fw_en = 0;
EXPORT_SYMBOL(dbg_print);
RGDATA_t api_aux ={(0b11111111<<24)|(0b11111111<<16)|(0b11111111<<8)|(0b11111100<<0)};
EXPORT_SYMBOL(api_aux);

u8 eth_linkup_wrapper_wa_flag = 0;
u16 eth_linkup_wrapper_delay_time = 2000;
u8 xpon_linkup_wrapper_wa_flag = 0;
u16 xpon_linkup_wrapper_delay_time = 2000;

EXPORT_SYMBOL(eth_linkup_wrapper_wa_flag);
EXPORT_SYMBOL(eth_linkup_wrapper_delay_time);
EXPORT_SYMBOL(xpon_linkup_wrapper_wa_flag);
EXPORT_SYMBOL(xpon_linkup_wrapper_delay_time);

 
u32 xsgmii_api(u8 serdes,u8 xsgmii,u8 mod,u8 rate,u8 an){
	u32 status = 0x00;
	if(api_aux.bit.b0 && (mod!=3) && (mod!=4)) printk("xsgmii_api: serdes %x,xsgmii %x,mod %x,rate %x,an %x\n",serdes, xsgmii, mod, rate, an);
	switch(serdes){
		case 0 :
			switch(xsgmii){
				case 1:
				...
				case 7:					
					...
					break;
                case 8 :
 					interrupt_fw_en = mod; //AN7583 New Add - Andy240408
					break;                   
				default:
					status = 0xff;
					ETH_xSGMII_Ver();
