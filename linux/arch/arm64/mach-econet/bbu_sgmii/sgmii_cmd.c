...
static int sgmii_api_pcie1_force_sgmii(void);
static int sgmii_api_pcie0_force_sgmii(void);
static int sgmii_api_usb0_force_sgmii(void);
static int sgmii_api_pon0_force_sgmii(void);
...
static int sgmii_sel_flag(sgmii_port_type port)
{
	...
	case SGMII_PORT_PCIE0:
		ret = get_serdes_interface_sel(ECNT_EVENT_PCIE0_HSGMII);
		break;
	case SGMII_PORT_PCIE1:
		ret = get_serdes_interface_sel(ECNT_EVENT_PCIE1_HSGMII);
		break;
	case SGMII_PORT_USB0:
		ret = get_serdes_interface_sel(ECNT_EVENT_USB3_HSGMII);
		break;
	case SGMII_PORT_PON0:
		ret = get_serdes_interface_sel(ECNT_EVENT_PON_HSGMII);
			break;
#endif
...
int sgmii_api_force_mode(sgmii_port_type port, sgmii_speed_type speed)
{
	...
	if (speed != SGMII_SPEED_2500M)
	{
		...
		{
			case SGMII_PORT_PCIE0:
				sgmii_api_pcie0_force_sgmii();
				break;
			case SGMII_PORT_PCIE1:
				sgmii_api_pcie1_force_sgmii();
				break;
			case SGMII_PORT_USB0:
				sgmii_api_usb0_force_sgmii();
				break;
			case SGMII_PORT_PON0:
				sgmii_api_pon0_force_sgmii();
				break;
			default: //unknow format
				return 0;
				break;	
		}
		if(speed != SGMII_SPEED_1000M)
		{
...
int sgmii_api_get_info(sgmii_port_type port, int *argc, int *argv[])
{
	...
	//unknow format, now only hsgmii = SGMII_SPEED_2500M = 0
		...
		/* ToDO */
#else
	if(port >= SGMII_PORT_UNKNOW || port <0 || *argc == NULL || *argc <2 || check_sel !=1 )
	{	
		return 0;
	}
#endif
