...
#define NSRAM_TEST_SIZE			0x60000 // 512K
#define NSRAM_TEST_SIZE_HALF	0x30000//256K

#define DRAM_SIZE	0x80000000
#define DRAM_PHYS	0x80000000
...
#ifdef TCSUPPORT_CPU_ARMV8_64
unsigned long high_tx_des0 = 0;
unsigned long high_rx_des0 = 0;

static unsigned long stress_test_pattern = 0x5a5a5a5a5a5a5a5a;
#endif
...
{
	int i = 0;
	printk("***compare src (0x%px) des(0x%px) len(0x%x)***\n",
						src_array, dst_array, len);
	for(i = 0; i < len; i++)
	{
		if(src_array[i] != dst_array[i])
		{
			printk("0x%px(0x%x) != 0x%px(0x%x)\n",
...
int compare_num(u8 * src_array,u8 num,u32 len)
{
	int i = 0;
	printk("***compare src (0x%lx) des(0xFF) len(0x%x)***\n",src_array, len);
	for(i = 0; i < len; i++)
	{
		if(src_array[i] != num)
		{
			printk("0x%lx(0x%x) != 0xFF\n",
								src_array, src_array[i]);
			return (i + 1);
		}
	}
		...
		dma_addr_t src_addr, dma_addr_t dst_addr, u32 NLS_bit, int swap_flag, int i)
{
	...
	src_addr_high = (src_addr & 0x300000000) >> 32 ;
	dst_addr_high = (dst_addr & 0x300000000) >> 32 ;

//	printk ("src_addr_high = %x dst_addr_high = %x",src_addr_high,dst_addr_high);
	if (*testLen > hsdma_pkt_len)
	{
		t_desc2 |= ECNT_HSDMA_DESC_PLEN(hsdma_pkt_len);
		t_desc2 |= ECNT_HSDMA_DESC_PKT_PTR_H(src_addr_high);
		r_desc2 |= ECNT_HSDMA_DESC_PLEN(hsdma_pkt_len);
		r_desc2 |= ECNT_HSDMA_DESC_PKT_PTR_H(dst_addr_high);

		*testLen -= hsdma_pkt_len;
	}
	else
	{
		t_desc2 |= ECNT_HSDMA_DESC_PLEN(*testLen);
		t_desc2 |= ECNT_HSDMA_DESC_PKT_PTR_H(src_addr_high);
		r_desc2 |= ECNT_HSDMA_DESC_PLEN(*testLen);
		r_desc2 |= ECNT_HSDMA_DESC_PKT_PTR_H(dst_addr_high);

		*testLen = 0;
	}
//	printk("ECNT_HSDMA_DESC_PKT_PTR_H s = %x ECNT_HSDMA_DESC_PKT_PTR_H d = %x", ECNT_HSDMA_DESC_PKT_PTR_H(src_addr_high), ECNT_HSDMA_DESC_PKT_PTR_H(dst_addr_high));
//	printk("t_desc2 = %x r_desc2 d = %x", t_desc2, r_desc2);
	...
	WRITE_ONCE(rxd->desc2, bswap32(r_desc2, swap_flag));
	WRITE_ONCE(txd->desc3, bswap32((src_addr + (hsdma_pkt_len * i)), swap_flag));
	WRITE_ONCE(txd->desc2, bswap32(t_desc2, swap_flag));
...
static void fill_data(dma_addr_t src, unsigned long pattern, unsigned long size)
{
	int i = 0;
	for(i = 0; i < size; i+=8)
	{
		wtMeml((src+i), pattern+i);
	}

}

static int compare_data(dma_addr_t src, unsigned long pattern, unsigned long size)
{
	int i = 0;
	for (i = 0; i < size; i+=8)
	{
		if (rdMeml((src+i)) != pattern+i)
		{
			printk("ERROR: compare data error at 0x%llx", src);
			return -1;
		}
	}
	return 0;
...
#ifdef TCSUPPORT_CPU_ARMV8_64
int HS_DmaMem2Mem_des(volatile struct hsdma_test_req *p_req, u32 NLS_bit, int chan_num)
{
	...
	ecnt_hsdma_high_des_test(0);

	ecnt_hsdma_reg_dump();

	memcpy_toio(high_tx_des0, ring->txd, ECNT_DMA_SIZE * sizeof(*ring->txd));
	memcpy_toio(high_rx_des0, ring->rxd, ECNT_DMA_SIZE * sizeof(*ring->txd));

#if 0
	for(i = 0; i < ECNT_DMA_SIZE; i+=8)
	{
		wtMeml((high_tx_des0+i), pattern+i);
	}	
#endif
	  ...
	  do
	  {
		  memcpy_fromio(ring->txd, high_tx_des0, ECNT_DMA_SIZE * sizeof(*ring->txd));
		  memcpy_fromio(ring->rxd, high_rx_des0, ECNT_DMA_SIZE * sizeof(*ring->txd));
...
void set_hsdma_block_test(dma_addr_t src_phys0, dma_addr_t dst_phys0, u32 testLen, u32 bsize)
{
    ...
    if (bsize==0)
		...
		ecnt_hsdma_set_burst_size(ECNT_HSDMA_BURST_256BYTES);
					...
					unsigned long count, void *data)
{
	...
	memcpy_toio(hg.nsram_base, hg.src_unc0, NSRAM_TEST_SIZE);
	wmb();
	/* readback npu sram data */
	readb((hg.nsram_base + NSRAM_TEST_SIZE - 1));
	rmb();
	...
	switch(type)
	{
		...
		case 27:
			...
			hg.src_unc0 = ecnt_hsdma_alloc_dma(0xf000, &(hg.src_phys0));
			if (hg.src_unc0 == NULL)
			{
			...
			req.src_addr0 = hg.src_phys0;
			req.dst_addr0 = 0xc0000000;
			req.src_array0 = hg.src_unc0;
			
			//fill_data(req.src_array0,stress_test_pattern+test_round, 0xf000);
		
			printk("High DRAM move Test\n");
			...
			do
			{
				...
				HS_DmaMem2Mem(&req,0,0);
				req.dst_addr0+= 0xf000;
				test_round ++;
				if ((req.dst_addr0 & 0xff000) == 0xff000)
					printk("copy to high test addr = %llx\n",req.dst_addr0);
			}while(req.dst_addr0 < DRAM_PHYS + DRAM_SIZE);

			test_round = 0;
//			req.dst_addr0 = req.src_addr0;
			req.src_addr1 = 0xc0000000;
			req.dst_addr1 = req.src_addr0;
			req.len1 = 0xf000;
			...
			do
			{
				reset_cpu_index();
				HS_DmaMem2Mem(&req,0,1);
				//fill_data(req.src_array0, stress_test_pattern + test_round,0x80000);
				req.src_addr1+= 0xf000;
				if (0 != (compare_data(req.src_array0, stress_test_pattern + test_round, 0xf000)))
				{
					break;
				}
				test_round ++;
				if ((req.src_addr1 & 0xff000) == 0xff000)
					printk("copy from high test addr = %llx\n",req.src_addr1);

			}while(req.src_addr1 < DRAM_PHYS + DRAM_SIZE);
		...
		case 29:
			...
			req.len0 = 128;
			ecnt_hsdma_disable_intr(ECNT_HSDMA_INT_ALL);
			memset_io((void *)hg.dst_unc0, 0xa5, 256);
			src_offset = 128;
			HS_DmaMem2Mem(&req, 0, 0);
			comapre(req.src_array0, req.dst_array0, req.len0);
			compare_num(req.dst_array0, 0xa5, req.len0);
			req.dst_array0 = (req.dst_array0 + src_offset);
			compare_num(req.dst_array0, 0xa5, req.len0);
			ecnt_hsdma_enable_intr(ECNT_HSDMA_INT_ALL);
		break;
...
static int RalinkHSdmaInit(void)
{
	...
	#ifdef TCSUPPORT_CPU_ARMV8_64
	get_high_dram_base(0, &high_tx_des0);
	get_high_dram_base(1, &high_rx_des0);
	#endif
	...
	hg.nsram_base = ioremap(NSRAM_PHYS, NSRAM_TEST_SIZE);
