...
unsigned long high_ram0 = 0;
unsigned long high_ram1 = 0;
...
int ecnt_hsdma_reg_access_test(unsigned int cnt)
{
	ecnt_dma_write(g_hsdma, ECNT_HSDMA_TX_BASE, cnt);
	ecnt_dma_write(g_hsdma, ECNT_HSDMA_DLYINT, cnt+1);
	if ((ecnt_dma_read(g_hsdma, ECNT_HSDMA_TX_BASE)!=cnt) || (ecnt_dma_read(g_hsdma, ECNT_HSDMA_DLYINT)!=(cnt+1))) {
		printk("(hsdma_read(TX_BASE):0x%x != 0x%x) or (hsdma_read(DLYINT):0x%x != 0x%x)\n",
				ecnt_dma_read(g_hsdma, ECNT_HSDMA_TX_BASE), cnt, ecnt_dma_read(g_hsdma, ECNT_HSDMA_DLYINT), cnt+1);
		return -1;
	}
	return 0;
}
EXPORT_SYMBOL(ecnt_hsdma_reg_access_test);

#ifdef HSDMA_TEST
u64 get_high_dram_base(u32 reg, u64 *data)
{
	if(reg == 0)
		*data = high_ram0;
	else
...
EXPORT_SYMBOL(get_high_dram_base);

int ecnt_hsdma_high_des_test(int mode)
{
	if (mode == 0)
	{
		ecnt_dma_write(g_hsdma, ECNT_HSDMA_TX_BASE, 0x60000000);
		ecnt_dma_write(g_hsdma, ECNT_HSDMA_TX_CNT , ECNT_DMA_SIZE | 0x1000);
		ecnt_dma_write(g_hsdma, ECNT_HSDMA_RX_BASE, 0x30000000);
		ecnt_dma_write(g_hsdma, ECNT_HSDMA_RX_CNT, ECNT_DMA_SIZE | 0x2000);
	}
	else
	{
		ecnt_dma_write(g_hsdma, ECNT_HSDMA_TX_BASE, 0x1e800000);
		//ecnt_dma_write(g_hsdma, ECNT_HSDMA_TX_CNT , ECNT_DMA_SIZE | 0x1000);
		ecnt_dma_write(g_hsdma, ECNT_HSDMA_RX_BASE, 0x1e840000);
		//ecnt_dma_write(g_hsdma, ECNT_HSDMA_RX_CNT, ECNT_DMA_SIZE | 0x2000);
	}

	return 0;
}
EXPORT_SYMBOL(ecnt_hsdma_high_des_test);
#endif
...
{
	return dma_alloc_coherent(hsdma2dev(g_hsdma), buf_size, phys, GFP_KERNEL | __GFP_ZERO);
}
...
void ecnt_hsdma_set_burst_size(u32 size)
{
	...
	if(size == ECNT_HSDMA_BURST_256BYTES)
	{
		ecnt_dma_rmw(g_hsdma, ECNT_HSDMA_GLO, ECNT_HSDMA_BURST_256BYTES, size);		
	}
	else
	{
		ecnt_dma_clr(g_hsdma, ECNT_HSDMA_GLO, ECNT_HSDMA_BURST_256BYTES);
					...
					struct ecnt_hsdma_pchan *pc, int chan_num)
{
	...
	ring->txd = dma_alloc_coherent(hsdma2dev(hsdma), pc->sz_ring, &(ring->tphys), GFP_KERNEL | __GFP_ZERO);
	if (!ring->txd)
	{
...
static int ecnt_hsdma_probe(struct platform_device *pdev)
{
	...
	high_ram0 = devm_ioremap_resource(&(pdev->dev), res);
	if (IS_ERR(high_ram0))
	{
		err = PTR_ERR(high_ram0);
		goto err_free_hsdma;
	}

	res = platform_get_resource(pdev, IORESOURCE_MEM, 2);
	high_ram1 = devm_ioremap_resource(&(pdev->dev), res);
	if (IS_ERR(high_ram1))
	{
		err = PTR_ERR(high_ram1);
		goto err_free_hsdma;
	}
#endif
...
static const struct ecnt_hsdma_soc ecnt_soc = {
	.ddone		= BIT(31),
	.nls		= BIT(29),
};

static const struct of_device_id ecnt_hsdma_match[] = {
	{ .compatible = "econet,ecnt-hsdma", .data = &ecnt_soc},
	{ /* sentinel */ }
};
