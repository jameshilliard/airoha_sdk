...
__attribute__((aligned(8))) static u8 dat_buf[MAX_DAT_SIZE];
__attribute__((aligned(8))) static u8 oob_buf[MAX_OOB_SIZE];
static bool pool_erased;
static bool isUpdateBmt = false;
...
*                                                              
***************************************************************/
static int get_remainder_bmt_count (void);
...
*                                                              *
***************************************************************/
void dump_bmt_info(int msg_lv, bmt_struct *bmt)
{
	int i;

	MSG(msg_lv, "BMT v%d.\n", bmt->version);
	MSG(msg_lv, "Remainder bmt count = %d\n", get_remainder_bmt_count());
	MSG(msg_lv, "Total %d mapping\n", bmt->mapped_count);
...
void dump_bbt_info(int msg_lv, init_bbt_struct *bbt)
{
	int i;

	MSG(msg_lv, "BBT was written in block:0x%04x\n", bbt_block_index);
	MSG(msg_lv, "BBT v%d.\n", bbt->version);
	MSG(msg_lv, "bad count:%d\n", bbt->badblock_count);
	for (i = 0; i < bbt->badblock_count; i++) {
		MSG(msg_lv, "  0x%04x\n", bbt->badblock_table[i]);
	}
}

int check_default_bad(u32 block_idx)
{
	u32 i;
	
	/* check default bad block */
	for (i = 0; i < init_bbt.badblock_count; i++) {
		if(block_idx == init_bbt.badblock_table[i]) {
			return 1;
		}
	}
...
int check_runtime_bad(u32 block_idx)
{
	u32 i;

	for (i = 0; i < bmt.mapped_count; i++) {
		if(block_idx == bmt.table[i].bad_index) {
			return 1;
		}
	}
...
static bool valid_bmt_data(phys_bmt_struct *phys_table)
{
    ...
    for (i = 0; i < phys_table->header.mapped_count; i++)
    {
        ...
        {
            MSG(0, "bmt block index error \n");
            return false;
        }
    }

    // pass check, valid bmt.
    MSG(0, "Valid BMT, version v%d\n", phys_table->header.version);
    return true;
}
...
static int load_bmt_data(int start, int pool_size)
{
    ...
    MSG(0, "begin to search BMT from block %d \n", bmt_index);
    ...
    for (bmt_index = start + pool_size - 1; bmt_index >= start; bmt_index--)
    {
        ...
        {
            MSG(0, "Skip bad block: %d \n", bmt_index);
            continue;
        }
        
        if (nand_read_page_bmt(PAGE_ADDR(bmt_index), dat_buf, oob_buf))
        {
            MSG(0, "Error found when read block: %d\n", bmt_index);
            continue;
        }
        ...
        else
        {
            ...
            MSG(0, "bmt found at block: %d, mapped block: %d\n", bmt_index, bmt.mapped_count);

            for (i = 0; i < bmt.mapped_count; i++)
            {
                if (!nand_block_bad_bmt(OFFSET(bmt.table[i].bad_index), BAD_BLOCK_RAW))
                {
                    MSG(0, "block %d is not mark bad, should be power lost last time\n", bmt.table[i].bad_index);
...
static int load_bbt_data(int start, int pool_size, init_bbt_struct *init_bbt)
{
    ...
    for(;bbt_index < (start + pool_size); bbt_index++)
    {
        ...
        else
        {
            ...
            MSG(0, "bbt found, bad block count: %d\n", init_bbt->badblock_count);

            for (i = 0; i < init_bbt->badblock_count; i++)
            {
                MSG(0, "init_bbt->badblock_table[%d]: %d \n", i, init_bbt->badblock_table[i]);
            }
...
static int find_available_block(bool start_from_end)
{
    ...
    int direction;
    MSG(0, "Try to find_available_block, pool_erase: %d\n", pool_erased);
    ...
    if (!pool_erased)
    {
        for (i = 0; i < bmt_block_count; i++)
        {
	        ...
	        {
	            MSG(0, "Skip bmt block %d \n", block + i);
	            continue;
	        }            

	        if ((block + i) == bbt_block_index)    
	        {
	            MSG(0, "Skip bbt block %d \n", block + i);
	            continue;
	        }  
            ...
            {
                MSG(0, "Skip mapped block %d \n", block + i);
                continue;
            }

            if (nand_erase_bmt(OFFSET(block + i)))
            {
                MSG(0, "Erase block %d fail\n", block + i);
...
static int get_remainder_bmt_count (void)
{
	...
	count = 0;
    for (i = 0; i < bmt_block_count; i++, block++)
    {
        if (block == bmt_block_index)
        {
            MSG(0, "Skip bmt block %d \n", block);
...
static int migrate_from_bad(u32 offset, update_reason_t reason, u8 *write_dat, u8 *write_oob)
{
    int page;
	...
	{
		/* When write fail, do not need to copy all page since the remainer page are all 0xFF */
		migrate_amount = error_page;
	}
	else
	{
		/* Need to copy all page when read fail. */
		migrate_amount = page_per_block;
	}
    ...
    if (!to_index)
    {
        MSG(0, "Cannot find an available block for BMT\n");
        return 0;
    }

    for (page = 0; page < migrate_amount; page++)
    {
		/* Do not need to read error page since system has data at write_dat. */
		if (page == error_page)
		{
			continue;
		}
        ...
        {
            MSG(0, "Write page %d fail\n", PAGE_ADDR(to_index) + page);
            mark_block_bad_bmt(OFFSET(to_index), BMT_BADBLOCK_GENERATE_LATER);
            return migrate_from_bad(offset, reason, write_dat, write_oob);
        }          
    }
    ...
    MSG(0, "Migrate from %d to %d done!\n",error_block, to_index);
...
static bool write_bmt_to_flash(u8 *dat, u8 *oob)
{
    ...
    if (need_erase)
    {
        ...
        {
            MSG(0, "BMT block erase fail, mark bad: 0x%x\n", bmt_block_index);
    ...
    MSG(0, "Write BMT to block %d success\n", bmt_block_index);
    return true;
}
...
static bool write_bbt_to_flash(u8 *dat, u8 *oob)
{
    ...
    {
        MSG(0, "Cannot find an available block for BBT\n");
        return false;
    }

    if ( nand_write_page_bmt(PAGE_ADDR(bbt_block_index), dat, oob) )
    {
        MSG(0, "Write BBT data fail \n");
    ...
    MSG(0, "Write BBT to block %d success\n", bbt_block_index);
    return true;
}
...
bmt_struct *reconstruct_bmt(bmt_struct * bmt)
{
    ...
    for (i = 0; i < bmt_block_count; i++, index++)
    {
        ...
        {
            MSG(0, "get bad index: 0x%x \n", bad_index);
            if (bad_index != 0xFFFF)
                MSG(0, "Invalid bad index found in block: %d \n", index);
            continue;
        }

        MSG(0, "Block 0x%x is mapped to bad block: 0x%x\n", index, bad_index);

        if (!nand_block_bad_bmt(OFFSET(bad_index), BAD_BLOCK_RAW))
        {
            mark_block_bad_bmt(OFFSET(bad_index), BAD_BLOCK_RAW);
            MSG(0, "block %d is not marked as bad, mark it\n", bad_index);
        }
        ...
        MSG(0, "Add mapping: %d -> %d to BMT\n", bad_index, index);
    }

    MSG(0, "Scan replace pool done, mapped block: %d\n", bmt->mapped_count);
...
init_bbt_struct* reconstruct_bbt(init_bbt_struct* init_bbt)
{
    ...
    {
        MSG(0, "scan_badblock_raw fail (%d)! \n", BBT_VERSION);
        return NULL;
    }
...
#endif
{
    ...
    {
        MSG(0, "Init bmt table, size: %d\n", size);
        bmt_block_count = size;
    }
    else
    {
        MSG(0, "Invalid bmt table size: %d\n", size);
        return NULL;
    }
    ...
    MSG(0, "bmt count: %d, system count: %d\n", bmt_block_count, system_block_count);
    ...
    if (bmt_block_index)
    {
        MSG(0, "Load bmt data success @ block %d \n", bmt_block_index);
        dump_bmt_info(0, &bmt);
        return &bmt;
    }
    else
    {
        MSG(0, "Load bmt data fail! \n");
...
init_bbt_struct* start_init_bbt(void)
{
    ...
    if(bbt_block_index)
    {
        MSG(0, "Load bbt data success \n");
        return &init_bbt;
    }  
    else
    {

        MSG(0, "Load bbt data fail! \n");
...
bool update_bmt(u32 offset, update_reason_t reason, u8 *dat, u8 *oob)
{
	...
	if (nand_chip_bmt == NULL)
	{
		MSG(0, "BMT does not init\n");
		return false;
	}

    bad_index = offset / BLOCK_SIZE_BMT;

	if ((bad_index < system_block_count)
		&& (bad_index != get_mapping_block_index_by_bmt(bad_index)))
	{
		MSG(0, "Block %d exists in BMT.\n", bad_index);
		return true;
	}

	/* Since system may update bmt twice at one command. Only support update bmt once. */
	/* ex. write page #5 fail or page #5 read over ECC threshold. Update BMT but page #0 read over ECC threshold, then cause update BMT twice. */
	if (isUpdateBmt == true)
	{
		MSG(0, "System is updating BMT\n");
		return true;
	}
	else
	{
    ...
    if ((reason == UPDATE_WRITE_FAIL) || (reason == UPDATE_READ_CHECK_FAIL))
    {
        if ( !(map_index = migrate_from_bad(offset, reason, dat, oob)) )
        {
			/* Disable update BMT flag */
			isUpdateBmt = false;
    ...
    else
    {
        if ( !(map_index = find_available_block(false)) )
        {
            ...
            MSG(0, "Cannot find block in pool \n");
            return false;
        }
    }
    ...
    if (bad_index >= system_block_count)     // mapped block become bad, find original bad block
    {
        ...
        MSG(0, "Mapped block becomes bad, orig bad block is %d \n", orig_bad_block);
...
int get_mapping_block_index_by_bmt(int index)
{
    ...
    {
        MSG(0, "BMT Given index exceed: %d > %d \n", index, system_block_count);
        return index;
    }
