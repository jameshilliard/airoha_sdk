...
#include <linux/ecnt_semaphore.h>
...
struct spi_chip_info {
	...
	u32 (*read)(struct map_info *map, unsigned long from, u32 to, u32 size);
	u32 (*read_manual)(struct mtd_info *mtd, unsigned long from, unsigned char *buf, unsigned long len);
	u32 (*write)(struct mtd_info *mtd, unsigned long from, u32 to, u32 size);
	u32 (*erase)(struct mtd_info *mtd, unsigned long from);
};
...
static u32
spiflash_write (struct mtd_info *mtd, unsigned long from, u32 to, u32 len)
{
	u8 cur_mode=0;
	u8 mode=0;
	static int cnt=0;
...
int mtd_spiflash_read(struct mtd_info *mtd, loff_t from, size_t len, size_t *retlen, u_char *buf)
{
	...
	*((__u32 *)(CR_SPI_BASE | SPI_FLASH_MM)) = reg0x28;
	...
	ret = chip_info->read_manual(mtd, (unsigned long) from, buf, len);
	if (ret){
#if defined(TC_SOC) && defined(CONFIG_MIPS_TC3262)
		up_Manual(&SPI_SEM);
#endif
...
#endif
	ret = chip_info->write(mtd, (unsigned long) buf, to, len);
	//spiflash_done();
	if (ret){
#if defined(TC_SOC) && defined(CONFIG_MIPS_TC3262)
		up_Manual(&SPI_SEM);
...
{
#ifdef TCSUPPORT_CPU_ARMV8
	#define FLASHBASE_FF48 (flash_base + 0xff48)
	char macaddr[7] = {0};
	if(index >= FLASHBASE_FF48 && index < (FLASHBASE_FF48 + 6))
	{
		get_ethaddr(macaddr, 6);
		return macaddr[index - FLASHBASE_FF48];
	}
	else
...
unsigned long ReadSPIDWord(unsigned long index)
{
	...
	struct mtd_info *mtd = get_mtd_device(NULL, 0);
	mtd_read(mtd, (loff_t)index, sizeof(dword), &retlen, &dword);
