...
struct bbt_nand_copy_resp bbt_nand_copy(u16 dest_blk, u16 src_blk, loff_t max_offset)
{
	...
	uint8_t oob[64];
	int i, ret;
	struct bbt_nand_copy_resp resp; 
	resp.ret = 0; 
...
int mtk_bmt_attach(struct mtd_info *mtd)
{
	...
	else if (of_property_read_bool(np, "mediatek,bmt-v2"))
