...
static int w25n01gv_m02gv_ecc_get_status(struct spinand_device *spinand,
					u8 status)
{
	switch (status & W25N01_M02GV_STATUS_ECC_MASK) {
	case W25N01_M02GV_STATUS_ECC_NO_BITFLIPS:
		return 0;

	case W25N01_M02GV_STATUS_ECC_1_BITFLIPS:
		return 1;
	
	case W25N01_M02GV_STATUS_ECC_UNCOR_ERROR:
		return -EBADMSG;
...
static int winbond_spinand_detect(struct spinand_device *spinand)
{
	...
	ret = spinand_match_and_init(spinand, &winbond_spinand_table,
				     ARRAY_SIZE(winbond_spinand_table), id[2] << 8 | id[3] );
