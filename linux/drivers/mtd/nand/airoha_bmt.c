...
static void variable_setup(void) { 
	...
	if ((total_blks - last_blk) > (total_blks * MAX_BMT_PERCENT))  { 
		panic("cant find bmt area inside resevearea\n");
	}
...
static void reconstruct_from_oob(void) { 
	...
	for ( i = total_blks -1 ; i >= system_blks ;i--) { 
		...
		if (bmt.header.size > MAX_RAW_BAD_BLOCK_SIZE)
			panic("bad block cound exceeded the maximum amount\n");
	}
...
retry: 
	...
	if (copy_len) { 
		struct bbt_nand_copy_resp resp = bbt_nand_copy(new_block , mapped_block , copy_len);
		if(retry_count-- > 0 && resp.ret ) {
			if (resp.fail_type == write_fail) { 
				mark_bad(new_block);
			}
			goto retry;
		}
	...
	if (!mapped_already_in_bmt)
		bmt.table[bmt.header.size++] = (bmt_entry){ .from = mapped_block, .to = new_block};
...
static int init(struct device_node *np) {
	...
	// generate pbbt
	// this is like a map , for the evaluation of bbt table 
	int i=0,j=0,k=0;
	for (i =0; i < system_blks;i++) { 
...
static  int get_mapping_block( int block) { 
	...
	int bbt_mapped_block = pbbt[block];
	int i = 0;
...
static int  debug( void* data , u64 cmd) { 
	...
	if (val == 0 ) { 
		...
		printk("adding :%d  block to bbt\n", _to);
		mark_bad_raw(_to);
		reconstruct_from_oob();
	} else if ( val == 13) { 
		printk("printk starting byte of first 5 blocks\n");
		for ( i = 0 ; i < 5; i++) { 
			char c; 
			struct mtd_oob_ops ops = { 
				.mode = MTD_OPS_PLACE_OOB,
			...
			bmtd._read_oob(bmtd.mtd, _to << bmtd.blk_shift, &ops);
			printk("%X ", c);
		} 
		printk("\n");
	}
