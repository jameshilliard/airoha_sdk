		...
		struct xhci_event_cmd *event)
{
    ...
    xhci_dbg(xhci, "cmd type: 0x%x\n", (le32_to_cpu(xhci->cmd_ring->dequeue->generic.field[3]) & TRB_TYPE_BITMASK));
		...
		union xhci_trb *event)
{
	...
	xhci_dbg(xhci, "port_status %x, port->port_reenabled:%d\n", port_status, port->port_reenabled);
		...
		if(port_status & USB_PORT_STAT_CONNECTION){
			...
			if(!(port_status & USB_PORT_STAT_SUPER_SPEED)){
				if(!isFPGA){
                    ...
                    temp = get_uphy_reg(port_id, 0x364);
					temp |= (1<<18);
					temp &= ~(1<<19);
					set_uphy_reg(port_id, 0x364,temp);
					printk("set SW pll %x\n", get_uphy_reg(port_id, 0x364));
                    #else
					temp = get_uphy_reg(0x364);
					temp |= (1<<18);
					temp &= ~(1<<19);
					set_uphy_reg(0x364,temp);
					printk("set sw pll %x\n", get_uphy_reg(0x364));
                    #endif
                #else
...
void xhci_handle_event(struct xhci_hcd *xhci){
    ...
    #if 1 /* update from new code 20221123 */
	...
	if ((le32_to_cpu(event->event_cmd.flags) & TRB_CYCLE) !=
        ...
        xhci_dbg(xhci, "Error cycle state\n");
        xhci_dbg(xhci, "xhci->event_ring->cycle_state:0x%x, event->event_cmd.flags:0x%x\n",
            xhci->event_ring->cycle_state, (le32_to_cpu(event->event_cmd.flags) & TRB_CYCLE));
		return;
	}
...
/*
    ...
    xhci_dbg(xhci, "event type: 0x%x\n", (le32_to_cpu(event->event_cmd.flags) & TRB_TYPE_BITMASK));
