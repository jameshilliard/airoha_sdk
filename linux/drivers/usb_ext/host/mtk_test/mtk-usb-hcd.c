...
struct usb_device *mtk_usb_alloc_dev(struct usb_device *parent, struct usb_bus *bus, unsigned port1){
	...
	if (root_hub)	/* Root hub always ok [and always wired] */
        ...
        /* refer to usb_alloc_dev() in drivers/usb/core/usb.c */
        #if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,55))
        dev->authorized = arht_usb_dev_authorized(dev, usb_hcd);
        #else /* for 3.18.21 */
		...
		unsigned int irqnum, unsigned long irqflags)
{
    ...
    /* refer to usb_add_hcd() in drivers/usb/core/hcd.c */
    #if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,55))
    hcd->dev_policy = hcd->wireless? 0 : 1;
    #else /* for 3.18.21 */
	...
	if (hcd->driver->irq) {
        ...
        /* IRQF_DISABLED no longer exists in kernel >= 4.1 */
		...
		struct device *dev, const char *bus_name)
{
    ...
    /* refer to usb_create_hcd() in drivers/usb/core/hcd.c */
    #if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,55))
    hcd->self.sysdev = dev;
    #else /* 3.18.21 */
