...
void enableAllClockPower(struct device *dev){
	...
	int u3_not_support=0;
	if(get_hcd_port_id(u3h_hw) == 0 && get_serdes_interface_sel(ECNT_EVENT_SERDES_SEL_USB1) != 0) // don't support usb 0 
	{
		u3_not_support = 1;
		printk("usb controller 1, u3_not_support  \n");
	}
	if(get_hcd_port_id(u3h_hw) == 1 && get_serdes_interface_sel(ECNT_EVENT_SERDES_SEL_USB2) != 0) // don't support usb 1
	{
		u3_not_support = 1;
		printk("usb controller 2, u3_not_support  \n");
	}
#endif
	...
	for(i=0; i<u3_port_num; i++){
		...
		if(u3_not_support == 1)
		{
			printk("usb serdes is not USB, don't init u3  \n");
#if defined(TCSUPPORT_AUTOBENCH)
			u4TimeoutCount = 0;
#endif
			break;
		}
		if(get_hcd_port_id(u3h_hw) == 1 && !isAN7581GT && !isAN7581FG && !isAN7551GT && !isAN7566GT){
			printk("PT series usb port 1 not support usb3.0 \n");
#if defined(TCSUPPORT_AUTOBENCH)
			u4TimeoutCount = 0;
#endif
			break;	
