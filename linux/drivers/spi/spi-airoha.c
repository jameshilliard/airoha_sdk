...
#define _SPI_NAND_OP_READ_FROM_CACHE_SINGLE_FAST \
	0x0B /* Read data from cache of SPI NAND chip, single speed*/
...
#define _SPI_NAND_OP_WRITE_DISABLE \
	0x04 /* Reseting the Write Enable Latch (WEL) */
#define _SPI_NAND_OP_PROGRAM_LOAD_SINGLE \
	0x02 /* Write data into cache of SPI NAND chip with cache reset, single speed */
...
#define _SPI_NAND_OP_PROGRAM_LOAD_RAMDON_QUAD \
...
#define WRITE_NFI_REG_WITH_MASK(a, b, c) \
	WRITE_NFI_REG(a, (READ_NFI_REG(a) & ~(b)) | (c))
...
static int airoha_spi_set_opfifo(struct airoha_snand *as, unsigned char op_cmd,
				 unsigned int op_len)
{
	WRITE_SPI_REG(_SPI_CONTROLLER_REGS_MANUAL_OPFIFO_WDATA,
...
static void airoha_spi_set_cs(struct airoha_snand *as,
			      SPI_CONTROLLER_CHIP_SELECT_T cs)
{
				      ...
				      unsigned int data_len)
{
	...
	for (idx = 0; idx < data_len; idx++) {
		...
		WRITE_SPI_REG(_SPI_CONTROLLER_REGS_MANUAL_DFIFO_WDATA,
			      ((*(ptr_data + idx)) & 0xff));
				     ...
				     unsigned int data_len)
{
	...
	for (idx = 0; idx < data_len; idx++) {
		...
		*(ptr_rtn_data + idx) =
			(READ_SPI_REG(
				_SPI_CONTROLLER_REGS_MANUAL_DFIFO_RDATA)) &
			0xff;
...
static int airoha_spi_write_one_byte_with_cmd(struct airoha_snand *as,
					      unsigned char cmd,
					      unsigned char *data)
{
...
static int airoha_spi_write_nbytes_with_cmd(struct airoha_snand *as,
					    unsigned char cmd,
					    unsigned char *ptr_data,
					    unsigned int len)
{
...
static int airoha_spi_read_nbytes(struct airoha_snand *as,
				  unsigned char *ptr_rtn_data, unsigned int len)
{
	unsigned int data_len, remain_len;
	remain_len = len;
	...
	while (remain_len > 0) {
		data_len = (remain_len > 0x1ff) ? 0x1ff : remain_len;
		airoha_spi_set_opfifo(as, 0x0c, data_len);
...
static int airoha_snand_nfi_init(struct airoha_snand *as)
{
	...
	/* Enable DMA */
	WRITE_NFI_REG(_SPI_NFI_REGS_INTR_EN,
		      READ_NFI_REG(_SPI_NFI_REGS_INTR_EN) | 0x0040);
	return 0;
}
...
{
	/* AutoFDM */
	WRITE_NFI_REG(_SPI_NFI_REGS_CNFG,
		      READ_NFI_REG(_SPI_NFI_REGS_CNFG) & ~(0x0200));
	/* HW ECC */
	WRITE_NFI_REG(_SPI_NFI_REGS_CNFG,
		      READ_NFI_REG(_SPI_NFI_REGS_CNFG) & ~(0x0100));
	/* DMA Burst */
	WRITE_NFI_REG(_SPI_NFI_REGS_CNFG,
		      READ_NFI_REG(_SPI_NFI_REGS_CNFG) | (0x0004));
	/* page format */
	switch (as->nfi_cfg.spare_size) {
	case 16: {
		WRITE_NFI_REG_WITH_MASK(_SPI_NFI_REGS_PAGEFMT, 0x0030,
					(0x0 << 4));
		break;
	}
	case 26: {
	...
	case 512: {
		WRITE_NFI_REG_WITH_MASK(_SPI_NFI_REGS_PAGEFMT, 0x0003,
					(0x0 << 0));
		break;
	}
	case 2048: {
	...
	/* sec num */
	WRITE_NFI_REG_WITH_MASK(_SPI_NFI_REGS_CON, 0xf000,
				as->nfi_cfg.sec_num << 12);
	/* enable cust sec size */
	WRITE_NFI_REG(_SPI_NFI_REGS_SECCUS_SIZE,
		      READ_NFI_REG(_SPI_NFI_REGS_SECCUS_SIZE) | (0x00010000));
	/* set cust sec size */
	WRITE_NFI_REG_WITH_MASK(_SPI_NFI_REGS_SECCUS_SIZE, 0x00001fff,
				(as->nfi_cfg.sec_size << 0));
	return 0;
}
					...
					u64 offs, size_t len, void *buf)
{
	...
	if (op->cmd.opcode == _SPI_NAND_OP_READ_FROM_CACHE_SINGLE ||
	    op->cmd.opcode == _SPI_NAND_OP_READ_FROM_CACHE_SINGLE_FAST) {
		rd_mode = 0x0;
	...
	dma_sync_single_for_device(as->dev, as->rx_dma_addr, as->buf_len,
				   DMA_FROM_DEVICE);
	mb();
	// set dma addr
	WRITE_NFI_REG(_SPI_NFI_REGS_STRADDR, as->rx_dma_addr);
	// set cust sec size
	WRITE_NFI_REG_WITH_MASK(_SPI_NFI_REGS_SNF_MISC_CTL2, 0x1fff,
	...
	WRITE_NFI_REG(_SPI_NFI_REGS_CNFG,
		      READ_NFI_REG(_SPI_NFI_REGS_CNFG) | 0x0002);
	WRITE_NFI_REG(_SPI_NFI_REGS_CNFG,
		      READ_NFI_REG(_SPI_NFI_REGS_CNFG) | 0x0001);
	WRITE_NFI_REG(_SPI_NFI_REGS_CMD, 0x00);
	// trigger dma start read
	WRITE_NFI_REG(_SPI_NFI_REGS_CON,
		      READ_NFI_REG(_SPI_NFI_REGS_CON) & ~(0x0100));
	WRITE_NFI_REG(_SPI_NFI_REGS_CON,
		      READ_NFI_REG(_SPI_NFI_REGS_CON) | (0x0100));

	ret = readl_poll_timeout(as->nfi_base + _SPI_NFI_REGS_SNF_STA_CTL1, val,
				 (val & 0x02000000) != 0, 0, 1000000);
	if (ret) {
		printk("[Error] Read DMA : Check READ FROM CACHE Done Timeout ! \n");
		return -1;
	}
	WRITE_NFI_REG(_SPI_NFI_REGS_SNF_STA_CTL1,
		      READ_NFI_REG(_SPI_NFI_REGS_SNF_STA_CTL1) | 0x02000000);
	ret = readl_poll_timeout(as->nfi_base + _SPI_NFI_REGS_INTR, val,
				 (val & 0x0040) != 0, 0, 1000000);
	if (ret) {
	...
	udelay(1);
	dma_sync_single_for_cpu(as->dev, as->rx_dma_addr, as->buf_len,
				DMA_FROM_DEVICE);
					 ...
					 u64 offs, size_t len, const void *buf)
{
	...
	if (op->cmd.opcode == _SPI_NAND_OP_PROGRAM_LOAD_QUAD ||
	    op->cmd.opcode == _SPI_NAND_OP_PROGRAM_LOAD_RAMDON_QUAD) {
		wr_mode = 0x2;
	} else {
		wr_mode = 0x0;
	}
	...
	airoha_spi_set_mode(as, SPI_CONTROLLER_MODE_MANUAL);
	dma_sync_single_for_cpu(as->dev, as->tx_dma_addr, as->buf_len,
				DMA_TO_DEVICE);
	memcpy(as->tx_buf, as->rx_buf, as->buf_len);
	memcpy(as->tx_buf + offs, buf, len);
	dma_sync_single_for_device(as->dev, as->tx_dma_addr, as->buf_len,
				   DMA_TO_DEVICE);
	mb();
	airoha_spi_set_mode(as, SPI_CONTROLLER_MODE_DMA);
	airoha_snand_nfi_reset(as);
	...
	WRITE_NFI_REG_WITH_MASK(_SPI_NFI_REGS_SNF_MISC_CTL2, 0x1fff0000,
				((as->nfi_cfg.sec_size * as->nfi_cfg.sec_num)
				 << 16));
	...
	WRITE_NFI_REG(_SPI_NFI_REGS_CON,
		      (READ_NFI_REG(_SPI_NFI_REGS_CON) & ~(0x0200)));
	WRITE_NFI_REG(_SPI_NFI_REGS_CON,
		      (READ_NFI_REG(_SPI_NFI_REGS_CON) | (0x0200)));
	udelay(1);
	...
	ret = readl_poll_timeout(as->nfi_base + _SPI_NFI_REGS_SNF_STA_CTL1, val,
				 (val & 0x04000000) != 0, 0, 1000000);
	if (ret) {
		printk("[Error] Read DMA : Check AHB Done Timeout ! \n");
		return -1;
	}
	WRITE_NFI_REG(_SPI_NFI_REGS_SNF_STA_CTL1,
		      READ_NFI_REG(_SPI_NFI_REGS_SNF_STA_CTL1) | 0x04000000);
	airoha_spi_set_mode(as, SPI_CONTROLLER_MODE_MANUAL);
	return len;
}

static int airoha_snand_op_transfer(struct airoha_snand *as,
				    const struct spi_mem_op *op)
{
	unsigned int idx;
	...
	for (idx = 0; idx < op->addr.nbytes; idx++) {
		unsigned char addr_data =
			(op->addr.val >> ((op->addr.nbytes - idx - 1) * 8)) &
			0xff;
		if (op->cmd.opcode == _SPI_NAND_OP_GET_FEATURE) {
			airoha_spi_write_one_byte_with_cmd(as, 0x11,
							   &addr_data);
		} else {
	...
	if (op->data.dir == SPI_MEM_DATA_IN) {
		...
		airoha_spi_write_nbytes_with_cmd(as, 0x8, op->data.buf.out,
						 op->data.nbytes);
	}
				...
				const struct spi_mem_op *op)
{
	...
	spinand = (struct spinand_device *)mem->drvpriv;
...
static bool is_nor(void)
{
	void __iomem *reg = ioremap(
		(_SPI_CONTROLLER_REGS_BASE + _SPI_CONTROLLER_REGS_SFC_STRAP),
		4);
	u32 val = readl(reg);
	iounmap(reg);
	return !(val & 0x02);
...
static is_parallel(void)
{
	void __iomem *reg = ioremap(0x1fa1155c, 4);
	u32 val = readl(reg);
	iounmap(reg);
	return (val & 0x04) == 0;
}
