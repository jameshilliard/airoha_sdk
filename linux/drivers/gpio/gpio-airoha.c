			  ...
			  int val, int out)
{
	...
	u32 dir = ioread32(ctrl->dir[gpio >> 4]);				// gpio >> 4 is equivalent to gpio / 16 and is more computationally efficient
	...
	iowrite32(dir, ctrl->dir[gpio >> 4]); 				// gpio >> 4 is equivalent to gpio / 16 and is more computationally efficient
...
static int airoha_gpio_probe(struct platform_device *pdev)
{
	...
	if(device_property_read_u32(dev, "gpio_base", &gpio_base)){
		printk("gpio_arioha probe - gpio_base could not be read from dts.");
	}
	else{
