...
inline void *airoha_ring_next_wptr(struct airoha_device *airoha,
						struct airoha_desc_ring *ring)
{
	void *ptr = ring->write;
...
inline void *airoha_ring_next_rptr(struct airoha_device *airoha,
						struct airoha_desc_ring *ring)
{
	void *ptr = ring->read;
					...
					struct eip93_descriptor_s *desc)
{
	...
	spin_lock_irqsave(&airoha->ring->write_lock, irqflags);

	rdesc = airoha_ring_next_wptr(airoha, &airoha->ring->rdr);

	if (IS_ERR(rdesc)) {
		spin_unlock_irqrestore(&airoha->ring->write_lock, irqflags);
		return -ENOENT;
	}

	cdesc = airoha_ring_next_wptr(airoha, &airoha->ring->cdr);
...
inline void *airoha_get_descriptor(struct airoha_device *airoha)
{
	...
	spin_lock_irqsave(&airoha->ring->read_lock, irqflags);

	cdesc = airoha_ring_next_rptr(airoha, &airoha->ring->cdr);

	if (IS_ERR(cdesc)) {
		spin_unlock_irqrestore(&airoha->ring->read_lock, irqflags);
		return ERR_PTR(-ENOENT);
	}

	memset(cdesc, 0, sizeof(struct eip93_descriptor_s));

	ptr = airoha_ring_next_rptr(airoha, &airoha->ring->rdr);
	if (IS_ERR(ptr)) {
		spin_unlock_irqrestore(&airoha->ring->read_lock, irqflags);
		return ERR_PTR(-ENOENT);
	}

	atomic_inc(&airoha->ring->free);
	spin_unlock_irqrestore(&airoha->ring->read_lock, irqflags);
...
static inline void airoha_free_sg_copy(const int len, struct scatterlist **sg)
{
	if (!*sg || !len)
		return;
...
static inline int airoha_make_sg_copy(struct scatterlist *src,
...
static inline bool airoha_is_sg_aligned(struct scatterlist *sg, u32 len,
						const int blksize)
{
	int nents;
...
int check_valid_request(struct airoha_cipher_reqctx *rctx)
{
	...
	if (authsize) {
		if (dst_nents == 1 && src_nents == 1) {
			src_align = airoha_is_sg_aligned(src, totlen_src, blksize);
			if (src ==  dst)
				dst_align = src_align;
			else
				dst_align = airoha_is_sg_aligned(dst,
						totlen_dst, blksize);
		} else {
			src_align = false;
			dst_align = false;
		...
		if (src == dst)
			...
			dst_align = airoha_is_sg_aligned(dst, totlen_dst, blksize);
	}

	copy_len = max(totlen_src, totlen_dst);
	if (!src_align) {
		err = airoha_make_sg_copy(src, &rctx->sg_src, copy_len, true);
		if (err)
			return err;
	}

	if (!dst_align) {
		err = airoha_make_sg_copy(dst, &rctx->sg_dst, copy_len, false);
		if (err)
			return err;
	}
				...
				const u32 flags)
{
	...
	switch ((flags & AIROHA_HASH_MASK)) {
	case AIROHA_HASH_SHA256:
		saRecord->saCmd0.bits.hash = 3;
		break;
	case AIROHA_HASH_SHA224:
		saRecord->saCmd0.bits.hash = 2;
		break;
	case AIROHA_HASH_SHA1:
		saRecord->saCmd0.bits.hash = 1;
		break;
	case AIROHA_HASH_MD5:
	...
	switch ((flags & AIROHA_MODE_MASK)) {
	case AIROHA_MODE_CBC:
		saRecord->saCmd1.bits.cipherMode = 1;
		break;
	case AIROHA_MODE_CTR:
		saRecord->saCmd1.bits.cipherMode = 2;
		break;
	case AIROHA_MODE_ECB:
 ...
 *
 */
static inline int airoha_scatter_combine(struct airoha_device *airoha,
			struct airoha_cipher_reqctx *rctx,
			u32 datalen, u32 split, int offsetin)
{
	...
	do {
		...
		if (n == 0)
			cdesc->userId |= AIROHA_DESC_LAST;

		/* Loop - Delay - No need to rollback
		 * Maybe refine by slowing down at AIROHA_RING_BUSY
		 */
again:
		err = airoha_put_descriptor(airoha, cdesc);
		if (err) {
			udelay(500);
			goto again;
		}
		/* Writing new descriptor count starts DMA action */
		writel(1, airoha->base + EIP93_REG_PE_CD_COUNT);
			...
			const u8 *reqiv, struct airoha_cipher_reqctx *rctx)
{
	...
	//printk("->");
	//printk("##################### async64[0x%llx]async[0x%llx]\n", (u64)async, virt_to_phys(async));
	...
	if (IS_DMA_IV(flags)) {
		...
		idx = airoha_get_free_saState(airoha);
		if (idx < 0)
			goto send_err;
		saState_pool = &airoha->ring->saState_pool[idx];
	...
	if (IS_DMA_IV(flags)) {
		//printk("(iv)reqivAddr(0x%llx)ivsize(%d)\n", reqiv, rctx->ivsize);
		rctx->saState_base = dma_map_single(airoha->dev, (void *)reqiv,
						rctx->ivsize, DMA_TO_DEVICE);
		if (dma_mapping_error(airoha->dev, rctx->saState_base))
			goto free_state;
	}
skip_iv:
	...
	cdesc.arc4Addr = ((uint32_t)(virt_to_phys(async)));
	cdesc.userId = flags;
	rctx->cdesc = &cdesc;
	 ...
	 */
	//printk("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n"); 
	dma_map_sg(airoha->dev, dst, rctx->dst_nents, DMA_BIDIRECTIONAL);
	if (src != dst)
		dma_map_sg(airoha->dev, src, rctx->src_nents, DMA_TO_DEVICE);

	err = airoha_scatter_combine(airoha, rctx, datalen, split, offsetin);

	return err;

free_state:
	if (rctx->saState) {
		saState_pool = &airoha->ring->saState_pool[rctx->saState_idx];
		saState_pool->in_use = false;
	}

	if (rctx->saState_ctr) {
		saState_pool = &airoha->ring->saState_pool[rctx->saState_ctr_idx];
		saState_pool->in_use = false;
	}
send_err:
	return err;
			...
			struct scatterlist *reqsrc, struct scatterlist *reqdst)
{
	...
	if (rctx->sg_src == rctx->sg_dst) {
		dma_unmap_sg(airoha->dev, rctx->sg_dst, rctx->dst_nents,
							DMA_BIDIRECTIONAL);
		goto process_tag;
	}

	dma_unmap_sg(airoha->dev, rctx->sg_src, rctx->src_nents,
							DMA_TO_DEVICE);

	if (rctx->sg_src != reqsrc)
		airoha_free_sg_copy(len +  rctx->authsize, &rctx->sg_src);
...
process_tag:
	...
	if (rctx->sg_dst != reqdst) {
		...
		airoha_free_sg_copy(len + rctx->authsize, &rctx->sg_dst);
	}
}
			...
			u8 *reqiv)
{
	...
	if (IS_DMA_IV(rctx->flags))
		dma_unmap_single(airoha->dev, rctx->saState_base, rctx->ivsize,
						DMA_TO_DEVICE);
