...
{
	if (!seg || !xhci->segment_pool) // COV, CID 153311
		return;
	if (seg->trbs) {
...
static void xhci_free_container_ctx(struct xhci_hcd *xhci,
			     struct xhci_container_ctx *ctx)
{
	if (!ctx || !xhci->device_pool) // COV, CID 153348
		return;
		...
		struct xhci_stream_ctx *stream_ctx, dma_addr_t dma)
{
	...
	{
		if (xhci->small_streams_pool) // COV, CID 185983
	...
	{
		if (xhci->medium_streams_pool) // COV, CID 185979
		...
		struct xhci_stream_info *stream_info)
{
	 ...
	 */
	if ((fls_ret = fls(stream_info->num_stream_ctxs)) > 2) // COV, CID 121258
		max_primary_streams = fls_ret - 2;
	else
		max_primary_streams = 1;
...
void xhci_free_virt_device(struct xhci_hcd *xhci, int slot_id)
{
	...
	if (slot_id == 0 || !xhci->devs[slot_id] || !xhci->dcbaa) // COV, CID 16901
		return;
		...
		__le32 __iomem *addr, u8 major_revision, int max_caps)
{
	...
	if (port_offset == 0 || (port_offset + port_count - 1) > num_ports)
		/* WTF? "Valid values are 鈥?鈥?to MaxPorts" */
		return;
