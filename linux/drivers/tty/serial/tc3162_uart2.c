...
#elif defined(TCSUPPORT_CPU_EN7523) && !defined(TCSUPPORT_CPU_EN7581)
#define TC3162_NR_PORTS				1	//UART2
#elif defined(TCSUPPORT_CPU_EN7580) && !defined(TCSUPPORT_CPU_EN7581)
#define TC3162_NR_PORTS				4	//UART2, UART3, UART4, UART5
#elif defined(TCSUPPORT_CPU_EN7581)
#define TC3162_NR_PORTS				4	//UART2, HSUART3, UART4, UART5
#else
#define TC3162_NR_PORTS				1 	// set 1 UART by default
#endif

typedef enum UART_NUM {
	UART_2 = 1,
#if TC3162_NR_PORTS > 1
	UART_3,
#if TC3162_NR_PORTS > 2
	UART_4,
#if TC3162_NR_PORTS > 3
	UART_5,
#endif
#endif
#endif
...
typedef enum UART_TYPE {
	UART_NORM = 1,
    UART_HIGH_SPEED,    /* identify high speed uart. */
} uart_type_e;

struct uart_desp_t {
    uint32_t uart_id;
    uint32_t uart_type;
#ifdef TCSUPPORT_CPU_ARMV8 /* for address mapping in ARM. */
    struct device *dev;
...
struct uart_desp_t ecnt_uarts[TC3162_NR_PORTS + 1] = {0};

struct uart_dma_private{
	dma_addr_t rx_phys;
	dma_addr_t tx_phys;
	char *rx_buffer;
	char *tx_buffer;
...
static struct uart_dma_private dma_info;


#ifdef TCSUPPORT_HIGH_SPEED_UART
uint32_t high_speed_uarts[] = {UART_3};
#endif

#ifdef TCSUPPORT_CPU_ARMV8 // if ARM
...
#endif // #ifdef TCSUPPORT_CPU_ARMV8
...
/* 8250 official register 1~8*/


#define dma_depth 0xFF

//dma register
#define csr_dma_tx_en		1 
#define csr_dma_rx_en		1 << 1
#define csr_dma_tx_rst		1 << 2
#define csr_dma_rx_rst		1 << 3

#define mem_size			0xFF * 128 //dma_depth * 128

#define ARHT_UART_DMA 0
#define ARHT_DMA_LOG 0
#define ARHT_DMA_CTRL	0x58
#define ARHT_RX_BSADDR	0x5c
#define ARHT_TX_BSADDR	0x60
#define ARHT_RX_PARA	0x64
#define ARHT_TX_PARA	0x68
#define ARHT_RX_IDX		0x6c
#define ARHT_TX_IDX		0x70

static void tc3162ser_stop_tx(struct uart_port *port);

#if ARHT_UART_DMA
void print_register(struct uart_port *port)
{
	printk(" [UART_DMA]ARHT_DMA_CTRL: 0x%x\n", UART_RDL(ARHT_DMA_CTRL + port->iobase));
	printk(" [UART_DMA]ARHT_TX_BSADDR: 0x%x\n", UART_RDL(ARHT_TX_BSADDR + port->iobase));
	printk(" [UART_DMA]ARHT_TX_PARA: 0x%x\n", UART_RDL(ARHT_TX_PARA + port->iobase));
	printk(" [UART_DMA]ARHT_TX_IDX: 0x%x\n", UART_RDL(ARHT_TX_IDX + port->iobase));
	printk(" [UART_DMA]ARHT_RX_BSADDR: 0x%x\n", UART_RDL(ARHT_RX_BSADDR + port->iobase));
	printk(" [UART_DMA]ARHT_RX_PARA: 0x%x\n", UART_RDL(ARHT_RX_PARA + port->iobase));
	printk(" [UART_DMA]ARHT_RX_IDX: 0x%x\n", UART_RDL(ARHT_RX_IDX + port->iobase));
}
// dma operation
int arht8250_tx_dma(struct uart_port *port)
{
	unsigned int dma_idx=0, cpu_idx = 0,  length =0, count = 0, copy_l = 0, cnt_idx , tmp_idx ;
	unsigned int rg_tx_idx = UART_RDL(ARHT_TX_IDX + port->iobase);
	...
	length = xmit->head - xmit->tail;
	tmp_idx = dma_info.tx_idx;
	
	printk("[UART_DMA] arht8250_tx_dma length :%d %d %d\n",length, dma_info.tx_idx , mem_size);
	
	if( (length + dma_info.tx_idx) > mem_size)
		count = (length + dma_info.tx_idx - mem_size)/ mem_size + 1;

	for( cnt_idx = 0; cnt_idx <= count ; cnt_idx ++)
	{
		printk("[UART_DMA] arht8250_tx_dma length :%c %d\n",*(((char*)xmit->buf) + xmit->tail), count);
		
		if(count == 0)
		{
			copy_l = length;
		}
		else if(cnt_idx != count)
		{
			copy_l = mem_size - dma_info.tx_idx;
		}
		else if (cnt_idx == count)
		{
			copy_l = length + tmp_idx - count * mem_size;
		}
		
		memcpy_toio(((char*)dma_info.tx_buffer) + dma_info.tx_idx, ((char*)xmit->buf) + xmit->tail , copy_l);

		#if ARHT_DMA_LOG
		ch = ((char*)xmit->buf) + xmit->tail;
		ch1 = ((char*)dma_info.tx_buffer) + dma_info.tx_idx;
		printk("[UART_DMA] CHAR: %c%c%c%c%c\n", *ch,*(ch+1), *(ch+2),*(ch+3),*(ch+4));
		printk("[UART_DMA] CHAR: %c%c%c%c%c\n", *ch1,*(ch1+1), *(ch1+2),*(ch1+3),*(ch1+4));
		printk("[UART_DMA] tail :%d, head :%d, copy_l :%d \n",xmit->tail, xmit->head, copy_l);
		print_register(port);
		#endif

		dma_idx = ((rg_tx_idx >> 16)+ copy_l) & (mem_size - 1);
		cpu_idx = rg_tx_idx & 0xFFFF;
		
		UART_WRL(ARHT_TX_IDX + port->iobase, dma_idx << 16 |cpu_idx);
		
		xmit->tail = (xmit->tail + copy_l) & (UART_XMIT_SIZE - 1);
		dma_info.tx_idx += copy_l;
		port->icount.tx += copy_l;
		
		if (dma_info.tx_idx >=  mem_size)
			dma_info.tx_idx -= mem_size;
...
int arht8250_rx_dma(struct uart_port *port)
{
	unsigned int length =0, tmout=0, idx = 0, depth, i;
	unsigned char *data , *data1;
	unsigned int cpu_idx = (UART_RDL( ARHT_RX_IDX + port->iobase)& 0xFFFF0000) >> 16;
	unsigned int dma_idx = (UART_RDL( ARHT_RX_IDX + port->iobase)& 0x0000FFFF);
	unsigned int flg;
	
	struct tty_struct *tty = port->state->port.tty;
	
	printk("[UART_DMA] length :%d, cpu_idx :%d, dma_idx : %d\n",length, cpu_idx , dma_idx);

	
	if(dma_idx == cpu_idx)
	{
		printk("[UART_DMA] No data");
		return 0;
	}
	
	print_register(port);
	dma_info.rx_idx = dma_info.rx_idx % mem_size;

	length = dma_idx - cpu_idx;
	data = ((char*)dma_info.rx_buffer) + dma_info.rx_idx;
	data1 = ((char*)dma_info.rx_buffer);
	#if ARHT_DMA_LOG
	printk("[UART_DMA] CHAR: %c%c%c%c%c\n", *data,*(data+1), *(data+2),*(data+3),*(data+3));
	printk("[UART_DMA] CHAR: %c%c%c%c%c\n", *data1,*(data1+1), *(data1+2),*(data1+3),*(data1+4));
	printk("[UART_DMA] length :%d, cpu_idx :%d, dma_idx : %d, dma_idx : %d\n",length, cpu_idx , dma_idx, dma_info.rx_idx);
	#endif

	depth = (UART_RDL( ARHT_RX_IDX + port->iobase)& 0xFFFF0000) >> 24;
	tmout = (UART_RDL( ARHT_RX_PARA + port->iobase)& 0x00FFFF00) >> 8 ;


	tty_insert_flip_string(&port->state->port, data, length);
	port->icount.rx += length;

	tty_flip_buffer_push(&port->state->port);

	dma_info.rx_idx += length;
	
	idx = ((((UART_RDL(ARHT_RX_IDX + port->iobase) >> 16)+ length)  << 16) | (UART_RDL(ARHT_RX_IDX + port->iobase)& 0x0000FFFF));
	UART_WRL(ARHT_RX_IDX + port->iobase, idx);
	#if ARHT_DMA_LOG
	print_register(port);
	#endif
...
int arht8250_request_dma(struct uart_port *port)
{
	unsigned int cfg_dma_ctrl = 0;
	unsigned int cfg_dma_para = 0;
	
	cfg_dma_ctrl = csr_dma_tx_en | csr_dma_rx_en  ;
		
	UART_WRL(ARHT_DMA_CTRL+ port->iobase, cfg_dma_ctrl);
	
	cfg_dma_para = (dma_depth << 20)|(UART_RDL(ARHT_TX_PARA + port->iobase)& 0x003FFFFF);

	UART_WRL(ARHT_TX_PARA + port->iobase, cfg_dma_para);
	UART_WRL(ARHT_RX_PARA + port->iobase, cfg_dma_para);

	UART_WRL(ARHT_RX_BSADDR + port->iobase, dma_info.rx_phys);
	UART_WRL(ARHT_TX_BSADDR + port->iobase, dma_info.tx_phys);

	printk("[UART_DMA] dma enable 0x%x \n ", UART_RDL(ARHT_DMA_CTRL + port->iobase));
	return 0;
	
}
EXPORT_SYMBOL_GPL(arht8250_request_dma);

void arht8250_release_dma(struct uart_port *port)
{
	unsigned int cfg_dma_ctrl = 0;

	cfg_dma_ctrl = ~csr_dma_tx_en | ~csr_dma_rx_en | csr_dma_tx_rst | csr_dma_rx_rst;

	UART_WRL(ARHT_DMA_CTRL + port->iobase, cfg_dma_ctrl);

	printk("[UART_DMA] dma disable 0x%x \n ", UART_RDL(ARHT_DMA_CTRL + port->iobase));
}
EXPORT_SYMBOL_GPL(arht8250_release_dma);
//dma_end
...
#ifdef TCSUPPORT_HIGH_SPEED_UART
static uint32_t tc3162_get_uclk_20M_high_speed(uint32_t baud)
{
    uint32_t uclk_20M = 0;
    
	if (baud <= 115200) {
    	uclk_20M = (((baud / 100) * UART_XYD_Y / ((UART_CRYSTAL_CLK_20M / 100) / UART_CRYSTAL_CLK_DIV_HIGH_SPEED)) * 
                    (UART_BRDH_20M << 8 | UART_BRDL_20M) * 8);
    } else {
        /* divided by 100 to aviod uint32_t overflow */
        uclk_20M = (((baud / 100) * UART_XYD_Y_HIGH_SPEED / ((UART_CRYSTAL_CLK_20M / 100) / UART_CRYSTAL_CLK_DIV_HIGH_SPEED)) * 
                    (UART_BRDH_20M << 8 | UART_BRDL_20M) * 8);
    }
    ...
    struct ktermios *termios, struct ktermios *old)
{
	...
	{
		if(port->unused[1] & UART_HWFC_ENABLE)
	...
	if (termios->c_cflag & UART_CFLAG_DEVON) {
		if(!isFPGA) {
			...
			if (port->unused[0] == UART_2) {
				...
				if(port->unused[1] == UART_HWFC_ENABLE) {
					SET_IOMUX_CTRL_BIT(RG_GPIO_UART2_CTS_RTS_MODE, 1);
				}
			} else if(port->unused[0] == UART_3) {
				SET_IOMUX_CTRL_BIT(RG_GPIO_UART3_MODE, 1);
				if(port->unused[1] == UART_HWFC_ENABLE) {
					SET_IOMUX_CTRL_BIT(RG_GPIO_UART3_CTS_RTS_MODE, 1);
				}
			} else if(port->unused[0] == UART_4) {
				SET_IOMUX_CTRL_BIT(RG_GPIO_UART4_MODE, 1);
			} else if(port->unused[0] == UART_5) {
				SET_IOMUX_CTRL_BIT(RG_GPIO_UART5_MODE, 1);
			}
#else
			SET_IOMUX_CTRL_BIT(RG_GPIO_UART2_MODE, 1);
#endif
		...
		if(!isFPGA) {
			...
			if (port->unused[0] == UART_2) {
				...
				SET_IOMUX_CTRL_BIT(RG_GPIO_UART2_CTS_RTS_MODE, 0);
			} else if(port->unused[0] == UART_3) {
				SET_IOMUX_CTRL_BIT(RG_GPIO_UART3_MODE, 0);
				SET_IOMUX_CTRL_BIT(RG_GPIO_UART3_CTS_RTS_MODE, 0);
			} else if(port->unused[0] == UART_4) {
				SET_IOMUX_CTRL_BIT(RG_GPIO_UART4_MODE, 0);
			} else if(port->unused[0] == UART_5) {
				SET_IOMUX_CTRL_BIT(RG_GPIO_UART5_MODE, 0);
			}
#else
			SET_IOMUX_CTRL_BIT(RG_GPIO_UART2_MODE, 0);
#endif
	...
	div_y = UART_XYD_Y;
#ifdef TCSUPPORT_HIGH_SPEED_UART
    if (ecnt_uarts[port->unused[0]].uart_type == UART_HIGH_SPEED) {
        div_x = tc3162_get_uclk_20M_high_speed(baud);
        if (baud > 115200) {
            
div_y = UART_XYD_Y_HIGH_SPEED;
        }
    } else {
...
#ifdef TCSUPPORT_CPU_ARMV8
	if (isEN7523 || isEN7581)
		printk("Init UART(s), still need to trun on for using.");
#else
	if(isEN751221) {
					...
					const char *options)
{
	...
	ecnt_uarts[device->port.unused[0]].base = device->port.membase;
...
#ifdef TCSUPPORT_HIGH_SPEED_UART
static void set_uart_types(void)
{
    int i = 0;
    if (high_speed_uarts != 0 && sizeof(high_speed_uarts) != 0) {
        for (i = 0; i < sizeof(high_speed_uarts)/sizeof(high_speed_uarts[0]); ++i) {
            ecnt_uarts[high_speed_uarts[i]].uart_type = UART_HIGH_SPEED;
        }
    }
}
#endif
...
static int ecnt_uart2_drv_probe(struct platform_device *pdev)
{
	...
	for(i = 0; i < TC3162_NR_PORTS; i++) {
        ecnt_uarts[UART_2 + i].uart_id = UART_2 + i;
        ecnt_uarts[UART_2 + i].uart_type = UART_NORM;

		platform_set_drvdata(pdev, &ecnt_uarts[UART_2 + i]);
		res = platform_get_resource(pdev, IORESOURCE_MEM, i);

		ecnt_uarts[i + 1].base = devm_ioremap_resource(&pdev->dev, res);
		if (IS_ERR(ecnt_uarts[i + 1].base)) {
			return PTR_ERR(ecnt_uarts[i + 1].base);
		}

		ecnt_uarts[i + 1].dev = &pdev->dev;

		ecnt_uarts[i + 1].irq  = platform_get_irq(pdev, i);

		if (ecnt_uarts[i + 1].irq <= 0)
			return ecnt_uarts[i + 1].irq;

		tc3162ser_ports[i].irq = ecnt_uarts[i + 1].irq ;
		tc3162ser_ports[i].iobase = ecnt_uarts[i + 1].base;
	}
#ifdef TCSUPPORT_HIGH_SPEED_UART
	printk ("\033[32;1m TCSUPPORT_HIGH_SPEED_UART is open :%d\n\033[0m", TC3162_NR_PORTS);
    set_uart_types();
#endif
	for(i = 0; i < TC3162_NR_PORTS; i++) {
		...
		div_y = UART_XYD_Y;
#ifdef TCSUPPORT_HIGH_SPEED_UART
        if (ecnt_uarts[UART_2 + i].uart_type == UART_HIGH_SPEED) {
            div_x = tc3162_get_uclk_20M_high_speed(UART_BAUDRATE);
        } else {
	...
	if (ecnt_uarts[UART_2 + i].uart_type == UART_HIGH_SPEED) {
			...
			cfg_dma_ctrl = csr_dma_tx_rst | csr_dma_rx_rst ;

			printk ("\033[32;1m ARHT_DMA_CTRL %d\n\033[0m", i);
			UART_WRL(ARHT_DMA_CTRL+ port->iobase, cfg_dma_ctrl);
			
			dma_info.tx_idx = 0;
			dma_info.rx_idx = 0;

			dma_info.tx_buffer = dma_alloc_coherent(&pdev->dev, mem_size , &(dma_info.tx_phys) , GFP_KERNEL | __GFP_ZERO);
			
			if (dma_info.tx_buffer == NULL)
			{
				printk("\nFAILED: alloc tx_buffer\n");
				goto err;
			}
			dma_info.rx_buffer = dma_alloc_coherent(&pdev->dev, mem_size , &(dma_info.rx_phys) , GFP_KERNEL | __GFP_ZERO);
			if (dma_info.rx_buffer == NULL)
			{
				printk("\nFAILED: alloc rx_buffer\n");
				goto err;
			}
		...
		err:
			if (dma_info.tx_buffer)
				dma_free_coherent(&pdev->dev, mem_size , dma_info.tx_buffer ,dma_info.tx_phys);
			if (dma_info.rx_buffer)
				dma_free_coherent(&pdev->dev, mem_size , dma_info.rx_buffer ,dma_info.rx_phys);
		}
#endif		
	}
	return ret;
