...
static char i2c_driver_version[]= "v1.01";
			       ...
			       int num)
{
	...
	if (i2c->clk_div > SM0CTL0_CLK_DIV_MAX){
		dev_warn(i2c->dev, "clock-frequency div > 4095, set to max value:4095\n");
		i2c->clk_div = SM0CTL0_CLK_DIV_MAX;
	}
	
	read_data = ioread32(i2c->base + REG_SIFMCTL0_REG) & ~SM0CTL0_CLK_DIV_MASK;
	iowrite32(((i2c->clk_div << 16) & SM0CTL0_CLK_DIV_MASK) |read_data, i2c->base + REG_SIFMCTL0_REG);
...
static int airoha_i2c_probe(struct platform_device *pdev)
{
	...
	i2c = devm_kzalloc(&pdev->dev, sizeof(struct airoha_i2c), GFP_KERNEL);
	...
	if (bus_freq == 0) {
		dev_warn(i2c->dev, "clock-frequency 0, set standard frequency 100k\n");
