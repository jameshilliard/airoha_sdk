...
static int mtk_pcie_startup_port_v2(struct mtk_pcie_port *port)
{
	...
	val = readl(port->base + PCIE_MMIO_CTRL);
	printk("===orig=====PCIE_MMIO_CTRL %x=======\n", val);		
	val |= 1;
	writel(val, port->base + PCIE_MMIO_CTRL);
	printk("===after====PCIE_MMIO_CTRL %x=======\n", val);		
...
int pcibios_map_irq(const struct pci_dev *dev, u8 slot, u8 pin)
{
	...
	printk("\n========pcibios_map_irq === slot=%d, irq0=%d, irq1=%d ===========", slot, irq0, irq1);		
