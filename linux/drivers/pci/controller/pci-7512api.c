...
#define isRC0_LINKUP		((regRead_PCIe(0x1fc00154) & 0x100) ? 1 : 0)  
#define isRC1_LINKUP		((regRead_PCIe(0x1fc20154) & 0x100) ? 1 : 0)	
#define isRC2_LINKUP		((regRead_PCIe(0x1fc40154) & 0x100) ? 1 : 0)	
#endif
...
//		return -1;	
	...
	if (idx == PCIE_DEV_RC0){
		...
		bus = 0; dev = 1; func = 0;
	}else if (idx == PCIE_DEV_RC2){
		bus = 0; dev = 2; func = 0;
	}else if (idx == PCIE_DEV_EP0){
		bus = 1; dev = 0; func = 0;
	}else if (idx == PCIE_DEV_EP1){
		bus = 2; dev = 0; func = 0;
	}else if (idx == PCIE_DEV_EP2){
		bus = 3; dev = 0; func = 0;
	}else{
		return -1;
	}
...
static int pcie_set_aspm(int idx,int sw)
{
	...
	if (idx == PCIE_DEV_RC2)
		return pcie_set_aspm_ext(0,2,0,sw);
	...
	if (idx == PCIE_DEV_EP2 && isRC2_LINKUP)
		return pcie_set_aspm_ext(3,0,0,sw);
...
static int pcie_get_aspm(int idx)
{
	...
	if (idx == PCIE_DEV_RC2)
		return pcie_get_aspm_ext(0,2,0);
	...
	if (idx == PCIE_DEV_EP2 && isRC2_LINKUP)
		return pcie_get_aspm_ext(3,0,0);
...
static int pcie_set_speed(int idx,unsigned int mode)
{
	...
	if (idx != PCIE_DEV_EP0 && idx != PCIE_DEV_EP1 && idx != PCIE_DEV_EP2)
		return -1;
  ...
  int pcie_api_set_phy_dbg(struct ecnt_pcie_data *data)
{
 ...
 {
#if defined(TCSUPPORT_CPU_EN7581) && defined(TCSUPPORT_AUTOBENCH)
	 data->retValue = pcie_7581_slt_test();
#else
	 data->retValue =0;
#endif
