...
void mt7512_pcie_reset(void)
{
	...
	{
		tmp = regRead_PCIe(0x1fc00080);
		printk("\n ===== before set 1fc00080=%x \n",tmp);
		regWrite_PCIe(0x1fc00080, (tmp &(~(1<<13))));
		tmp = regRead_PCIe(0x1fc00080);
		printk("\n ===== after set 1fc00080=%x \n",tmp);
	}
	if(isAN7581CT)
	{
		virtAddr1 = ioremap((phys_addr_t)0x1fc20080, 4);
		tmp = readl(virtAddr1);
		printk("\n ===== before set 1fc20080=%x \n",tmp);
		writel((tmp &(~(1<<13))), virtAddr1);
		tmp = readl(virtAddr1);
		printk("\n ===== after set 1fc20080=%x \n",tmp);
	}
...
#if 1 //LTSSM monitor
	u32 time0[CHECK_LEN], time1[CHECK_LEN], ltssm0[CHECK_LEN], ltssm1[CHECK_LEN];
	u32 ltssm0_cnt, ltssm1_cnt;
	...
	while(monitor_cnt <  M_CNT) // monitor about 50ms, every read/write action in while loop affact 1 tick time.
	{
		...
		ltssm0_now = regRead_PCIe(0x1fc00150) & 0x1f000000;
		
		virtAddr = ioremap ((phys_addr_t)0x1fc20150,4);// guess  this read take about 2us....
		ltssm1_now = readl(virtAddr) & 0x1f000000;

		if((ltssm0_now != ltssm0_last) && (ltssm0_cnt < CHECK_LEN))
		{
		...
		if((ltssm1_now != ltssm1_last) && (ltssm1_cnt < CHECK_LEN))
		{
	...
	{
		printk("%05dus, lane0: 0x%x\n", time0[x]*2, ltssm0[x] >> 24);
	}
	printk("lane1\n");
	for(x=0; x<ltssm1_cnt; x++)
	{
		printk("%05dus, lane1: 0x%x\n", time1[x]*2, ltssm1[x] >> 24);
	}
	...
	if(1==serdes_ret)//port0,1lane
	{
		printk("serdes_ret = %d ,1lane ,need relink \n", serdes_ret );
		/*============check if err then relink begin=================================*/
	    //port0
	    
		tmp = regRead_PCIe(0x1fc00154);
		while((((tmp >> 16) !=0) || !(tmp & BIT(8)))&& (retry_cnt < RETRY_LIMIT))
		{
			/*before reset host,need to pull device low*/
			//port0 port1 0x1fb00088 bit29 bit 26
			tmp = regRead_PCIe(0x1fb00088);
			regWrite_PCIe(0x1fb00088, (tmp & (~(1<<29) )));

			//rc0 reset
			tmp = regRead_PCIe(0x1fb00834);
			regWrite_PCIe(0x1fb00834, (tmp | (1<<26)));
			...
			tmp = regRead_PCIe(0x1fc00154);
			printk("retry %d: 1fc00154 = 0x%x , 1fc00150 = %x, 1fc00018 = %x \n", retry_cnt, tmp, regRead_PCIe(0x1fc00150), regRead_PCIe(0x1fc00018) );
		...
		while((((tmp >> 16) !=0) || !(tmp & BIT(8))) && (retry_cnt < RETRY_LIMIT))
		{
			...
			//port0 port1 0x1fb00088 bit29 bit 26
			tmp = regRead_PCIe(0x1fb00088);
			regWrite_PCIe(0x1fb00088, (tmp & (~(1<<26) )));

			//rc0 reset
			tmp = regRead_PCIe(0x1fb00834);
			regWrite_PCIe(0x1fb00834, (tmp | (1<<27)));
			...
			virtAddr = ioremap ((phys_addr_t)0x1fc20338,4); //preset to use (final)
			writel(0x1018020F ,virtAddr);
			...
			printk("retry %d : 1fc20154 = 0x%x , 1fc20150 = %x, 1fc20018 = %x \n",retry_cnt,  tmp , tmp1, tmp2 );

			retry_cnt++;
		}
    	/*============check if err then relink end===============================================================*/
	}
	else
	{
		printk("serdes_ret = %d ,2lane or no PCIe,don't need relink \n", serdes_ret );
	}
...
static int mtk_pcie_parse_port(struct mtk_pcie_port *port)
{
	...
	printk("\n=====pcie_parse_port=22===base_num=%d===port->reg_base=%x===========",base_num,port->reg_base);
	...
	printk("\n=====pcie_parse_port=33==ecnt_pcie->mac0_base=%lx=%lx=%lx========\n",ecnt_pcie->mac0_base,ecnt_pcie->mac1_base,ecnt_pcie->mac2_base);
...
{	
	...
	if (isAN7581ST || isAN7581FD || isAN7581DT || isAN7552ST)
		return 1;
...
int pcibios_map_irq(const struct pci_dev *dev, u8 slot, u8 pin)
{
	...
	irq2=ecnt_pcie->irq_pcie2;	
	
	printk("\n========pcibios_map_irq === id=%d, irq0=%d, irq1=%d, irq2=%d ===========", id, irq0, irq1, irq2);		
	...
	if(isFPGA)
{
	...
	regWrite_PCIe(0x1fc00140, 0x1f0000);
	check_reg = regRead_PCIe(0x1fc01000);
	printk("\n===============PCIe check===tmp_reg=========%x===========",check_reg);
	if(0x689914c3!=check_reg)
		return 0;	
}
	...
	if(0==base_num)
		{
			...
			if(!isFPGA)
				{
					serdes_ret = get_serdes_interface_sel(ECNT_EVENT_SERDES_SEL_WIFI1);
					printk("7581 wifi_serdes_select0: serdes_ret=%d !\n",serdes_ret);
	...
	else if(1==base_num)
		{
			...
			if(!isFPGA)
				{
					serdes_ret = get_serdes_interface_sel(ECNT_EVENT_SERDES_SEL_WIFI2);
					printk("7581 wifi_serdes_select1: serdes_ret=%d !\n",serdes_ret);
					if((0==serdes_ret)||(1==serdes_ret))
						printk("7581 wifi_serdes_select1 match serdes_select0 !\n");
					else
						printk("7581 wifi_serdes_select1 !!!!!!not match!!!!! serdes_select0 !\n");
				}
			#endif
		}
	else if(2==base_num)
		{
			...
			if(!isFPGA)
				{
					serdes_ret = get_serdes_interface_sel(ECNT_EVENT_SERDES_SEL_USB2);
					printk("7581 wifi_serdes_select2: serdes_ret=%d !\n",serdes_ret);
					if(1==serdes_ret)
						pcie_phy_init(2);
				}
			#endif
