...
void Rx_PR_Fw_Pre_Cal(char speed, char lane)
{
	...
	{
		Reg_R_then_W(lane_offset_dig + 0x1fa5b174, cdr_pr_idac_tmp, 10, 0 ); //rg_ipath_idac = cdr_pr_ida, 20230525 Biker add
...
//*******  JCPLL SETTING (phase 1, NO SSC for K TXPLL)
 ...
 RG_PXP_2L_JCPLL_TCL_VTP_EN.hal.rg_pxp_2l_jcpll_spare_l = 0x20; //JCPLL: Wait BG ready 8抙00=>8抙20 
...
//*******  PR / RX mode setting, divider, etc? ADD_DIG_RESERVE_27.dat.value = Reg_R(PCIE_PMA0, PMA_OFFSET, _ADD_DIG_RESERVE_27);
 ...
 if(load_k_en == 1)
 {
	...
	//>>>>  Workaround for gen1 auto K (20230525)
	 SS_RX_FLL_b.dat.value = Reg_R(PCIE_PMA0, PMA_OFFSET, _SS_RX_FLL_b);
	 SS_RX_FLL_b.hal.rg_load_en = 0x1; 
	 Reg_W(PCIE_PMA0, PMA_OFFSET, _SS_RX_FLL_b, SS_RX_FLL_b.dat.value); // write addr: 0x1FA5B19C = 0x1

	 SS_RX_FLL_b.dat.value = Reg_R(PCIE_PMA1, PMA_OFFSET, _SS_RX_FLL_b);
	 SS_RX_FLL_b.hal.rg_load_en = 0x1; 
	 Reg_W(PCIE_PMA1, PMA_OFFSET, _SS_RX_FLL_b, SS_RX_FLL_b.dat.value); // write addr: 0x1FA5C19C = 0x1

	 ADD_DIG_RESERVE_14.dat.value = Reg_R(PCIE_PMA0, PMA_OFFSET, _ADD_DIG_RESERVE_14);
	 tmp = ADD_DIG_RESERVE_14.hal.rg_dig_reserve_14;
	 tmp = (0x0<<16) | (tmp & (~(0x1<<16)));
	 ADD_DIG_RESERVE_14.hal.rg_dig_reserve_14 = tmp;
	 Reg_W(PCIE_PMA0, PMA_OFFSET, _ADD_DIG_RESERVE_14, ADD_DIG_RESERVE_14.dat.value); // write addr: 0x1FA5B8C0 = 0x100648

	 ADD_DIG_RESERVE_14.dat.value = Reg_R(PCIE_PMA1, PMA_OFFSET, _ADD_DIG_RESERVE_14);
	 tmp = ADD_DIG_RESERVE_14.hal.rg_dig_reserve_14;
	 tmp = (0x0<<16) | (tmp & (~(0x1<<16)));
	 ADD_DIG_RESERVE_14.hal.rg_dig_reserve_14 = tmp;
	 Reg_W(PCIE_PMA1, PMA_OFFSET, _ADD_DIG_RESERVE_14, ADD_DIG_RESERVE_14.dat.value); // write addr: 0x1FA5C8C0 = 0x100648
	//<<<< Workaround for gen1 auto K (20230525)
	...
	//>>>>  Workaround for gen1 auto K (20230525)
	 ADD_DIG_RESERVE_14.dat.value = Reg_R(PCIE_PMA0, PMA_OFFSET, _ADD_DIG_RESERVE_14);
	 tmp = ADD_DIG_RESERVE_14.hal.rg_dig_reserve_14;
	 tmp = (0x0<<20) | (tmp & (~(0x1<<20)));
	 ADD_DIG_RESERVE_14.hal.rg_dig_reserve_14 = tmp;
	 Reg_W(PCIE_PMA0, PMA_OFFSET, _ADD_DIG_RESERVE_14, ADD_DIG_RESERVE_14.dat.value); // write addr: 0x1FA5B8C0 = 0x648

	 ADD_DIG_RESERVE_14.dat.value = Reg_R(PCIE_PMA1, PMA_OFFSET, _ADD_DIG_RESERVE_14);
	 tmp = ADD_DIG_RESERVE_14.hal.rg_dig_reserve_14;
	 tmp = (0x0<<20) | (tmp & (~(0x1<<20)));
	 ADD_DIG_RESERVE_14.hal.rg_dig_reserve_14 = tmp;
	 Reg_W(PCIE_PMA1, PMA_OFFSET, _ADD_DIG_RESERVE_14, ADD_DIG_RESERVE_14.dat.value); // write addr: 0x1FA5C8C0 = 0x648

	udelay(10);

	 ADD_DIG_RESERVE_14.dat.value = Reg_R(PCIE_PMA0, PMA_OFFSET, _ADD_DIG_RESERVE_14);
	 tmp = ADD_DIG_RESERVE_14.hal.rg_dig_reserve_14;
	 tmp = (0x1<<20) | (tmp & (~(0x1<<20)));
	 ADD_DIG_RESERVE_14.hal.rg_dig_reserve_14 = tmp;
	 Reg_W(PCIE_PMA0, PMA_OFFSET, _ADD_DIG_RESERVE_14, ADD_DIG_RESERVE_14.dat.value); // write addr: 0x1FA5B8C0 = 0x100648
 ...
 // check ro_idacf, reset CDR if idacf value not match
 u32 lane_offset_dig, retry_cnt;
 if(load_k_en == 1)
 {
	
	for(lane_offset_dig=0; lane_offset_dig<0x2000; lane_offset_dig = lane_offset_dig + 0x1000)
		for(retry_cnt=0; retry_cnt<5; retry_cnt++)
		{

			tmp = Reg_R(lane_offset_dig + 0x1fa5b000, 0, 0x8c4);
			printk("lane_%d apply ro_idacf = %x\n",lane_offset_dig>>12, tmp);

			if ((Reg_R(lane_offset_dig + 0x1fa5b000, 0, 0x174 ) & 0x7ff ) !=  tmp)
			{
				if(lane_offset_dig ==0)
				{
					//PCIe_G3_L0_Relink_Set();
					 ADD_DIG_RESERVE_14.dat.value = Reg_R(PCIE_PMA0, PMA_OFFSET, _ADD_DIG_RESERVE_14);
					 tmp = ADD_DIG_RESERVE_14.hal.rg_dig_reserve_14;
				...
				printk("lane_%d ro_idacf not match!\n", lane_offset_dig>>12); //wait  ~10ms to see if idacf change		
				
			}else
			{			
				//printk("lane_%d ro_idacf check ok\n", lane_offset_dig>>12);
				break;
			}
		}
...
void PCIe_G3_L0_Relink_Set(void)
{
	...
	//>>>>  if gen1 auto K idacf not match, reset CDR (20230906)
	 rg_type_t(HAL_ADD_DIG_RESERVE_14) ADD_DIG_RESERVE_14;	
	 rg_type_t(HAL_SS_DA_XPON_PWDB_0) SS_DA_XPON_PWDB_0;
	 u32 tmp;
...
static int pcie_phy_drv_probe(struct platform_device *pdev)
{
	...
	printk("PCIe phy driver version: 7581.10.20230911 \n reset CDR if idacf value not match\n");	//reset CDR if idacf value not match
