...
int irq0 = 0; /* PCIe Port0 irq */

/*====================regs==================*/
u32 get_pcie_mac0_data(u32 reg)
{
	return readl(mac0_base+ reg);
}

void set_pcie_mac0_data(u32 reg, u32 val)
{
	writel(val, mac0_base + reg); 
}
...
{	
	...
	switch(reg&0xffff0000)
		...
		case 0x1fc20000:
			...
			break;
		case 0x1fc30000:
			val=get_pcie_mac0_data(reg&0x1ffff);
			break;
		}
	return val;		  
}		
...
{                                                	
	...
	switch(reg&0xffff0000)
		...
		case 0x1fc20000:
			...
			break;
		case 0x1fc30000:
			set_pcie_mac0_data(reg&0x1ffff,val);
			break;
		}
}
...
{	
    ...
    //7581MSI all port
	msg_addr = mac0_addr + PCIE_MSI_SET_BASE_REG;
...
void mt7512_pcie_reset(void)
{
	...
	regWrite_PCIe(0x1fc30044, 0x23020133);
	regWrite_PCIe(0x1fc35030, 0x50500032);
		pcie_phy_init(0);
		pcie_phy_init(1);
	}
	...
	regWrite_PCIe(0x1fc20100, 0x50500032);
	regWrite_PCIe(0x1fc20338, 0x1018020F);
	...
	if(!isFPGA)
	{
		pcie_PhyDebug(0);//monitor LTSSM
		/*wait link up*/
		mdelay(800); 
		pcie_PhyDebug(1);//Link status reg
	}
	else
	{
	/*wait link up*/
...
void mt7512_pcie_reset0(void)
{
	...
	if(!isFPGA)
	{
		Get_PCIe_Debug_Selector(0);
	}
	
	//only reset 1time in port0 init
    tmp = regRead_PCIe(0x1fb00834);
    regWrite_PCIe(0x1fb00834, (tmp | (1<<29)));
    mdelay(100);
    regWrite_PCIe(0x1fb00834, (tmp & (~(1<<29))));
    ...
    tmp = regRead_PCIe(0x1fb00834);
    regWrite_PCIe(0x1fb00834, (tmp | ( 1<<26 )));//bit 26 pcie0
    mdelay(100);

	/*before reset host,need to pull device low*/
	//port0 port1 0x1fb00088 bit29 bit 26
	tmp = regRead_PCIe(0x1fb00088);
	regWrite_PCIe(0x1fb00088, (tmp & (~(1<<29 ))));//bit 29 pcie0
	mdelay(1);
	...
	if(!isFPGA)
	{
		PCIe_LTSSM_Monitor_Port(0);//monitor LTSSM		
		/*wait link up*/
		mdelay(400); 		
		PCIe_Link_Status_Check_Port(0);//Link status reg
		PCIe_GDMP_Config(0);
	}
	else
	{
...
static int mtk_pcie_startup_port(struct mtk_pcie_port *port)
{
	...
	dev_info(port->dev, "pcie rc 0 linkup success\n");
...
static int mtk_pcie_setup_irq(struct mtk_pcie_port *port)
{
	...
	port->irq = platform_get_irq(pdev, 0);
	printk("=======pcie_setup_irq0: i = 0, irq_msi = %d ",  port->irq);				
	irq0 = port->irq;
...
static int mtk_pcie_parse_port(struct mtk_pcie_port *port)
{
	...
	printk("\n=====pcie_parse_port=11===base_num0===port->reg_base=%x===========",port->reg_base);


	mac0_base = port->base;
#ifdef TCSUPPORT_PCIE_MSI
	mac0_addr = port->reg_base;
#endif

	printk("\n=====pcie_parse_port=22==mac0_base=%lx===mac0_addr=%lx=======\n",mac0_base,mac0_addr);
...
static int mtk_pcie_setup(struct mtk_pcie_port *port)
{
	...
	/*===========for PCIe pbus setting begin============================================ */	
	printk("===AN7583 Pbus for PCIe init===");
...
int pcibios_map_irq0(const struct pci_dev *dev, u8 slot, u8 pin)
{
	
	printk("\n========pcibios_map_irq ===  irq0=%d ===========", irq0);		
...
static int mtk_pcie_probe(struct platform_device *pdev)
{
	...
	printk("\n===============pcie_probe 0=====for msi kernel API===========");
#endif
	if(isFPGA)
	{
		//check whether PCIe exsits or not
		regWrite_PCIe(0x1fc20140, 0x1f0000);
		check_reg = regRead_PCIe(0x1fc21000);
