...
ISREVENTREREAD:
	...
	//	} else if (atmRxIrqReport & LIRQ_TX_U_DONE) {
			...
			if( vc < ATM_VC_MAX ) { //COV, CID 139735
			    atmRxDescrp = atmRxVcDescrp[vc];
...
unsigned long long int res_time_sum_64; //COV, CID 139469
#endif
	...
	int count, int *eof, void *data)
{
	...
	if(oam_success_cnt){ //COV, CID 139469
		res_time_sum_64 = res_time_sum;
		do_div(res_time_sum_64, oam_success_cnt);  
		res_time_sum = (0xFFFFFFFF & res_time_sum_64);
	} else {
		res_time_sum = 0;
	}
	...
	unsigned long count, void *data)
{
	...
	ms = tick_acc;
	ms_64 = ms; //COV, CID 139442
	do_div(ms_64, one_tick_unit); 
	ms = (0xFFFFFFFF & ms_64);
   	res_time_sum += ms;
	if(res_time_min > ms)
   		res_time_min = ms;
...
static int tsarm_tcqos_write_proc(struct file *file, const char *buffer,
	unsigned long count, void *data){
	char qos_disc[11]; //COV, CID 139709
	int len;
...
static int tsarm_qosred_write_proc(struct file *file, const char *buffer,
	...
	char get_buf[33]; //COV, CID 139738
	uint32 reg_tmp;
...
static int tsarm_qosred_update_write_proc(struct file *file, const char *buffer,
	...
	if(count > sizeof(get_buf)-1)
		len = sizeof(get_buf)-1;//COV, CID 139718
	else
		len = count;
	/* use the copy_from_user function to copy buffer data to our get_buf */
	if(copy_from_user(get_buf, buffer, len))
...
#endif
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CT_COM)
extern u32 mld_snooping_packet_check;
extern u32 mld_enable;
extern u32 mld_debug;
extern u32 mld_detect_len;
extern u32 mld_max_rate;

static int mld_small_rate_control_read_proc(char *page, char **start, off_t off,
	int count, int *eof, void *data)
{
	int len;

	len = sprintf(page, "\nMLD Snooping packet check is [%d]\nMLD enable [%d] Debug [%d] Small packet detect size [%d] Small packet rate [%d] bps\n", mld_snooping_packet_check, mld_enable, mld_debug, mld_detect_len, mld_max_rate * HZ * 8);
...
static int mld_small_rate_control_write_proc(struct file *file, const char *buffer,
	unsigned long count, void *data)
{
	...
	if(sscanf(val_string, "%d %d %d %d %d", &mld_snooping_packet_check, &mld_enable, &mld_debug, &mld_detect_len, &mld_max_rate) != 5)
	{
		printk("mld_small_rate_control_write_proc: input error! need input 5 u32 parameter\nmld_snooping_packet_check, mld_enable, mld_debug, mld_detect_len, mld_max_rate\n");
		mld_max_rate = mld_max_rate / HZ / 8;
		return count;
	}
	mld_max_rate = mld_max_rate / HZ / 8;
	return count;
...
#endif
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CT_COM)
	tsarm_proc = create_proc_entry("tc3162/mld_small_rate_control", 0, NULL);
	tsarm_proc->read_proc = mld_small_rate_control_read_proc;
	tsarm_proc->write_proc = mld_small_rate_control_write_proc;
#endif/*TCSUPPORT_COMPILE*/
...
#endif
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CT_COM)
	remove_proc_entry("tc3162/mld_small_rate_control", NULL);
#endif/*TCSUPPORT_COMPILE*/
#ifdef CONFIG_QOS
#ifdef TCSUPPORT_SBTHROUGHPUT_ENHANCE
