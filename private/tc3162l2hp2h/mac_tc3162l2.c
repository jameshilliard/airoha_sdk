...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_DYNAMIC_VLAN)
int dyVLAN_Enable = 0;
dyVLAN_OUI_t dyVLAN_OUI[MAX_OUI];
dyVLAN_PORT_t dyVLAN_PORT[PORTNUM];

static int
dyVLAN_read_proc(char *buf, char **start, off_t off, int count,
                 int *eof, void *data)
{
	int len = 0;

	len = sprintf(buf, "%d-%d %d-%d %d-%d %d-%d\n", 
		dyVLAN_PORT[0].state, dyVLAN_PORT[0].pvc,
		dyVLAN_PORT[1].state, dyVLAN_PORT[1].pvc,
		dyVLAN_PORT[2].state, dyVLAN_PORT[2].pvc,
		dyVLAN_PORT[3].state, dyVLAN_PORT[3].pvc
		);
...
/* the received OUI format would be XX:XX:XX*/
static int 
dyVLAN_write_proc(struct file *file, const char *buffer,
	unsigned long count, void *data)
{
	int i;
	char strOUI[128];
	if (count > sizeof(strOUI) - 1)
		return -EINVAL;

	if (copy_from_user(strOUI, buffer, count))
		return -EFAULT;

	strOUI[count] = '\0';
	sscanf(strOUI, "%d %hhx:%hhx:%hhx %d %hhx:%hhx:%hhx %d %hhx:%hhx:%hhx %d %hhx:%hhx:%hhx %d %hhx:%hhx:%hhx %d", 
		&dyVLAN_Enable, 
		dyVLAN_OUI[0].oui, dyVLAN_OUI[0].oui+1, dyVLAN_OUI[0].oui+2, &(dyVLAN_OUI[0].pvc),
		dyVLAN_OUI[1].oui, dyVLAN_OUI[1].oui+1, dyVLAN_OUI[1].oui+2, &(dyVLAN_OUI[1].pvc),
		dyVLAN_OUI[2].oui, dyVLAN_OUI[2].oui+1, dyVLAN_OUI[2].oui+2, &(dyVLAN_OUI[2].pvc),
		dyVLAN_OUI[3].oui, dyVLAN_OUI[3].oui+1, dyVLAN_OUI[3].oui+2, &(dyVLAN_OUI[3].pvc),
		dyVLAN_OUI[4].oui, dyVLAN_OUI[4].oui+1, dyVLAN_OUI[4].oui+2, &(dyVLAN_OUI[4].pvc)

	);
	
	for(i = 0; i < MAX_OUI; i++){
		dyVLAN_PORT[i].state = 0;
	}
	return count;
}
#endif/*TCSUPPORT_COMPILE*/
...
#ifdef TCSUPPORT_KERNEL_API
	ledBlinkOn(LED_ETHER_100M_STATUS, 1);
#else
...
#ifdef TCSUPPORT_KERNEL_API
	ledBlinkOn(LED_ETHER_10M_STATUS, 1);
#else
...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_DYNAMIC_VLAN)
		uint8 *src_mac = oldMbp->data + ETH_ALEN;
		int i;
		if(swicVendor == SWIC_TC2206){
			uint8 pid = *(oldMbp->data+ETH_ALEN*2 +3) & 0x03; /* Ingress PID 3 bit */		
			if(dyVLAN_Enable){/* Dynamic VLAN is enabled */
				for(i = 0; i < MAX_OUI; i++){
					if(memcmp(src_mac, dyVLAN_OUI[i].oui, 3) == 0){
						dyVLAN_PORT[pid].state = 1;
						dyVLAN_PORT[pid].pvc = dyVLAN_OUI[i].pvc;
						break;
					}
				}
			}
...
#endif
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_DYNAMIC_VLAN)	
	struct proc_dir_entry *dyVLAN_proc;
#endif/*TCSUPPORT_COMPILE*/
...
#endif
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_DYNAMIC_VLAN)
	/* dynamic vlan related proc */
	dyVLAN_proc = create_proc_entry("tc3162/dyVLAN", 0, NULL);
	dyVLAN_proc->read_proc = dyVLAN_read_proc;
	dyVLAN_proc->write_proc = dyVLAN_write_proc;
#endif/*TCSUPPORT_COMPILE*/
...
#endif	
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_DYNAMIC_VLAN)	
	remove_proc_entry("tc3162/dyVLAN", 0);
#endif/*TCSUPPORT_COMPILE*/
#ifndef TC2031_SUPPORT
