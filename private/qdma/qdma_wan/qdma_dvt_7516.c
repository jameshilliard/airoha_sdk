...
/*make sure that all INTs  are bind to qdma_int_src*/
...
uint qdma_int_src = QDMA_INT1; /*default as QDMA_INT1*/
...
int qdma_set_dbg_cntr_info(QDMA_DBG_CNTR_T *dbgCntrPtr)
{
	...
	if(dbgCntrPtr->cntrIdx>=CONFIG_MAX_CNTR_NUM) {
		QDMA_ERR("Fault: dbg Cntr counter index should between 0 and %d\n", CONFIG_MAX_CNTR_NUM-1) ; 
		return -EINVAL ;
	}
	if(dbgCntrPtr->cntrSrc<0 || dbgCntrPtr->cntrSrc>=QDMA_DBG_CNTR_SRC_ITEMS) {
		QDMA_ERR("Fault: dbg Cntr source should between 0 and %d\n", QDMA_DBG_CNTR_SRC_ITEMS-1) ; 
		return -EINVAL ;
	}
...
static int tc_init(char subcmd[32], uint base)
{
    ...
    if(!strcmp(subcmd, "umac")) {
    	...
    	qdmaDisableInt(base, INT_MASK_EPON_MAC, qdma_int_src, QDMA_INT_ENABLE0) ;
    ...
    QDMA_MSG(DBG_ST, "CSR info: INFO:%.8x, GLG:%.8x, INT_MASK:%.8x\n", 
    														...
    														qdmaGetGlbCfg(base), 
    														qdmaGetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0)) ;
...
static int tc_loopback_basic_test(uint base, uint round , uint ring_index)
{
    ...
    uint i=0, j=0, curPktCnt=0, rx_done_int_cnt=0 ;
    uint txRingIdx=0, rxRingIdx=0, result=0 ;

    for(txRingIdx=0 ; txRingIdx<TX_RING_NUM; txRingIdx++) 
    {
        if( RX_RING_NUM == ring_index )
        {
            for(rxRingIdx=0 ; rxRingIdx<RX_RING_NUM; rxRingIdx++) 
            {
                qdmaSetLpbkToRXRing(base, rxRingIdx);
                printk("force to Rx Ring %d\n",rxRingIdx);
                for(j=0; j<round; j++) {					
                    ...
                    curPktCnt = 1+random32()%700 ;
                    curPattern = random32()%5 ;
                    qdma_st_packet_gen(channel, queue, curPktCnt, 0, pattern[curPattern], txRingIdx) ;
                    msleep(1000) ;
                    rx_done_int_cnt = gpQdmaPriv->counters.intRxDone[rxRingIdx] ;

                    printk("txRing:%d, rxRing:%d, Round:%d, Pattern:%.2x, Lens:48~2000, Tx Pkts:%d, Tx OK Pkts:%d, Rx OK Pkts:%d, Rx_DONE_INT:%d, tx_cpu_idx:%.8x, tx_dma_idx:%.8x", 
                    txRingIdx, rxRingIdx, j, pattern[curPattern], curPktCnt, counters[channel][queue].tx_frames, counters[channel][queue].rx_frames, rx_done_int_cnt, IO_GREG(QDMA_CSR_TX_CPU_IDX(base, txRingIdx)), IO_GREG(QDMA_CSR_TX_DMA_IDX(base, txRingIdx))) ;

                    if((curPktCnt == counters[channel][queue].rx_frames) && (curPktCnt == rx_done_int_cnt)){
            ...
            if(ring_index<0 || ring_index >= RX_RING_NUM ){
                QDMA_ERR("RX Ring Num should between 0 and %d.\n", (RX_RING_NUM-1)) ;
                return -EINVAL;
            }
            qdmaSetLpbkToRXRing(base, ring_index);
            printk("force to Rx Ring %d\n",ring_index);
            for(j=0; j<round; j++) 
            {					
                memset(counters, 0, sizeof(QDMA_DbgCounters_T)*CONFIG_QDMA_CHANNEL*CONFIG_QDMA_QUEUE) ;
                gpQdmaPriv->counters.intRxDone[ring_index] = 0 ;
                curPktCnt = 1+random32()%700 ;
                curPattern = random32()%5 ;
                qdma_st_packet_gen(channel, queue, curPktCnt, 0, pattern[curPattern], txRingIdx) ;
                msleep(1000) ;
                rx_done_int_cnt = gpQdmaPriv->counters.intRxDone[ring_index] ;

                printk("txRing:%d, rxRing:%d, Round:%d, Pattern:%.2x, Lens:48~2000, Tx Pkts:%d, Tx OK Pkts:%d, Rx OK Pkts:%d, Rx_DONE_INT:%d, tx_cpu_idx:%.8x, tx_dma_idx:%.8x", 
                txRingIdx, ring_index , j, pattern[curPattern], curPktCnt, counters[channel][queue].tx_frames, counters[channel][queue].rx_frames, rx_done_int_cnt, IO_GREG(QDMA_CSR_TX_CPU_IDX(base, txRingIdx)), IO_GREG(QDMA_CSR_TX_DMA_IDX(base, txRingIdx))) ;
...
/*for single test for debug*/
static int tc_loopback_basic_test_debug(uint base, uint npackets , uint ring_index, uint txRingIdx)
{
    uint channel = random32() % CONFIG_QDMA_CHANNEL ;
    uint queue = random32() % CONFIG_QDMA_QUEUE ;
    unchar curPattern=0, pattern[5] = {0x00, 0xFF, 0x5A, 0xA5, 0x01} ;
    uint j=0, curPktCnt=0, rx_done_int_cnt=0 ;
    uint rxRingIdx=0, result=0 ;

    qdmaSetLpbkToRXRing(base, ring_index);
    printk("from Tx%d force to Rx%d, GLB_CFG :%.8x \n", txRingIdx, ring_index, qdmaGetGlbCfg(base));
...
/*NO_TX_DSCP_INT & NO_RX_DSCP_INT*/
static int tc_csr1(uint base, uint packets, uint lens,int txRingIdx, int rxRingIdx)
{
    uint oldInt0 = qdmaGetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0) ; 
    uint oldInt1 = qdmaGetIntMask(base, qdma_int_src, QDMA_INT_ENABLE1) ; 
    uint channel = random32() % CONFIG_QDMA_CHANNEL ;
    uint queue = random32() % CONFIG_QDMA_QUEUE ;

    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0, (INT_MASK_NO_TX_CPU_DSCP | INT_MASK_TX0_DONE | 
    												      INT_MASK_NO_RX1_CPU_DSCP |INT_MASK_NO_RX0_CPU_DSCP)) ;
    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE1, INT_MASK_NO_RX_CPU_DSCP_2_15) ;

    qdmaSetLpbkToRXRing(base, rxRingIdx);
    printk("TestPath: From TxRing%d to RxRing%d.\n", txRingIdx , rxRingIdx);

    memset(counters, 0, sizeof(QDMA_DbgCounters_T)*CONFIG_QDMA_CHANNEL*CONFIG_QDMA_QUEUE) ;
    gpQdmaPriv->counters.intNoRxDscp[rxRingIdx] = 0 ;
    gpQdmaPriv->counters.intNoTxDscp[txRingIdx] = 0 ;
    gpQdmaPriv->devCfg.waitTxMaxNums = 0 ;
    gpQdmaPriv->devCfg.countDown = (random32()%(gpQdmaPriv->txDscpNum[txRingIdx]-1))+1 ;
    ...
    printk("INT_CSR:%.8x %.8x, Packets:%d, NO_TX_DSCP_INT Cnt:%d, NO_RX_DSCP_INT Cnt:%d, CntDownRound:%d\n\n", 
        qdmaGetIntStatus(base),qdmaGetIntStatus1(base), packets, gpQdmaPriv->counters.intNoTxDscp[txRingIdx], 
    ...
    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0, oldInt0) ;
    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE1, oldInt1) ;
    return 0;
}

/*notice macResource Limit*/
static int tc_csr2(uint base, uint packets, uint lens, int ringIdx)
{
    uint oldInt = qdmaGetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0) ; 
    uint channel = random32() % CONFIG_QDMA_CHANNEL ;
    uint queue = random32() % CONFIG_QDMA_QUEUE ;
    uint intNums = 0;

    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0, (INT_STATUS_TX0_DONE|INT_STATUS_HWFWD_DSCP_EMPTY)) ;

#if 0
    printk("int_enable0 mask:0x%08x\n", qdmaGetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0));
    printk("int_enable1 mask:0x%08x\n", qdmaGetIntMask(base, qdma_int_src, QDMA_INT_ENABLE1));
    printk("int_enable2 mask:0x%08x\n", qdmaGetIntMask(base, qdma_int_src, QDMA_INT_ENABLE2));
    printk("glbCfg:%.8x, delayTime %d\n\n", qdmaGetGlbCfg(base), delayTime);
#endif	
    memset(counters, 0, sizeof(QDMA_DbgCounters_T)*CONFIG_QDMA_CHANNEL*CONFIG_QDMA_QUEUE) ;
    ...
    intNums = packets/(CONFIG_HWFWD_DSCP_NUM+CONFIG_RX_DSCP_NUM[RING_IDX_0]);
    ...
    if((__st_check_counters() == 0) && (gpQdmaPriv->counters.intNoLinkDscp >= intNums) && (gpQdmaPriv->counters.intNoLinkDscp <= (intNums*2))) {
        QDMA_LOG("csr2: Test Result:Pass\n\n") ;
    } else {
        QDMA_LOG("csr2: Test Result:Failed\n\n") ;
    }
    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0, oldInt) ;
    return 0;
}

/*Done Bit check Coherent */
static int tc_done_bit_check_test(uint base)
{
    uint oldInt0 = qdmaGetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0) ; 
    uint oldInt1 = qdmaGetIntMask(base, qdma_int_src, QDMA_INT_ENABLE1) ; 
    uint oldInt2 = qdmaGetIntMask(base, qdma_int_src, QDMA_INT_ENABLE2) ; 
    ...
    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0, (INT_MASK_TX_COHERENT | INT_MASK_TX0_DONE | INT_MASK_RX0_COHERENT | INT_MASK_RX1_COHERENT)) ;
    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE2, INT_MASK_RX_COHERENT_2_15) ;
    qdmaDisableInt(base, (INT_MASK_NO_RX0_CPU_DSCP |INT_MASK_NO_RX1_CPU_DSCP | INT_MASK_RX0_DONE | INT_MASK_RX1_DONE), qdma_int_src, QDMA_INT_ENABLE0) ;
    qdmaDisableInt(base, INT_MASK_RX_DONE_2_15, qdma_int_src, QDMA_INT_ENABLE1) ;
    ...
    for(rxRingIdx=0; rxRingIdx<RX_RING_NUM; rxRingIdx++) 
    {
    	...
    	for(txRingIdx=0; txRingIdx<TX_RING_NUM; txRingIdx++) 
    	{
    		...
    		{
    			if(i==0)
    				qdmaSetGlbCfg(base, (qdmaGetGlbCfg(base) & (~GLB_CFG_CHECK_DONE))) ;
    			else
    				qdmaSetGlbCfg(base, (qdmaGetGlbCfg(base) | GLB_CFG_CHECK_DONE)) ;
    			...
    			if(txRingIdx == 0){
    				packets = 1 + random32() % (min(CONFIG_RX_DSCP_NUM[rxRingIdx], CONFIG_TX0_DSCP_NUM)-1) ;
    			}else{
    				packets = 1 + random32() % (min(CONFIG_RX_DSCP_NUM[rxRingIdx], CONFIG_TX1_DSCP_NUM)-1) ;
    			}
    				
    			round_cnt = 1 + random32() % 10 ;
...
done_bit_out:
    ...
    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0, oldInt0) ;
    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE1, oldInt1) ;
    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE2, oldInt2) ;
...
static int tc_csr4(uint base, uint packets, uint lens, int ringIdx)
{
    ...
    uint oldRxDly = qdmaGetRxDelayIntCfg(base, 0) ;
    uint channel = random32() % CONFIG_QDMA_CHANNEL ;
    uint queue = random32() % CONFIG_QDMA_QUEUE ;
    ...
    for(i=0, j=0 ; j<2 ; j++) {
        ...
        for( ; i<itemMax[j] ; i++) {
            ...
            printk("Rx Delay Pkt:%d, Time:%d*20us, DlyCSR:%.8x, Packets:%d, RX_INT Count:%d\n\n", 
                                        ((qdmaGetRxDelayIntCfg(base, 0)&DLY_INT_RXMAX_PINT_MASK)>>DLY_INT_RXMAX_PINT_SHIFT) ,
                                        ((qdmaGetRxDelayIntCfg(base, 0)&DLY_INT_RXMAX_PTIME_MASK)>>DLY_INT_RXMAX_PTIME_SHIFT),
                                        qdmaGetRxDelayIntCfg(base, 0), packets, gpQdmaPriv->counters.intRxDone[ringIdx]) ;
            if((i != 0) && (i != 5)) {
    ...
    qdmaSetRxDelayIntVal(base, 0, oldRxDly) ;
    return 0;
}
#endif

/*should be set delaytime in 7516 FPGA*/
static int tc_rx_dly_int_test(uint base, int rxRingIdx, int txRingIdx, int npackets, int lens)
{
	...
	uint result=0 ;
	int rxIdx=0, txIdx=0;
    uint csr[11] = {0, ((DLY_INT_RXDLY_INT_EN(rxRingIdx)) | (10<<DLY_INT_RXMAX_PINT_SHIFT(rxRingIdx)) | (0<<DLY_INT_RXMAX_PTIME_SHIFT(rxRingIdx))), 
                      ((DLY_INT_RXDLY_INT_EN(rxRingIdx)) | (20<<DLY_INT_RXMAX_PINT_SHIFT(rxRingIdx)) | (0<<DLY_INT_RXMAX_PTIME_SHIFT(rxRingIdx))), 
                      ((DLY_INT_RXDLY_INT_EN(rxRingIdx)) | (40<<DLY_INT_RXMAX_PINT_SHIFT(rxRingIdx)) | (0<<DLY_INT_RXMAX_PTIME_SHIFT(rxRingIdx))), 
                      ((DLY_INT_RXDLY_INT_EN(rxRingIdx)) | (50<<DLY_INT_RXMAX_PINT_SHIFT(rxRingIdx)) | (0<<DLY_INT_RXMAX_PTIME_SHIFT(rxRingIdx))), 
                    0, ((DLY_INT_RXDLY_INT_EN(rxRingIdx)) | (0<<DLY_INT_RXMAX_PINT_SHIFT(rxRingIdx)) | (20<<DLY_INT_RXMAX_PTIME_SHIFT(rxRingIdx))), 
                      ((DLY_INT_RXDLY_INT_EN(rxRingIdx)) | (0<<DLY_INT_RXMAX_PINT_SHIFT(rxRingIdx)) | (40<<DLY_INT_RXMAX_PTIME_SHIFT(rxRingIdx))), 
                      ((DLY_INT_RXDLY_INT_EN(rxRingIdx)) | (0<<DLY_INT_RXMAX_PINT_SHIFT(rxRingIdx)) | (80<<DLY_INT_RXMAX_PTIME_SHIFT(rxRingIdx))), 
                      ((DLY_INT_RXDLY_INT_EN(rxRingIdx)) | (0<<DLY_INT_RXMAX_PINT_SHIFT(rxRingIdx)) | (160<<DLY_INT_RXMAX_PTIME_SHIFT(rxRingIdx)))} ;
	
	rxIdx = rxRingIdx ;
	txIdx = txRingIdx ;
	oldRxDly = qdmaGetRxDelayIntCfg(base, rxIdx);

	qdmaSetLpbkToRXRing(base, rxIdx);
    ...
    for(i=0, j=0 ; j<2 ; j++) {
        ...
        for( ; i<itemMax[j] ; i++) {
			qdmaSetRxDelayIntVal(base, rxIdx, csr[i]);
			printk("csrVal %.8x ,maskVal %.8x shift %d \n\n", csr[i], DLY_INT_RXDLY_MASK(rxIdx), DLY_INT_RXDLY_SHIFT(rxIdx));
			printk("rxdlycsr %.8x ,glbCfg:%.8x RxDlyCfg:%.8x \n\n", QDMA_CSR_RX_DELAY_INT_CFG(base, rxIdx), qdmaGetGlbCfg(base), qdmaGetRxDelayIntCfg(base, rxIdx));
			
			gpQdmaPriv->counters.intRxDone[rxIdx] = 0 ;
            qdma_st_packet_gen(channel, queue, npackets, lens, 0x01, txIdx) ;
            msleep(100) ;
			
            printk("Tx%d to Rx%d Delay Pkt:%d, Time:%d*20us, DlyCSR:%.8x, Packets:%d, RX_INT Count:%d\n\n", txIdx, rxIdx,
		                                        ((qdmaGetRxDelayIntCfg(base, rxIdx) & DLY_INT_RXMAX_PINT_MASK(rxIdx))>>DLY_INT_RXMAX_PINT_SHIFT(rxIdx)) ,
		                                        ((qdmaGetRxDelayIntCfg(base, rxIdx) & DLY_INT_RXMAX_PTIME_MASK(rxIdx))>>DLY_INT_RXMAX_PTIME_SHIFT(rxIdx)),
		                                        qdmaGetRxDelayIntCfg(base, rxIdx), npackets, gpQdmaPriv->counters.intRxDone[rxIdx]) ;
            if((i != 0) && (i != 5)) {
                if(gpQdmaPriv->counters.intRxDone[rxIdx] > prevIntRxDone)
                    result = -1;
            }               
            prevIntRxDone = gpQdmaPriv->counters.intRxDone[rxIdx] ;
            msleep(2000) ;
        }
    }

    if(result == 0)
        QDMA_LOG("RX_DLY_INT: Test Result:Pass\n\n") ;
    else
        QDMA_LOG("RX_DLY_INT: Test Result:Failed\n\n") ;
    
    qdmaSetRxDelayIntVal(base, rxIdx, oldRxDly) ;
    return 0;
}
...
void delay1us(int period, int number)
{
	...
	volatile unsigned int  timer1_ldv = TIMER1_LDV_GET;
	int same_count = 0;
	tick_acc = 0;
 	timer_last = TIMER1_VLR_GET;
	do {
...
static int tc_send_packet(uint base, int npackets, int lens, int rxRingIdx, int txRingIdx)
{
    uint channel = random32() % CONFIG_QDMA_CHANNEL ;
    uint queue = random32() % CONFIG_QDMA_QUEUE ;
	int i = 0;
    ...
    gpQdmaPriv->counters.intRxDone[rxRingIdx] = 0;
    gpQdmaPriv->counters.intTxDone[rxRingIdx] = 0;
    qdma_st_packet_gen(channel, queue, npackets, lens, 0x01, txRingIdx) ;
    __st_print_counters() ;

    printk("RX INT cntr: %d , TX Done cntr: %d\n", gpQdmaPriv->counters.intRxDone[rxRingIdx], gpQdmaPriv->counters.intTxDone[rxRingIdx]);
...
static int tc_mul_int_test(uint base, int rxRingIdx, int txRingIdx, int npackets, int lens)
{
    uint channel = random32() % CONFIG_QDMA_CHANNEL ;
    uint queue = random32() % CONFIG_QDMA_QUEUE ;
    int i = 0;

    if(rxRingIdx == RX_RING_NUM ){
        for(i=0;i<RX_RING_NUM;i++){
            qdmaSetLpbkToRXRing(base, i);
...
/*TX DLY INT DE comments that this feature does not need test, ignore it !!! */
#if 0
...
static int tc_rx_dscp_low_test(uint base)
{
    uint channel = 0 ;
    uint queue = 0 ;
    uint rxIdx = 0, txIdx = 0 ;
    int retVal=0;
    uint oldInt0 = qdmaGetIntMask(base,qdma_int_src, QDMA_INT_ENABLE0) ;
    uint oldInt1 = qdmaGetIntMask(base,qdma_int_src, QDMA_INT_ENABLE1) ; 
    uint oldInt2 = qdmaGetIntMask(base,qdma_int_src, QDMA_INT_ENABLE2) ; 
    uint oldGlb = qdmaGetGlbCfg(base);
    int oldRxRingThr[RX_RING_NUM] = {0};

    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0, (	INT_STATUS_RX1_DSCP_LOW | INT_STATUS_RX0_DSCP_LOW |
    				INT_STATUS_TX0_DONE |INT_MASK_NO_TX_CPU_DSCP|INT_STATUS_NO_RX1_CPU_DSCP | INT_STATUS_NO_RX0_CPU_DSCP) );

    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE2, INT_STATUS_RX_DSCP_LOW_2_15);

    qdmaDisableInt(base, (INT_MASK_NO_RX0_CPU_DSCP|INT_MASK_NO_RX1_CPU_DSCP
    						|INT_MASK_RX0_DONE | INT_MASK_RX1_DONE), qdma_int_src, QDMA_INT_ENABLE0) ;
    qdmaDisableInt(base, INT_MASK_RX_DONE_2_15, qdma_int_src, QDMA_INT_ENABLE1) ;
	
    for(rxIdx = 0; rxIdx < RX_RING_NUM; rxIdx++){
        oldRxRingThr[rxIdx] = qdmaGetRxRingThrh(base,rxIdx);
        qdmaSetRxRingThrh(base, rxIdx, (CONFIG_RX_DSCP_NUM[rxIdx]-5)) ;
    }

    for(txIdx=0; txIdx < TX_RING_NUM; txIdx++) {
        for(rxIdx = 0; rxIdx < RX_RING_NUM; rxIdx++){
            qdmaSetLpbkToRXRing(base, rxIdx);
            printk("from Tx%d to Rx%d ,glb cfg %.8x ringthr%d %d\n",txIdx, rxIdx, qdmaGetGlbCfg(base), rxIdx, qdmaGetRxRingThrh(base,rxIdx) );
            memset(counters, 0, sizeof(QDMA_DbgCounters_T)*CONFIG_QDMA_CHANNEL*CONFIG_QDMA_QUEUE) ;
            gpQdmaPriv->counters.intRxDscpLow[rxIdx] = 0 ;
            channel = random32() % CONFIG_QDMA_CHANNEL ;
            queue = random32() % CONFIG_QDMA_QUEUE ;
            qdma_st_packet_gen(channel, queue, CONFIG_RX_DSCP_NUM[rxIdx], 0, 0x01, txIdx) ;
            qdma_bm_receive_packets(0, rxIdx) ;
            msleep(1000) ;
            printk("TX%d to RX%d, Packets:%d, RX%d_DSCP_LOW Cnt %d ", 
                txIdx, rxIdx, CONFIG_RX_DSCP_NUM[rxIdx], rxIdx, gpQdmaPriv->counters.intRxDscpLow[rxIdx]) ;
            if(gpQdmaPriv->counters.intRxDscpLow[rxIdx] > 0){
            		printk("Pass.\n\n");
            }else{
            	printk("Failed**** \n\n");
            	retVal = -1;
    ...
    if(retVal == 0) {
        QDMA_LOG("RX_DSCP_LOW_INT Test Result:Pass\n\n") ;
    } else {
        QDMA_LOG("RX_DSCP_LOW_INT Test Result:Failed\n\n") ;
    }
    qdmaSetGlbCfg(base, oldGlb);
    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0, oldInt0) ;
    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE1, oldInt1) ;
    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE2, oldInt2) ;
    for(rxIdx = 0; rxIdx < RX_RING_NUM; rxIdx++){
    	qdmaSetRxRingThrh(base, rxIdx, oldRxRingThr[rxIdx]) ;
    }
...
static int tc_tk2(uint base, uint packets, uint lens, int ringIdx)
{
	...
	for(thrshldLen=packets; thrshldLen<=32; thrshldLen=thrshldLen<<1) {
		...
		if((entryLen == (thrshldLen-1)-(thrshldLen+1)/2) && (headIdx == (headIdxBase+(thrshldLen+1)/2) % depthLen))
		...
		entryLen = qdmaGetIrqEntryLen(base);
		headIdx = qdmaGetIrqHeadIdx(base);
		if((entryLen == 0) && (headIdx == (headIdxBase+thrshldLen-1) % depthLen))
...
static int dbg_cntr_init(int rxRingIdx) {
	/*  CPU TX Config */
	...
	dbg_cntr_config(34, DBG_CNTR_SRC_CPU_RX, QDMA_ENABLE, QDMA_ENABLE, QDMA_DISABLE, -1, -1, rxRingIdx);	/*	34: CPU_RX Ring_Idx All */
	
	dbg_cntr_config(35, DBG_CNTR_SRC_CPU_RX, QDMA_DISABLE, QDMA_DISABLE, QDMA_ENABLE, 1, 7, -1);	/*	35: CPU_RX channel_1  queue_7 */
	dbg_cntr_config(36, DBG_CNTR_SRC_CPU_RX, QDMA_DISABLE, QDMA_DISABLE, QDMA_ENABLE, 1, 1, -1);	/*	36: CPU_RX channel_1  queue_1 */
...
static int dbg_cntr_test(uint base, uint packets, uint lens)
{
    ...
    uint rx_cpu_total_cnt=0, rx_cpu_ring_cnt[RX_RING_NUM]={0}, rx_fwd_total_cnt=0, rx_fwd_stream_cnt=0 ;
    ...
    for(index=0; index<DBG_CNTR_LOOP_NUM; index++) {
        ...
        for(txRingIdx=0; txRingIdx<TX_RING_NUM; txRingIdx++) {
            for(rxRingIdx=0; rxRingIdx<RX_RING_NUM; rxRingIdx++) {
				...
				(expectedVal[32] = rx_cpu_total_cnt); (expectedVal[33] = rx_fwd_total_cnt); (expectedVal[34] = rx_cpu_ring_cnt[rxRingIdx]); (expectedVal[35] = npackets);
...
/*dbg cntr test*/
//rxRing num need from 2 to 16,be careful
...
#if 0
/*not used ,tx block tested by other items */
static int tc_tk5(uint base, uint packets, uint lens, int ringIdx)
{
    ...
    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0, INT_STATUS_TX0_DONE);
...
/*EN7516: qos weight changed to 16 bit*/
...
{
    uint oldInt=0 ;
    oldInt = qdmaGetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0) ;
...
static int tc_tc2(uint base, uint packets, uint lens, int ringIdx)
{
    ...
    uint i=0, j=0, k=0 ;
	uint result=0 ;
    printk("Case 2: (Channel, Queue, Pattern, Lens) Any Combination, Packets:%d\n", 1000*CONFIG_QDMA_CHANNEL*CONFIG_QDMA_QUEUE*5) ;
    for(i=0 ; i<CONFIG_QDMA_CHANNEL ; i++) {
        for(j=0 ; j<CONFIG_QDMA_QUEUE ; j++) {
            for(k=0 ; k<5 ; k++) {
                memset(counters, 0, sizeof(QDMA_DbgCounters_T)*CONFIG_QDMA_CHANNEL*CONFIG_QDMA_QUEUE) ;
                qdma_st_packet_gen(i, j, 1000, 1, pattern[k], ringIdx) ;
                msleep(1000) ;
                if(counters[i][j].tx_frames == counters[i][j].rx_frames) {
...
/*IRQ Depth Test ,need tx dscpNum larger than irq depth,tx1024, irq 512, should not modify irqDepth in runtime*/
static int tc_tc6(uint base, uint packets, uint lens, int ringIdx)
{
    uint oldInt=0, depth=0, entryIndex=0 ;
    uint i=0, j=0 ;
    uint npackets=0;
    int channel = random32() % CONFIG_QDMA_CHANNEL;
    int queue = random32() % CONFIG_QDMA_QUEUE;
	

    oldInt = qdmaGetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0) ;
    depth = qdmaGetIrqDepth(base) ;     
    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0, (INT_MASK_RX0_DONE|INT_MASK_RX1_DONE|INT_MASK_IRQ_FULL)) ;
	
    printk("int_enable0 mask:0x%08x, irqDepth %d \n", qdmaGetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0),depth);
    memset(counters, 0, sizeof(QDMA_DbgCounters_T)*CONFIG_QDMA_CHANNEL*CONFIG_QDMA_QUEUE) ;
	
    npackets = depth + 100;
         
    printk("Case 6: IRQ Queue Depth:%d, Pattern:RAND, Lens:RAND, Packets:%d\n", depth, npackets) ;
    qdma_st_packet_gen(CONFIG_QDMA_CHANNEL, CONFIG_QDMA_QUEUE, npackets, 0, 0x01, ringIdx) ;
    msleep(1000) ;
    ...
    printk("IRQ_Entry_Len: %d\n", qdmaGetIrqEntryLen(base)) ;
    if(__st_check_counters() == 0) {
        printk("IRQ depth:%d, Packets:%d, Test Result:Pass\n", depth, npackets) ;
    } else {
        printk("IRQ depth:%d, Packets:%d, Test Result:Failed\n", depth, npackets) ;
    }
...
static int tc_tc7(uint base, uint packets, uint lens, int ringIdx)
{
    ...
    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0, curInt);
    memset(counters, 0, sizeof(QDMA_DbgCounters_T)*CONFIG_QDMA_CHANNEL*CONFIG_QDMA_QUEUE) ;
    // qdma_st_packet_gen(CONFIG_QDMA_CHANNEL, CONFIG_QDMA_QUEUE, packets, 1, 1, ringIdx) ;
    qdma_st_packet_gen(channel, queue, packets, 1, 1, ringIdx) ;
    msleep(1000) ;	
    printk("Packets:%d sent done\n\n", packets) ;

    curInt |= INT_MASK_IRQ_FULL;  
    qdmaSetIntMask(base, qdma_int_src, QDMA_INT_ENABLE0, curInt);
    msleep(1000) ;
...
static int qdma_st_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{
	...
	sscanf(val_string, "%s %s %d %d %d %d", cmd, subcmd, &value, &ringIdx, &msec, &index) ;
    ...
    /* for other csr verification */
        ...
        if(packets<0 || packets >=QDMA_INT_NUM){
            printk("Error Int Src!!!  range[1~%d]\n", (QDMA_INT_NUM-1)); 
        }else{
            qdma_int_src = packets;
            printk("set QDMA_INT_SRC as %d\n", qdma_int_src);
            printk("Warn: make sure QDMA_INT_SRC same with ringBind intSrc!!!!\n");
        }
    }else if(!strcmp(cmd, "loopback_basic_test") || !strcmp(cmd, "tcall")) {
    	tc_loopback_basic_test(base, packets, value);
	}else if(!strcmp(cmd, "loopshort")) {
    	tc_loopback_basic_test_debug(base, packets, value, ringIdx);
	}
	...
	/* CHCEK_DONE bit enable/disable verification  */
	else if(!strcmp(cmd, "coherent_test")){
		tc_done_bit_check_test(base);
	}else if(!strcmp(cmd, "rx_low_test")){
		tc_rx_dscp_low_test(base);
	}else if(!strcmp(cmd, "rx_dly_test")){
		tc_rx_dly_int_test(base, rxRingIdx, txRingIdx, packets, value);
	}else if(!strcmp(cmd, "rx_dly_test")){
		tc_rx_dly_int_test(base, rxRingIdx, txRingIdx, packets, value);
	}else if(!strcmp(cmd, "tc_send")){
		tc_send_packet(base, packets, value, rxRingIdx, txRingIdx);
	}else if(!strcmp(cmd, "udelay")){
		delayTime = packets;  /*us*/
        printk("set delaytime as %d us\n", delayTime);
	}else if(!strcmp(cmd, "tc_mul_int")){
		tc_mul_int_test(base, rxRingIdx, txRingIdx, packets, value);
	}
    #if 0
	...
	}else if(!strcmp(cmd, "tk3_dbgcntr")){
		dbg_cntr_test(base, packets,lens);
    	...
    	if(!strcmp(cmd, "tc6") || !strcmp(cmd, "tcall")) {	/*IRQq Depth Test*/
