...
DEFINE_SPINLOCK(rtl_mdio_lock);

#define RTL_MDIO_LOCK()           \
		do {						  \
			spin_lock_bh(&rtl_mdio_lock);\
		} while(0)

#define RTL_MDIO_UNLOCK()           \
		do {							\
			spin_unlock_bh(&rtl_mdio_lock);\
		} while(0)
#endif /* CONFIG_TP_IMAGE */

#if defined(MDC_MDIO_OPERATION)

extern void rtl8372_mdio_write(unsigned int phy_addr, unsigned int phy_reg, unsigned int phy_data);
extern void rtl8372_mdio_read(unsigned int phy_addr, unsigned int phy_reg, unsigned int* pData);
extern void rtl8372_mdio_read_h(unsigned int phy_addr, unsigned int phy_reg, unsigned short* pData);
...
#define MDC_MDIO_WRITE(preamableLength, phyID, regID, data)	rtl8372_mdio_write(phyID, regID, data)
#define MDC_MDIO_READ(preamableLength, phyID, regID, pData) rtl8372_mdio_read(phyID, regID, pData)
#define MDC_MDIO_READ_H(preamableLength, phyID, regID, pData) rtl8372_mdio_read_h(phyID, regID, pData)
