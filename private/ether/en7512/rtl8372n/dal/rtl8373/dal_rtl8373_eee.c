...
rtk_api_ret_t dal_rtl8373_eee_macForceSpeedEn_set(rtk_port_t port, rtk_eee_speedInMacForceMode_t speed, rtk_enable_t enable)
{
	...
	if((retVal = rtl8373_setAsicRegBit(RTL8373_MAC_FORCE_MODE_CTRL1_ADDR(port), speed,  (rtk_uint32)enable)) != RT_ERR_OK)
	 ...
	 if(speed == EEE_MAC_FORCE_SPEED_2P5G)
	 	{
	if((retVal = dal_rtl8373_phy_writeBits(1<<port, 7, 62, 1, (rtk_uint32)enable)) != RT_ERR_OK)
        return retVal;  
	 	}
	
	 if(speed == EEE_MAC_FORCE_SPEED_1000M)
	 	{
	if((retVal = dal_rtl8373_phy_writeBits(1<<port, 7, 60, 1<<2, (rtk_uint32)enable)) != RT_ERR_OK)
		return retVal;
	 	}

	 if(speed == EEE_MAC_FORCE_SPEED_100M)
	 	{
	if((retVal = dal_rtl8373_phy_writeBits(1<<port, 7, 60, 1<<1, (rtk_uint32)enable)) != RT_ERR_OK)
        return retVal;  
		}
    return RT_ERR_OK;	
}
...
/* Function Name:
 *      dal_rtl8373_eee_sdsEn_set
 * Description:
 *      Set 8224 sds 10gr EEE enable.
 * Input:
 *      sds_index - serdes index.
 *      enable  - enable EEE status.
 * Output:
 *      None
 * Return:
 ...
 *      RT_ERR_ENABLE - Invalid enable input.
 * Note:
 *      This API set serdes reg to enable 8224 10GR EEE 
 */

rtk_api_ret_t dal_rtl8373_eee_sdsEn_set(rtk_uint32 sds_index, rtk_enable_t enable)
{
	rtk_uint32 retVal = 0;

	if((retVal =  dal_rtl8373_sds_regbits_write(sds_index, 6, 3, 1<<15, (rtk_uint32)enable)) != RT_ERR_OK)
		return retVal;
	return RT_ERR_OK;
}
