 ...
 */
rtk_api_ret_t rtk_port_rtct_init(rtk_port_t port)
{
    rtk_api_ret_t retVal;
    ...
    RTK_API_LOCK();
    retVal = RT_MAPPER->port_rtct_init(port);
    RTK_API_UNLOCK();
...
/* Function Name:
 *      rtk_port_txPacketMaxLength_set
 * Description:
 ...
 *      None
 */
rtk_api_ret_t rtk_port_txPacketMaxLength_set(rtk_port_t port, rtk_uint32 type, rtk_uint32 pktLen)
{
    rtk_api_ret_t retVal;

    if (NULL == RT_MAPPER->port_txPktMaxLen_set)
        return RT_ERR_DRIVER_NOT_FOUND;

    RTK_API_LOCK();
    retVal = RT_MAPPER->port_txPktMaxLen_set(port, type, pktLen);
    RTK_API_UNLOCK();
...
/* Function Name:
 *      rtk_port_txPacketMaxLength_get
 * Description:
 ...
 *      None
 */
rtk_api_ret_t rtk_port_txPacketMaxLength_get(rtk_port_t port, rtk_uint32 type, rtk_uint32 *pPktLen)
{
    rtk_api_ret_t retVal;

    if (NULL == RT_MAPPER->port_txPktMaxLen_get)
        return RT_ERR_DRIVER_NOT_FOUND;

    RTK_API_LOCK();
    retVal = RT_MAPPER->port_txPktMaxLen_get(port, type, pPktLen);
    RTK_API_UNLOCK();
