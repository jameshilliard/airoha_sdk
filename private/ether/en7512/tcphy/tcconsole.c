...
extern int (*send_to8851_hook)(struct sk_buff* skb, int hsgmii_mask);

#if defined(TCSUPPORT_SISM_HOST)
extern void fe_vip_tc_console_setup(uint8_t enable);
#endif

extern int (*xsi_max_tx_hook)(struct sk_buff * skb);
#ifdef WAN2LAN
extern int masko;
#endif
...
#ifdef WAN2LAN
		if(send_to_hsgmii_hook != NULL && (masko & 0xf0) != 0)
			send_to_hsgmii_hook(skb, masko);
#if defined(TCSUPPORT_SWITCH_8851)
		if(send_to8851_hook !=NULL && (masko & 0xff)!=0)
		{
			
			send_to8851_hook(skb,masko);
		}
#endif
		if((masko & 0x0f) == 0) 
		{
...
#if defined(TCSUPPORT_CPU_ARMV8) && !defined(TCSUPPORT_HOST_TCCONSOLE)
	return 0;
#else
	if ((skb->data[tc_pro_idx] == 0xaa) && (skb->data[tc_pro_idx+1] == 0xaa)) {
#ifndef  TCSUPPORT_HOST_TCCONSOLE
		if (adsl_dev_ops){
#endif
#ifdef TC_CONSOLE_ENABLE  /*Ryan_20091019*/
			...
			switch(skb->data[tcc_cmd_idx]){
				case MAC_RTS_START:
					...
					/* Add 0xaaaa to VIP when tc console start. */
					fe_vip_tc_console_setup(1);
#endif
					getTcconsoleFile();
				...
				case MAC_RTS_STOP:
#if TCSUPPORT_SISM_HOST
					/* Remove 0xaaaa from VIP when tc console stop. */
					fe_vip_tc_console_setup(0);
#endif
			...
			skb->dev = dev_get_by_name(&init_net, "ae_wan");
...
tcconsole_init(void){
	...
	#endif
	rcu_assign_pointer(macSend_hook, macSend);
}
...
tcconsole_exit(void){
	...
	#endif
	rcu_assign_pointer(macSend_hook, NULL);
}
#endif
