...
#endif
#ifndef QOS_H_PRIORITY
#define 	QOS_H_PRIORITY		0x20
#endif
#ifndef QOS_M_PRIORITY
#define 	QOS_M_PRIORITY		0x30
#endif
#ifndef QOS_L_PRIORITY
...
extern struct net_device* (*portbind_get_outdev_by_indev_ct_hook)(unsigned char* indev_name);
...
extern  void eth_led_tx_hook(uint8 port_mask);
extern  void eth_led_rx_hook(uint8 pvid);
#endif
#endif


#if defined(CONFIG_MIPS_RT63365) || defined(TCSUPPORT_CPU_ARMV8)
...
int use_soc_lan = 0;
EXPORT_SYMBOL(use_soc_lan);
int soc_switch_port = -1;
EXPORT_SYMBOL(soc_switch_port);
#ifdef TCSUPPORT_QTN_ISOLATION
int isolation_enable = 1;
#else
int isolation_enable = 0;
#endif
EXPORT_SYMBOL(isolation_enable);

unsigned char asym_test_soc_switch = 0xff;
...
uint8 macSTagFlag = 0;
EXPORT_SYMBOL(macSTagFlag);
uint8 macVlanFlag = 0;
uint8 macRxPortFlag = 0;
uint8 esd_sw_patch = 1;/*Default is enabled the ESD software patch*/

#if defined(TCSUPPORT_MULTI_SWITCH_EXT)
/* internal switch port-5 for extend switch */
char extend_switch_port = 5;
EXPORT_SYMBOL(extend_switch_port);
eth_port_vlan_t eth_port_vlan[MAX_ETH_ITF_NUM];
EXPORT_SYMBOL(eth_port_vlan);
char inter_switch_lanport_num = 4;
EXPORT_SYMBOL(inter_switch_lanport_num);
char extend_switch_lanport_num = 4;
EXPORT_SYMBOL(extend_switch_lanport_num);
unsigned char inter_switch_mask = 0x0F;
EXPORT_SYMBOL(inter_switch_mask);
unsigned char extend_switch_mask = 0xF0;
EXPORT_SYMBOL(extend_switch_mask);
char lan_port_map_ext[ MAX_ETH_ITF_NUM ];
EXPORT_SYMBOL(lan_port_map_ext);
#else
char extend_switch_port = -1;
EXPORT_SYMBOL(extend_switch_port);
eth_port_vlan_t eth_port_vlan[MAX_ETH_ITF_NUM];
EXPORT_SYMBOL(eth_port_vlan);
unsigned char inter_switch_mask = 0xFF;
EXPORT_SYMBOL(inter_switch_mask);
unsigned char extend_switch_mask = 0x0;
EXPORT_SYMBOL(extend_switch_mask);
char lan_port_map_ext[ MAX_ETH_ITF_NUM ] = {0};
EXPORT_SYMBOL(lan_port_map_ext);
#endif
...
int wan_port_id=-1;
/* map to switch ports */
EXPORT_SYMBOL(wan_port_id);

char lan_port_map[MAX_ETH_ITF_NUM];
EXPORT_SYMBOL(lan_port_map);
 
char switch_port_map[RT63365_SWIC_PORTNUM];
EXPORT_SYMBOL(switch_port_map);
#endif

int eth_gQueueMask = 0;
EXPORT_SYMBOL(eth_gQueueMask);
...
uint32 switch_reg_read2(uint32 ext_switch, uint32 reg)
{
    if(ext_switch){
        return gswPbusRead(reg-GSW_BASE);
    }else{
...
uint32 switch_reg_write2(uint32 ext_switch, uint32 reg, uint32 wdata)
{
    if(ext_switch){
        gswPbusWrite((reg-GSW_BASE), wdata);
    }else{
...
/*extend switch reg read and write*/
uint32 gswRegRead_extend(uint32 reg, uint32 is_extend_gsw)
{
    if(is_extend_gsw){
        return gswPbusRead_extend((reg-GSW_BASE), GSW_PHY_OFFSET_23);
    }else{
        return switch_reg_read(reg);
    }
}
...
{
    if(is_extend_gsw){
        gswPbusWrite_extend((reg-GSW_BASE), wdata,  GSW_PHY_OFFSET_23);
    }else{
        switch_reg_write(reg, wdata);
    }
...
#if defined(CONFIG_MIPS_RT63365) || defined(TCSUPPORT_CPU_ARMV8)
#if defined(TCSUPPORT_MULTI_SWITCH_EXT)
inline uint8 isLANPortInExtSwitch(uint8 port_mask, uint16 *vlan_id_ext, uint8 *is_port_extend)
{
	uint8 port = 0;
	uint8 phyport = 0;
	uint8 value = 0;
	...
	for (port=0; port<MAX_ETH_ITF_NUM; port++){
		if (port_mask & (1<<port)){
			phyport = lan_port_map[port];			
			if(phyport == extend_switch_port){
                *vlan_id_ext = eth_port_vlan[port].vlan_id;
                *is_port_extend = 1;
            }else{
                *is_port_extend = 0;
                *vlan_id_ext = 0;
...
inline uint8 macEN7512STagRemap(uint8 port_mask)
{
	...
	for (port=0; port<MAX_ETH_ITF_NUM; port++){
		if (port_mask & (1<<port)){
			...
			if (phyport<RT63365_SWIC_PORTNUM){
				value |= (1<<phyport);
			}
		}
	}
...
void rgmii0_interface_init(int hw_port){
    uint32 reg;

    if(isEN751221)
    {
        /*set rgmii interface into rgmii mode*/
        regWrite32(0xBFA2015C,regRead32(0xBFA2015C)|0x1);
    }

    /* set 0x5e30b for soc switch port 0 with rgmii0 interface */
    ...
    if(hw_port ==5 || hw_port==0)
	    write_reg_word(GSW_PMCR(hw_port), reg);
}
...
void init_ethernet_port_map()
{
...
#if defined(TCSUPPORT_MULTI_SWITCH_EXT)
	memset(lan_port_map_ext, -1, MAX_ETH_ITF_NUM*sizeof(char));
#endif
	memset(switch_port_map, -1, RT63365_SWIC_PORTNUM*sizeof(char));

#if defined(TCSUPPORT_MANUAL_ETHERNET_PORTMAP)
#if defined(TCSUPPORT_MULTI_SWITCH_EXT)
	sscanf(TCSUPPORT_ETHERNET_PORTMAP, "%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d", &wan_port, 
								&lan[0], &lan[1], &lan[2], &lan[3], &lan[4], &lan[5],
								&lan[6], &lan[7], &lan[8], &lan[9], &lan[10]);
#else
...
#endif
	for (port=0; port<MAX_ETH_ITF_NUM; port++){
        if(lan[port]/10){   // port id = 1X,means soc lan portX with ext switch
            if(use_ext_switch){
                use_soc_lan = 1;
            }
            soc_switch_port = lan[port]%10;
            lan[port] = soc_switch_port;
        }
        #if SUPPORT_EXTERNAL_SWITCH
        {
            if (0<=lan[port]  && lan[port]<9){
                if (port < 4)
                {
                ...
                {
                    lan_port_map2[port-4] = (char)lan[port] - 4;
                }
            }
        }
        #else
		...
		#endif
        if((use_soc_lan && lan[port]==0) || (!use_ext_switch && lan[port]==5)){
            rgmii0_interface_init(lan[port]);
        }
	}
#else
        ...
        lan_port_map[2] = 3;
        lan_port_map[3] = 4;
#elif defined(TCSUPPORT_CPU_EN7521)
        wan_port_id = -1;
        lan_port_map[0] = 1;
        lan_port_map[1] = 2;
        lan_port_map[2] = 3;
    ...
    if(isFPGA){
        ...
        /*if use ether wan, port mapping need to modify*/
            lan_port_map[0] = 0;
            lan_port_map[1] = 1;
            lan_port_map[2] = 2;
            lan_port_map[3] = 3;     
...
static void acl_for_soc_ether_lan(void)
{
    uint32 value;
...
int macEN7512STagEnable(int chanID)
{
    ...
    if(use_ext_switch){
        ...
        if(use_soc_lan){
            value = gswPbusRead(0x2610);
            value |= (1<<PT_OPTION);   /*external switch port6 enable PT_OPTION*/
            gswPbusWrite(0x2610, value);

            value = regRead32(GSW_PCR(6));  /*soc switch port6 ACL enable */
            value |= (1<<ACL_EN);
            regWrite32(GSW_PCR(6), value);

            value = regRead32(GSW_PVC(5));
            value &= ~0xc0;                 /* set soc switch port 5 as user port */
            value |= (1<<PORT_STAG);        /* set soc switch port 5 special tag enable */
            regWrite32(GSW_PVC(5), value);
            
            value = regRead32(GSW_PVC(6));
            value &= ~0xc0;                 /* set soc switch port 6 as user port */
            value |= (1<<PORT_STAG);        /* set soc switch port 6 special tag enable */
            value |= (1<<8);                /* set soc switch port 6 consistent mode*/
            regWrite32(GSW_PVC(6), value);
            
            value = regRead32(GSW_PVC(0));
            value &= ~0xc0;                 /* set soc switch port 0 as user port */
            regWrite32(GSW_PVC(0), value);
            
            acl_for_soc_ether_lan();
        }
    }else{
        /*Enable Special Tag function*/
	...
	if(CPORT_CFG_SPTAG_ENABLE)
	{
		value = 0x3;	
		regWrite32(GSW_CPORT_SPTAG_CFG, value);
	}
	else{ 
		if(isEN7523)
		{
	    ...
	    regWrite32(GDMA1_VLAN_GEN, value);
	}
...
uint32 getQueueForSwitchbySkbMark(struct sk_buff *skb)
{
	uint32 queue;
	...
	else if(queuemark_enable == 1)
	{
		queue = (skb->mark>>queuemark_shift) & ((1<<queuemark_width) - 1);
	} 
	else
	{
		queue = 0;
...
int checkQueueForSwitchBySkbMark(struct sk_buff* skb)
{
	uint32 queue = 0;
...
int isDefaultQueue_2(struct sk_buff *skb)
{
	int queue=0;
	...
	if(eth_gQueueMask & (1 << ((queue >> 4) -1 ))){ 	/* default queue */
		return 0;
	}else{
		return 1;
	}
...
{	
	...
	switch (qos_flag) {
		case QOS_SW_PQ:
			...
			break;
		case QOS_SW_CAR:
			txq = CAR_QUEUE_NUM - ((skb->mark & QOS_FILTER_MARK)>> 4);
			if(txq >= CAR_QUEUE_NUM)
				txq = CAR_QUEUE_NUM - 1;
			break;
	#if 1/*def TCSUPPORT_HW_QOS*/
		case QOS_HW_WRR:
			if(isDefaultQueue_2(skb)){ 	/* default queue */
				txq = 0;
			}
			else
			/* HW WRR mode */
...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CMCC) || defined(TCSUPPORT_CT_CONGW_CH)
static inline int TagPktDSC(struct sk_buff *skb, uint8 pvid){
    char landev[10] = {0};
    struct net_device *wandev = NULL;
    sprintf(landev, "%s%d", lanNamePre(), pvid);
    if(portbind_get_outdev_by_indev_ct_hook)
        wandev = portbind_get_outdev_by_indev_ct_hook(landev);
    if(wandev !=NULL){
		dev_put(wandev);
        if((wandev->vlan_mode == 3 || wandev->vlan_mode == 4) && ((skb->lan_vlan_tci) & 0x0FFF)!= 0)
            return -1;
        else{
            return 0;
        }
...
static inline struct sk_buff * Insert_4Byte_to_Skb(struct sk_buff * bp)
{
	...
	else 
	{
		...
		/* Don't unshare tso skbs, modified for tso performance test. */
		if(!(bp->tso_mark & TSO_ENABLE_MARK))
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,15,0)
	else if(is5GWiFiInterface(skb->original_dev)){  /* rai0*/
        return WIFI1_RAIX_TO_LAN;
    }else if(is24GWiFiInterface(skb->original_dev)){    /* ra0*/
...
inline struct sk_buff *macEN7512STagInsert(ethTxMsg_t *TxMsg, struct sk_buff *bp)
{
	uint8 *cp;
	uint8 port_mask=0;
	uint8 pvid = 0;
    ...
    /*EN7580: spTag should be put in TxMsg, not skb-data.*/
    /*here, alloc tmp buffer to calculate spTag*/
#if defined(TCSUPPORT_CPU_EN7580)
    uint8 fake_data[16];
...
#if defined(TCSUPPORT_MULTI_SWITCH_EXT)
	uint8 lan_idx = 0;
	uint16	vlan_id_ext = 0;
	uint8 is_port_extend = 0;
	lan_idx = skbFromWhichVirtualEthItf(bp);
	if((lan_idx >=0) && (lan_idx < MAX_ETH_ITF_NUM)){
		if(lan_port_map[lan_idx] == extend_switch_port){
            cp = bp->data;
            if(!is_multicast_ether_addr(cp) || is_broadcast_ether_addr(cp) || bp->is_unknown_mul){
			bp = Insert_4Byte_to_Skb(bp);
			cp = bp->data;
			*(uint16 *)(cp+12) = htons(0x8100);
			/* default setting extend LAN interface 4~7 => vlan 1~4 */
			    *(uint16 *)(cp+14) = htons(eth_port_vlan[lan_idx].vlan_id);
            }
		}
	}
#endif
    ...
    cp = bp->data;
	tag0 = ntohs(*(uint16 *)(cp+12)); /* tpid*/
	tag1 = ntohs(*(uint16 *)(cp+14)); /* tag1 = x = eth0.x*/
	if(TCSUPPORT_FWC_QOS_VAL)
	{
...
#if defined(TCSUPPORT_MULTI_USER_ITF) || defined(TCSUPPORT_MULTI_SWITCH_EXT)
				if ((GET_ETHER_ITF_MARK(bp->mark) >= 1) && (GET_ETHER_ITF_MARK(bp->mark) <= 4))
#else
	...
	{
        /*EN7580: not change skb->data, so no need do unshare*/
#if !defined(TCSUPPORT_CPU_EN7580)
#if defined(TCSUPPORT_TSO_ENABLE)
		/* Don't unshare tso skbs, modified for tso performance test. */
		if(!(bp->tso_mark & TSO_ENABLE_MARK))
        ...
        if ((eth_itf >=0) && (eth_itf < MAX_ETH_ITF_NUM))
        {
		...
		if (bp->mark & SKBUF_COPYTOLAN){
            port_mask = ((masko_on_off ? masko : 0) | (wlan_masko_on_off ? wlan_masko : 0)) & 0xF;
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CT_LOOPDETECT)		
			if(temp_mask=isDetectPacket(cp, bp->len)){
				port_mask = temp_mask;
				isLoopDetectPkt = 1;
#if defined(TCSUPPORT_MULTI_SWITCH_EXT)
                isLANPortInExtSwitch(port_mask, &vlan_id_ext, &is_port_extend);
#endif
			}
#endif/*TCSUPPORT_COMPILE*/
...
#if (!defined(TCSUPPORT_MULTI_USER_ITF)&&!defined(TCSUPPORT_MULTI_SWITCH_EXT)) || defined(TCSUPPORT_HOST_TCCONSOLE)
        if ((bp->mark & SKBUF_TCCONSOLE) || (bp->data[12]==0xaa)){
            port_mask = 0x0f;
            port_mask = macEN7512STagRemap(port_mask);
        }
#endif		
		...
		*/
		/*EN7580: VPM fix as 0. no need calculate*/
#if defined(TCSUPPORT_CPU_EN7580)
        vpm = 0;
        cp = fake_data;
#else
		if(!(tag0 == 0x8100 || tag0 == 0x88a8 || tag0 == cpu_port_tpid))/*untag packet*/
		{
			...
			cp = bp->data;
#if defined(TCSUPPORT_MULTI_SWITCH_EXT)
			/*ogiginal set 0xffff, external set 0x8100 and vlan */
			if(is_port_extend){
				tag0 = 0x8100;
				*(uint16 *)(cp+14) = htons(vlan_id_ext);
			}else
				tag0 = 0xffff;
#else
		...
		if(isComeFromWan(bp) == 1)
			*(uint16 *)(cp+12) = htons((vpm << 8) + (1<<wan_port_id)| (1<<15) | ((getPbitForSwitchQos(bp) & 0x7)<<12));
		else
#endif
		{	
			uint16 stag_tmp = 0;
			//add user pbit in stag for switch to do qos
			stag_tmp = (vpm << 8) + (port_mask) | (stag_up<<12) + (rgmii_itf << 10);  /* rgmii flag use DRM bit*/
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CT_DS_LIMIT)
			queue = dataspeed_limit_queue_hook(bp); /* remark the queue index. */
#else/*TCSUPPORT_COMPILE*/
			if(ds_qos == 0)     //downstream qos in switch
			{
				queue = getQueueForSwitchbySkbMark(bp);	
			}
#endif/*TCSUPPORT_COMPILE*/
    			stag_tmp |= (queue << 12);
    			stag_tmp |= (1 << 15);
			*(uint16 *)(cp+12) = htons(stag_tmp) ;
		}
		if(vpm == 0)
		{
#if defined(TCSUPPORT_WAN_ETHER)
			...
			{
				if(g_stag_to_vtag == 1)
					*(uint16 *)(cp+14) = htons(5);
				else
					*(uint16 *)(cp+14) = htons(wan_port_id+1);
			}
			else
#endif
				*(uint16 *)(cp+14) = htons(0);
		}
	}

    /*EN7580: sptag is stored in tx msg. here, need to be changed*/
#if !defined(TCSUPPORT_CPU_EN7580)
#if defined(TCSUPPORT_GPON_MAPPING) && defined(TCSUPPORT_GPON_DOWNSTREAM_MAPPING)
    ...
    port_mask = ntohs(*(uint16 *)(cp+12)) & 0x3F;
    
    if(use_soc_lan == 1){
        if(port_mask != (1 << soc_switch_port))
            *(uint16 *)(cp+12) |= htons(STAG_PASSTHROUGH);
    }

#if defined(TCSUPPORT_CPU_EN7580)
    TxMsg->raw.sp_tag =  ntohs(*(uint16 *)(cp+12));
#endif
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,15,0)
	...
	bp->ecnt_sk_buff.stag_inserted = 1; 
	#endif
#endif
...
inline int macEN7512STagRemove(QDMA_RxMsg_Ether_T *rxmsg, struct sk_buff *bp)
{
	uint8 *cp=NULL;
	...
	cp = bp->data;
    /*EN7580: get spTag info from RxMsg*/
#if defined(TCSUPPORT_CPU_EN7580)
    sp_tag = rxmsg->sptag;
    ether_type = *(uint16 *)(cp+12);
#else
	sp_tag = ntohs(*(uint16 *)(cp+12));
#endif
    tci = ntohs(*(uint16 *)(cp+14));

#if defined(TCSUPPORT_LAN_VLAN) || defined(TCSUPPORT_LAN_VLAN_RANGE) || defined(TCSUPPORT_CDS) || defined(TCSUPPORT_CMCC) || defined(TCSUPPORT_CT_CONGW_CH) || defined(TCSUPPORT_CT_VLAN_BIND)
#if defined(TCSUPPORT_CPU_EN7580)
	if((ether_type == htons(ETH_P_8021Q)) || 
        (ether_type == htons(ETH_P_QinQ_88a8))){ // with 820.1q tag
        bp->lan_vlan_tci_valid = 1;
        bp->lan_vlan_tci = tci;
    }else
#else
    if((sp_tag & 0x0300) >> 8){ // with 820.1q tag
        bp->lan_vlan_tci_valid = 1;
        bp->lan_vlan_tci = tci;
    }else
#endif
	...
	if( g_stag_to_vtag == 1)
	{
		if(port_num<RT63365_SWIC_PORTNUM){
			pvid = switch_port_map[ port_num ]; /* get lan port id 			*/
			
#if defined(TCSUPPORT_MULTI_SWITCH_EXT)
			int lan_idx = 0;
			/* extend switch port , mapping by VLAN */
			/* LAN4~7 mapping VLAN1~4  default set */
			if(port_num == extend_switch_port){
                bp->dlf_resv1 = tci;
				for(lan_idx = 0; lan_idx < MAX_ETH_ITF_NUM; lan_idx ++){
					if(eth_port_vlan[lan_idx].eth_port_state == 1 &&eth_port_vlan[lan_idx].vlan_id == tci){
						pvid = lan_idx;
						break;
					}
				}
		...
		*/
		if(vpm != 0)
		{
			...
			switch(vpm)/*check vpm*/
			{
				...
				case 1:/*8100*/
					*(uint16 *)(cp+16) = htons(0x8100);
					break;

				case 2:/*88a8*/
					*(uint16 *)(cp+16) = htons(0x88a8);
					break;

				case 3:/*defined by register*/
					tpid = Get_Tpid_By_Port(pvid);
					*(uint16 *)(cp+16) = htons(tpid);
					break;
	...
	else
	{
        ...
        for(;eth_intf < MAX_ETH_ITF_NUM; eth_intf++)
        {
...
#if defined(TCSUPPORT_MULTI_SWITCH_EXT)
		/* extend switch port , mapping by VLAN */
		/* LAN4~7 mapping VLAN1~4 */
		uint16 ether_type = 0;
		for(eth_intf = 0;eth_intf < MAX_ETH_ITF_NUM; eth_intf++){
			if((port_num == extend_switch_port) && (tci == eth_port_vlan[eth_intf].vlan_id)){
				setSkbFromWhichVirtualEthItf(bp,eth_intf);
				ether_type = *(uint16 *)(cp+16);//reset vlan and vlantag
				tci = *(uint16 *)(cp+18);
				if((htons(ETH_P_8021Q) == ether_type) || 
					(htons(ETH_P_QinQ_88a8) == ether_type) ||
					 (htons(ETH_P_QinQ_9100) == ether_type)){ // 820.1q tag
					bp->lan_vlan_tci_valid = 1;
					bp->lan_vlan_tci = tci;
				}else{
					bp->lan_vlan_tci_valid = 0;
        ...
        /*EN7580: no need  change skb-data. */
#if defined(TCSUPPORT_CPU_EN7580)
#if defined(TCSUPPORT_MULTI_SWITCH_EXT)
		/* extend switch port , mapping by VLAN */
		/* so remove the special vlan also */
		if(vpm == 1 && port_num == extend_switch_port){
			*(uint16 *)(cp+12) = htons(0x8100);
			memmove(bp->data+4, bp->data, 12);
			skb_pull(bp, 4);
		}
...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CMCC) || defined(TCSUPPORT_CT_CONGW_CH)
#if defined(TCSUPPORT_SEPERATED_ETH_ITF)
    if(TagPktDSC(bp, pvid + 1) == -1)
#else
    if(TagPktDSC(bp, pvid) == -1)
#endif
		return -1;
#endif/*TCSUPPORT_COMPILE*/
...
int checkSkbFromSocSwitchPort(struct sk_buff* skb)
{
	int lan_port_id = -1;
	int switch_port_id = -1;

	if(skb->dev)
	{
		if( isLANInterface(skb->dev) )
			lan_port_id = getSwitchLANIndex(skb->dev);

		if(-1 != lan_port_id)
			switch_port_id = lan_port_map[lan_port_id];

		if(0xfe == asym_test_soc_switch)
			printk("soc_switch_port = %d,switch_port_id = %d\n",soc_switch_port,switch_port_id);
		
		if(switch_port_id == asym_test_soc_switch)
			return 1;

		if(switch_port_id == soc_switch_port)
			return 1;
		else
			return 0;
	}
...
static inline uint32 acl_write_pattern(acl_pattern_t *acl_pattern_info, u8 index)
{
	...
	switch_reg_write( VAWD1 , acl_data1 );
	switch_reg_write( VAWD2 , acl_data2 );
	switch_reg_write( VTCR , ACL_PATTERN_WRITE(index) );
...
static inline uint32 acl_write_mask(u32 acl_mask, u8 index)
{
	...
	switch_reg_write( VAWD1 , acl_data1 );
	switch_reg_write( VAWD2 , acl_data2 );
	switch_reg_write( VTCR , ACL_MASK_WRITE(index) );
...
static inline uint32 acl_write_rule(acl_control_1_t *acl_ctl_info_1, acl_control_2_t *acl_ctl_info_2 , u8 index)
{
	...
	switch_reg_write( VAWD1 , acl_data1 );
	switch_reg_write( VAWD2 , acl_data2 );
	switch_reg_write( VTCR , ACL_RULE_WRITE(index) );
...
static inline uint32 acl_write_rate(acl_rate_t *acl_rate_info , u8 index)
{
	...
	switch_reg_write( VAWD1 , acl_data1 );
	switch_reg_write( VAWD2 , acl_data2 );
	switch_reg_write( VTCR , ACL_RATE_WRITE(index) );
	return 1;
}
...
uint32 acl_set_upstream_ratelimit_for_special(u32 rate , u8 enable , u8 mode)
{
	...
	for( i = 0 ; i < 5 ; i++ )
	{
		value = switch_reg_read(GSW_PORT_CTL(i));
		value = value | (1 << 10) ;
		switch_reg_write(GSW_PORT_CTL(i) , value);
	}
...
uint32 acl_set_upstream_ratelimit_for_mac(u8 *mac , uint32 rate , u8 enable, u8 index , u8 mode)
{
	...
	acl_control_2_t acl_ctl_info_2 ;
	acl_rate_t acl_rate_info ;
	uint32 dmac_rule_max = DMAC_RULE_NUM;
	uint32 smac_rule_max = SMAC_RULE_NUM;	
...
void trgmii_interface_init(void){
    ...
    //SOC RX delay
    write_reg_word(GSW_BASE + 0x7a10, 0x4);
	write_reg_word(GSW_BASE + 0x7a18, 0x4);
	write_reg_word(GSW_BASE + 0x7a20, 0x4);
	write_reg_word(GSW_BASE + 0x7a28, 0x4);
	write_reg_word(GSW_BASE + 0x7a30, 0x4);
#endif  //YMC mark_20191107
  ...
  //Disable internal FE Phy
    ...
    //FW version
	//printk("EN7512 TRGMII V1.0");    //YMC mark_20191107
    if(use_rgmii1){
        /* set 0x5e30b for switch port 5 force link up*/
        reg = (IPG_CFG_SHORT << IPG_CFG_PN_SHIFT) | MAC_MODE_PN | FORCE_MODE_PN | 
            MAC_TX_EN_PN | MAC_RX_EN_PN | BKOFF_EN_PN | BACKPR_EN_PN | 
            (PN_SPEED_1000M<<FORCE_SPD_PN_SHIFT) | FORCE_DPX_PN | FORCE_LNK_PN;
        switch_reg_write(GSW_PMCR(5), reg);
    }
...
static void acl_for_rgmii_itf(void){
	acl_pattern_t acl_pattern_info ;
	acl_control_1_t acl_ctl_info_1 = {0};
	acl_control_2_t acl_ctl_info_2 = {0};
...
void macSetGSW(macAdapter_t *mac_p)
{
    ...
    if(use_soc_lan == 1){
        /* set cpu port as port 6 on soc switch*/
        reg = (0x40<<MFC_BC_FFP_SHIFT) | (0x40<<MFC_UNM_FFP_SHIFT) | (0x40<<MFC_UNU_FFP_SHIFT) | 
            MFC_CPU_EN  | (6<<MFC_CPU_PORT_SHIFT);
        write_reg_word(GSW_MFC, reg);
    }

#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CHT_IGMP_NOLEAVEFLOOD)	 || defined(TCSUPPORT_CZ_H108N)	
	...
	if (isFPGA) {
        ...
        // phy_api_phy_ouid    phy_api_7512
            ...
            if (read_reg_word(GSW_PMSR(0)) == 0xdeadbeef) {
                if(SWITCH_SUPPORT_4PORTS_INFPGA){
                    reg |= 0x1f << 24;
                    phy_add_start = 0;
                    phy_add_end = 4;
                }else{
	...
	reg |= (phy_add_end<<PHY_END_ADDR_SHIFT) | (phy_add_start<<PHY_ST_ADDR_SHIFT);
        ...
        for(phy_add = phy_add_start;phy_add<=phy_add_end;phy_add++){
			...
			tcMiiStationWrite(phy_add, 0x0, 0x1200);
        }
#endif
	}else{
    ...
    switch_vlan_setting();
#endif
#ifndef TCSUPPORT_CPU_EN7528
	if(isEN751627){
		reg = read_reg_word(GSW_PCR(6));
		reg |= 0x3;
		write_reg_word(GSW_PCR(6), reg);
	}
#endif

	if(use_rgmii1)
	{
		rgmii0_interface_init(5);
	}



	if(SUPPORT_SWITCH_PERPORT_RX_ACTION){

		//keep clock for arl setting
		reg = read_reg_word(GSW_CKGCR);
		reg &= 0xffffff00;
		write_reg_word(GSW_CKGCR, reg);
		if(CPORT_CFG_DEFINE_IN_SWITCH){
			reg = read_reg_word(GSW_CPORT_CFG);
			//disable fe2switch crc and set fe2switch ipg = 2
			//enable padding & set fe port port fc enable

			reg |= 1<<31;
			reg |= (1<<26);
			reg = (reg & (~(0x3<<24))) | (0x2 <<24);
			reg = (reg & (~(0xff<<8))) | (0x2 <<8);
			
			write_reg_word(GSW_CPORT_CFG, reg);

		}else{

			//disable port 6 rx crc
			//disable fe2switch crc and set fe2switch ipg = 2
			...
			//enable cport_rxg_busy
			write_reg_word(GSW_DGCR, 0x2);
		}
	}
...
**      function name: init extend switch reg
**      descriptions:
**          init 8 eth_port extern switch regisiter
**            
**      parameters:
**         
**             
...
*/
void extend_switch_reg_init(void)
{
    uint8 port =0;
    uint32 value=0;
    /*reset switch*/
    gswPbusWrite_extend( 0x7000, 0x3, GSW_1);
    /*port 0 --> vlan 1*/
    gswPbusWrite_extend( EXT_GSW_VLAN_CTL(0), 0x810006c0, GSW_1);
    gswPbusWrite_extend( EXT_GSW_PORT_CTL(0), 0x00ff0003, GSW_1);
    gswPbusWrite_extend( EXT_GSW_PORT_BASED_VLAN1(0), 0x00010008, GSW_1);
    /*port 1 --> vlan 2*/
    gswPbusWrite_extend( EXT_GSW_VLAN_CTL(1), 0x810006c0, GSW_1);
    gswPbusWrite_extend( EXT_GSW_PORT_CTL(1), 0x00ff0003, GSW_1);
    gswPbusWrite_extend( EXT_GSW_PORT_BASED_VLAN1(1), 0x00010009, GSW_1);
    /*port 2 --> vlan 3*/
    gswPbusWrite_extend( EXT_GSW_VLAN_CTL(2), 0x810006c0, GSW_1);
    gswPbusWrite_extend( EXT_GSW_PORT_CTL(2), 0x00ff0003, GSW_1);
    gswPbusWrite_extend( EXT_GSW_PORT_BASED_VLAN1(2), 0x0001000A, GSW_1);
    /*port 3 --> vlan 4*/
    gswPbusWrite_extend( EXT_GSW_VLAN_CTL(3), 0x810006c0, GSW_1);
    gswPbusWrite_extend( EXT_GSW_PORT_CTL(3), 0x00ff0003, GSW_1);
    gswPbusWrite_extend( EXT_GSW_PORT_BASED_VLAN1(3), 0x0001000B, GSW_1);
    /*port 4 --> vlan 4*/
    gswPbusWrite_extend( EXT_GSW_VLAN_CTL(4), 0x810006c0, GSW_1);
    gswPbusWrite_extend( EXT_GSW_PORT_CTL(4), 0x00ff0003, GSW_1);
    gswPbusWrite_extend( EXT_GSW_PORT_BASED_VLAN1(4), 0x0001000C, GSW_1);
    /*security mode*/
    gswPbusWrite_extend( EXT_GSW_VLAN_CTL(CPU_PORT_OF_GSW_1), 0x81000000, GSW_1);
    gswPbusWrite_extend( EXT_GSW_PORT_CTL(CPU_PORT_OF_GSW_1), 0x00ff0003, GSW_1);

    /*vlan 8 table for port 0 & 5*/
    port =0;
    value = 0x50000001 | ((1<<CPU_PORT_OF_GSW_1) | (1<<port))<<16;
    gswPbusWrite_extend( REG_VAWD1_ADDR, value, GSW_1);
    gswPbusWrite_extend( REG_VAWD2_ADDR, 0x00000000, GSW_1);
    gswPbusWrite_extend( REG_VTCR_ADDR, 0x80001008, GSW_1);
    
    /*vlan 9 table for port 1 & 5*/
    port =1;
    value = 0x50000001 | ((1<<CPU_PORT_OF_GSW_1) | (1<<port))<<16;
    gswPbusWrite_extend( REG_VAWD1_ADDR, value, GSW_1);
    gswPbusWrite_extend( REG_VAWD2_ADDR, 0x00000000, GSW_1);
    gswPbusWrite_extend( REG_VTCR_ADDR, 0x80001009, GSW_1);
    
    /*vlan 10 table for port 2 & 5*/
    port =2;
    value = 0x50000001 | ((1<<CPU_PORT_OF_GSW_1) | (1<<port))<<16;
    gswPbusWrite_extend( REG_VAWD1_ADDR, value, GSW_1);
    gswPbusWrite_extend( REG_VAWD2_ADDR, 0x00000000, GSW_1);
    gswPbusWrite_extend( REG_VTCR_ADDR, 0x8000100a, GSW_1);
    
    /*vlan 11 table for port 3 & 5*/
    port = 3;
    value = 0x50000001 | ((1<<CPU_PORT_OF_GSW_1) | (1<<port))<<16;
    gswPbusWrite_extend( REG_VAWD1_ADDR, value, GSW_1);
    gswPbusWrite_extend( REG_VAWD2_ADDR, 0x00000000, GSW_1);
    gswPbusWrite_extend( REG_VTCR_ADDR, 0x8000100b, GSW_1);
    
    /*vlan 12 table for port 4 & 5*/
    port = 4;
    value = 0x50000001 | ((1<<CPU_PORT_OF_GSW_1) | (1<<port))<<16;
    gswPbusWrite_extend( REG_VAWD1_ADDR, value, GSW_1);
    gswPbusWrite_extend( REG_VAWD2_ADDR, 0x00000000, GSW_1);
    gswPbusWrite_extend( REG_VTCR_ADDR, 0x8000100C, GSW_1);


    write_reg_word(GSW_PMCR(5), 0x0005e30b);

    /* ingress/egress rate limit calculate (4byte CRC + 12byte IPG + 8byte Preamble) */
    value = gswPbusRead_extend((GSW_GIRLC-GSW_BASE), GSW_1);
    gswPbusWrite_extend((GSW_GIRLC-GSW_BASE), (value & ~0xFF)|0x18, GSW_1);
    value = gswPbusRead_extend((GSW_GERLC-GSW_BASE), GSW_1);
    gswPbusWrite_extend((GSW_GERLC-GSW_BASE), (value & ~0xFF)|0x18, GSW_1);

     /* global mac control: set Rx Jumbo to 9K Bytes */
    gswPbusWrite_extend(0x30e0, 0x3f27, GSW_1);

    /* disable extend port5 sa learning*/
    gswPbusWrite_extend(EXT(GSW_PSC(5)), 0xFFF10, GSW_PHY_OFFSET_23);

    /* GSW 1 rgmii config */
    if(GSW_1){  // phy add 23 swtich config port6
    
        // Directly access phy mdc  & enable port6 rgmii mode
        gswPbusWrite_extend( GSW_CFG_M_HWTRAP-GSW_BASE, 0x0101669f | ((GSW_1>>3)<<11), GSW_1);
        if(CPU_PORT_OF_GSW_1 == 6){
            gswPbusWrite_extend( EXT_GSW_MAC_CTL_REG(CPU_PORT_OF_GSW_1), 0x0005e30b, GSW_1);
            // p6 rgmii center-align
            gswPbusWrite_extend( EXT_P6_IFT_MODE, 2, GSW_1);
            // P6 rgmii TX reset
            gswPbusWrite_extend( TRGMII_TX_CTL, 0x20000, GSW_1);
            gswPbusWrite_extend( TRGMII_TX_CTL, 0x10020000, GSW_1);
            gswPbusWrite_extend( TRGMII_TX_CTL, 0x20000, GSW_1);
            // enahance tx power of soc port5
            write_reg_word( RGMII_TX_POWER_2, 2);
            gswPbusWrite_extend( TRGMII_RX_CTL, 0x80020000, GSW_1);
            gswPbusWrite_extend( TRGMII_RX_CTL, 0x20000, GSW_1);
        }
    }
    ...
    if(GSW_2){  
        // Directly access phy mdc  & enable port6 rgmii mode
        gswPbusWrite_extend( GSW_CFG_M_HWTRAP-GSW_BASE, 0x0101669f|((GSW_2>>3)<<11), GSW_2);
        if(CPU_PORT_OF_GSW_2 == 6){
            gswPbusWrite_extend( EXT_GSW_MAC_CTL_REG(CPU_PORT_OF_GSW_2), 0x0005e30b, GSW_2);
            // p6 rgmii edge-align
            gswPbusWrite_extend( EXT_P6_IFT_MODE, 0, GSW_2);
            // P6 rgmii TX reset
            gswPbusWrite_extend( TRGMII_TX_CTL, 0x10020000, GSW_2);
            gswPbusWrite_extend( TRGMII_TX_CTL, 0x20000, GSW_2);
        }
    }
    ...
    if(GSW_3){  
        // Directly access phy mdc  & enable port6 rgmii mode
        gswPbusWrite_extend( GSW_CFG_M_HWTRAP-GSW_BASE, 0x0101669f|((GSW_3>>3)<<11), GSW_3);
        if(CPU_PORT_OF_GSW_3 == 6){
            gswPbusWrite_extend( EXT_GSW_MAC_CTL_REG(CPU_PORT_OF_GSW_3), 0x0005e30b, GSW_3);
            // p6 rgmii edge-align
            gswPbusWrite_extend( EXT_P6_IFT_MODE, 0, GSW_3);
            // P6 rgmii TX reset
            gswPbusWrite_extend( TRGMII_TX_CTL, 0x10020000, GSW_3);
            gswPbusWrite_extend( TRGMII_TX_CTL, 0x20000, GSW_3);
        }
    }
    return;
}
/*_____________________________________________________________________________
**      function name: init eth_port_vlan 
**      descriptions:
**          init 8 eth_port extern switch regisiter
**            
**      parameters:
...
**____________________________________________________________________________
*/
void extend_switch_para_init(void)
{
    int i = 0;

    for(i = 0; i < 4 ; i++){
        eth_port_vlan[i].eth_port_state = 0;
        eth_port_vlan[i].vlan_id = -1;
    }
    for(i = 4; i < 8 ; i++){
        eth_port_vlan[i].eth_port_state = 1;
        eth_port_vlan[i].vlan_id = i + 4;
    }
    for(i = 8; i < 11 ; i++){
        eth_port_vlan[i].eth_port_state = -1;
        eth_port_vlan[i].vlan_id = -1;
...
int mt7530_switch_init(void)
{
	...
	if(use_ext_switch == 0)
	{
        ...
        reg = read_reg_word(GSW_IMC);
        reg = reg | ((0x06 << IMC_IGMP_RPT_FW_SHIFT) | (0x06 << IMC_MLD_RPT_FW_SHIFT));
        write_reg_word(GSW_IMC, reg);
#endif
...
#if defined (TCSUPPORT_MULTI_SWITCH_EXT)
		extend_switch_reg_init();
		extend_switch_para_init();
#if defined(TCSUPPORT_CPU_EN7580)
		/* mt7530 add per port vlan, EN7580 add special tag, port5 mac learning disable*/
		write_reg_word(GSW_PSC(extend_switch_port), 0xFFF10);
#endif
#endif
	...
	else
	{
        ...
        /* SOC L2LEN check enable will drop stag packet with vlan from ext switch*/
        /* so disable soc switch l2len check*/
    	reg=read_reg_word (GSW_AGC);
    	reg &= (~(0x1<<L2LEN_CHK));
    	write_reg_word (GSW_AGC, reg);
	}
	
    if(use_soc_lan == 1){
        reg = read_reg_word(GSW_PCR(5));
        reg &= ~(0xFF<<PORT_MATRIX);
        reg |= (0x40<<PORT_MATRIX);  /* port5 only forward to cpu port*/
        write_reg_word(GSW_PCR(5), reg);
        
        reg = read_reg_word(GSW_PCR(soc_switch_port));
        reg &= ~(0xFF<<PORT_MATRIX);
        reg |= (0x40<<PORT_MATRIX);  /* soc lan port only forward to cpu port*/
        write_reg_word(GSW_PCR(soc_switch_port), reg);
	}

    #ifdef TCSUPPORT_CF_UNIFIED_PLATFORM
	for(i = 0;i <= 4;i++)
		switch_reg_write(GSW_PSC(i),0xFFF10);
    #endif
