...
int eth_vlan_set_write_proc(struct file *file, const char *buffer,
	unsigned long count, void *data)
{
	char val_string[32];
	int tx_len;
	...
	ivl_val = simple_strtoul(val_string, NULL, 16);
    printk("loop_val %8X",ivl_val);
	if(ivl_val == 0)
		ivl_en = 0;
	else
		ivl_en = 1;
...
int eth_loopback_rx_hook(void *msg_p, struct sk_buff *skb)
{
    int page_index = 0;
    unsigned int rxmsg0,rxmsg1,rxmsg2;
    if (macLoopback & LOOPBACK_MSG) {
        QDMA_API_GET_RXMSG(ECNT_QDMA_LAN,msg_p,RXMSG_FIELD_WHOLE_MSG0,&rxmsg0);
        QDMA_API_GET_RXMSG(ECNT_QDMA_LAN,msg_p,RXMSG_FIELD_WHOLE_MSG1,&rxmsg1);
        QDMA_API_GET_RXMSG(ECNT_QDMA_LAN,msg_p,RXMSG_FIELD_WHOLE_MSG2,&rxmsg2);
