...
ECONET SOFTWARE.
***************************************************************/
#ifndef _FOE_FDB_7583_H
#define _FOE_FDB_7583_H
...
#define PPE_FOE_SIZE_64 80 //7581 64byte extend to 80byte.
#define PPE_FOE_SIZE_80 80

#define IS_64_BYTE_MODE 0
#define IS_80_BYTE_MODE 1

#define PPE_FOE_SIZE_BYTE_MODE IS_64_BYTE_MODE // 0 means foe size is 64byte

#define PPE_FOE_ENTRY_NUM	16

#define PPE_TYPE_DEFAULT	PPE_TYPE_L2B_IPV4
#define PPE_FOE_SIZE_DEFAULT	PPE_FOE_SIZE_64
#define DRAM_HASH_IDX_BASE	(0x2000)
...
#define FOE_NUM_SIZE_8 	8
#define FOE_NUM_SIZE_16 16
#define FOE_NUM_SIZE_48 48
#define FOE_NUM_SIZE_56 56

#define FOE_NUM_DEFAULT FOE_NUM_SIZE_56
#define SRAM_FOE_NUM_DEFAULT FOE_NUM_SIZE_8
...
#define FP_NPU			    (6)
#define FP_TDMA				(7)
#define FP_WDMA				(7)/*P7 OQ10&11 => WDMA RX0&RX1*/
#define FP_ETH_SERDES       FP_GDMA3 
#define WDMA_RX0_OQ		10
#define WDMA_RX1_OQ		11
...
#define TUNNEL_TABLE_NUM (64)
#define TUNNEL_DOWNSTART (TUNNEL_TABLE_NUM/2)

#define IFC_ID_IN_HASH        0xff
...
#define SRAM_TB_EN							(1<<0)
#define DRAM_TB_EN							(1<<16)
#define SRAM_HASH1_EN						(1<<8)

#define SRAM_HASH1_MODE_SHIFT				12
#define SRAM_HASH1_MODE_MASK				(0xf<<SRAM_HASH1_MODE_SHIFT)
#define SRAM_HASH0_MODE_SHIFT				4
#define SRAM_HASH0_MODE_MASK				(0xf<<SRAM_HASH0_MODE_SHIFT)
#define DRAM_HASH_MODE_SHIFT				20
#define DRAM_HASH_MODE_MASK					(0xf<<DRAM_HASH_MODE_SHIFT)

#define sramFlowTableEnable()				IO_SBITS(PPE_TB_HASH_CFG, SRAM_TB_EN)
#define sramFlowTableDisable()				IO_CBITS(PPE_TB_HASH_CFG, SRAM_TB_EN)
#define isSramFlowTableEnable()				(IO_GREG(PPE_TB_HASH_CFG) & SRAM_TB_EN)
#define dramFlowTableEnable()				IO_SBITS(PPE_TB_HASH_CFG, DRAM_TB_EN)
#define dramFlowTableDisable()				IO_CBITS(PPE_TB_HASH_CFG, DRAM_TB_EN)
#define isDramFlowTableEnable()				(IO_GREG(PPE_TB_HASH_CFG) & DRAM_TB_EN)
#define sramHash1ModeEnable()				IO_SBITS(PPE_TB_HASH_CFG, SRAM_HASH1_EN)
#define sramHash1ModeDisable()				IO_CBITS(PPE_TB_HASH_CFG, SRAM_HASH1_EN)
#define isSramHash1ModeEnable()				(IO_GREG(PPE_TB_HASH_CFG) & SRAM_HASH1_EN)

#define ppeSetSramHash1Mode(val)			IO_SMASK(PPE_TB_HASH_CFG, SRAM_HASH1_MODE_MASK, SRAM_HASH1_MODE_SHIFT, val)
#define ppeGetSramHash1Mode()				IO_GMASK(PPE_TB_HASH_CFG, SRAM_HASH1_MODE_MASK, SRAM_HASH1_MODE_SHIFT)
#define ppeSetSramHash0Mode(val)			IO_SMASK(PPE_TB_HASH_CFG, SRAM_HASH0_MODE_MASK, SRAM_HASH0_MODE_SHIFT, val)
#define ppeGetSramHash0Mode()				IO_GMASK(PPE_TB_HASH_CFG, SRAM_HASH0_MODE_MASK, SRAM_HASH0_MODE_SHIFT)
#define ppeSetDramHashMode(val)				IO_SMASK(PPE_TB_HASH_CFG, DRAM_HASH_MODE_MASK, DRAM_HASH_MODE_SHIFT, val)
#define ppeGetDramHashMode()				IO_GMASK(PPE_TB_HASH_CFG, DRAM_HASH_MODE_MASK, DRAM_HASH_MODE_SHIFT)
/************************************************************************
*               M A C R O S
*************************************************************************
*/
...
#define FOE_4TB_SIZ_TOTAL   (gPpeFoeNum+hwnat_dram_idx_base)
...
typedef struct  {
	...
	uint32_t port_ag:11;	/* port account group */
	...
	uint32_t fpidx:4;		/* force to  PSE port */
...
#ifdef __BIG_ENDIAN
	uint32_t hop0:3;	/* hop id 0 */
	uint32_t hop1:3;	/* hop id 1 */
	uint32_t hop2:3;	/* hop id 2 */
	uint32_t hop3:3;	/* hop id 3 */
	uint32_t :20;
#else
	uint32_t :20;
	uint32_t hop3:3;	/* hop id 3 */
...
#ifdef __BIG_ENDIAN
	uint32_t tunnel_mtu: 16;
	uint32_t acnt_grp3:  7;
	uint32_t meter_grp3: 4;
	uint32_t meter_grp2: 5;
#else
	uint32_t meter_grp2: 5;
	uint32_t meter_grp3: 4;
	uint32_t acnt_grp3:  7;
...
struct _ipv6_3t_route {
	...
	union{
		struct{	
			#ifdef __BIG_ENDIAN
			uint32_t rev_idx_low:12;
			uint32_t smac_idx:4;
			uint32_t pppoe_id:16;
			#else
			uint32_t pppoe_id:16;
			uint32_t smac_idx:4;
			uint32_t rev_idx_low:12;
...
#if 0
	uint32_t resv1:30;
	/* pkt_dir: ask kevin, used for packet direction */
	uint32_t pkt_dir:2;
	uint32_t resv2;
	uint32_t resv3:27;
	/* classify_index: sw classify, now we have IFC, could we discard this function? */
    uint32_t classify_index:5;/*TCSUPPORT_XPON_HAL_API_EXT*/
#endif
...
#define GetInfo2Mgrp(info2)	 (0)
#define SetInfo2Mgrp(iblk2, value)


static inline void set_ppe_entry_dpi (struct FoeEntry *foe_entry, enum FoeIPAct type, unsigned int value) 
{
	if(type == IPV4_HNAPT)
		foe_entry->ipv4_hnapt.dpi = (value&0x1);
	else if(type == IPV6_5T_ROUTE)
		foe_entry->ipv6_5t_route.dpi = (value&0x1);
	else if(type == L2_BRIDGE)
		foe_entry->l2_bridge32.dpi = (value&0x1);
	else if(type == IPV4_DSLITE)
		foe_entry->ipv4_dslite.dpi = (value&0x1);
...
{
#ifdef __BIG_ENDIAN
	foe_entry->ipv4_hnapt.resv2 = ((Src[0] << 24) | (Src[1] << 16) | (Src[2] << 8) | Src[3]);
	foe_entry->ipv4_hnapt.vlan2 = ((Src[4] << 8) | Src[5]);
#else
	foe_entry->ipv4_hnapt.resv2 = ((Src[3] << 24) | (Src[2] << 16) | (Src[1] << 8) | Src[0]);
	foe_entry->ipv4_hnapt.vlan2 = ((Src[5] << 8) | Src[4]);
#endif
}

static inline void set_dualEntry_info2(struct ipv4_hnapt_dual_info2 *dual_info2, struct FoeEntry *foe_entry)
{
	memcpy((unsigned char *)(dual_info2->new_dmac_hi), (unsigned char *)(&foe_entry->ipv4_hnapt.resv2), 4);
	memcpy((unsigned char *)(dual_info2->new_dmac_lo), (unsigned char *)(&foe_entry->ipv4_hnapt.vlan2), 2);
	memcpy((unsigned char *)(dual_info2->new_smac_hi), (unsigned char *)(&foe_entry->ipv4_hnapt.dmac_hi), 4);
	memcpy((unsigned char *)(dual_info2->new_smac_lo), (unsigned char *)(&foe_entry->ipv4_hnapt.dmac_lo), 2);
...
static inline void set_channel_info (struct FoeEntry * foe_entry, enum FoeIPAct type, unsigned char value) {
	...
	2. channel 6~9 -- external switch port 1~4
	3. channel 10  -- hsgmii pcie0
	4. channel 11  -- hsgmii pcie1
	5. channel 12  -- hsgmii usb
	*/
    if(type == IPV4_HNAPT)
...
static inline int get_ppe_entry_smac_index (struct FoeEntry * foe_entry, enum FoeIPAct type) {
    ...
    {
        if(0xf == foe_entry->ipv6_3t_route.smac_idx)
            return 0x10;/*workround for hw remove keep smac bit issue,now smac idx 0xf means keep smac*/
        else
            return foe_entry->ipv6_3t_route.smac_idx;
    }
    else if(type == IPV6_5T_ROUTE)
    {
        if(0xf == foe_entry->ipv6_5t_route.smac_idx)
            return 0x10;
        else
            return foe_entry->ipv6_5t_route.smac_idx;
    }
    return 0;
}
...
static inline void set_ppe_entry_smac_index (struct FoeEntry * foe_entry, enum FoeIPAct type, unsigned char value) {
    if(0x10 == value)/*workround for hw remove keep smac bit issue,now smac idx 0xf means keep smac*/
        foe_entry->ipv6_5t_route.smac_idx = 0xf;
    else
	    foe_entry->ipv6_5t_route.smac_idx = value;
    return;
}

static inline int get_ppe_entry_tunnel_ip_index (struct FoeEntry * foe_entry, enum FoeIPAct type) {
    if(type == IPV6_6RD)
        return foe_entry->ipv6_6rd.ip4_idx;
    else if(type == IPV4_DSLITE)
        return foe_entry->ipv4_dslite.ip6_idx;
    return 0;
}

static inline void set_ppe_entry_tunnel_ip_index (struct FoeEntry * foe_entry, enum FoeIPAct type, unsigned char value) {
    if(type == IPV6_6RD)
        foe_entry->ipv6_6rd.ip4_idx = value;
    else if(type == IPV4_DSLITE)
        foe_entry->ipv4_dslite.ip6_idx= value;
    return;
}
...
static inline void set_ppe_entry_tunnel_ip (struct FoeEntry * foe_entry, enum FoeIPAct type, void * valPtr, int vlaue_flag) {
    return;
}
...
{
	foe_entry->ipv6_3t_route.snpt = snpt;
	foe_entry->ipv6_3t_route.dnpt = dnpt;
	foe_entry->ipv6_3t_route.rev_idx_high = (foe_num>>12)&0x7;
	foe_entry->ipv6_3t_route.rev_idx_low = foe_num&0xfff;
...
static inline void set_ppe_nptv6_info_mode1(struct FoeEntry * foe_entry,unsigned char snpt,unsigned char dnpt,unsigned char prefix_id,unsigned prefix_len) 
{
	foe_entry->ipv6_3t_route.snpt = snpt;
	foe_entry->ipv6_3t_route.dnpt = dnpt;
	foe_entry->ipv6_3t_route.prefix_id = prefix_id;
	foe_entry->ipv6_3t_route.prefix_len = prefix_len;
...
{
	if(type == IPV4_HNAPT){
		foe_entry->ipv4_hnapt.tunnel = tunnel;
		foe_entry->ipv4_hnapt.tunnel_id= tunnel_id;
	}else if(type == IPV6_5T_ROUTE){
		foe_entry->ipv6_5t_route.tunnel = tunnel;
		foe_entry->ipv6_5t_route.tunnel_id= tunnel_id;

	}else if(type == L2_BRIDGE){

		foe_entry->l2_bridge32.tunnel = tunnel;
		foe_entry->l2_bridge32.tunnel_id= tunnel_id;
	}else if(type == IPV6_3T_ROUTE){
        foe_entry->ipv6_3t_route.tunnel = tunnel;
		foe_entry->ipv6_3t_route.tunnel_id= tunnel_id;
	}
	return;
}
...
{
	if(type == IPV4_HNAPT){
		foe_entry->ipv4_hnapt.hop0 = hop0;
		foe_entry->ipv4_hnapt.hop1 = hop1;
		foe_entry->ipv4_hnapt.hop2 = hop2;
		foe_entry->ipv4_hnapt.hop3 = hop3;
	}else if(type == IPV6_5T_ROUTE){
		foe_entry->ipv6_5t_route.hop0 = hop0;
		foe_entry->ipv6_5t_route.hop1 = hop1;
		foe_entry->ipv6_5t_route.hop2 = hop2;
		foe_entry->ipv6_5t_route.hop3 = hop3;
	}else if(type == L2_BRIDGE){

		foe_entry->l2_bridge32.hop0 = hop0;
		foe_entry->l2_bridge32.hop1 = hop1;
		foe_entry->l2_bridge32.hop2 = hop2;
		foe_entry->l2_bridge32.hop3 = hop3;
	}else if(type == IPV6_3T_ROUTE){

		foe_entry->ipv6_3t_route.hop0 = hop0;
		foe_entry->ipv6_3t_route.hop1 = hop1;
		foe_entry->ipv6_3t_route.hop2 = hop2;
		foe_entry->ipv6_3t_route.hop3 = hop3;
	}
	return;
}
...
{
	if(type == IPV4_HNAPT){
		foe_entry->ipv4_hnapt.tunnel_mtu= tunnel_mtu;
	}else if(type == IPV6_5T_ROUTE){
		foe_entry->ipv6_5t_route.tunnel_mtu= tunnel_mtu;
	}else if(type == L2_BRIDGE){
		foe_entry->l2_bridge32.tunnel_mtu= tunnel_mtu;
	}else if(type == IPV6_3T_ROUTE){
        foe_entry->ipv6_3t_route.tunnel_mtu= tunnel_mtu;
	}
	return;
}
...
{
    if((type == IPV4_HNAPT)||(type == IPV4_DSLITE)){
		foe_entry->ipv4_hnapt.acnt_grp3=(value&0x7f);
	}else if(type == IPV6_5T_ROUTE){
		foe_entry->ipv6_5t_route.acnt_grp3=(value&0x7f);
	}else if(type == L2_BRIDGE){
		foe_entry->l2_bridge32.acnt_grp3=(value&0x7f);
	}else if(type == IPV6_3T_ROUTE){
        foe_entry->ipv6_3t_route.acnt_grp3=(value&0x7f);
	}
	return;
}
...
{
    if((type == IPV4_HNAPT)||(type == IPV4_DSLITE)){
		foe_entry->ipv4_hnapt.meter_grp2=(value&0x1f);
	}else if(type == IPV6_5T_ROUTE){
		foe_entry->ipv6_5t_route.meter_grp2=(value&0x1f);
	}else if(type == L2_BRIDGE){
		foe_entry->l2_bridge32.meter_grp2=(value&0x1f);
	}else if(type == IPV6_3T_ROUTE){
        foe_entry->ipv6_3t_route.meter_grp2=(value&0x1f);
	}
	return;
}
...
{
    if((type == IPV4_HNAPT)||(type == IPV4_DSLITE)){
		foe_entry->ipv4_hnapt.meter_grp3=(value&0xf);
	}else if(type == IPV6_5T_ROUTE){
		foe_entry->ipv6_5t_route.meter_grp3=(value&0xf);
	}else if(type == L2_BRIDGE){
		foe_entry->l2_bridge32.meter_grp3=(value&0xf);
	}else if(type == IPV6_3T_ROUTE){
        foe_entry->ipv6_3t_route.meter_grp3=(value&0xf);
	}
	return;
}
...
{
    if((type == IPV6_5T_ROUTE)||(type == IPV6_3T_ROUTE))
        foe_entry->ipv6_3t_route.snpt = value&0x1;
...
{
    if((type == IPV6_5T_ROUTE)||(type == IPV6_3T_ROUTE))
        foe_entry->ipv6_3t_route.dnpt = value&0x1;
...
static inline void set_ppe_entry_nptv6_mode0_foe_num(struct FoeEntry *foe_entry, enum FoeIPAct type, unsigned int value) 
{
    if((type == IPV6_5T_ROUTE)||(type == IPV6_3T_ROUTE))
    {
        foe_entry->ipv6_3t_route.rev_idx_high = (value>>12)&0x7;
        foe_entry->ipv6_3t_route.rev_idx_low = value&0xfff;
    }
...
{
    if((type == IPV6_5T_ROUTE)||(type == IPV6_3T_ROUTE))
        foe_entry->ipv6_3t_route.prefix_id = value&0xf;
...
{
    if((type == IPV6_5T_ROUTE)||(type == IPV6_3T_ROUTE))
        foe_entry->ipv6_3t_route.prefix_len = value&0x7f;
...
static inline void set_ppe_l2b_entry_sptag(struct FoeEntry *foe_entry, enum FoeIPAct type, unsigned int value) 
{
    if(type == L2_BRIDGE)
	{
		foe_entry->l2_bridge32.sptag = value;
	}
...
{
	if(type == IPV4_HNAT){
		foe_entry->ipv4_hnapt.sport = (value >> 16) & 0xffff;
		foe_entry->ipv4_hnapt.dport = value & 0xffff;
	}else if(type == IPV6_3T_ROUTE){
		foe_entry->ipv6_5t_route.sport = (value >> 16) & 0xffff;
		foe_entry->ipv6_5t_route.dport = value & 0xffff;
	}
