...
#if defined(TCSUPPORT_CPU_EN7516)
#define DISABLE		0
#define ENABLE		1
#endif
...
#if defined(TCSUPPORT_CPU_EN7516)
extern void set_ring_buffer_mode_auto(int enable);
#endif
...
#endif
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CHT_DSL_LED)
extern int chkLedStatus(uint8 led_no);
...
extern void os_TCIfQuery(unsigned short query_id, void *result1, void *result2);
...
down_proc:
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_C1_NEW_GUI) || defined(TCSUPPORT_CT)
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,15,0)
			ledTurnOn(LED_DSL_HANDSHAKE_STATUS);
#endif
			if (lastModemst != ADSL_MODEM_STATE_WAIT_INIT) {
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,15,0)
				ledTurnOff(LED_DSL_TRAINING_STATUS);
				ledTurnOff(LED_DSL_SHOWTIME_STATUS);
#endif
...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_C1_NEW_GUI)
			if (lastModemst != ADSL_MODEM_STATE_INIT) {
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,15,0)
				ledTurnOff(LED_DSL_SHOWTIME_STATUS);
				ledTurnOff(LED_DSL_HANDSHAKE_STATUS);
#endif
		...
		case ADSL_MODEM_STATE_UP:
			...
			#endif
			#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CHT_DSL_LED)
			if(chkLedStatus(LED_DSL_SHOWTIME_STATUS)==0){
				/*The DSL LED will be turn off at traffic,so we need to turn on it*/
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,15,0)
				ledTurnOn(LED_DSL_SHOWTIME_STATUS);
#endif
			}
...
int doAdslInfoState(int argc, char *argv[], void *p)
{
	...
	memset(&adslChannelOperData, 0, sizeof(T_adslChannelOperData));
	ops->query(ADSL_QUERY_STATUS, &modemst, NULL );
    	if (modemst == ADSL_MODEM_STATE_UP)
	    	ops->query(ADSL_QUERY_CH_OP_DATA, &adslChannelOperData, NULL);
...
{
	uint8 modemst, mode, channel, line_state, pm_state = 0;
	uint32 linerate_ds = 0, linerate_us = 0;
	T_adslChannelOperData adslChannelOperData;
	T_NTadslFarEndLineOperData adslFarEndLineOperData;
	T_NTadslNearEndLineOperData adslNearEndLineOperData;
	PMS_PARAMS pms;
	...
	memset(&CellOperdata, 0, sizeof(CELL_COUNT1));
	ops->query(ADSL_QUERY_STATUS, &modemst, NULL );
//    	if (modemst == ADSL_MODEM_STATE_UP){
		ops->query(ADSL_QUERY_MODE, &mode, NULL);
		ops->query(ADSL_QUERY_CH_OP_DATA, &adslChannelOperData, NULL);
...
int adslFileOperate(char*paravalue)
{
	...
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	writelen = ecnt_kernel_fs_write(file,(char*)paravalue,strlen(paravalue),&file->f_pos);
	set_fs(old_fs);
...
#endif
/* Enable ring buffer mode when DMT is training in 7516 platform */
#if defined(TCSUPPORT_CPU_EN7516)
	switch (event) {
		case DSL_EVENT_DOWN:
		case DSL_EVENT_UP:
			set_ring_buffer_mode_auto(DISABLE);
			break;
		case DSL_EVENT_TRAIN:
			set_ring_buffer_mode_auto(ENABLE);
			break;
	}
#endif
	return 0;
...
#if/*TCSUPPORT_COMPILE*/ defined(CWMP) && defined(TCSUPPORT_TTNET)
...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_H108NS_OTE)
	index += sprintf( stateADSL+index, "last far-end HEC error fast: %u\n",
						WanIfcfgStatsLastShowTime_counter_set.far_end.HecNotInterleaved);
	index += sprintf( stateADSL+index, "last far-end HEC error interleaved: %u\n",
						WanIfcfgStatsLastShowTime_counter_set.far_end.HecInterleaved);
#endif/*TCSUPPORT_COMPILE*/
	return index;
}
...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_H108NS_OTE)
	index += sprintf( stateADSL+index, "total far-end HEC error fast: %u\n",
						WanIfcfgStatsTotal_counter_set.far_end.HecNotInterleaved);
	index += sprintf( stateADSL+index, "total far-end HEC error interleaved: %u\n",
						WanIfcfgStatsTotal_counter_set.far_end.HecInterleaved);
#endif/*TCSUPPORT_COMPILE*/
...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_H108NS_OTE)
	index += sprintf( stateADSL+index, "curdaySt far-end HEC error fast: %u\n",
						WanIfcfgStatsCurdaySt_counter_set.far_end.HecNotInterleaved);
	index += sprintf( stateADSL+index, "curdaySt far-end HEC error interleaved: %u\n",
						WanIfcfgStatsCurdaySt_counter_set.far_end.HecInterleaved);
#endif/*TCSUPPORT_COMPILE*/
...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_H108NS_OTE)
	index += sprintf( stateADSL+index, "quthour far-end HEC error fast: %u\n",
						WanIfcfgStatsQuthrSt_counter_set.far_end.HecNotInterleaved);
	index += sprintf( stateADSL+index, "quthour far-end HEC error interleaved: %u\n",
						WanIfcfgStatsQuthrSt_counter_set.far_end.HecInterleaved);
#endif/*TCSUPPORT_COMPILE*/
...
getVDSLInterfaceCfg(char *vdslCfg)
{
	...
	switch(tr98VdslInterfaceCfg.CurrentProfiles) {
		...
		case VDSL2_PROFILE_30A:
			...
			break;
		case VDSL2_PROFILE_35B:
       			index += sprintf( vdslCfg+index, "35b\n");	
			break;
		default:
...
int tcadslcmd_init(void)
{
	...
	adsl_timer.data = 0x0;
#else
	timer_setup(&adsl_timer, adslTimer, 0);
	adsl_timer.expires = jiffies + (HZ * 2);
#endif
...
int query_adsl_link_rate(int *down_rate, int *up_rate){
	...
	*down_rate = adslChannelOperData.actualBitrateNearInt + adslChannelOperData.actualBitrateNearFast;
	*up_rate = adslChannelOperData.actualBitrateFarInt + adslChannelOperData.actualBitrateFarFast;
#else	
