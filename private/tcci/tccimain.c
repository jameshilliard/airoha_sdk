...
char sifm_info[128] = {0};
#ifdef TCSUPPORT_MIPS_SYNCTYPE10
extern unsigned int sync_type10;
#endif
int cmd_reg_add(char *cmd_name, cmds_t *cmds_p)
{
	return 0;
...
print_out_cmds:
		...
		for(i=0,cmdp = tab;cmdp->name != NULL;cmdp++){
			...
			if(cmdp->flags &0x0f != 0x02)
				continue;
#endif/*TCSUPPORT_COMPILE*/
			if(strlen(cmdp->name) < 16)
...
static unsigned long *reg_ptr;

static int reg_read_output_proc(char *page, char **start, off_t off,
	int count, int *eof, void *data)
{
	int len;
...
static int reg_read_input_proc(struct file *file, const char *buffer,
	unsigned long count, void *data)
{
	char val_string[160];
	...
	reg_ptr = (unsigned long *)simple_strtoul(val_string, NULL, 16);
...
void set_sifm_info(const char *fmt, ...)
{
	va_list args;
	char str_info[128]= {0};

	va_start(args, fmt);
	vsprintf(str_info, fmt, args);
	va_end(args);
	
	if (!strlen(str_info))
		return;

	if (strlen(sifm_info) + strlen(str_info) > sizeof(sifm_info) - 1)
		return;
	
	strcat(sifm_info, str_info);
}
static int sifm_info_read_proc(char *page, char **start, off_t off,
	int count, int *eof, void *data)
{
	int len;
	len = sprintf(page, "%s", sifm_info);
...
#ifdef TCSUPPORT_MIPS_SYNCTYPE10
static int sync_type_read_proc(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
	printk("spinlock sync cmd use %s\n", (sync_type10 & 0x1) ? "type 0x10" : "type 0");
	printk("spinunlock first sync cmd use %s\n", (sync_type10 & 0x2) ? "type 0x10" : "type 0");
	printk("spinunlock second sync cmd use %s\n", (sync_type10 & 0x4) ? "type 0x10" : "type 0");

	return 0 ;
}
static int sync_type_write_proc(struct file *file, const char *buffer,
		unsigned long count, void *data)
{
	char get_buf[8];
	int len = 0;
...
#ifdef TCSUPPORT_NPU
unsigned char npu_wifi_fast_flag = 0;
static int npu_wifi_fast_read_proc(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
	int len;
...
static int npu_wifi_fast_write_proc(struct file *file, const char *buffer,
		unsigned long count, void *data)
{
	char get_buf[8];
	int len = 0;
	...
	printk("\r\n ===type = %d====",type);
	isMailBoxSuccess = WIFI_MAIL_API_SET_WAIT_FAST_FLAG(0, type);
	if(isMailBoxSuccess != 1) {
		printk("Error: NPU_WIFI_OFFLOAD Set RX fast flag fail \n");
	}else{
		npu_wifi_fast_flag = type;
	}
...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_XPON_HAL_API)
	struct proc_dir_entry *reg_read_proc;
#endif/*TCSUPPORT_COMPILE*/
	struct proc_dir_entry *sifm_info_proc;
#ifdef TCSUPPORT_MIPS_SYNCTYPE10
	struct proc_dir_entry *sync_type_proc;
#endif
#ifdef TCSUPPORT_NPU
	struct proc_dir_entry *npu_wifi_fast_proc;
#endif
...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_XPON_HAL_API)
	/* read register */
	reg_read_proc = create_proc_entry("tc3162/reg_read", 0, NULL);
	reg_read_proc->read_proc = reg_read_output_proc;
	reg_read_proc->write_proc = reg_read_input_proc;
#endif/*TCSUPPORT_COMPILE*/

	sifm_info_proc = create_proc_entry("tc3162/sifm_info", 0, NULL);
	if (sifm_info_proc)
		sifm_info_proc->read_proc = sifm_info_read_proc;

#ifdef TCSUPPORT_MIPS_SYNCTYPE10
	sync_type_proc = create_proc_entry("tc3162/sync_type", 0, NULL) ;
	if(sync_type_proc) {
		sync_type_proc->read_proc = sync_type_read_proc ;
		sync_type_proc->write_proc = sync_type_write_proc ;
	}
#endif
#ifdef TCSUPPORT_NPU
	npu_wifi_fast_proc = create_proc_entry("tc3162/npu_wifi_fast", 0, NULL) ;
	if(npu_wifi_fast_proc) {
		npu_wifi_fast_proc->read_proc = npu_wifi_fast_read_proc ;
		npu_wifi_fast_proc->write_proc = npu_wifi_fast_write_proc ;
	}
#endif
...
#endif	
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_XPON_HAL_API)
	remove_proc_entry("tc3162/reg_read", NULL);
#endif/*TCSUPPORT_COMPILE*/
	remove_proc_entry("tc3162/sifm_info", NULL);
#ifdef TCSUPPORT_MIPS_SYNCTYPE10
	remove_proc_entry("tc3162/sync_type", NULL);
#endif
#ifdef TCSUPPORT_NPU
	remove_proc_entry("tc3162/npu_wifi_fast", NULL);
#endif
    #ifndef TCSUPPORT_CPU_ARMV8
	tcadslcmd_exit();
    #endif
