...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CUC) || defined(TCSUPPORT_CT_PON_CY)
...
int macMT7530SetPauseTime(u16 pauseTime, u8 port)
{
	...
	//FC for port6
	phy_value = (macMT7530ReadReg(0x3600) & ~(0x0030)) | (pauseTime ? (0x0030) : 0);	
	macMT7530WriteReg(0x3600, phy_value);
#endif/*TCSUPPORT_COMPILE*/
	phy_addr = macMT7530GetPortPhyAddr(port);
	phy_reg = MII_ADVERTISE;
...
int macMT7530SetDsTrtcmEnable(u8 enable)
{
	...
	{		
		...
		/* internal switch port0-port4 Q0-Q7 control 10(yellow)-11(red) */
		for(i=0; i<5; i++)
		{
			reg = 0x180c + 0x100 * i;
			value = (1<<31);
...
int macMT7530GetPortAllMac(u8 port, mt7530_switch_port_mac *outptr_PortMac)
{
	...
	while (1)
	{
		...
		{
            value = macMT7530ReadReg(REG_TSRA1_ADDR);
            outptr_PortMac->portmac[i].mac[0] = (value >> 24) & 0xFF;
			outptr_PortMac->portmac[i].mac[1] = (value >> 16) & 0xFF;
			outptr_PortMac->portmac[i].mac[2] = (value >>  8) & 0xFF;
			outptr_PortMac->portmac[i].mac[3] = (value      ) & 0xFF;
			value = macMT7530ReadReg(REG_TSRA2_ADDR);
			outptr_PortMac->portmac[i].mac[4]  = (value >> 24) & 0xFF;
			outptr_PortMac->portmac[i].mac[5]  = (value >> 16) & 0xFF;
            i++;
...
{	
    ...
    { 
        ...
        {       
    		...
    		value = macMT7530ReadReg(switch_reg);
    		value|= (1<<15);
    		value&=(0xfffff080);
    		macMT7530WriteReg((switch_reg), value);
    ...
    {  
		...
		for( queue = 0; queue < 4;queue++)
        {
			...
			value = macMT7530ReadReg(switch_reg);
			value &= (~(0xf<<24));
			value|= (((qos_wrr_info[3-queue]-1)&0xf)<<24);
			macMT7530WriteReg((switch_reg), value);
		}
	}
    //PQ_WRR mode
        ...
        { 
            if(wrr_queue_cnt<1 || wrr_queue_cnt>3)
            {
                	printk("==>doQoSDiscHWPQ_WRR(): wrong parameters. \r\n"
                		   "		usage: \r\n"
                		   "		qoscmd discpline HWPQ_WRR wrr_queue_cnt w3 w2 w1, wrr queue cnt support 1-3\r\n");
                    return -1;
            }
            
            //1. SP mode
            for(i=0; i<(4-wrr_queue_cnt); i++)
            {
                queue = 3-i;
                //Port n Queue x min.rate control enable		
                switch_reg = 0x1000 + queue*0x8 + port * 0x100;
...
int macMT7530Setqueuemark(mt7530_switch_queue_info paramext)
{
		...
		int i,j,k;
	//	u8 queuemark_enable,queuemark_shift,queuemark_width;
...
//	if (macMT7530_APIDbgDumpEn) printk(KERN_DEBUG "%s(%04X)\n", __FUNCTION__, api_param.cmdid);
	...
	portId = temport;
#endif/*TCSUPPORT_COMPILE*/
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CT_PON_CY) || defined(TCSUPPORT_CUC_C5_4P)
...
// add for storm control
        ...
        {
            ret_value = macMT753SetPortUpDown(portId, (u16)(u32)(api_param.paramext_ptr));
        }
            break;
...
int macMT7530SetPortMatrix(int group[6], int type)
{
    ...
    for(lan_port = 0; lan_port < 6; lan_port++)
    {
    ...
    if((type == 1) || (type == 2)){        
        for(switch_port = 0; switch_port < 6; switch_port++){
            set_port_matrix(switch_port,(vlanbind_matrix[switch_port] & portbind_matrix[switch_port]));
        }
    }
