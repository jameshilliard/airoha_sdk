/* //BBN_Linux/DEV/main/tclinux_phoenix/modules/private/tcphy/tcetherphy_7580.c */
/************************************************************************
...
#if defined(TCSUPPORT_CPU_EN7580) || defined(TCSUPPORT_AUTOBENCH) // allen_20171109
...
/************************************************************************
*
*            Variables and Functions for 7580 GE PHY (ID=0x9491)
*
**************************************************************************/
#ifdef LINUX_OS 
#define MT7580Ge_PORTNUM 				4		
#define MT7580Ge_PHY_INIT_LDATA_LEN 	4
#define MT7580Ge_PHY_INIT_PERPDATA_LEN 	0
#define MT7580Ge_INIT_SET_NUM 			1
#define MT7580Ge_PHY_INIT_SET_NUM 		1

#define MT7580Ge_PHY_INIT_CL45_GDATA_LEN 	14 	
#define MT7580Ge_PHY_INIT_CL45_LDATA_LEN 	18	 
#define MT7580Ge_PHY_INIT_CL45_PERPDATA_LEN 0
#define MT7580Ge_PHY_INIT_CL45_SET_NUM 		1

#define MT7580Ge_PHY_INIT_TR_LDATA_LEN 		5
#define MT7580Ge_PHY_INIT_TR_PERPDATA_LEN 	1
#define MT7580Ge_PHY_INIT_TR_SET_NUM 		1

typedef struct mt7580Ge_cfg_data_s
{
	char 		name[10];
	cfg_data_t 	ldata[MT7580Ge_PHY_INIT_LDATA_LEN];
	cfg_data_t 	perpdata[MT7580Ge_PHY_INIT_PERPDATA_LEN];			// per port register setting	
}mt7580Ge_cfg_data_t;

typedef struct mt7580Ge_cfg_cl45data_s
{
	cfg_cl45data_t gdata[MT7580Ge_PHY_INIT_CL45_GDATA_LEN];
	cfg_cl45data_t ldata[MT7580Ge_PHY_INIT_CL45_LDATA_LEN];
	cfg_cl45data_t perpdata[MT7580Ge_PHY_INIT_CL45_PERPDATA_LEN];	// per port register setting	
}mt7580Ge_cfg_cl45data_t;

typedef struct mt7580Ge_cfg_trdata_s
{
	cfg_trdata_t ldata[MT7580Ge_PHY_INIT_TR_LDATA_LEN];
	cfg_trdata_t perpdata[MT7580Ge_PHY_INIT_PERPDATA_LEN];			// per port register setting	
}mt7580Ge_cfg_trdata_t;

const uint32 mt7580Ge_page_sel_addr = 31;

mt7580Ge_cfg_data_t mt7580Ge_cfg[MT7580Ge_PHY_INIT_SET_NUM]=
{
	{
		{"Ge_1.0"}, 
    	//local data	
...
mt7580Ge_cfg_cl45data_t mt7580Ge_cfg_cl45[MT7580Ge_PHY_INIT_CL45_SET_NUM]=
{
    {	//globle data
     	{
			...
			{0x1f, 0x0272, 0x7cff},					// allen_20191224 ps_op 0x7fff/0x3cff, for PHYD bug, need to workaround 

			{0x1f, 0x0268, 0x0728},					
			{0x1f, 0x0269, 0x144f},					// allen_20191224 ps_dri  //0x244f/0x4344
			{0x1f, 0x0271, 0x7e0e},					// allen_20191224 0x7fff/0x4e13, for PHYD bug, need to workaround
     	...
     	//local data
     	{
			...
			{0x1e,0x003d,0x0c00},					// RX vbuffer bypass pwd
			{0x1e,0x0198,0x0001},					// allen_20180530, TX vld bypass pwd
			{0x1e,0x003e,0xc000},					// allen_20190902 allen_20180530, TX vld bypass pwd (pair A/B for EEE)
...
mt7580Ge_cfg_trdata_t mt7580Ge_cfg_tr[MT7580Ge_PHY_INIT_TR_SET_NUM]=
{
    {	//local data
     	{ 
...
static int32 MT7580GePhy_erroversum_A = 0;
static int32 MT7580GePhy_erroversum_B = 0;
static int32 MT7580GePhy_erroversum_C = 0;
static int32 MT7580GePhy_erroversum_D = 0;

//static void mt7580GePhyConfig(int argc, char *argv[], void *p);
void 		mt7580GePhyCfgLoad(uint8 idx);
//static void mt7580GePhyCfgTx10AmpSave(void);
void 		mt7580GePhyGRCfgCheck(void);
void 		mt7580GePhyLRCfgCheck(uint8 port_num);
void 		mt7580GePhyCfgCheck(void);

#ifdef MTPHY_DEBUG
//ESD detect
static int16 mt7580GePhy_Up2Down_cnt_gbt[5] = {0,0,0,0,0};
static int16 mt7580GePhy_Up2Down_cnt_hbt[5] = {0,0,0,0,0};
static uint8 mt7580GePhy_Up2DownFastHbt_detect[5] = {0,0,0,0,0};
static uint8 mt7580GePhy_Up2DownFastgbt_detect[5] = {0,0,0,0,0};

static bool 	mt7580GePhy_link_fail_detect_flag = 0;
//static uint8 	mt7580GePhy_reset_needed_perport[5] = {0,0,0,0,0};
//static uint8 	mt7580GePhy_reset_needed_flag = 0;
//static uint8 	mt7580GePhy_esdphy_init_flag = 1; 		// default: enable

//Disable EEE if link fail
static int16 	mt7580GePhy_Up2Down_cnt_tbt[5] = {0,0,0,0,0};
static uint8 	mt7580GePhy_Up2DownFastTbt_detect[5] = {0,0,0,0,0};
static uint8 	mt7580GePhy_linkup_check_timer[5] = {0,0,0,0,0};
static bool 	mt7580GePhyDisEEEHappened[5] = {0,0,0,0,0};
static uint8 	mt7580GePhy_linkup_check_cnt[5] = {0,0,0,0,0};
static uint8 	mt7580GePhyDisNextPage_cnt[5] = {0,0,0,0,0};
static bool 	mt7580GePhyDisNextPage_flag[5] = {0,0,0,0,0};
static uint8 	mt7580GePhyDisSlaveMode_cnt[5] = {0,0,0,0,0};  
static bool 	mt7580GePhyDisSlaveMode_flag[5] = {0,0,0,0,0};
int32 			mt7580GePhyReadProbe(uint8 port_num, uint8 mode, uint8 ch_num);
int32 			mt7580GePhyReadCoeff(uint8 port_num, uint8 mode, uint8 ch_num);
void 			mt7580GePhyDbgPorbeSet(uint8 prb_num, uint8  port_num, uint16 flg_num, uint16 clk_num );
int32 			mt7580GePhyDbgPorbeRead(uint8 prb_num,uint8  port_num);
#endif

#ifdef MTPHY_DEBUG 
int32 	mt7580GePhyReadErrOverSum(uint8 port_num);
int32 	mt7580GePhyReadAdcSum(uint8 port_num);
void 	mt7580GePhyDispProbe100(uint8 port_num, bool long_msg);
void 	mt7580GePhySwDispMessage(uint8 port_num);
uint16 	mt7580GePhyReadSnrSum(uint8 port_num, uint16 cnt);
void 	mt7580GePhyUpdateErrOverSum(uint8 port_num);
void 	mt7580GePhySwErrOverMonitor(uint8 port_num);
void 	mt7580GePhyUp2DownFastHbt(uint8 port_num);
void 	mt7580GePhyUp2DownFastTbt(uint8 port_num);
bool 	mt7580GePhyLinkFailDetect (uint8 port_num, uint8 mode);		//0:10BT or 100BT, 1:100BT only, 2:10BT only,3:10BT or 100BT or 1000BT
void 	mt7580GePhyDisEEENeeded(uint8 port_num);
void 	mt7580GePhyDisNextPageNeeded(uint8 port_num);
#endif //TCPHY_DEBUG

void 	mt7580GePhySwPatch(void);
...
void	GECal_Rext_T(uint8 phyaddr, uint32 delay);			 
...
void mt7580GePhySwDispMessage(uint8 port_num)
{
    ...
    switch(mtphy_link_state[port_num]) 
    {
        ...
        case ST_LINK_DOWN2UP: 
			...
			{
				mt7580GePhyReadProbe(port_num, Probe_VgaState, 0);
				mt7580GePhyReadProbe(port_num, Probe_MSE_ALL, 0);
				mt7580GePhyReadProbe(port_num, Probe_MSE_Slicer_err_thres, 0);
				mt7580GePhyReadProbe(port_num, Probe_Slicer_Err_Over_Sum_ALL, 0);
			}
            break;
...
void mt7580GePhyUpdateErrOverSum(uint8 port_num)
{
        ...
        && (mtphy_link_state[port_num] == ST_LINK_UP)) 
    {
        ...
        err_over_cnt_prev_D = ErovcntD[port_num].err_over_cnt;
      	...
      	// mt7580GePhyReadProbe(port_num,Probe_MSE_Sum,0);
       	mt7580GePhyReadProbe(port_num,Probe_Slicer_Err_Over_Sum,0);		//Pair A
       	mt7580GePhyReadProbe(port_num,Probe_Slicer_Err_Over_Sum,1);		//Pair B
       	mt7580GePhyReadProbe(port_num,Probe_Slicer_Err_Over_Sum,2);		//Pair C
       	mt7580GePhyReadProbe(port_num,Probe_Slicer_Err_Over_Sum,3);		//Pair D
        ...
        MT7580GePhy_erroversum_A = val_A;
        MT7580GePhy_erroversum_B = val_B;
        MT7580GePhy_erroversum_C = val_C;
        MT7580GePhy_erroversum_D = val_D;
    }
    else 
    {
        MT7580GePhy_erroversum_A = -1;
        MT7580GePhy_erroversum_B = -1;
        MT7580GePhy_erroversum_C = -1;
        MT7580GePhy_erroversum_D = -1;
    }
}
...
void mt7580GePhySwErrOverMonitor(uint8 port_num)
{
    if (tcPhy_disp_level < 3) 
        return; 			// inactive
	#if debug_flag
    if (MT7580GePhy_erroversum_A>0)
    {
        TCPHYDISP3("tcphy[%d]: ErrOver A=%ld\r\n",port_num,MT7580GePhy_erroversum_A);          
    }
    if (MT7580GePhy_erroversum_B>0)
    {
        TCPHYDISP3("tcphy[%d]: ErrOver B=%ld\r\n",port_num,MT7580GePhy_erroversum_B);          
    }
    if (MT7580GePhy_erroversum_C>0)
    {
        TCPHYDISP3("tcphy[%d]: ErrOver C =%ld\r\n",port_num,MT7580GePhy_erroversum_C);          
    }
    if (MT7580GePhy_erroversum_D>0)
    {
        TCPHYDISP3("tcphy[%d]: ErrOver D=%ld\r\n",port_num,MT7580GePhy_erroversum_D);          
    }
    #endif
}

void mt7580GePhyUp2DownFastgbt(uint8 port_num)
{
	const uint8 mt7580GePhy_Up2Down_gbt_Thd = 10;
	const uint8 mt7580GePhy_Up2Down_gbt_ub = 40;

	if(mr1E_A2.final_speed_1000 & !Nmr1[port_num].link_status) 
	{
		if(mt7580GePhy_Up2Down_cnt_gbt[port_num]<mt7580GePhy_Up2Down_gbt_ub) 
		{
			mt7580GePhy_Up2Down_cnt_gbt[port_num]+=3;
		}
    }
	else if(!mr1E_A2.final_speed_1000 & !Nmr1[port_num].link_status)
	{
		if(mt7580GePhy_Up2Down_cnt_gbt[port_num]>0) 
		{
			mt7580GePhy_Up2Down_cnt_gbt[port_num]--;
		}
	}
	else if(mr1E_A2.final_speed_1000 & Nmr1[port_num].link_status) 
	{	//adjust link-up time to clear counter
		mt7580GePhy_Up2Down_cnt_gbt[port_num]=0;
	}
	
	if(mt7580GePhy_Up2Down_cnt_gbt[port_num] > mt7580GePhy_Up2Down_gbt_Thd) 
	{
	    mt7580GePhy_Up2Down_cnt_gbt[port_num]=0;
		mt7580GePhy_Up2DownFastgbt_detect[port_num]=1;
		#ifdef TCPHY_DEBUG
		TCPHYDISP4("set mt7580GePhy_Up2DownFastgbt_detect[%d] to 1.\r\n",port_num); 
		#endif
	}
	else 
	{
		mt7580GePhy_Up2DownFastgbt_detect[port_num]=0;
	}  
}

void mt7580GePhyUp2DownFastHbt(uint8 port_num)
{
	const uint8 mt7580GePhy_Up2Down_hbt_Thd = 10;
	const uint8 mt7580GePhy_Up2Down_hbt_ub = 40;

	if(mr1E_A2.final_speed_100 & !Nmr1[port_num].link_status) 
	{
		if(mt7580GePhy_Up2Down_cnt_hbt[port_num] < mt7580GePhy_Up2Down_hbt_ub) 
		{
			mt7580GePhy_Up2Down_cnt_hbt[port_num] += 3;
		}
    }
	else if(!mr1E_A2.final_speed_100 & !Nmr1[port_num].link_status) 
	{
		if(mt7580GePhy_Up2Down_cnt_hbt[port_num]>0) 
		{
			mt7580GePhy_Up2Down_cnt_hbt[port_num]--;
		}
	}
	else if(mr1E_A2.final_speed_100 & Nmr1[port_num].link_status) 
	{	//adjust link-up time to clear counter
		mt7580GePhy_Up2Down_cnt_hbt[port_num]=0;
	}
	
	if(mt7580GePhy_Up2Down_cnt_hbt[port_num]> mt7580GePhy_Up2Down_hbt_Thd) 
	{
	    mt7580GePhy_Up2Down_cnt_hbt[port_num]=0;
		mt7580GePhy_Up2DownFastHbt_detect[port_num]=1;
		#ifdef TCPHY_DEBUG
		TCPHYDISP4("set mt7580GePhy_Up2DownFastHbt_detect[%d] to 1.\r\n",port_num); 
		#endif
	}
	else 
	{
		mt7580GePhy_Up2DownFastHbt_detect[port_num]=0;
	}  
}

void mt7580GePhyUp2DownFastTbt(uint8 port_num)
{
	const uint8 mt7580GePhy_Up2Down_Tbt_Thd = 10;
	const uint8 mt7580GePhy_Up2Down_Tbt_ub = 40;
	const uint8 linkup_check_timer_done = 1;
	
    if(mr1E_A2.final_speed_10 & !Nmr1[port_num].link_status) 
	{
		mt7580GePhy_linkup_check_timer[port_num]=0;
		if(mt7580GePhy_Up2Down_cnt_tbt[port_num] < mt7580GePhy_Up2Down_Tbt_ub) 
		{
			mt7580GePhy_Up2Down_cnt_tbt[port_num]+=5;
		}
    }
	else if(!mr1E_A2.final_speed_10 & !Nmr1[port_num].link_status) 
	{
		mt7580GePhy_linkup_check_timer[port_num]=0;
		if(mt7580GePhy_Up2Down_cnt_tbt[port_num]>0) 
		{
			mt7580GePhy_Up2Down_cnt_tbt[port_num]--;
		}
	}
	else if(mr1E_A2.final_speed_10 & Nmr1[port_num].link_status ) 
	{
		if((mt7580GePhy_linkup_check_timer[port_num] == linkup_check_timer_done)) 
		{
			mt7580GePhy_Up2Down_cnt_tbt[port_num]=0;
			mt7580GePhy_linkup_check_timer[port_num]=0;
		}
		else if((mt7580GePhy_linkup_check_timer[port_num] < linkup_check_timer_done))
		{
			mt7580GePhy_linkup_check_timer[port_num]++;
		}
	}
	
	if(mt7580GePhy_Up2Down_cnt_tbt[port_num]> mt7580GePhy_Up2Down_Tbt_Thd) 
	{
	    mt7580GePhy_Up2Down_cnt_tbt[port_num]=0; 
		mt7580GePhy_Up2DownFastTbt_detect[port_num]=1;
		#ifdef TCPHY_DEBUG
		TCPHYDISP4("set mt7580GePhy_Up2DownFastTbt_detect[%d] to 1.\r\n",port_num); 
		#endif
	}
	else 
	{
		mt7580GePhy_Up2DownFastTbt_detect[port_num]=0;
	}	
}

bool mt7580GePhyLinkFailDetect(uint8 port_num, uint8 mode)
{
	switch(mode)
	{
		case TbtOrHbtOrGbt:
			if(sw_FixUp2DownFast_flag 
				&(mt7580GePhy_Up2DownFastTbt_detect[port_num]
				||mt7580GePhy_Up2DownFastHbt_detect[port_num]||mt7580GePhy_Up2DownFastgbt_detect[port_num])) 
			{
				mt7580GePhy_link_fail_detect_flag =1;
		        #ifdef TCPHY_DEBUG
		        TCPHYDISP3("mt7580GePhy_link_fail_detect_flag(%d)=%d\r\n", port_num, mt7580GePhy_link_fail_detect_flag); 
		        #endif
			}
			else 
			{
				mt7580GePhy_link_fail_detect_flag =0;
			}	
			break;

		case HbtOnly:
			if(sw_FixUp2DownFast_flag & mt7580GePhy_Up2DownFastHbt_detect[port_num]) 
			{
				mt7580GePhy_link_fail_detect_flag =1;
		        #ifdef TCPHY_DEBUG
		        TCPHYDISP3("Hbt: mt7580GePhy_link_fail_detect_flag(%d)=%d\r\n", port_num, mt7580GePhy_link_fail_detect_flag); 
		        #endif
			}
			else 
			{
		...
		case TbtOnly:
			if(sw_FixUp2DownFast_flag & mt7580GePhy_Up2DownFastTbt_detect[port_num]) 
			{
				mt7580GePhy_link_fail_detect_flag =1;
		       	#ifdef TCPHY_DEBUG
		        TCPHYDISP3("Tbt: mt7580GePhy_link_fail_detect_flag(%d)=%d\r\n", port_num, mt7580GePhy_link_fail_detect_flag); 
		        #endif
			}
			else 
			{
	...
	return mt7580GePhy_link_fail_detect_flag;
}

void mt7580GePhyDisNextPageNeeded(uint8 port_num)
{
	const uint8 mt7580GePhyDisNextPage_cntdone = 5;
	const uint8 mt7580GePhyDisSlaveMode_cntdone = 3; 
   
   	if((mtphy_link_state[port_num] == ST_LINK_UP)||(mt7580GePhyDisEEEHappened[port_num])) 
 	{
   		mt7580GePhyDisNextPage_cnt[port_num] = 0;
   	}
   	else if((mr5.LPNextAble) && !(mr7_3D.lp_eee_100||mr7_3D.lp_eee_1000||mr7_3D.lp_eee_10g)
   		&& (mt7580GePhyDisNextPage_cnt[port_num] <= mt7580GePhyDisNextPage_cntdone)) 
   	{
   		mt7580GePhyDisNextPage_cnt[port_num]++;
   	}

   	if(mt7580GePhyDisNextPage_cnt[port_num] > mt7580GePhyDisNextPage_cntdone) 
	{
   		mt7580GePhyDisNextPage_flag[port_num] = 1;
		mt7580GePhyDisNextPage_cnt[port_num] = 0;
   	}
   	else 
	{
   		mt7580GePhyDisNextPage_flag[port_num]=0;
   	}
	...
	if((mtphy_link_state[port_num] != ST_LINK_UP) && (mtphy_link_state[port_num] != ST_LINK_DOWN) &&
		(mt7580GePhyDisSlaveMode_cnt[port_num] <= mt7580GePhyDisSlaveMode_cntdone))
	{
		mt7580GePhyDisSlaveMode_cnt[port_num]++;
	}

	if(mt7580GePhyDisSlaveMode_cnt[port_num] > mt7580GePhyDisSlaveMode_cntdone)
	{
		mt7580GePhyDisSlaveMode_flag[port_num] = 1;
		mt7580GePhyDisSlaveMode_cnt[port_num] = 0;
	}
	else
	{
		mt7580GePhyDisSlaveMode_flag[port_num] = 0;
	}
}

void mt7580GePhyDisEEENeeded(uint8 port_num)
{
    const uint8 mt7580GePhy_linkup_check_done = 1;
	uint8 phy_base = 8, phyaddr;
	
	phyaddr = phy_base + port_num;
	
    if((mtphy_link_state[port_num] == ST_LINK_UP)
		&& (mt7580GePhy_linkup_check_cnt[port_num] < mt7580GePhy_linkup_check_done)) 
	{
		mt7580GePhy_linkup_check_cnt[port_num]++;
    }
	else if(mtphy_link_state[port_num] != ST_LINK_UP)
	{
		mt7580GePhy_linkup_check_cnt[port_num] = 0;
    }
	
    if(mt7580GePhyDisEEEHappened[port_num] && mt7580GePhy_linkup_check_cnt[port_num]==mt7580GePhy_linkup_check_done) 
	{	//extend link-up time
		mtEMiiRegWrite(phyaddr, 0x07, 0x3c, 0x06);
		mt7580GePhyDisEEEHappened[port_num]=0;
    }
	else if(mt7580GePhyLinkFailDetect(port_num,3) || mt7580GePhyDisNextPage_flag[port_num]) 
	{
		mtEMiiRegWrite(phyaddr, 0x07, 0x3c, 0x0000);	// disable next page
		tcMiiStationWrite(0,0x00,0x1200);				// re-start AN
		mt7580GePhyDisEEEHappened[port_num]=1;
    }
	//TCPHYDISP3("mt7580GePhy_linkup_check_cnt[%d]=%d\r\n", port_num, mt7580GePhy_linkup_check_cnt[port_num]);
	//TCPHYDISP3("mt7580GePhyDisEEEHappened[%d]=%d\r\n", port_num, mt7580GePhyDisEEEHappened[port_num]);
}
...
void mt7580GePhySwPatch(void)
{
    ...
    for(phyaddr = gphy_base; phyaddr < (gphy_base+MT7580Ge_PORTNUM); phyaddr++) 
	{
		pn = phyaddr - phy_base;
        mtPhyReadReg(pn,1);
		...
		if(((reg9>>9)&0x1) == 1) 								// check 1000Mbps capbility
		{
			...
			//mt7580GePhyDisNextPageNeeded(pn); 
			//mt7580GePhyUp2DownFastgbt(pn);
			//mt7580GePhyUp2DownFastHbt(pn);
			//mt7580GePhyUp2DownFastTbt(pn);
			//mt7580GePhyDisEEENeeded(pn);
 
			if((mtphy_link_state[pn] != ST_LINK_UP) && (mt7580GePhyDisSlaveMode_flag[pn] == 1) && (((reg9>>12)&0x1)==1)) 
			{
				tcMiiStationWrite(phyaddr, 0x09, 0x0600); 		// disable manual  
				//tcMiiStationWrite(phyaddr, 0x00, 0x1200); 		// re-start AN
				//mt7580GePhyDisSlaveMode_cnt[pn] = 0;
				//mt7580GePhyDisSlaveMode_flag[pn] = 0;
			}
		...
		#ifdef MTPHY_DEBUG
		// display message
		mt7580GePhySwDispMessage(pn);
		#endif

		#ifdef MTPHY_DEBUG
		if(sw_ErrOverMonitor_flag)
			mt7580GePhyUpdateErrOverSum(pn);
		if(sw_ErrOverMonitor_flag) 
			mt7580GePhySwErrOverMonitor(pn); // call after mt7580FEUpdateErrOverSum()
		#endif
		#endif
    }
}
...
void mt7580GePhyCfgLoad(uint8 idx)
{
    ...
    for( pn=0; pn < MT7580Ge_PORTNUM; pn++)		
	{
		phyAddr = phyAddr_base + pn;
	
		TCPHYDISP3("\n 7580Ge, phyaddr= (%d,%d) \n", phyAddr_base, phyAddr);
		for( i=0; i<MT7580Ge_PHY_INIT_CL45_GDATA_LEN; i++ )
		{         
			mtEMiiRegWrite(phyAddr, mt7580Ge_cfg_cl45[current_idx].gdata[i].dev_num, mt7580Ge_cfg_cl45[current_idx].gdata[i].reg_num, mt7580Ge_cfg_cl45[current_idx].gdata[i].val);
		}
		for( i=0; i<MT7580Ge_PHY_INIT_CL45_LDATA_LEN; i++ )
		{         
			mtEMiiRegWrite(phyAddr, mt7580Ge_cfg_cl45[current_idx].ldata[i].dev_num, mt7580Ge_cfg_cl45[current_idx].ldata[i].reg_num, mt7580Ge_cfg_cl45[current_idx].ldata[i].val);
		}
		for( i=0; i<MT7580Ge_PHY_INIT_LDATA_LEN; i++ )
		{         
			tcMiiStationWrite(phyAddr, mt7580Ge_cfg[current_idx].ldata[i].reg_num, mt7580Ge_cfg[current_idx].ldata[i].val);
		}
		for( i=0; i<MT7580Ge_PHY_INIT_TR_LDATA_LEN; i++ )
		{         
			mtPhyMiiWrite_TrDbg( phyAddr, mt7580Ge_cfg_tr[current_idx].ldata[i].reg_typ, mt7580Ge_cfg_tr[current_idx].ldata[i].reg_num, mt7580Ge_cfg_tr[current_idx].ldata[i].val,0);
		}
	}
	
	
    TCPHYDISP4("mtphy: CfgLoad %s\r\n",  mt7580Ge_cfg[current_idx].name);
}
#endif  
#endif
...
static int doPhysetPowerSaving (int argc, char *argv[], void *p);
static int doPhyset7580_0R (int argc, char *argv[], void *p);		
static int doPhysetLED (int argc, char *argv[], void *p);			
...
static const cmds_t ethertphyprint[] =
{
	...
	{"setPowerSaving", 	doPhysetPowerSaving,	0x02, 1, "setPowerSaving <on|off> <0|1|2|3> "},
	{"set7580_0R", 		doPhyset7580_0R,		0x02, 1, "set7580_0R <on|off>"},
	{"setLED", 			doPhysetLED,			0x02, 1, "setLED <0|1|2>"},
...
int tcPhyVerLookUp(uint16 ephy_addr)
{
    ...
    else if(rval == EPHY_ID_7580Ge)  
	{
        tcPhyVer = tcPhyVer_7580Ge;		
        tcPhyPortNum = 4;
        TCPHYDISP1(" 7580 Ge, \n");
        tcPhyFlag = 1;
        tcPhyInitFlag = 1;
...
int ePhyInit(uint16 ephy_addr)
{
	...
	{			    
		...
		case tcPhyVer_7580Ge: 		// EN7580 Ge
			if(ephy_addr_base == 0)
				ephy_addr_base += 8;
			break;
	}
...
static int doPhySwVer(int argc, char *argv[], void *p)
{
	...
	{    
		case tcPhyVer_7580Ge: 
			printf("%s, \r\n", mt7580Ge_cfg[0].name);
		 	printf("tcPhyVer_7580Ge, tcPhyVer=0x%02lX \r\n", tcPhyVer);
			break;	
...
int doPhyGphyTestMode (int argc, char *argv[], void *p)
{
	...
	else	
	{
		...
		if((reg3 == EPHY_ID_7580Ge)||(reg3 == EPHY_ID_7512Ge)||(reg3==EPHY_ID_7522Ge))
		{
...
{ 	// for 7512 (linux)
   	...
   	if(phychkval_flag)
	{
		...
		else if((reg3 == EPHY_ID_7580Ge)||(reg3 == EPHY_ID_7530Ge_7512Fe))
		{ 	// Force-speed
			//mr_dplx = 0; // mr_anen=0, should be half duplex for coverity 345692
			if(mr_speed==0x00)
	 		{
		 		printf(" Force-%d%s\r\n",10,(mr_dplx?"F":"H"));
			}
		...
		{ 	// link-up
			if((reg3==EPHY_ID_7580Ge)||(reg3 == EPHY_ID_7530Ge_7512Fe)) 
			{
				if(lr_speed == 0x04)
				{
			...
			{	// 100Mbps/1000Mbps
				...
				{
					#if debug_flag
					val=mt7580GePhyReadProbe(phyAddr, Probe_VgaState, 0);
					printf("7580 VgaStateA, B, C, D = 0x%x, 0x%x, 0x%x, 0x%x\r\n",((val>>4) & 0x1F),((val>>9) & 0x1F),((val>>14) & 0x1F),((val>>19) & 0x1F));
				
					val=mt7580GePhyReadProbe(phyAddr,Probe_MSE,0);
					printf("7580 lch_mse_mdcA =0x%x\r\n",val);
					val=mt7580GePhyReadProbe(phyAddr,Probe_MSE,1);
					printf("7580 lch_mse_mdcB =0x%x\r\n",val);
					val=mt7580GePhyReadProbe(phyAddr,Probe_MSE,2);
					printf("7580 lch_mse_mdcC =0x%x\r\n",val);
					val=mt7580GePhyReadProbe(phyAddr,Probe_MSE,3);
					printf("7580 lch_mse_mdcD =0x%x\r\n",val);
			
					val=mt7580GePhyReadProbe(phyAddr,Probe_MSE_Slicer_err_thres,0);
					printf("slicer_err_thres =0x%x\r\n",val);
					val=mt7580GePhyReadProbe(phyAddr,Probe_Slicer_Err_Over_Sum,0);
					printf("ProbeSlicerErrOverSumA =0x%x\r\n",val);
					val=mt7580GePhyReadProbe(phyAddr,Probe_Slicer_Err_Over_Sum,1);
					printf("ProbeSlicerErrOverSumB =0x%x\r\n",val);
					val=mt7580GePhyReadProbe(phyAddr,Probe_Slicer_Err_Over_Sum,2);
					printf("ProbeSlicerErrOverSumC =0x%x\r\n",val);
					val=mt7580GePhyReadProbe(phyAddr,Probe_Slicer_Err_Over_Sum,3);
...
int doPhyForceMode(int argc, char *argv[], void *p) 	
{
	...
	else if(argc == 4) // tce forcemode <phyaddr> <an|force> <10|100|1000>
	{
		...
		else if(stricmp(argv[2], "force") == 0)
		{
			...
			else if((stricmp(argv[3], "1000") == 0)&&(reg3==EPHY_ID_7580Ge))
			{
...
int doPhySLTMode(int argc, char *argv[], void *p) 	// allen_20171109
{
	...
	if(argc == 3) // tce sltmode <phyaddr> <10|100|1000> 
	{
		...
		else if((stricmp(argv[2], "1000") == 0)&&(reg3==EPHY_ID_7580Ge))
		{
...
void doEphySetSETMode(uint8 set_mode, uint16 parameter)    // allen_20190709
{
	...
	else if(set_mode == 1)	// set Power Saving
	{
		...
		{
			mtEMiiRegWrite(9, 0x1f, 0x271, 0x7e14);
			mtEMiiRegWrite(9, 0x1f, 0x269, 0x444f);
		}
		else if(parameter == 1)	// for 110m
		{
			mtEMiiRegWrite(9, 0x1f, 0x271, 0x7e0e);   // allen_20191224
			//mtEMiiRegWrite(9, 0x1f, 0x272, 0x7cff);   // allen_20191224
			mtEMiiRegWrite(9, 0x1f, 0x269, 0x144f);   // allen_20191224
		}
		else if(parameter == 2)	
		{
			mtEMiiRegWrite(9, 0x1f, 0x271, 0x7c63);
			//mtEMiiRegWrite(9, 0x1f, 0x272, 0x7cff);
...
int doPhygetCALReg (int argc, char *argv[], void *p)
{
	...
	mtEMiiRegWrite( 9, 0x1e, 0x0015, 0x0004);	// gating, cutoff others pair 
	mtEMiiRegWrite(10, 0x1e, 0x0015, 0x0004);	// gating, cutoff others pair
	mtEMiiRegWrite(11, 0x1e, 0x0015, 0x0004);	// gating, cutoff others pair
	mtEMiiRegWrite(12, 0x1e, 0x0015, 0x0004);	// gating, cutoff others pair

	GECal_Rext_flag = 0;
	while(GECal_Rext_flag == 0)			
		GECal_Rext_T(9, 100);
...
int doPhyset7580_0R(int argc, char *argv[], void *p)	
{				
	uint16 	phyaddr = 0, port_num = 0;
	uint16 	reg0_temp = 0, dev1e_145_temp = 0;
...
int doPhysetPowerSaving(int argc, char *argv[], void *p) 	
{
	...
	for(phyaddr = 9; phyaddr<=12; phyaddr++)
	{
		...
		else if(PowerSaving_mode == 3)	// for 10m
		{
			tcMiiStationWrite(phyaddr, 0x17, 0x00b0);
			mtEMiiRegWrite(phyaddr, 0x1e, 0x3e, 0x0000);
			mtEMiiRegWrite(phyaddr, 0x1e, 0x5c, 0x4444);
...
void doGePhyALLAnalogCal(uint8 phyaddr) // for EN7512 GBE 
{
	...
	uint8	tx_amp_temp, tx_amp_reg, cnt=0, tx_amp_reg_100, tst_offset=0, tst_offset_p9d=0, hbt_offset=0, gbe_offset=0;
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		...
		while(all_ana_cal_status < ANACAL_ERROR)
		{
			...
			else
			{
				...
				{
					all_ana_cal_status = ANACAL_SATURATION;  // need to FT
					printf(" GE Tx offset AnaCal Saturation!  (%d)\r\n", tx_offset_temp);
				}
			}
		}
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		...
		while(all_ana_cal_status < ANACAL_ERROR)
		{
			...
			else if(((mtEMiiRegRead(9, 0x1e, 0x017a)>>8)&0x1) != ad_cal_comp_out_init) 
			{
				...
				reg_temp = mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg_100)&(~0x00ff);
				mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|((tx_amp_temp - tst_offset))));	// for tst
				if((phyaddr== 9) && (calibration_pair == ANACAL_PAIR_D))
						{
					mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|((tx_amp_temp+tst_offset_p9d))));
				}
...
void doGePhyALLAnalogCal_R45(uint8 phyaddr) 
{
	uint8	rg_zcal_ctrl, all_ana_cal_status, phyaddr_p0;
	uint16	ad_cal_comp_out_init;
	uint16	dev1e_e0_ana_cal_r5, reg0_temp, dev1e_145_temp; 
	int 	calibration_polarity, tx_offset_temp;
	uint16	calibration_pair, cal_temp;
	uint16	tx_offset_reg_shift, tx_amp_reg_shift; 
	...
	uint16	tx_amp_offset_tst_p9a=0, tx_amp_offset_tst_p9b=0, tx_amp_offset_tst_p9c=0, tx_amp_offset_tst_p9d=0;
	uint16  tx_amp_offset_tst_p10a=0, tx_amp_offset_tst_p10b=0, tx_amp_offset_tst_p10c=0, tx_amp_offset_tst_p10d=0, tx_amp_offset_tst_p11a=0, tx_amp_offset_tst_p11b=0, tx_amp_offset_tst_p11c=0, tx_amp_offset_tst_p11d=0;
	uint16	tx_amp_offset_tst_p12a=0, tx_amp_offset_tst_p12b=0, tx_amp_offset_tst_p12c=0, tx_amp_offset_tst_p12d=0; 
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		...
		else
		{
			...
			{
				tx_amp_offset_tst_p9a = 5; // 2;
				tx_amp_offset_tst_p9b = 7; // 2;
				tx_amp_offset_tst_p9c = 9; // 2;
				tx_amp_offset_tst_p9d = 7; // 2;
				tx_amp_offset_tst_p10a = 4; // 2;
				tx_amp_offset_tst_p10b = 5; // 2;
				tx_amp_offset_tst_p10c = 10; // 2;
				tx_amp_offset_tst_p10d = 5; // 2;
				tx_amp_offset_tst_p11a = 4; // 2;
				tx_amp_offset_tst_p11b = 6; // 2;
				tx_amp_offset_tst_p11c = 9; // 2;
				tx_amp_offset_tst_p11d = 6; // 2;
				tx_amp_offset_tst_p12a = 3; // 2;
				tx_amp_offset_tst_p12b = 7; // 2;
				tx_amp_offset_tst_p12c = 9; // 2;
				tx_amp_offset_tst_p12d = 6; // 2;

				if((phyaddr == 9))
				{
					if(calibration_pair == ANACAL_PAIR_A)
						tx_amp_temp_tst = tx_amp_temp - tx_amp_offset_tst_p9a;
					else if(calibration_pair == ANACAL_PAIR_B)
						tx_amp_temp_tst = tx_amp_temp - tx_amp_offset_tst_p9b;
					else if(calibration_pair == ANACAL_PAIR_C)
						tx_amp_temp_tst = tx_amp_temp - tx_amp_offset_tst_p9c;
					else if(calibration_pair == ANACAL_PAIR_D)
						tx_amp_temp_tst = tx_amp_temp - tx_amp_offset_tst_p9d;	
				}
				else if(phyaddr == 10)
				{
					if(calibration_pair == ANACAL_PAIR_A)
						tx_amp_temp_tst = tx_amp_temp - tx_amp_offset_tst_p10a;
					else if(calibration_pair == ANACAL_PAIR_B)
						tx_amp_temp_tst = tx_amp_temp - tx_amp_offset_tst_p10b;
					else if(calibration_pair == ANACAL_PAIR_C)
						tx_amp_temp_tst = tx_amp_temp - tx_amp_offset_tst_p10c;
					else if(calibration_pair == ANACAL_PAIR_D)
						tx_amp_temp_tst = tx_amp_temp - tx_amp_offset_tst_p10d;
				}
				else if(phyaddr == 11)
				{
					if(calibration_pair == ANACAL_PAIR_A)
						tx_amp_temp_tst = tx_amp_temp - tx_amp_offset_tst_p11a;
					else if(calibration_pair == ANACAL_PAIR_B)
						tx_amp_temp_tst = tx_amp_temp - tx_amp_offset_tst_p11b;
					else if(calibration_pair == ANACAL_PAIR_C)
						tx_amp_temp_tst = tx_amp_temp - tx_amp_offset_tst_p11c;
					else if(calibration_pair == ANACAL_PAIR_D)
						tx_amp_temp_tst = tx_amp_temp - tx_amp_offset_tst_p11d;	
				}
				else  // phyaddr == 12
				{
					if(calibration_pair == ANACAL_PAIR_A)
						tx_amp_temp_tst = tx_amp_temp - tx_amp_offset_tst_p12a;
					else if(calibration_pair == ANACAL_PAIR_B)
						tx_amp_temp_tst = tx_amp_temp - tx_amp_offset_tst_p12b;
					else if(calibration_pair == ANACAL_PAIR_C)
						tx_amp_temp_tst = tx_amp_temp - tx_amp_offset_tst_p12c;
					else if(calibration_pair == ANACAL_PAIR_D)
						tx_amp_temp_tst = tx_amp_temp - tx_amp_offset_tst_p12d;
				}
			}
			...
			if((all_ana_cal_status == ANACAL_SATURATION)||(tx_amp_temp_tst==0))
			{
				...
				if((calibration_pair == ANACAL_PAIR_B)||(calibration_pair == ANACAL_PAIR_D))
				{
					mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|(0x0020)));  // for tst	
				}
			}
			else
			{
			...
			{
				if((calibration_pair == ANACAL_PAIR_A))
					mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|(0x4000)));  // for gbe
				else if(calibration_pair == ANACAL_PAIR_C)
					mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|(0x1000)));  // for gbe
				else
					mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|(0x2000)));  // for gbe
			}
			else
			{
				mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|(tx_amp_temp_tst<<tx_amp_reg_shift)));	// for gbe	// 1e_12, 1e_17, 1e_18, 1e_21[13:8]
			}
			...
			else
			{
				...
				tx_amp_offset_hbt_p0_b = 2;
				tx_amp_offset_hbt_p1_a = 2;
				tx_amp_offset_hbt_p1_b = 0; // 2;
				tx_amp_offset_hbt_p2_a = 2;
				tx_amp_offset_hbt_p2_b = 1; // 2;
				tx_amp_offset_hbt_p3_a = 2;
				tx_amp_offset_hbt_p3_b = 2;
				tx_amp_offset_hbt_p4_a = 2;
				tx_amp_offset_hbt_p4_b = 2;
...
void	GECal_Rext_T(uint8 phyaddr, uint32 delay)									
{
	...
	rg_zcal_ctrl = 0x20;  								// start with 0 dB
	dev1e_e0_ana_cal_r5 = mtEMiiRegRead(phyaddr, 0x1e, 0x00e0) & 0xff00;
	...
	else
	{
		...
		//printf("  GE Rext AnaCal Done! (%d)(0x%x)  \r\n", cnt, rg_zcal_ctrl);
		GECal_Rext_flag = 1;
		printf("  GE Rext init (0x%x)  \r\n", (dev1e_e0_ana_cal_r5>>8));
...
void	GECal_tx_offset(uint8 phyaddr, uint32 delay)
{
	...
	uint16	calibration_pair=0, cal_temp=0;
	uint16	tx_offset_reg_shift=0;
	uint16	tx_offset_reg=0, reg_temp=0, cnt=0;
