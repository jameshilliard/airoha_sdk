/* //BBN_Linux/DEV/main/tclinux_phoenix/modules/private/tcphy/tcetherphy_7516.c */
/************************************************************************
...
#if defined(TCSUPPORT_CPU_EN7516) || defined(TCSUPPORT_AUTOBENCH) // allen_20171109
...
static uint8  mt7516_0R_flag = 0;		// allen_20190102
...
static uint8 tcPhyPortNum = 5; 		// set value in tcPhyVerLookUp
//static uint8 tcGPhyPortNum = 5;
...
uint32 mtPhyReadReg(uint8 port_num, uint8 reg_num)  // for GEPHY
{
	...
	{
		//if((tcPhyVer == tcPhyVer_7512Ge)||(tcPhyVer == tcPhyVer_7512Fe)||(tcPhyVer == tcPhyVer_7522Ge)||(tcPhyVer == tcPhyVer_7522Fe))
		if((tcPhyVer == tcPhyVer_7516Ge))	
			ephy_addr_base += 8;
	}
...
void 		FECal_R45(uint8 port_num, uint32 delay);		// for EN7512 // allen_20170621
static uint8 	FECal_R45_flag = 0;
...
static void mt7512FELRCfgLoad(uint8 idx, uint8 port_num)
{
	...
	if(isEN7526c)
	{
    	tcPhyWriteGReg(port_num, 5, 22, 0x0030);	// 100Mb tx p2z_mid
		tcPhyWriteGReg(port_num, 5, 25, 0x0248);	// 100Mb tx z2n_mid
		tcPhyWriteGReg(port_num, 5, 27, 0x02f0);	// 100Mb tx z2n_ovs_post
		tcPhyWriteGReg(port_num, 5, 28, 0x0230);	// 100Mb tx n2z_mid
	}
...
void mt7512FESwPatch(void)
{
    ...
    for(pn=0; pn<MT7512FE_PORTNUM; pn++)
	{
		...
		if(tcphy_link_state[pn] != ST_LINK_UP)
		{ 
			//if(((tcPhyReadLReg(pn, 0, 22)>>10)&0x1) == 1) // allen_20170425 test code for extend mode(agc=17)
			if(mt7512FEReadProbe(pn, ProbeAgccode) >= 16)
			{
				//tcPhyWriteLReg(pn, 0, 16, 0x6000); 
				//tcPhyWriteLReg(pn, 1, 20, 0x0014);
				//tcPhyWriteLReg(pn, 1, 19, 0x4400);
			}
			else
			{
				//tcPhyWriteLReg(pn, 0, 16, 0x0000); 
				//tcPhyWriteLReg(pn, 1, 20, 0x000c);
				//tcPhyWriteLReg(pn, 1, 19, 0x0400);
			}
		}
...
int32 mtPhyMiiRead_TrDbg(uint8 phyaddr, char *type, uint32 data_addr , uint8 ch_num)
{
	...
	{
		printf("Wrong TR register Type !");  // for CID:140269
    	return(0xFFFF);
	}
...
#define MT7530_PHY_INIT_LDATA_LEN 		2
#define MT7530_PHY_INIT_PERPDATA_LEN 	0
#define MT7530_PHY_INIT_SET_NUM 		1

#define MT7530_PHY_INIT_CL45_GDATA_LEN 		3	// allen_20150206	
#define MT7530_PHY_INIT_CL45_LDATA_LEN 		9	// allen_20161031
#define MT7530_PHY_INIT_CL45_PERPDATA_LEN 	0
#define MT7530_PHY_INIT_CL45_SET_NUM 		1

#define MT7530_PHY_INIT_TR_LDATA_LEN 		3
    ...
    {	//globle data
     	{
			...
			{0x1f, 0x0044, 0x0040}          // allen_20170405
		},
     	//local data
     	{
			...
			{0x1e,0x0041,0x3333}, // for RX voltage
			{0x1e,0x0016,0x0018}, // allen_20170405
			{0x1e,0x0008,0x02f0}, // z2n_ovs_post   // allen_20170405
        	
			{0x1e,0x0013,0x0000}, // allen_20190902
			{0x1e,0x0014,0x0000}, // allen_20190902
...
void mt7530GePhySwPatch(void)
{
    ...
    for(pn=0; pn<MT7530_PORTNUM; pn++)
	{
		...
		if(((reg9>>9)&0x1)==1) // check 1000Mbps capbility  // allen_20140530
		{
			...
			//mt7530GePhyDisNextPageNeeded(pn); // 20161129
			//mt7530GePhyUp2DownFastgbt(pn);	// 20161129 
			//mt7530GePhyUp2DownFastHbt(pn);	// 20161129
			//mt7530GePhyUp2DownFastTbt(pn);	// 20161129
			//mt7530GePhyDisEEENeeded(pn);		// 20161129
			...
			if(mtphy_link_state[pn] == ST_LINK_DOWN2UP) // link up
			{
			...
			if((mtphy_link_state[pn] != ST_LINK_UP)&&(mt7530_15R_flag == 1))
			{
				if(reg9 == 0x2700)
				{
					mtEMiiRegWrite(pn, 0x1e,  0x5c, 0x7777); // for 15R IOT
					mtEMiiRegWrite(pn, 0x1e,  0x45, 0x0706); // for 15R IOT
					mtEMiiRegWrite(pn, 0x1e,  0x46, 0x0707); // for 15R IOT
					mtEMiiRegWrite(pn, 0x1f, 0x108, 0xd010);		// 15R match
					mtEMiiRegWrite(pn, 0x1f,  0x44, 0x0000);		// for 1000 diff A&B
				}
				else if(reg0 == 0x2100)
				{
					mtEMiiRegWrite(pn, 0x1e,  0x5c, 0x7777); // for 15R IOT
					mtEMiiRegWrite(pn, 0x1e,  0x45, 0x0706); // for 15R IOT
					mtEMiiRegWrite(pn, 0x1e,  0x46, 0x0707); // for 15R IOT
					mtEMiiRegWrite(pn, 0x1f, 0x108, 0xd010);
				}
				else
				{
					mtEMiiRegWrite(pn, 0x1e,  0x5c, 0x5555);
					//mtEMiiRegWrite(pn, 0x1f, 0x108, 0x1010);		
					mtEMiiRegWrite(pn, 0x1e,  0x45, 0x0404);	
					mtEMiiRegWrite(pn, 0x1e,  0x46, 0x0404);	
					//mtEMiiRegWrite(pn, 0x1f,  0x44, 0x00a0);
				}
			}

			if(mt7530_15R_L_flag == 1)	// 20161129 for Lecroy scope
			{
				if((pn == 1))
				{
					if(reg9 == 0x2700)
					{
						...
						mtEMiiRegWrite(pn, 0x1e, 0x96, 0x8000);
						mtEMiiRegWrite(pn, 0x1e, 0x97, 0xa000);
						mtEMiiRegWrite(pn, 0x1e, 0x16, 0x0017);
						mtEMiiRegWrite(pn, 0x1f, 0x44, 0x0080);

						mtEMiiRegWrite(pn, 0x1f, 0x108, 0xd010);		// 15R match
						mtEMiiRegWrite(pn, 0x1e,  0x45, 0x0605);		// R offset
						mtEMiiRegWrite(pn, 0x1e,  0x46, 0x0605);		// R offset
						//mtEMiiRegWrite(pn, 0x1f,  0x44, 0x0080);		// for 1000 diff A&B
					}
					else if(reg0 == 0x2100)
					{	
						mtEMiiRegWrite(pn, 0x1e,  0x5c, 0x7777); // for 15R IOT
					...
					else
					{
						...
						mtEMiiRegWrite(pn, 0x1e, 0x96, 0x0000);
						mtEMiiRegWrite(pn, 0x1e, 0x97, 0x0000);
						//mtEMiiRegWrite(pn, 0x1f, 0x44, 0x0070);
						//mtEMiiRegWrite(pn, 0x1f, 0x108, 0x1010);		
						mtEMiiRegWrite(pn, 0x1e,  0x45, 0x0404);	
						mtEMiiRegWrite(pn, 0x1e,  0x46, 0x0404);	
						//mtEMiiRegWrite(pn, 0x1f,  0x44, 0x00a0);
				...
				else if((pn == 2)||(pn == 3)||(pn == 4))
				{
					if(reg9 == 0x2700)
					{
						...
						mtEMiiRegWrite(pn, 0x1e, 0x96, 0x8000);
						mtEMiiRegWrite(pn, 0x1e, 0x97, 0xa000);
						mtEMiiRegWrite(pn, 0x1e, 0x16, 0x0018);
						mtEMiiRegWrite(pn, 0x1f, 0x44, 0x0070);
			...
			if(mt7530_0R_L_flag == 1)
			{
				if(pn == 4)
				{
					...
					{
						mtEMiiRegWrite(pn, 0x1e, 0x16, 0x0018);
						mtEMiiRegWrite(pn, 0x1f, 0x44, 0x0090);
					}
					else
					{
						mtEMiiRegWrite(pn, 0x1f, 0x44, 0x00a0);
	...
	#if 0
    if((tcMiiStationRead(0,0x9)!=0x2700)&&(tcMiiStationRead(1,0x9)!=0x2700)&&(tcMiiStationRead(2,0x9)!=0x2700)&&(tcMiiStationRead(3,0x9)!=0x2700)&&(tcMiiStationRead(4,0x9)!=0x2700))
	{
		if((tcMiiStationRead(0,0x0)!=0x2100)&&(tcMiiStationRead(1,0x0)!=0x2100)&&(tcMiiStationRead(2,0x0)!=0x2100)&&(tcMiiStationRead(3,0x0)!=0x2100)&&(tcMiiStationRead(4,0x0)!=0x2100))
		{
			//mtEMiiRegWrite(2, 0x1f, 0x108, 0x1010); // for 15R IOT, reduce Tx amp	// 1f_108=0xd010: for FH IOT (Intel i217) // allen_20170202
		}
		//mtEMiiRegWrite(2, 0x1f,  0x44, 0x00a0);	// 1f_44=0x80: for FH IOT (Intel i217) // allen_20170202
	}	
	#endif
}
#endif
...
#define MT7512Ge_PHY_INIT_CL45_GDATA_LEN 	13 	
#define MT7512Ge_PHY_INIT_CL45_LDATA_LEN 	19 	 
#define MT7512Ge_PHY_INIT_CL45_PERPDATA_LEN 0
#define MT7512Ge_PHY_INIT_CL45_SET_NUM 		1

#define MT7512Ge_PHY_INIT_TR_LDATA_LEN 		2
    ...
    {	//globle data
     	{
			...
			{0x1f, 0x0268, 0x0728},		// ps_dri_tst
			{0x1f, 0x0269, 0x244f},		// ps_dri_gbe
			{0x1f, 0x0271, 0x4e13},		// ps_op
     	...
     	//local data
     	{
			...
			{0x1e,0x0000,0x01b0},	// 0->1, middle
			{0x1e,0x0001,0x01be},	// 0->1, ov1
			{0x1e,0x0002,0x01be},	// 0->1, ov2, z2p_ovs
			{0x1e,0x0003,0x0090},	// 1->0, middle
			{0x1e,0x0004,0x0205},	// 1->0, ov1
			{0x1e,0x0005,0x0204},	// 1->0, ov2, p2z_ovs

			{0x1e,0x0006,0x039c},	// 0->-1, middle
			{0x1e,0x0007,0x03c0},	// 0->-1, ov1
			{0x1e,0x0008,0x03bc},	// 0->-1, ov2, z2n_ovs
			{0x1e,0x0009,0x0297},	// -1->0, middle
			{0x1e,0x000a,0x0005},	// -1->0, ov1
			{0x1e,0x000b,0x0006},	// -1->0, ov2, n2z_ovs
			...
			//{0x1e,0x0201,0x4002},	// for 7522 HEC setting
			{0x1e,0x0041,0x3333},	// hvga_bias // allen_20170202: enhance rx voltage
...
mt7512Ge_cfg_trdata_t mt7512Ge_cfg_tr[MT7512Ge_PHY_INIT_TR_SET_NUM]=
{
    {	//local data
     	{ 
			...
			//{"DSPF",0x000e,0x800008}
			{"PMA",0x00,0x00002b},  // allen_20170202 : enhance rx 
			{"PMA",0x03,0x082422}
     	},
...
void 	doGePhyALLAnalogCal_R45(uint8 phyaddr); 			// for EN7512 // allen_20170621 
void	GECal_R45(uint8 phyaddr, uint32 delay);				// for EN7512 // allen_20170621
...
void mt7512GePhySwPatch(void)
{
    ...
    for(phyaddr = gphy_base; phyaddr < (gphy_base+MT7512Ge_PORTNUM); phyaddr++) // allen_20140822
	{
		...
		if(((reg9>>9)&0x1) == 1) // check 1000Mbps capbility
		{
			...
			if(mtphy_link_state[pn] == ST_LINK_UP)
			{
...
void mt7512GePhyCfgLoad(uint8 idx)
{
	...
	if(isEN7526c)	// allen_20160923
	{
		mtEMiiRegWrite(phyAddr, 0x1e, 0x201, 0x4002);	
		//mtEMiiRegWrite(phyAddr, 0x1e, 0x016, 0x2410);
		//mtEMiiRegWrite(phyAddr, 0x1e, 0x018, 0x090f);	
		mtEMiiRegWrite(phyAddr, 0x1e, 0x03d, 0x0c00);		// txpwd for R50
	}
	else // for 7512
	{
		mtEMiiRegWrite(phyAddr, 0x1e, 0x012, 0x2c10);	
		mtEMiiRegWrite(phyAddr, 0x1e, 0x017, 0x0410);
	}
...
/************************************************************************
*
*            Variables and Functions for 7516 GE PHY (ID=0x?)
*
**************************************************************************/
#ifdef LINUX_OS 
#define MT7516Ge_PORTNUM 				5		
#define MT7516Ge_PHY_INIT_LDATA_LEN 	3
#define MT7516Ge_PHY_INIT_PERPDATA_LEN 	0
#define MT7516Ge_INIT_SET_NUM 			1
#define MT7516Ge_PHY_INIT_SET_NUM 		1

#define MT7516Ge_PHY_INIT_CL45_GDATA_LEN 	14 	
#define MT7516Ge_PHY_INIT_CL45_LDATA_LEN 	24 	 
#define MT7516Ge_PHY_INIT_CL45_PERPDATA_LEN 0
#define MT7516Ge_PHY_INIT_CL45_SET_NUM 		1

#define MT7516Ge_PHY_INIT_TR_LDATA_LEN 		4
#define MT7516Ge_PHY_INIT_TR_PERPDATA_LEN 	1
#define MT7516Ge_PHY_INIT_TR_SET_NUM 		1

typedef struct mt7516Ge_cfg_data_s
{
	char 		name[10];
	cfg_data_t 	ldata[MT7516Ge_PHY_INIT_LDATA_LEN];
	cfg_data_t 	perpdata[MT7516Ge_PHY_INIT_PERPDATA_LEN];			// per port register setting	
}mt7516Ge_cfg_data_t;

typedef struct mt7516Ge_cfg_cl45data_s
{
	cfg_cl45data_t gdata[MT7516Ge_PHY_INIT_CL45_GDATA_LEN];
	cfg_cl45data_t ldata[MT7516Ge_PHY_INIT_CL45_LDATA_LEN];
	cfg_cl45data_t perpdata[MT7516Ge_PHY_INIT_CL45_PERPDATA_LEN];	// per port register setting	
}mt7516Ge_cfg_cl45data_t;

typedef struct mt7516Ge_cfg_trdata_s
{
	cfg_trdata_t ldata[MT7516Ge_PHY_INIT_TR_LDATA_LEN];
	cfg_trdata_t perpdata[MT7516Ge_PHY_INIT_PERPDATA_LEN];			// per port register setting	
}mt7516Ge_cfg_trdata_t;

const uint32 mt7516Ge_page_sel_addr = 31;

mt7516Ge_cfg_data_t mt7516Ge_cfg[MT7516Ge_PHY_INIT_SET_NUM]=
{
	{
		{"Ge_1.0"}, 
    	//local data	
     	{ 	//{0x00,0x4000}
       		//for reference {0x1f,0x2A30},{0x0d,0x1002}, 	//TEST 0D//force txfil value at test page
       		//{0x1f,0x0003}, 								// lpi (21=0x15, 26=0x1a)
       		//{0x15,0x206a},
       		//{0x1a,0x0604}
       		{0x09, 0x0600},
       		{0x17, 0x00b0},	// allen_20190902
...
mt7516Ge_cfg_cl45data_t mt7516Ge_cfg_cl45[MT7516Ge_PHY_INIT_CL45_SET_NUM]=
{
    {	//globle data
     	{
			//for reference {0x1f,0x0032,0x0C24},	// disable lpi_1000_pwr_seq_en
			//{0x1f, 0x0103, 0x0020}, 				// invert clock of DFF for phase-mux data input 
			//{0x1f, 0x0104, 0x0608}, 				// disable PLL 
			//{0x1f, 0x0104, 0x2608}, 				// enable PLL 
			//{0x1f, 0x0416, 0x0047}, 				// mcc_acc_ref_th setting 
			/*{0x1f, 0x027c, 0x0c0c}, 				// for 10BASE-T amplitude */
			/*{0x1f, 0x0021, 0x800a}, 				*/
			/*{0x1f, 0x0024, 0xc007}, 				*/
			/*{0x1f, 0x0025, 0x003f},  				*/
			{0x1f, 0x0273, 0x1000}, 				// i2mpb_tbh_ots
			{0x1f, 0x0272, 0x14ff},					// allen_20190902 ps_op 0x3cff, for PHYD bug, need to workaround 

			{0x1f, 0x0268, 0x0728},					
			{0x1f, 0x0269, 0x111f},					// allen_20190902 ps_dri  //0x4344
			{0x1f, 0x0271, 0x7ca8},					// allen_20190902 0x4e13, for PHYD bug, need to workaround
			{0x1f, 0x0044, 0x0030},					// pair delay (digital)
     	...
     	{
        	//for reference {0x1e,0x003D,0x0800},	//modify Pair A DC for 100M
        	//{0x1e,0x00a6,0x0000} 					// disable TX power control 
    		//{0x7,0x003c,0x0000},  				//disable EEE mode   GbE: IOT, wait 7511 E4 ECO
			#if 0 // keep E1 setting 
        	{0x1e,0x0001,0x0158}, 					// for 100BASE-T TX 
        	{0x1e,0x0003,0x0043}, 					// for 100BASE-T TX 
        	{0x1e,0x0004,0x0207}, 					// for 100BASE-T TX 
        	{0x1e,0x0007,0x0357}, 					// for 100BASE-T TX 
        	{0x1e,0x0008,0x02f0}, 					// for 100BASE-T TX 
        	{0x1e,0x000a,0x0006} 					// for 100BASE-T TX 
			#endif 
			/*{0x1e,0x0012,0x7230},  				// 0x7210   tx gain*/
			/*{0x1e,0x0045,0x0303},  				// 0x0202  R offset*/
			/*{0x1e,0x0046,0x0303},   				// 0x0303 // for 100BASE-T TX  R offset */

			{0x1e,0x0000,0x0190},					// 0->1, middle			// 1e_0 = 0x01b0
			{0x1e,0x0001,0x01ca},					// 0->1, ov1			// 1e_1 = 0x01be
			{0x1e,0x0002,0x01be},					// 0->1, ov2, z2p_ovs	// 1e_2 = 0x01be
			{0x1e,0x0003,0x009a},					// 1->0, middle			// 1e_3 = 0x0090
			{0x1e,0x0004,0x0205},					// 1->0, ov1			// 1e_4 = 0x0205
			{0x1e,0x0005,0x0204},					// 1->0, ov2, p2z_ovs	// 1e_5 = 0x0204

			{0x1e,0x0006,0x038a},					// 0->-1, middle		// 1e_6 = 0x039c
			{0x1e,0x0007,0x03cd},					// 0->-1, ov1			// 1e_7 = 0x03c0
			{0x1e,0x0008,0x03bc},					// 0->-1, ov2, z2n_ovs	// 1e_8 = 0x03bc
			{0x1e,0x0009,0x0297},					// -1->0, middle		// 1e_9 = 0x0297
			{0x1e,0x000a,0x0005},					// -1->0, ov1			// 1e_a = 0x0005
			{0x1e,0x000b,0x0006},					// -1->0, ov2, n2z_ovs	// 1e_b = 0x0006
			...
			{0x1e,0x003e,0x0000},					// allen_20190902
...
mt7516Ge_cfg_trdata_t mt7516Ge_cfg_tr[MT7516Ge_PHY_INIT_TR_SET_NUM]=
{
    {	//local data
     	{ 
			//for reference {"PMA",0x20,0x0700},
			...
			//{"DSPF",0x0011,0x040004},
			//{"DSPF",0x000e,0x800008}
			{"PMA",0x00,0x00002b},  				// enhance rx 
			{"PMA",0x17,0x060671},	// allen_20190902
			{"PMA",0x18,0x0e2e00},  // allen_20190902
			{"PMA",0x03,0x082422}
     	},
...
static int32 MT7516GePhy_erroversum_A = 0;
static int32 MT7516GePhy_erroversum_B = 0;
static int32 MT7516GePhy_erroversum_C = 0;
static int32 MT7516GePhy_erroversum_D = 0;

//static void mt7516GePhyConfig(int argc, char *argv[], void *p);
void 		mt7516GePhyCfgLoad(uint8 idx);
//static void mt7516GePhyCfgTx10AmpSave(void);
void 		mt7516GePhyGRCfgCheck(void);
void 		mt7516GePhyLRCfgCheck(uint8 port_num);
void 		mt7516GePhyCfgCheck(void);

#ifdef MTPHY_DEBUG
//ESD detect
static int16 mt7516GePhy_Up2Down_cnt_gbt[5] = {0,0,0,0,0};
static int16 mt7516GePhy_Up2Down_cnt_hbt[5] = {0,0,0,0,0};
static uint8 mt7516GePhy_Up2DownFastHbt_detect[5] = {0,0,0,0,0};
static uint8 mt7516GePhy_Up2DownFastgbt_detect[5] = {0,0,0,0,0};

static bool 	mt7516GePhy_link_fail_detect_flag = 0;
//static uint8 	mt7516GePhy_reset_needed_perport[5] = {0,0,0,0,0};
//static uint8 	mt7516GePhy_reset_needed_flag = 0;
//static uint8 	mt7516GePhy_esdphy_init_flag = 1; 		// default: enable

//Disable EEE if link fail
static int16 	mt7516GePhy_Up2Down_cnt_tbt[5] = {0,0,0,0,0};
static uint8 	mt7516GePhy_Up2DownFastTbt_detect[5] = {0,0,0,0,0};
static uint8 	mt7516GePhy_linkup_check_timer[5] = {0,0,0,0,0};
static bool 	mt7516GePhyDisEEEHappened[5] = {0,0,0,0,0};
static uint8 	mt7516GePhy_linkup_check_cnt[5] = {0,0,0,0,0};
static uint8 	mt7516GePhyDisNextPage_cnt[5] = {0,0,0,0,0};
static bool 	mt7516GePhyDisNextPage_flag[5] = {0,0,0,0,0};
static uint8 	mt7516GePhyDisSlaveMode_cnt[5] = {0,0,0,0,0};  
static bool 	mt7516GePhyDisSlaveMode_flag[5] = {0,0,0,0,0};
int32 			mt7516GePhyReadProbe(uint8 port_num, uint8 mode, uint8 ch_num);
int32 			mt7516GePhyReadCoeff(uint8 port_num, uint8 mode, uint8 ch_num);
void 			mt7516GePhyDbgPorbeSet(uint8 prb_num, uint8  port_num, uint16 flg_num, uint16 clk_num );
int32 			mt7516GePhyDbgPorbeRead(uint8 prb_num,uint8  port_num);
#endif

#ifdef MTPHY_DEBUG 
int32 	mt7516GePhyReadErrOverSum(uint8 port_num);
int32 	mt7516GePhyReadAdcSum(uint8 port_num);
void 	mt7516GePhyDispProbe100(uint8 port_num, bool long_msg);
void 	mt7516GePhySwDispMessage(uint8 port_num);
uint16 	mt7516GePhyReadSnrSum(uint8 port_num, uint16 cnt);
void 	mt7516GePhyUpdateErrOverSum(uint8 port_num);
void 	mt7516GePhySwErrOverMonitor(uint8 port_num);
void 	mt7516GePhyUp2DownFastHbt(uint8 port_num);
void 	mt7516GePhyUp2DownFastTbt(uint8 port_num);
bool 	mt7516GePhyLinkFailDetect (uint8 port_num, uint8 mode);		//0:10BT or 100BT, 1:100BT only, 2:10BT only,3:10BT or 100BT or 1000BT
void 	mt7516GePhyDisEEENeeded(uint8 port_num);
void 	mt7516GePhyDisNextPageNeeded(uint8 port_num);
#endif //TCPHY_DEBUG

void 	mt7516GePhySwPatch(void);
...
//void	GECal_R50(uint8 phyaddr, uint32 delay);				 
...
void 	doEphySetSETMode(uint8 set_mode, uint16 parameter); 			 
...
void mt7516GePhySwDispMessage(uint8 port_num)
{
    ...
    switch(mtphy_link_state[port_num]) 
    {
        ...
        case ST_LINK_DOWN2UP: 
			...
			if(mr1E_A2.final_speed_1000 ||mr1E_A2.final_speed_100 ||mr1E_A2.final_speed_10)
			{
				mt7516GePhyReadProbe(port_num, Probe_VgaState, 0);
				mt7516GePhyReadProbe(port_num, Probe_MSE_ALL, 0);
				mt7516GePhyReadProbe(port_num, Probe_MSE_Slicer_err_thres, 0);
				mt7516GePhyReadProbe(port_num, Probe_Slicer_Err_Over_Sum_ALL, 0);
			}
            break;
...
void mt7516GePhyUpdateErrOverSum(uint8 port_num)
{
        ...
        && (mtphy_link_state[port_num] == ST_LINK_UP)) 
    {
        ...
        err_over_cnt_prev_D = ErovcntD[port_num].err_over_cnt;
      	...
      	// mt7516GePhyReadProbe(port_num,Probe_MSE_Sum,0);
       	mt7516GePhyReadProbe(port_num,Probe_Slicer_Err_Over_Sum,0);		//Pair A
       	mt7516GePhyReadProbe(port_num,Probe_Slicer_Err_Over_Sum,1);		//Pair B
       	mt7516GePhyReadProbe(port_num,Probe_Slicer_Err_Over_Sum,2);		//Pair C
       	mt7516GePhyReadProbe(port_num,Probe_Slicer_Err_Over_Sum,3);		//Pair D
        val_A = ErovcntA[port_num].err_over_cnt - err_over_cnt_prev_A;
        val_B = ErovcntB[port_num].err_over_cnt - err_over_cnt_prev_B;
        val_C = ErovcntC[port_num].err_over_cnt - err_over_cnt_prev_C;
        val_D = ErovcntD[port_num].err_over_cnt - err_over_cnt_prev_D;
        ...
        MT7516GePhy_erroversum_A = val_A;
        MT7516GePhy_erroversum_B = val_B;
        MT7516GePhy_erroversum_C = val_C;
        MT7516GePhy_erroversum_D = val_D;
    }
    else 
    {
        MT7516GePhy_erroversum_A = -1;
        MT7516GePhy_erroversum_B = -1;
        MT7516GePhy_erroversum_C = -1;
        MT7516GePhy_erroversum_D = -1;
    }
}
...
void mt7516GePhySwErrOverMonitor(uint8 port_num)
{
    if (tcPhy_disp_level < 3) 
        return; 			// inactive
	#if debug_flag
    if (MT7516GePhy_erroversum_A>0)
    {
        TCPHYDISP3("tcphy[%d]: ErrOver A=%ld\r\n",port_num,MT7516GePhy_erroversum_A);          
    }
    if (MT7516GePhy_erroversum_B>0)
    {
        TCPHYDISP3("tcphy[%d]: ErrOver B=%ld\r\n",port_num,MT7516GePhy_erroversum_B);          
    }
    if (MT7516GePhy_erroversum_C>0)
    {
        TCPHYDISP3("tcphy[%d]: ErrOver C =%ld\r\n",port_num,MT7516GePhy_erroversum_C);          
    }
    if (MT7516GePhy_erroversum_D>0)
    {
        TCPHYDISP3("tcphy[%d]: ErrOver D=%ld\r\n",port_num,MT7516GePhy_erroversum_D);          
    }
    #endif
}

void mt7516GePhyUp2DownFastgbt(uint8 port_num)
{
	const uint8 mt7516GePhy_Up2Down_gbt_Thd = 10;
	const uint8 mt7516GePhy_Up2Down_gbt_ub = 40;

	if(mr1E_A2.final_speed_1000 & !Nmr1[port_num].link_status) 
	{
		if(mt7516GePhy_Up2Down_cnt_gbt[port_num]<mt7516GePhy_Up2Down_gbt_ub) 
		{
			mt7516GePhy_Up2Down_cnt_gbt[port_num]+=3;
		}
    }
	else if(!mr1E_A2.final_speed_1000 & !Nmr1[port_num].link_status)
	{
		if(mt7516GePhy_Up2Down_cnt_gbt[port_num]>0) 
		{
			mt7516GePhy_Up2Down_cnt_gbt[port_num]--;
		}
	}
	else if(mr1E_A2.final_speed_1000 & Nmr1[port_num].link_status) 
	{	//adjust link-up time to clear counter
		mt7516GePhy_Up2Down_cnt_gbt[port_num]=0;
	}
	
	if(mt7516GePhy_Up2Down_cnt_gbt[port_num] > mt7516GePhy_Up2Down_gbt_Thd) 
	{
	    mt7516GePhy_Up2Down_cnt_gbt[port_num]=0;
		mt7516GePhy_Up2DownFastgbt_detect[port_num]=1;
		#ifdef TCPHY_DEBUG
		TCPHYDISP4("set mt7516GePhy_Up2DownFastgbt_detect[%d] to 1.\r\n",port_num); 
		#endif
	}
	else 
	{
		mt7516GePhy_Up2DownFastgbt_detect[port_num]=0;
	}  
}

void mt7516GePhyUp2DownFastHbt(uint8 port_num)
{
	const uint8 mt7516GePhy_Up2Down_hbt_Thd = 10;
	const uint8 mt7516GePhy_Up2Down_hbt_ub = 40;

	if(mr1E_A2.final_speed_100 & !Nmr1[port_num].link_status) 
	{
		if(mt7516GePhy_Up2Down_cnt_hbt[port_num] < mt7516GePhy_Up2Down_hbt_ub) 
		{
			mt7516GePhy_Up2Down_cnt_hbt[port_num] += 3;
		}
    }
	else if(!mr1E_A2.final_speed_100 & !Nmr1[port_num].link_status) 
	{
		if(mt7516GePhy_Up2Down_cnt_hbt[port_num]>0) 
		{
			mt7516GePhy_Up2Down_cnt_hbt[port_num]--;
		}
	}
	else if(mr1E_A2.final_speed_100 & Nmr1[port_num].link_status) 
	{	//adjust link-up time to clear counter
		mt7516GePhy_Up2Down_cnt_hbt[port_num]=0;
	}
	
	if(mt7516GePhy_Up2Down_cnt_hbt[port_num]> mt7516GePhy_Up2Down_hbt_Thd) 
	{
	    mt7516GePhy_Up2Down_cnt_hbt[port_num]=0;
		mt7516GePhy_Up2DownFastHbt_detect[port_num]=1;
		#ifdef TCPHY_DEBUG
		TCPHYDISP4("set mt7516GePhy_Up2DownFastHbt_detect[%d] to 1.\r\n",port_num); 
		#endif
	}
	else 
	{
		mt7516GePhy_Up2DownFastHbt_detect[port_num]=0;
	}  
}

void mt7516GePhyUp2DownFastTbt(uint8 port_num)
{
	const uint8 mt7516GePhy_Up2Down_Tbt_Thd = 10;
	const uint8 mt7516GePhy_Up2Down_Tbt_ub = 40;
	const uint8 linkup_check_timer_done = 1;
	
    if(mr1E_A2.final_speed_10 & !Nmr1[port_num].link_status) 
	{
		mt7516GePhy_linkup_check_timer[port_num]=0;
		if(mt7516GePhy_Up2Down_cnt_tbt[port_num] < mt7516GePhy_Up2Down_Tbt_ub) 
		{
			mt7516GePhy_Up2Down_cnt_tbt[port_num]+=5;
		}
    }
	else if(!mr1E_A2.final_speed_10 & !Nmr1[port_num].link_status) 
	{
		mt7516GePhy_linkup_check_timer[port_num]=0;
		if(mt7516GePhy_Up2Down_cnt_tbt[port_num]>0) 
		{
			mt7516GePhy_Up2Down_cnt_tbt[port_num]--;
		}
	}
	else if(mr1E_A2.final_speed_10 & Nmr1[port_num].link_status ) 
	{
		if((mt7516GePhy_linkup_check_timer[port_num] == linkup_check_timer_done)) 
		{
			mt7516GePhy_Up2Down_cnt_tbt[port_num]=0;
			mt7516GePhy_linkup_check_timer[port_num]=0;
		}
		else if((mt7516GePhy_linkup_check_timer[port_num] < linkup_check_timer_done))
		{
			mt7516GePhy_linkup_check_timer[port_num]++;
		}
	}
	
	if(mt7516GePhy_Up2Down_cnt_tbt[port_num]> mt7516GePhy_Up2Down_Tbt_Thd) 
	{
	    mt7516GePhy_Up2Down_cnt_tbt[port_num]=0; 
		mt7516GePhy_Up2DownFastTbt_detect[port_num]=1;
		#ifdef TCPHY_DEBUG
		TCPHYDISP4("set mt7516GePhy_Up2DownFastTbt_detect[%d] to 1.\r\n",port_num); 
		#endif
	}
	else 
	{
		mt7516GePhy_Up2DownFastTbt_detect[port_num]=0;
	}	
}

bool mt7516GePhyLinkFailDetect(uint8 port_num, uint8 mode)
{
	switch(mode)
	{
		case TbtOrHbtOrGbt:
			if(sw_FixUp2DownFast_flag 
				&(mt7516GePhy_Up2DownFastTbt_detect[port_num]
				||mt7516GePhy_Up2DownFastHbt_detect[port_num]||mt7516GePhy_Up2DownFastgbt_detect[port_num])) 
			{
				mt7516GePhy_link_fail_detect_flag =1;
		        #ifdef TCPHY_DEBUG
		        TCPHYDISP3("mt7516GePhy_link_fail_detect_flag(%d)=%d\r\n", port_num, mt7516GePhy_link_fail_detect_flag); 
		        #endif
			}
			else 
			{
				mt7516GePhy_link_fail_detect_flag =0;
			}	
			break;

		case HbtOnly:
			if(sw_FixUp2DownFast_flag & mt7516GePhy_Up2DownFastHbt_detect[port_num]) 
			{
				mt7516GePhy_link_fail_detect_flag =1;
		        #ifdef TCPHY_DEBUG
		        TCPHYDISP3("Hbt: mt7516GePhy_link_fail_detect_flag(%d)=%d\r\n", port_num, mt7516GePhy_link_fail_detect_flag); 
		        #endif
			}
			else 
			{
		...
		case TbtOnly:
			if(sw_FixUp2DownFast_flag & mt7516GePhy_Up2DownFastTbt_detect[port_num]) 
			{
				mt7516GePhy_link_fail_detect_flag =1;
		       	#ifdef TCPHY_DEBUG
		        TCPHYDISP3("Tbt: mt7516GePhy_link_fail_detect_flag(%d)=%d\r\n", port_num, mt7516GePhy_link_fail_detect_flag); 
		        #endif
			}
			else 
			{
	...
	return mt7516GePhy_link_fail_detect_flag;
}

void mt7516GePhyDisNextPageNeeded(uint8 port_num)
{
	const uint8 mt7516GePhyDisNextPage_cntdone = 5;
	const uint8 mt7516GePhyDisSlaveMode_cntdone = 3; 
   
   	if((mtphy_link_state[port_num] == ST_LINK_UP)||(mt7516GePhyDisEEEHappened[port_num])) 
 	{
   		mt7516GePhyDisNextPage_cnt[port_num] = 0;
   	}
   	else if((mr5.LPNextAble) && !(mr7_3D.lp_eee_100||mr7_3D.lp_eee_1000||mr7_3D.lp_eee_10g)
   		&& (mt7516GePhyDisNextPage_cnt[port_num] <= mt7516GePhyDisNextPage_cntdone)) 
   	{
   		mt7516GePhyDisNextPage_cnt[port_num]++;
   	}

   	if(mt7516GePhyDisNextPage_cnt[port_num] > mt7516GePhyDisNextPage_cntdone) 
	{
   		mt7516GePhyDisNextPage_flag[port_num] = 1;
		mt7516GePhyDisNextPage_cnt[port_num] = 0;
   	}
   	else 
	{
   		mt7516GePhyDisNextPage_flag[port_num]=0;
   	}

	// === for gphy eee iot with realtek : default slave mode
	if((mtphy_link_state[port_num] != ST_LINK_UP) && (mtphy_link_state[port_num] != ST_LINK_DOWN) &&
		(mt7516GePhyDisSlaveMode_cnt[port_num] <= mt7516GePhyDisSlaveMode_cntdone))
	{
		mt7516GePhyDisSlaveMode_cnt[port_num]++;
	}

	if(mt7516GePhyDisSlaveMode_cnt[port_num] > mt7516GePhyDisSlaveMode_cntdone)
	{
		mt7516GePhyDisSlaveMode_flag[port_num] = 1;
		mt7516GePhyDisSlaveMode_cnt[port_num] = 0;
	}
	else
	{
		mt7516GePhyDisSlaveMode_flag[port_num] = 0;
	}
}

void mt7516GePhyDisEEENeeded(uint8 port_num)
{
    const uint8 mt7516GePhy_linkup_check_done = 1;
	uint8 phy_base = 8, phyaddr;
	
	phyaddr = phy_base + port_num;
	
    if((mtphy_link_state[port_num] == ST_LINK_UP)
		&& (mt7516GePhy_linkup_check_cnt[port_num] < mt7516GePhy_linkup_check_done)) 
	{
		mt7516GePhy_linkup_check_cnt[port_num]++;
    }
	else if(mtphy_link_state[port_num] != ST_LINK_UP)
	{
		mt7516GePhy_linkup_check_cnt[port_num] = 0;
    }
	
    if(mt7516GePhyDisEEEHappened[port_num] && mt7516GePhy_linkup_check_cnt[port_num]==mt7516GePhy_linkup_check_done) 
	{	//extend link-up time
		mtEMiiRegWrite(phyaddr, 0x07, 0x3c, 0x06);
		mt7516GePhyDisEEEHappened[port_num]=0;
    }
	else if(mt7516GePhyLinkFailDetect(port_num,3) || mt7516GePhyDisNextPage_flag[port_num]) 
	{
		mtEMiiRegWrite(phyaddr, 0x07, 0x3c, 0x0000);	// disable next page
		tcMiiStationWrite(0,0x00,0x1200);				// re-start AN
		mt7516GePhyDisEEEHappened[port_num]=1;
    }
	//TCPHYDISP3("mt7516GePhy_linkup_check_cnt[%d]=%d\r\n", port_num, mt7516GePhy_linkup_check_cnt[port_num]);
	//TCPHYDISP3("mt7516GePhyDisEEEHappened[%d]=%d\r\n", port_num, mt7516GePhyDisEEEHappened[port_num]);
}
...
void mt7516GePhySwPatch(void)
{
    uint8 	pn;
	uint32 	reg0, reg1, reg9, reg10;
	uint8 	gphy_base=8, phyaddr, phy_base=8;
    ...
    for(phyaddr = gphy_base; phyaddr < (gphy_base+MT7516Ge_PORTNUM); phyaddr++) 
	{
		pn = phyaddr - phy_base;
        mtPhyReadReg(pn,1);
        reg0 = tcMiiStationRead(phyaddr, 0x0);
		...
		if(((reg9>>9)&0x1) == 1) 								// check 1000Mbps capbility
		{
			...
			//mt7516GePhyDisNextPageNeeded(pn); 
			//mt7516GePhyUp2DownFastgbt(pn);
			//mt7516GePhyUp2DownFastHbt(pn);
			//mt7516GePhyUp2DownFastTbt(pn);
			//mt7516GePhyDisEEENeeded(pn);
 
			if((mtphy_link_state[pn] != ST_LINK_UP) && (mt7516GePhyDisSlaveMode_flag[pn] == 1) && (((reg9>>12)&0x1)==1)) 
			{
				tcMiiStationWrite(phyaddr, 0x09, 0x0600); 		// disable manual  
				tcMiiStationWrite(phyaddr, 0x00, 0x1200); 		// re-start AN
				//mt7516GePhyDisSlaveMode_cnt[pn] = 0;
				//mt7516GePhyDisSlaveMode_flag[pn] = 0;
			}
		...
		#ifdef MTPHY_DEBUG
		// display message
		mt7516GePhySwDispMessage(pn);
		#endif

		#ifdef MTPHY_DEBUG
		if(sw_ErrOverMonitor_flag)
			mt7516GePhyUpdateErrOverSum(pn);
		if(sw_ErrOverMonitor_flag) 
			mt7516GePhySwErrOverMonitor(pn); // call after mt7516FEUpdateErrOverSum()
		#endif
		#endif
    }
}
...
{
	int pn, i;
	uint16 phyAddr, phyAddr_base = 8;
	current_idx = idx;

	eco_rev = VPint(0xbfb00064);

    // global registers
    //for( pn=0; pn<tcGPhyPortNum; pn++)
    for( pn=0; pn < MT7516Ge_PORTNUM; pn++)		
	{
		phyAddr = phyAddr_base + pn;
	
		TCPHYDISP3("\n 7516Ge, phyaddr= (%d,%d) \n", phyAddr_base, phyAddr);
		for( i=0; i<MT7516Ge_PHY_INIT_CL45_GDATA_LEN; i++ )
		{         
			mtEMiiRegWrite(phyAddr, mt7516Ge_cfg_cl45[current_idx].gdata[i].dev_num, mt7516Ge_cfg_cl45[current_idx].gdata[i].reg_num, mt7516Ge_cfg_cl45[current_idx].gdata[i].val);
		}
		for( i=0; i<MT7516Ge_PHY_INIT_CL45_LDATA_LEN; i++ )
		{         
			mtEMiiRegWrite(phyAddr, mt7516Ge_cfg_cl45[current_idx].ldata[i].dev_num, mt7516Ge_cfg_cl45[current_idx].ldata[i].reg_num, mt7516Ge_cfg_cl45[current_idx].ldata[i].val);
		}
		for( i=0; i<MT7516Ge_PHY_INIT_LDATA_LEN; i++ )
		{         
			tcMiiStationWrite(phyAddr, mt7516Ge_cfg[current_idx].ldata[i].reg_num, mt7516Ge_cfg[current_idx].ldata[i].val);
		}
		for( i=0; i<MT7516Ge_PHY_INIT_TR_LDATA_LEN; i++ )
		{         
			mtPhyMiiWrite_TrDbg( phyAddr, mt7516Ge_cfg_tr[current_idx].ldata[i].reg_typ, mt7516Ge_cfg_tr[current_idx].ldata[i].reg_num, mt7516Ge_cfg_tr[current_idx].ldata[i].val,0);
		}
	}
	#if 0
	if(eco_rev >= 0x00050003) 
	{
		for(pn=0; pn < tcPhyPortNum; pn++)
		{
			phyAddr = phyAddr_base + pn;
			...
			mtEMiiRegWrite(phyAddr, 0x1e, 0x0122, 0xffa0);
			mtEMiiRegWrite(phyAddr, 0x1e, 0x00a6, 0x0350);
			mtEMiiRegWrite(phyAddr, 0x1f, 0x0417, 0x7775);  
			//mtEMiiRegWrite(phyAddr, 0x1e, 0x00a6, 0x0350); 
...
//static int doMacSendRandom(int argc, char *argv[], void *p); // for CID:139282
#ifndef PURE_BRIDGE
...
static int doPhyset7516_0R (int argc, char *argv[], void *p);		// allen_20190102
static int doPhysetPowerSaving (int argc, char *argv[], void *p);   // allen_20190709

static int doPhyForceLED (int argc, char *argv[], void *p);
static int doPhyset10M_random (int argc, char *argv[], void *p);
static int doPhyset10M_fix (int argc, char *argv[], void *p);
static int doPhyset10M_LP (int argc, char *argv[], void *p);
...
static const cmds_t ethertphyprint[] =
{
	...
	{"set7516_0R", 		doPhyset7516_0R,		0x02, 1, "set7516_0R <on|off>"},		// allen_20170621
	{"setPowerSaving", 	doPhysetPowerSaving,	0x02, 1, "setPowerSaving <on|off> <0|1|2|3> "},   // allen_20190709
	{"forceLED", 		doPhyForceLED,			0x02, 1, "forceLED <phyaddr> <enable|disable>"}, 
	{"set10M_random", 	doPhyset10M_random,		0x02, 1, "set10M_random <phyaddr> "}, 
	{"set10M_fix", 		doPhyset10M_fix,		0x02, 1, "set10M_fix <phyaddr> "}, 
	{"set10M_LP", 		doPhyset10M_LP,			0x02, 1, "set10M_LP <phyaddr> "}, 
...
**=========================================================================*/
void tcephydbgcmd(void)  // for CID:139863
{
...
int tcPhyInit(void)
{
	uint16	ephy_addr, port_num, rval;
	...
	else
	{
		...
		for(ephy_addr = 8; ephy_addr <= 12; ephy_addr++) // allen_20190215 for external 7530
		{
    	...
    	mt7516_0R_flag = 0;   // allen_20190215

    	// for external 7530 init   // allen_20190215
    	for(ephy_addr = 0; ephy_addr <= 31; ephy_addr++)
    			{
			rval = tcMiiStationRead(ephy_addr, 3);
			if(rval == EPHY_ID_7530Ge_7512Fe)
    				{
			tcMiiStationWrite(ephy_addr, 0x9,  0x0600);

			mtEMiiRegWrite(ephy_addr,  0x7, 0x3c, 0x0000);
			mtEMiiRegWrite(ephy_addr, 0x1e, 0xa6, 0x0350);
			mtEMiiRegWrite(ephy_addr, 0x1e, 0x41, 0x3333);
    				}	
    			}	
    		}			
	tcephydbgcmd();
...
int ePhyInit(uint16 ephy_addr)
{
   ...
   // uint16 start_addr = ephy_addr;
	...
	{			    
		...
		case tcPhyVer_7522Fe: 		// MT7522 FE
		case tcPhyVer_7522Ge: 		// MT7522 Ge
		case tcPhyVer_7516Ge: 		// EN7516 Ge
			if(ephy_addr_base == 0)
				ephy_addr_base += 8;
			break;
	}
...
static int doPhySwVer(int argc, char *argv[], void *p)
{
	...
	{    
		case tcPhyVer_7516Ge: 
			printf("%s, \r\n", mt7516Ge_cfg[0].name);
		 	printf("tcPhyVer_7516Ge, tcPhyVer=0x%02lX \r\n", tcPhyVer);
			break;	
...
int doPhyGphyTestMode (int argc, char *argv[], void *p)
{
	...
	else	
	{
		...
		if((reg3 == EPHY_ID_7516Ge)||(reg3 == EPHY_ID_7512Ge)||(reg3==EPHY_ID_7522Ge))
		{
...
static int doPhySwPatch (int argc, char *argv[], void *p)
{
	...
	else if(stricmp(argv[1], "dbg") == 0) // allen_20140822
	{
		...
		//for(pn=0; pn<4; pn++)
		//{
			//if((tcPhyVer == tcPhyVer_7512Ge)||(tcPhyVer == tcPhyVer_7522Ge))
...
{ 	// for 7512 (linux)
	...
	//if((tcPhyVer == tcPhyVer_7512Fe)||(tcPhyVer == tcPhyVer_7512Ge)||(tcPhyVer == tcPhyVer_mt7505FE)||(tcPhyVer == tcPhyVer_7522Fe)||(tcPhyVer == tcPhyVer_7522Ge))
	if((tcPhyVer == tcPhyVer_7516Ge))
	{
		ephy_addr_base = 8;
	}
   	...
   	if(phychkval_flag)
	{
		...
		//if((reg3==EPHY_ID_7512Ge)||(reg3==EPHY_ID_7522Ge)||((reg3 == EPHY_ID_7530Ge_7512Fe)&&(((r15_temp>>13)&0x1) == 1)))
		if((reg3 == EPHY_ID_7516Ge))
		{									
		...
		//else if((reg3==EPHY_ID_7512Ge)||(reg3 == EPHY_ID_7522Ge)||(reg3 == EPHY_ID_7530Ge_7512Fe)||(reg3 == EPHY_ID_7522Fe))
		else if((reg3 == EPHY_ID_7516Ge))
		{ 	// Force-speed
			if(mr_speed==0x00)
	 		{
		 		printf(" Force-%d%s\r\n",10,(mr_dplx?"F":"H"));
...
#if 0
			//if((tcPhyVer==tcPhyVer_7512Fe)||(tcPhyVer==tcPhyVer_7512Ge)||(tcPhyVer==tcPhyVer_7522Fe)||(tcPhyVer==tcPhyVer_7522Ge))
			if((tcPhyVer == tcPhyVer_7516Ge))
			{
				//printf(" tcphy[%ld]: adc_avg=%ld/1000\r\n", phyAddr, mt7512FEReadAdcSum(port_num));  

				val=mt7516GePhyReadProbe(phyAddr, Probe_VgaState, 0);
				printf("7516 VgaStateA, B, C, D = 0x%x, 0x%x, 0x%x, 0x%x\r\n",((val>>4) & 0x1F),((val>>9) & 0x1F),((val>>14) & 0x1F),((val>>19) & 0x1F));
				
				val=mt7516GePhyReadProbe(phyAddr,Probe_MSE,0);
				printf("7516 lch_mse_mdcA =0x%x\r\n",val);
				val=mt7516GePhyReadProbe(phyAddr,Probe_MSE,1);
				printf("7516 lch_mse_mdcB =0x%x\r\n",val);
				val=mt7516GePhyReadProbe(phyAddr,Probe_MSE,2);
				printf("7516 lch_mse_mdcC =0x%x\r\n",val);
				val=mt7516GePhyReadProbe(phyAddr,Probe_MSE,3);
				printf("7516 lch_mse_mdcD =0x%x\r\n",val);
			
				val=mt7516GePhyReadProbe(phyAddr,Probe_MSE_Slicer_err_thres,0);
				printf("slicer_err_thres =0x%x\r\n",val);
				val=mt7516GePhyReadProbe(phyAddr,Probe_Slicer_Err_Over_Sum,0);
				printf("ProbeSlicerErrOverSumA =0x%x\r\n",val);
				val=mt7516GePhyReadProbe(phyAddr,Probe_Slicer_Err_Over_Sum,1);
				printf("ProbeSlicerErrOverSumB =0x%x\r\n",val);
				val=mt7516GePhyReadProbe(phyAddr,Probe_Slicer_Err_Over_Sum,2);
				printf("ProbeSlicerErrOverSumC =0x%x\r\n",val);
				val=mt7516GePhyReadProbe(phyAddr,Probe_Slicer_Err_Over_Sum,3);
		...
		{ 	// link-up
			//if((reg3==EPHY_ID_7512Ge)||(reg3 == EPHY_ID_7522Ge)||((reg3 == EPHY_ID_7530Ge_7512Fe)&&(((r15_temp>>13)&0x1) == 1))) // 7512GE, 7530GE
			if((reg3==EPHY_ID_7516Ge)) 
			{
				if(lr_speed == 0x04)
				{
			...
			{	// 100Mbps/1000Mbps
				...
				//else if((reg3==EPHY_ID_7512Ge)||(reg3==EPHY_ID_7522Ge))
				if((reg3 == EPHY_ID_7516Ge))
				{
					#if debug_flag
					val=mt7516GePhyReadProbe(phyAddr, Probe_VgaState, 0);
...
int doPhyForceMode(int argc, char *argv[], void *p) 	
{
	...
	{    
		...
		else if(stricmp(argv[2], "force") == 0)
		{
			...
			{
				tcMiiStationWrite(phyaddr, 0x00, 0x00);
	...
	else if(argc == 4) // tce forcemode <phyaddr> <an|force> <10|100|1000>
	{
		...
		else if(stricmp(argv[2], "force") == 0)
		{
			...
			else if((stricmp(argv[3], "1000") == 0)&&(reg3==EPHY_ID_7516Ge))
			{
...
int doPhySLTMode(int argc, char *argv[], void *p) 	// allen_20171109
{
	...
	if(argc == 3) // tce sltmode <phyaddr> <10|100|1000> 
	{
		...
		else if((stricmp(argv[2], "1000") == 0)&&(reg3==EPHY_ID_7516Ge))
		{
...
void doEphySetSETMode(uint8 set_mode, uint16 parameter)    // allen_20190709
{
	...
	else if(set_mode == 1)	// set Power Saving
	{
		...
		{
			mtEMiiRegWrite(9, 0x1f, 0x271, 0x7ca8);
...
int doPhyForceGE (int argc, char *argv[], void *p) // allen_20131120
{
	...
	//if((reg3==EPHY_ID_7512Ge)||(reg3==EPHY_ID_7522Ge)||(reg3 == EPHY_ID_7522Ge)||((reg3 == EPHY_ID_7530Ge_7512Fe)&&(((r15_temp>>13)&0x1) == 1))) // GE // allen_20140108
	if((reg3 == EPHY_ID_7516Ge))
	{
		if (argc == 4) 
		{
...
int doPhygetCALReg (int argc, char *argv[], void *p)
{
	...
	if(cal_case == 1) // R50
	{
		...
		//while(FECal_R50_flag == 0)				// cmd
			FECal_R50(port_num, delay);				// cmd: getCAL	
	}
...
{				
	uint16 	phyaddr = 0;
	uint16	reg3 = 0, reg15=0;

	if((stricmp(argv[1], "on") == 0))
	{
	   	for(phyaddr = 0; phyaddr<=31; phyaddr ++)
   		{
			reg3 = tcMiiStationRead(phyaddr, 0x3);
			reg15 = tcMiiStationRead(phyaddr, 0xf);

			if((reg3 == EPHY_ID_7530Ge_7512Fe) && ((reg15>>13)&0x1) == 1)  // for 7530
   		{
			...
			//tcMiiStationWrite(phyaddr, 0x14, 0x3a45); 
			//tcMiiStationWrite(phyaddr, 0x1f, 0x8000);
				mtEMiiRegWrite(phyaddr, 0x1e, 0x41, 0x3333);
				mtEMiiRegWrite(phyaddr, 0x1e, 0xa6, 0x0350);
				mtEMiiRegWrite(phyaddr, 0x7,  0x3c, 0x0000);
				printf(" 7530 set 5R (phyaddr=%d) \r\n", phyaddr);
			}
   		}
   		mt7530_5R_flag = 1; 
	}
...
int doPhyset7516_0R(int argc, char *argv[], void *p)	// allen_20190102
{				
	uint16 	ephy_addr = 0, port_num = 0;
	uint16 	reg0_temp = 0, dev1e_145_temp = 0;
	...
	{
		for(ephy_addr = 8; ephy_addr <= 12; ephy_addr++)
		{
			tcMiiStationWrite(ephy_addr, 0x0, 0x0800);
			tcMiiStationWrite(ephy_addr, 0x0, 0x3100);

			#if 1
			tcMiiStationWrite(ephy_addr, 0x1f, 0x0000);					// g0 
			reg0_temp = tcMiiStationRead(ephy_addr, 0x0);				// keep the default value
			tcMiiStationWrite(ephy_addr, 0x0,  0x0140);					// set [12]AN disable, [8]full duplex, [13/6]1000Mbps

			mtEMiiRegWrite(ephy_addr, 0x1f, 0x0100, 0xc000);			// BG voltage output
			dev1e_145_temp = mtEMiiRegRead(ephy_addr, 0x1e, 0x0145);
			mtEMiiRegWrite(ephy_addr, 0x1e, 0x0145, 0x1010);			// fix mdi
			mtEMiiRegWrite(ephy_addr, 0x1e, 0x0185, 0x0000);			// disable tx slew control
			#endif
		
			printf(" 7516_5R set (%d) \r\n", ephy_addr);
    		tcPhyVerLookUp(ephy_addr);
			ePhyInit(ephy_addr);
    		if(tcPhyInitFlag == 1)
    		{
     		...
     		mtEMiiRegWrite(ephy_addr, 0x1e, 0x0040, 0x0000);
     		mtEMiiRegWrite(ephy_addr, 0x1e, 0x0145, 0x1000);			// auto-crossover
     		tcMiiStationWrite(ephy_addr, 0x0, 0x0800);
			tcMiiStationWrite(ephy_addr, 0x0, 0x3100);
    	}
	...
	else if((stricmp(argv[1], "on") == 0))
	{
		for(ephy_addr = 8; ephy_addr <= 12; ephy_addr++)
		{
    		...
    		if(tcPhyInitFlag == 1)
    		{
    			//ePhyInit(ephy_addr);
    			//GECal_flag = 0;
			    doGePhyALLAnalogCal(ephy_addr); 
    			if(GECal_flag == 0)
				    doGePhyALLAnalogCal(ephy_addr); 
     		}	
     		//mtEMiiRegWrite(ephy_addr, 0x1e, 0x0040, 0x8000);
     		mtEMiiRegWrite(ephy_addr, 0x1e, 0x0145, 0x1000);			// auto-crossover
     		tcMiiStationWrite(ephy_addr, 0x0, 0x0800);
			tcMiiStationWrite(ephy_addr, 0x0, 0x3100);
    	}
	...
	else
	{
		#if debug_flag
		if(mt7516_0R_flag == 0)
			printf("[5R] 1e_40 = 0x%x \r\n", mtEMiiRegRead(12, 0x1e, 0x40));
		else
			printf("[0R] 1e_40 = 0x%x \r\n", mtEMiiRegRead(12, 0x1e, 0x40));
		#endif
	}
...
int doPhysetPowerSaving(int argc, char *argv[], void *p)    // allen_20190709	
{
	uint16 phyaddr = 0, PowerSaving_mode = 0;

	if((stricmp(argv[1], "on") == 0))
	{
		if(argc == 3) 
		{ 
			PowerSaving_mode = checked_atoi(argv[2]);
		}
		else
		{
			PowerSaving_mode = 1;		// for 110m	
		}
	}
	else if((stricmp(argv[1], "off") == 0))
	{
		PowerSaving_mode = 0;			// default
	}
	else
	{
		printf("Usage: setPowerSaving <on|off> <0|1|2|3> \r\n");
	}
	
	// ===== settings ========================================
	// global register
	if(PowerSaving_mode == 0)	// default
	{
		mtEMiiRegWrite(9, 0x1f, 0x271, 0x7fff);
		mtEMiiRegWrite(9, 0x1f, 0x269, 0x444f);
	}
	else if(PowerSaving_mode == 1)	// for 110m
	{
		mtEMiiRegWrite(9, 0x1f, 0x271, 0x7ca8);
		mtEMiiRegWrite(9, 0x1f, 0x272, 0x14ff);
		mtEMiiRegWrite(9, 0x1f, 0x269, 0x111f);
	}
	else if(PowerSaving_mode == 2)	
	{
		mtEMiiRegWrite(9, 0x1f, 0x271, 0x7c63);
		mtEMiiRegWrite(9, 0x1f, 0x272, 0x14ff);
		mtEMiiRegWrite(9, 0x1f, 0x269, 0x000f);
	}
	else if(PowerSaving_mode == 3)	
	{
		mtEMiiRegWrite(9, 0x1f, 0x271, 0x7c42);
		mtEMiiRegWrite(9, 0x1f, 0x272, 0x14ff);
		mtEMiiRegWrite(9, 0x1f, 0x269, 0x000f);
...
int doPhyset10M_random(int argc, char *argv[], void *p) 	
{
	uint16 port_num = 0, phyaddr = 0;
	uint32 reg3 = 0, r15_temp = 0, dev_1f_24_temp = 0, dev_1f_24 = 0;
...
int doPhyset10M_fix(int argc, char *argv[], void *p) 	
{
	uint16 port_num = 0, phyaddr = 0;
	uint32 reg3 = 0, r15_temp = 0, dev_1f_24_temp = 0, dev_1f_24 = 0;
...
int doPhyset10M_LP(int argc, char *argv[], void *p) 	
{
	uint16 port_num = 0, phyaddr = 0;
	uint32 reg3 = 0, r15_temp = 0, dev_1f_24_temp = 0, dev_1f_24 = 0;
...
const uint8	ZCAL_TO_R45ohm_TBL[64] =
{
	...
	101,	99,		97,		79,		77,		75,		74,		72,		70,		69,		67,		66,		65,		47,		46,		45,    // 16 - 31
	43,		42,		41,		40,		39,		38,		37,		36,		34,		34,		33,		32,		15,		14,		13,		12,
	11,		10,		10,		9,		8,		7,		7,		6,		5,		4,		4,		3,		2,		2,		1,		1		
};
...
void doGePhyALLAnalogCal(uint8 phyaddr) // for EN7512 GBE 
{
	...
	uint8	tx_amp_temp, tx_amp_reg, cnt=0, tx_amp_reg_100, tst_offset=0, hbt_offset=0;
	...
	mtEMiiRegWrite( 8, 0x1e, 0x00db, 0x1100);	// 1e_db[12]:rg_cal_ckinv, [8]:rg_ana_calen, [4]:rg_rext_calen, [0]:rg_zcalen_a
	mtEMiiRegWrite( 8, 0x1e, 0x00dc, 0x0000);	// 1e_dc[0]:rg_txvos_calen
	mtEMiiRegWrite( 8, 0x1e, 0x00e1, 0x0000);	// 1e_e1[4]:select 1V
	
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		rg_zcal_ctrl = 0x20;  						// start with 0 dB
		dev1e_e0_ana_cal_r5 = (mtEMiiRegRead( 8, 0x1e, 0x00e0) & (~0x003f));
		mtEMiiRegWrite( 8, 0x1e, 0x00e0, (dev1e_e0_ana_cal_r5 | rg_zcal_ctrl));	// 1e_e0[5:0]:rg_zcal_ctrl
		...
		ad_cal_comp_out_init = (mtEMiiRegRead( 8, 0x1e, 0x017a)>>8) & 0x1;		// 1e_17a[8]:ad_cal_comp_out	
		...
		while(all_ana_cal_status < ANACAL_ERROR)
		{
			...
			mtEMiiRegWrite( 8, 0x1e, 0x00e0, (dev1e_e0_ana_cal_r5 | rg_zcal_ctrl));
			all_ana_cal_status = allGeAnaCalWait(100, phyaddr); // delay 20 usec
			...
			else if(((mtEMiiRegRead( 8, 0x1e, 0x017a)>>8)&0x1) != ad_cal_comp_out_init) 
			{
		...
		{	
			...
			rg_zcal_ctrl = ZCAL_TO_R50ohm_TBL[rg_zcal_ctrl];
			GECal_flag = 0;
		}
		else if(all_ana_cal_status == ANACAL_SATURATION)
		{
			rg_zcal_ctrl = 0x20; 
			rg_zcal_ctrl = ZCAL_TO_R50ohm_TBL[rg_zcal_ctrl];
			GECal_flag = 0;
	...
	mtEMiiRegWrite( 8, 0x1e, 0x00db, 0x0000);
	// *** R50 Cal end ***
	...
	mtEMiiRegWrite( 8, 0x1e, 0x00db, 0x0100);	// 1e_db[12]:rg_cal_ckinv, [8]:rg_ana_calen, [4]:rg_rext_calen, [0]:rg_zcalen_a
	mtEMiiRegWrite( 8, 0x1e, 0x00dc, 0x0001);	// 1e_dc[0]:rg_txvos_calen
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		...
		while(all_ana_cal_status < ANACAL_ERROR)
		{
			...
			all_ana_cal_status = allGeAnaCalWait(200, phyaddr); // delay 20 usec
			if(all_ana_cal_status == 0)
			{
				all_ana_cal_status = ANACAL_ERROR;	
				printf(" GE Tx amp AnaCal ERROR!   \r\n");
			...
			else if(((mtEMiiRegRead( 8, 0x1e, 0x017a)>>8)&0x1) != ad_cal_comp_out_init) 
			{
				...
				//if(calibration_pair == ANACAL_PAIR_A) // coverity : same branch
				//{
					mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|((tx_amp_temp+tst_offset)<<tx_amp_reg_shift)));
				//}
				//else //if(calibration_pair == ANACAL_PAIR_B) // pair b, c, d
				//{
				//	mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|((tx_amp_temp+tst_offset)<<tx_amp_reg_shift)));	// for gbe(DAC), 0xc
				//}
		...
		else if(all_ana_cal_status == ANACAL_SATURATION)
		{
			...
			mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|(tx_amp_temp))); 								// for tst
			GECal_flag = 0;
		}
		else
		{
			...
			if((all_ana_cal_status == ANACAL_SATURATION)||(tx_amp_temp==0))
				...
				mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|(tx_amp_temp+tst_offset)));  					// for tst
			...
			{
				mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|((tx_amp_temp+hbt_offset)<<tx_amp_reg_shift)));	// for 100
			}
		}
	}
	mtEMiiRegWrite(phyaddr, 0x1e, 0x017d, 0x0000);
...
void doGePhyALLAnalogCal_R45(uint8 phyaddr) 
{
	...
	uint16	tx_amp_offset_hbt, tx_amp_offset_hbt_p2, tx_amp_offset_hbt_p3, tx_amp_offset_hbt_p4, tx_amp_offset_tbt, tx_amp_offset_tbt_p4, tx_amp_temp_hbt, tx_amp_temp_tst, tx_amp_temp_tbt;
	...
	uint16	tx_amp_offset_tst, tx_amp_offset_tst_p8_p9, tx_amp_offset_tst_p10, tx_amp_offset_tst_p11;
	...
	printf(" 7516 GbE eco_rev (0x%x) &0xff(0x%x)\r\n", eco_rev, (eco_rev&0xff));
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		...
		{	
			rg_zcal_ctrl = 0x20;  // 0 dB
			rg_zcal_ctrl = ZCAL_TO_R45ohm_TBL[rg_zcal_ctrl];
			GECal_flag = 0;
		}
		else if(all_ana_cal_status == ANACAL_SATURATION)
		{
		...
		{
			printf("  GE R45 zcal_idx (%d) (0x%x) \r\n",  rg_zcal_ctrl, (rg_zcal_ctrl));

			if(isEN7527H)
			//if(1)
			{
				if(phyaddr == 9)
					rg_zcal_ctrl = ZCAL_TO_R45ohm_TBL[rg_zcal_ctrl-2];	
				else if(phyaddr == 10)
					rg_zcal_ctrl = ZCAL_TO_R45ohm_TBL[rg_zcal_ctrl-17];
				else if(phyaddr == 11)
					rg_zcal_ctrl = ZCAL_TO_R45ohm_TBL[rg_zcal_ctrl-14];
				else if(phyaddr == 12)
					rg_zcal_ctrl = ZCAL_TO_R45ohm_TBL[rg_zcal_ctrl-1];
			}	
			else
			{
			rg_zcal_ctrl = ZCAL_TO_R45ohm_TBL[rg_zcal_ctrl];
			...
			rg_zcal_ctrl = ZCAL_TO_R45ohm_TBL[rg_zcal_ctrl+0x7];	   // allen_20170630 (+7 : 99->70)
			#else
			rg_zcal_ctrl = rg_zcal_ctrl + 10;
			printf("  GE R45 zcal_idx (%d) (0x%x) \r\n",  rg_zcal_ctrl, (rg_zcal_ctrl));
			rg_zcal_ctrl = ZCAL_TO_R45ohm_TBL[rg_zcal_ctrl];	   // allen_20170524 (+16: 79->40)
			#endif
			#endif
			printf("  GE R45 AnaCal Done! (%d) (0x%x)(0x%x) \r\n", cnt, rg_zcal_ctrl, (rg_zcal_ctrl|0x80));
			GECal_flag = 1;
		}
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		...
		{
		// ======================================================================================
		// 		7516 E1 EPHY Digital bug workaround  (1)
		//		da_p0[5:0] = reg_p0[5:0]
		//		da_p1[5:1] = reg_p1[4:0]
		//		da_p2[5:1] = reg_p2[3:0], reg_p1[5]
		//		da_p3[5:1] = reg_p3[2:0], reg_p2[5:4]
		...
		if(phyaddr == 8) 			// p0
		{
			...
			tx_amp_reg_shift_eco_msb = 0;
			tx_amp_temp_eco_msb = ((tx_amp_temp>>0) & 0x3f)<<tx_amp_reg_shift_eco_msb;
		}
		else if(phyaddr == 9) 		// p1
		{
			tx_amp_reg_shift_eco_lsb = 0;
			...
			tx_amp_reg_shift_eco_msb = 0;	// p1[4:0]
			tx_amp_temp_eco_msb = ((tx_amp_temp>>1))<<tx_amp_reg_shift_eco_msb;
		}
		else if(phyaddr == 10)		// p2
		{
			tx_amp_reg_shift_eco_lsb = 5;	// p1[5]
			tx_amp_temp_eco_lsb = (tx_amp_temp>>1) & 0x1;
			//reg_temp_eco = (mtEMiiRegRead((phyaddr-1), 0x1e, tx_amp_reg) & (~0xff00));
			//mtEMiiRegWrite((phyaddr-1), 0x1e, tx_amp_reg, (reg_temp_eco|((tx_amp_temp_eco_lsb<<tx_amp_reg_shift_eco_lsb)<<tx_amp_reg_shift)));	// 1e_12, 1e_17, 1e_19, 1e_21
			if(calibration_pair == ANACAL_PAIR_A)
			{
				reg_temp_eco = (mtEMiiRegRead((phyaddr-1), 0x1e, tx_amp_reg) & (~0x8000));
				mtEMiiRegWrite((phyaddr-1), 0x1e, tx_amp_reg, (reg_temp_eco|(((tx_amp_temp_eco_lsb<<tx_amp_reg_shift_eco_lsb)<<tx_amp_reg_shift)&(~0x4000))));	// 1e_12, 17, 19, 21
			}
			else
			{
				reg_temp_eco = (mtEMiiRegRead((phyaddr-1), 0x1e, tx_amp_reg) & (~0x2000));
				mtEMiiRegWrite((phyaddr-1), 0x1e, tx_amp_reg, (reg_temp_eco|(((tx_amp_temp_eco_lsb<<tx_amp_reg_shift_eco_lsb)<<tx_amp_reg_shift)&(~0x1000))));	// 1e_12, 17, 19, 21
			}

			tx_amp_reg_shift_eco_msb = 0;	// p2[3:0]
			tx_amp_temp_eco_msb = ((tx_amp_temp>>2))<<tx_amp_reg_shift_eco_msb;
		}
		else if(phyaddr == 11)		// p3
		{
			tx_amp_reg_shift_eco_lsb = 4;	// p2[5:4]
			tx_amp_temp_eco_lsb = (tx_amp_temp>>1) & 0x3;
			//reg_temp_eco = (mtEMiiRegRead((phyaddr-1), 0x1e, tx_amp_reg) & (~0xff00));
			//mtEMiiRegWrite((phyaddr-1), 0x1e, tx_amp_reg, (reg_temp_eco|((tx_amp_temp_eco_lsb<<tx_amp_reg_shift_eco_lsb)<<tx_amp_reg_shift)));	// 1e_12, 1e_17, 1e_19, 1e_21
			if(calibration_pair == ANACAL_PAIR_A)
			{
				reg_temp_eco = (mtEMiiRegRead((phyaddr-1), 0x1e, tx_amp_reg) & (~0xc000));
				mtEMiiRegWrite((phyaddr-1), 0x1e, tx_amp_reg, (reg_temp_eco|(((tx_amp_temp_eco_lsb<<tx_amp_reg_shift_eco_lsb)<<tx_amp_reg_shift)&(~0x2000))));	// 1e_12, 17, 19, 21
			}
			else
			{
				reg_temp_eco = (mtEMiiRegRead((phyaddr-1), 0x1e, tx_amp_reg) & (~0x3000));
				mtEMiiRegWrite((phyaddr-1), 0x1e, tx_amp_reg, (reg_temp_eco|(((tx_amp_temp_eco_lsb<<tx_amp_reg_shift_eco_lsb)<<tx_amp_reg_shift)&(~0x0800))));	// 1e_12, 17, 19, 21
			}

			tx_amp_reg_shift_eco_msb = 0;	// p3[2:0]
			tx_amp_temp_eco_msb = ((tx_amp_temp>>3))<<tx_amp_reg_shift_eco_msb;
		}
		else if(phyaddr == 12)		// p4
		{
			tx_amp_reg_shift_eco_lsb = 3;	// p3[5:3]
			tx_amp_temp_eco_lsb = (tx_amp_temp>>1) & 0x7;
			//reg_temp_eco = (mtEMiiRegRead((phyaddr-1), 0x1e, tx_amp_reg) & (~0xff00));
			//mtEMiiRegWrite((phyaddr-1), 0x1e, tx_amp_reg, (reg_temp_eco|((tx_amp_temp_eco_lsb<<tx_amp_reg_shift_eco_lsb)<<tx_amp_reg_shift)));	// 1e_12, 1e_17, 1e_19, 1e_21
			if(calibration_pair == ANACAL_PAIR_A)
			{
				reg_temp_eco = (mtEMiiRegRead((phyaddr-1), 0x1e, tx_amp_reg) & (~0xe000));
				mtEMiiRegWrite((phyaddr-1), 0x1e, tx_amp_reg, (reg_temp_eco|(((tx_amp_temp_eco_lsb<<tx_amp_reg_shift_eco_lsb)<<tx_amp_reg_shift)&(~0x1000))));	// 1e_12, 17, 19, 21
			}
			else
			{
				reg_temp_eco = (mtEMiiRegRead((phyaddr-1), 0x1e, tx_amp_reg) & (~0x3800));
				mtEMiiRegWrite((phyaddr-1), 0x1e, tx_amp_reg, (reg_temp_eco|(((tx_amp_temp_eco_lsb<<tx_amp_reg_shift_eco_lsb)<<tx_amp_reg_shift)&(~0x0400))));	// 1e_12, 17, 19, 21
			}

			tx_amp_reg_shift_eco_msb = 0;	// p4[1:0]
			tx_amp_temp_eco_msb = ((tx_amp_temp>>4))<<tx_amp_reg_shift_eco_msb;
		}
		mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|(tx_amp_temp_eco_msb<<tx_amp_reg_shift)));		// 1e_12, 1e_17, 1e_19, 1e_21
		// ======================================================================================
		//mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|(tx_amp_temp<<tx_amp_reg_shift)));		// 1e_12, 1e_17, 1e_19, 1e_21
		}
		else
		...
		while(all_ana_cal_status < ANACAL_ERROR)
		{
			...
			{
			// ======================================================================================
			// 		7516 E1 EPHY Digital bug workaround  (2)
			//		da_p0[5:0] = reg_p0[5:0]
			//		da_p1[5:1] = reg_p1[4:0]
			//		da_p2[5:1] = reg_p2[3:0], reg_p1[5]
			//		da_p3[5:1] = reg_p3[2:0], reg_p2[5:4]
			...
			if(phyaddr == 8) 			// p0
			{
				...
				tx_amp_reg_shift_eco_msb = 0;
				tx_amp_temp_eco_msb = ((tx_amp_temp>>0))<<tx_amp_reg_shift_eco_msb;
			}
			else if(phyaddr == 9) 		// p1
			{
				tx_amp_reg_shift_eco_lsb = 0;
			...
			#endif
			//printf(" CAL Tx amp, p(%d), tx_amp = 0x%x ====== \r\n", phyaddr, tx_amp_temp);
			
			//if(phyaddr == 8)
			//	printf(" CAL Tx amp, p(%d), 1e_(0x%x)=0x%x  \r\n", phyaddr, tx_amp_reg, mtEMiiRegRead(phyaddr,0x1e,tx_amp_reg));
			//else
			//	printf(" CAL Tx amp, p(%d), 1e_(0x%x)=0x%x, p(%d), 1e_(0x%x)=0x%x  \r\n", (phyaddr-1), tx_amp_reg, mtEMiiRegRead((phyaddr-1),0x1e,tx_amp_reg), phyaddr, tx_amp_reg, mtEMiiRegRead(phyaddr,0x1e,tx_amp_reg));
			
			all_ana_cal_status = allGeAnaCalWait_TxAmp(100, phyaddr); // delay 20 usec
			...
			else if(((mtEMiiRegRead(8, 0x1e, 0x017a)>>8)&0x1) != ad_cal_comp_out_init) 
			{
				...
				//{
					...
					{
					// ======================================================================================
					// 		7516 E1 EPHY Digital bug workaround  (3)
					//		da_p0[5:0] = reg_p0[5:0]
					//		da_p1[5:1] = reg_p1[4:0]
					//		da_p2[5:1] = reg_p2[3:0], reg_p1[5]
					//		da_p3[5:1] = reg_p3[2:0], reg_p2[5:4]
				...
				//{
					...
					{
					// ======================================================================================
					// 		7516 E1 EPHY Digital bug workaround  (4)
					//		da_p0[5:0] = reg_p0[5:0]
					//		da_p1[5:1] = reg_p1[4:0]
					//		da_p2[5:1] = reg_p2[3:0], reg_p1[5]
					//		da_p3[5:1] = reg_p3[2:0], reg_p2[5:4]
				...
				//printf(" GE Tx amp AnaCal Done! (0x%x)(0x%x)\r\n", tx_amp_reg, mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg));
			}
			else
			{
				if((tx_amp_temp == 0x3f)||(tx_amp_temp == 0x00))	
		...
		else
		{
			...
			if(isEN7527H)  // tst from RFB #004
			{
				tx_amp_offset_tst = 5;
			tx_amp_offset_tst_p8_p9 = 9;
				printf(" GE(%d) Tx amp tst done (0x%x)\r\n", phyaddr, tx_amp_temp);

			if((phyaddr == 8)||(phyaddr == 9))
				tx_amp_temp_tst = tx_amp_temp + tx_amp_offset_tst_p8_p9;
			else
				tx_amp_temp_tst = tx_amp_temp + tx_amp_offset_tst;
			
				printf(" GE(%d) Tx amp tst done (0x%x)\r\n", phyaddr, tx_amp_temp_tst);
			}
			else if(isEN7516G)
			{
				tx_amp_offset_tst = 13;
				tx_amp_offset_tst_p8_p9 = 13;
				tx_amp_offset_tst_p10 = 14;
				tx_amp_offset_tst_p11 = 16;

				if((phyaddr == 8)||(phyaddr == 9))
					tx_amp_temp_tst = tx_amp_temp + tx_amp_offset_tst_p8_p9;
				else if(phyaddr == 10)
					tx_amp_temp_tst = tx_amp_temp + tx_amp_offset_tst_p10;
				else if(phyaddr == 11)
					tx_amp_temp_tst = tx_amp_temp + tx_amp_offset_tst_p11;	
				else
					tx_amp_temp_tst = tx_amp_temp + tx_amp_offset_tst;
			}
			else
			{
				//printf(" (%d)(%d) e3 debug (0x%x) \r\n", phyaddr, calibration_pair, tx_amp_temp);
				
				tx_amp_offset_tst = 12;
				tx_amp_offset_tst_p8_p9 = 14; //12;
				tx_amp_offset_tst_p10 = 13;
				tx_amp_offset_tst_p11 = 15;
				...
				//printf(" (%d)(%d) e3 debug (0x%x) \r\n", phyaddr, calibration_pair, tx_amp_temp_tst);		
			}
			...
			else
			{
				...
				mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|(tx_amp_temp_eco_msb)));		// 1e_16, 1e_18, 1e_20, 1e_22 (for tst)
				// ======================================================================================
				}
				else
				{
					//printf(" (%d)(%d) e3 debug (0x%x) \r\n", phyaddr, calibration_pair, mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg_100));
					mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|(tx_amp_temp_tst)));  // for tst
					//printf(" (%d)(%d) e3 debug (0x%x) \r\n", phyaddr, calibration_pair, mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg_100));
					//printf(" (%d)(%d) e3 debug (0x%x)(0x%x) \r\n", phyaddr, calibration_pair, reg_temp, tx_amp_temp_tst);
				}
				#endif
			}
			...
			//printf("  reg_temp(0x%x) \r\n", reg_temp);

			if(isEN7527H)  // hbt from RFB #044
			{
				tx_amp_offset_hbt = 0;		// p9
				tx_amp_offset_hbt_p2 = 0;	// p10
				tx_amp_offset_hbt_p3 = 5;	// p11
				tx_amp_offset_hbt_p4 = 5;	// p12
				
				if(phyaddr == 12)
					tx_amp_temp_hbt = tx_amp_temp - tx_amp_offset_hbt_p4;
				else if(phyaddr == 11)
					tx_amp_temp_hbt = tx_amp_temp - tx_amp_offset_hbt_p3;	
				else if(phyaddr == 10)
					tx_amp_temp_hbt = tx_amp_temp + tx_amp_offset_hbt_p2;	
				else
					tx_amp_temp_hbt = tx_amp_temp + tx_amp_offset_hbt;
			}
			else
			{
				//printf(" (%d)(%d) e3 debug (0x%x) \r\n", phyaddr, calibration_pair, tx_amp_temp);
				...
				// E3 ECNT0057_061
			tx_amp_offset_hbt = 3;
				tx_amp_offset_hbt_p0_a = 2;
				tx_amp_offset_hbt_p0_b = 3;
				tx_amp_offset_hbt_p1_a = 1;
				tx_amp_offset_hbt_p1_b = 3;
				tx_amp_offset_hbt_p2_a = 2;
				tx_amp_offset_hbt_p2_b = 3;
				tx_amp_offset_hbt_p3_a = 2;
				tx_amp_offset_hbt_p3_b = 3;
				tx_amp_offset_hbt_p4_a = 3;
				tx_amp_offset_hbt_p4_b = 4;
			
				if(((phyaddr == 8))&&(calibration_pair == ANACAL_PAIR_A))
					tx_amp_temp_hbt = tx_amp_temp + tx_amp_offset_hbt_p0_a;
				else if(((phyaddr == 8))&&(calibration_pair == ANACAL_PAIR_B))
					tx_amp_temp_hbt = tx_amp_temp + tx_amp_offset_hbt_p0_b;
				else if(((phyaddr == 9))&&(calibration_pair == ANACAL_PAIR_A))
					tx_amp_temp_hbt = tx_amp_temp + tx_amp_offset_hbt_p1_a;
				else if(((phyaddr == 9))&&(calibration_pair == ANACAL_PAIR_B))
					tx_amp_temp_hbt = tx_amp_temp + tx_amp_offset_hbt_p1_b;	
				else if(((phyaddr == 10))&&(calibration_pair == ANACAL_PAIR_A))
					tx_amp_temp_hbt = tx_amp_temp + tx_amp_offset_hbt_p2_a;
				else if(((phyaddr == 10))&&(calibration_pair == ANACAL_PAIR_B))
					tx_amp_temp_hbt = tx_amp_temp + tx_amp_offset_hbt_p2_b;
				else if(((phyaddr == 11))&&(calibration_pair == ANACAL_PAIR_A))
					tx_amp_temp_hbt = tx_amp_temp + tx_amp_offset_hbt_p3_a;
				else if(((phyaddr == 11))&&(calibration_pair == ANACAL_PAIR_B))
					tx_amp_temp_hbt = tx_amp_temp + tx_amp_offset_hbt_p3_b;	
				else if(((phyaddr == 12))&&(calibration_pair == ANACAL_PAIR_A))
					tx_amp_temp_hbt = tx_amp_temp + tx_amp_offset_hbt_p4_a;	
				else if(((phyaddr == 12))&&(calibration_pair == ANACAL_PAIR_B))
					tx_amp_temp_hbt = tx_amp_temp + tx_amp_offset_hbt_p4_b;
			else
				tx_amp_temp_hbt = tx_amp_temp + tx_amp_offset_hbt;
			
				//printf(" (%d)(%d) e3 debug (0x%x) \r\n", phyaddr, calibration_pair, tx_amp_temp_hbt);	
			}
			...
			else
			{
				...
				{
				// ======================================================================================
				// 		7516 E1 EPHY Digital bug workaround  (6) (hbt/tst)
				//		da_p0[5:0] = reg_p0[5:0]
				//		da_p1[5:1] = reg_p1[4:0]
				//		da_p2[5:1] = reg_p2[3:0], reg_p1[5]
				//		da_p3[5:1] = reg_p3[2:0], reg_p2[5:4]
				...
				if(phyaddr == 8) 			// p0
				{
					...
					tx_amp_reg_shift_eco_msb = 0;
					tx_amp_temp_eco_msb = ((tx_amp_temp_hbt>>0))<<tx_amp_reg_shift_eco_msb;
				}
				else if(phyaddr == 9) 		// p1
				{
					tx_amp_reg_shift_eco_lsb = 0;
					...
					tx_amp_reg_shift_eco_msb = 0;	// p1[4:0]
					tx_amp_temp_eco_msb = ((tx_amp_temp_hbt>>1))<<tx_amp_reg_shift_eco_msb;
				}
				else if(phyaddr == 10)		// p2
				{
					tx_amp_reg_shift_eco_lsb = 5;	// p1[5]
					tx_amp_temp_eco_lsb = (tx_amp_temp_hbt>>1) & 0x1;
					if(calibration_pair == ANACAL_PAIR_A)
					{
						reg_temp_eco = (mtEMiiRegRead((phyaddr-1), 0x1e, tx_amp_reg_100) & (~0x8000));						
						mtEMiiRegWrite((phyaddr-1), 0x1e, tx_amp_reg_100, (reg_temp_eco|(((tx_amp_temp_eco_lsb<<tx_amp_reg_shift_eco_lsb)<<tx_amp_reg_shift)&(~0x4000))));	// 1e_16, 18, 20, 22
					}
					else
					{
						reg_temp_eco = (mtEMiiRegRead((phyaddr-1), 0x1e, tx_amp_reg_100) & (~0x2000));
						mtEMiiRegWrite((phyaddr-1), 0x1e, tx_amp_reg_100, (reg_temp_eco|(((tx_amp_temp_eco_lsb<<tx_amp_reg_shift_eco_lsb)<<tx_amp_reg_shift)&(~0x1000))));	// 1e_16, 18, 20, 22
					}
					
					tx_amp_reg_shift_eco_msb = 0;	// p2[3:0]
					tx_amp_temp_eco_msb = ((tx_amp_temp_hbt>>2))<<tx_amp_reg_shift_eco_msb;
				}
				else if(phyaddr == 11)		// p3
				{
					tx_amp_reg_shift_eco_lsb = 4;	// p2[5:4]
					tx_amp_temp_eco_lsb = (tx_amp_temp_hbt>>1) & 0x3;
					
					if(calibration_pair == ANACAL_PAIR_A)
					{
						reg_temp_eco = (mtEMiiRegRead((phyaddr-1), 0x1e, tx_amp_reg_100) & (~0xc000));
						mtEMiiRegWrite((phyaddr-1), 0x1e, tx_amp_reg_100, (reg_temp_eco|(((tx_amp_temp_eco_lsb<<tx_amp_reg_shift_eco_lsb)<<tx_amp_reg_shift)&(~0x2000))));	// 1e_16, 18, 20, 22
					}	
					else
					{
						reg_temp_eco = (mtEMiiRegRead((phyaddr-1), 0x1e, tx_amp_reg_100) & (~0x3000));
						mtEMiiRegWrite((phyaddr-1), 0x1e, tx_amp_reg_100, (reg_temp_eco|(((tx_amp_temp_eco_lsb<<tx_amp_reg_shift_eco_lsb)<<tx_amp_reg_shift)&(~0x0800))));	// 1e_16, 18, 20, 22
					}

					tx_amp_reg_shift_eco_msb = 0;	// p3[2:0]
					tx_amp_temp_eco_msb = ((tx_amp_temp_hbt>>3))<<tx_amp_reg_shift_eco_msb;
				}
				else if(phyaddr == 12)		// p4
				{
					tx_amp_reg_shift_eco_lsb = 3;	// p3[5:3]
					tx_amp_temp_eco_lsb = (tx_amp_temp_hbt>>1) & 0x7;
					if(calibration_pair == ANACAL_PAIR_A)
					{
						reg_temp_eco = (mtEMiiRegRead((phyaddr-1), 0x1e, tx_amp_reg_100) & (~0xe000));
						mtEMiiRegWrite((phyaddr-1), 0x1e, tx_amp_reg_100, (reg_temp_eco|(((tx_amp_temp_eco_lsb<<tx_amp_reg_shift_eco_lsb)<<tx_amp_reg_shift)&(~0x1000))));	// 1e_16, 18, 20, 22
					}
					else
					{
						reg_temp_eco = (mtEMiiRegRead((phyaddr-1), 0x1e, tx_amp_reg_100) & (~0x3800));
						mtEMiiRegWrite((phyaddr-1), 0x1e, tx_amp_reg_100, (reg_temp_eco|(((tx_amp_temp_eco_lsb<<tx_amp_reg_shift_eco_lsb)<<tx_amp_reg_shift)&(~0x0400))));	// 1e_16, 18, 20, 22
					}
					
					tx_amp_reg_shift_eco_msb = 0;	// p4[1:0]
					tx_amp_temp_eco_msb = ((tx_amp_temp_hbt>>4))<<tx_amp_reg_shift_eco_msb;
				}
				mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|(tx_amp_temp_eco_msb<<tx_amp_reg_shift)));		// 1e_16, 1e_18, 1e_20, 1e_22 (for tst)
				// ======================================================================================
				}
				else
				{
					//printf(" (%d)(%d) e3 debug (0x%x) \r\n", phyaddr, calibration_pair, mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg_100));
					mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|((tx_amp_temp_hbt)<<tx_amp_reg_shift)));
					//printf(" (%d)(%d) e3 debug (0x%x) \r\n", phyaddr, calibration_pair, mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg_100));
					//printf(" (%d)(%d) e3 debug (0x%x)(0x%x) \r\n", phyaddr, calibration_pair, reg_temp, tx_amp_temp_hbt);
				}
				#endif
			}
			...
			tx_amp_offset_tbt = 3;
			tx_amp_offset_tbt_p4 = 5;
			if(((phyaddr == 12))&&(calibration_pair == ANACAL_PAIR_A))
				tx_amp_temp_tbt = tx_amp_temp - tx_amp_offset_tbt_p4;
			else
			...
			else
			{
				...
				{
				// ======================================================================================
				// 		7516 E1 EPHY Digital bug workaround  (7) (tbt)
				//		da_p0[5:0] = reg_p0[5:0]
				//		da_p1[5:1] = reg_p1[4:0]
				//		da_p2[5:1] = reg_p2[3:0], reg_p1[5]
				//		da_p3[5:1] = reg_p3[2:0], reg_p2[5:4]
				...
				if(phyaddr == 8) 			// p0
				{
					...
					tx_amp_reg_shift_eco_msb = 0;
					tx_amp_temp_eco_msb = ((tx_amp_temp_tbt>>0))<<tx_amp_reg_shift_eco_msb;
				}
				else if(phyaddr == 9) 		// p1
				{
					tx_amp_reg_shift_eco_lsb = 0;
					...
					tx_amp_reg_shift_eco_msb = 0;	// p1[4:0]
					tx_amp_temp_eco_msb = ((tx_amp_temp_tbt>>1))<<tx_amp_reg_shift_eco_msb;
				}
				else if(phyaddr == 10)		// p2
				{
					tx_amp_reg_shift_eco_lsb = 5;	// p1[5]
					tx_amp_temp_eco_lsb = (tx_amp_temp_tbt>>1) & 0x1;
					reg_temp_eco = (mtEMiiRegRead((phyaddr-1), 0x1e, tx_amp_reg) & (~0x0020));
					mtEMiiRegWrite((phyaddr-1), 0x1e, tx_amp_reg, (reg_temp_eco|((tx_amp_temp_eco_lsb<<tx_amp_reg_shift_eco_lsb)&(~0x0010))));	// 1e_12, 17, 19, 21

					tx_amp_reg_shift_eco_msb = 0;	// p2[3:0]
					tx_amp_temp_eco_msb = ((tx_amp_temp_tbt>>2))<<tx_amp_reg_shift_eco_msb;
				}
				else if(phyaddr == 11)		// p3
				{
					tx_amp_reg_shift_eco_lsb = 4;	// p2[5:4]
					tx_amp_temp_eco_lsb = (tx_amp_temp_tbt>>1) & 0x3;
					reg_temp_eco = (mtEMiiRegRead((phyaddr-1), 0x1e, tx_amp_reg) & (~0x0030));
					mtEMiiRegWrite((phyaddr-1), 0x1e, tx_amp_reg, (reg_temp_eco|((tx_amp_temp_eco_lsb<<tx_amp_reg_shift_eco_lsb)&(~0x0008))));	// 1e_12, 17, 19, 21

					tx_amp_reg_shift_eco_msb = 0;	// p3[2:0]
					tx_amp_temp_eco_msb = ((tx_amp_temp_tbt>>3))<<tx_amp_reg_shift_eco_msb;
				}
				else if(phyaddr == 12)		// p4
				{
					tx_amp_reg_shift_eco_lsb = 3;	// p3[5:3]
					tx_amp_temp_eco_lsb = (tx_amp_temp_tbt>>1) & 0x7;
					reg_temp_eco = (mtEMiiRegRead((phyaddr-1), 0x1e, tx_amp_reg) & (~0x0038));
					mtEMiiRegWrite((phyaddr-1), 0x1e, tx_amp_reg, (reg_temp_eco|((tx_amp_temp_eco_lsb<<tx_amp_reg_shift_eco_lsb)&(~0x0004))));	// 1e_12, 17, 19, 21

					tx_amp_reg_shift_eco_msb = 0;	// p4[1:0]
					tx_amp_temp_eco_msb = ((tx_amp_temp_tbt>>4))<<tx_amp_reg_shift_eco_msb;
				}
				mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|(tx_amp_temp_eco_msb)));		// 1e_12, 1e_17, 1e_19, 1e_21 (for tbt)
				// ======================================================================================
			}
				else
				{
					mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|(tx_amp_temp_tbt)));
				}
				#endif
			}
		}
	...
	while(all_ana_cal_status < ANACAL_ERROR)
	{
		...
		else
		{
			if((rg_zcal_ctrl == 0x3F)||(rg_zcal_ctrl == 0x00))	
			{
				...
				printf(" GE Rxet cal (%d)(%d)(%d)(0x%x)  \r\n", cnt, ad_cal_comp_out_init, ((mtEMiiRegRead(8, 0x1e, 0x017a)>>8)&0x1), mtEMiiRegRead(phyaddr, 0x1e, 0xe0));
		}
	}
...
void	GECal_R50(uint8 phyaddr, uint32 delay)									// for EN7512 
{
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		...
		else if(calibration_pair == ANACAL_PAIR_C)
		{
			...
			if(isEN7526c)
			{
				mtEMiiRegWrite(phyaddr, 0x1e, 0x0175, (ad_cal_comp_out_init | ((((rg_zcal_ctrl-4)<<8)&0xff00) | 0x8000)));		
			}
		}
		else // if(calibration_pair == ANACAL_PAIR_D)
		{
			...
			if(isEN7526c)
			{
				mtEMiiRegWrite(phyaddr, 0x1e, 0x0175, (ad_cal_comp_out_init | ((((rg_zcal_ctrl-4)<<0)&0x00ff) | 0x8000)));		
			}
		}
		//mtEMiiRegWrite(phyaddr, 0x1e, 0x00e0, ((rg_zcal_ctrl<<8)|rg_zcal_ctrl));
	}
...
void	GECal_R45(uint8 phyaddr, uint32 delay)		// for EN7512 // allen_20170524 
{
	uint8	rg_zcal_ctrl, all_ana_cal_status;
	uint16	ad_cal_comp_out_init;
	uint16	dev1e_e0_ana_cal_r5; 
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		...
		{
			rg_zcal_ctrl = ZCAL_TO_R45ohm_TBL[rg_zcal_ctrl];	// r45 mapping table
			printf("  GE R45 AnaCal Done! (%d) (0x%x)(0x%x) \r\n", cnt, rg_zcal_ctrl, (rg_zcal_ctrl|0x80));
		}
		...
		else // if(calibration_pair == ANACAL_PAIR_D)
		{
			...
			if(isEN7526c)
			{
				mtEMiiRegWrite(phyaddr, 0x1e, 0x0175, (ad_cal_comp_out_init | ((((rg_zcal_ctrl-4)<<8)&0x00ff) | 0x8000)));		
			}
		}
		//mtEMiiRegWrite(phyaddr, 0x1e, 0x00e0, ((rg_zcal_ctrl<<8)|rg_zcal_ctrl));
	}
...
void	GECal_tx_amp(uint8 phyaddr, uint32 delay)									// for EN7516 
{
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
			...
			else if(((mtEMiiRegRead(phyaddr, 0x1e, 0x017a)>>8)&0x1) != ad_cal_comp_out_init) 
			{
				...
				else //if(calibration_pair == ANACAL_PAIR_B)
				{
					...
					if(isEN7526c)
					{
						...
						{
							mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|((tx_amp_temp+8)<<tx_amp_reg_shift)));
						}
					}
				}
		...
		else
		{
			...
			{
				//if(calibration_pair == ANACAL_PAIR_A)
				//	mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, 0x2410);	// 0x2410
				//else if(calibration_pair == ANACAL_PAIR_B) 
				//	mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, 0x090f);	// 0x090f
			}
		}
	}
	mtEMiiRegWrite(phyaddr, 0x1e, 0x017d, 0x0000);
...
uint8 allGeAnaCalWait_R45(uint32 delay, uint8 phyaddr)  
{
	...
	mtEMiiRegWrite( 8, 0x1e, 0x017c, 0x0001);	// da_calin_flag pull high
	
	udelay(delay);

	all_ana_cal_status = mtEMiiRegRead( 8, 0x1e, 0x017b) & 0x1;
	mtEMiiRegWrite( 8, 0x1e, 0x017c, 0x0000);	// da_calin_flag pull low
...
void doFEPhyALLAnalogCal(uint8 port_num) // for EN7512 FE 
{
	...
	tcPhyWriteLReg(port_num, 0, 26, 0x5603);		// fix mdi  // allen_20170405
	...
	{
		FECal_R50(port_num, 100);  // allen_20160608
		cnt++;
		if(FECal_R50_flag == 0)
	...
	{
		FECal_TxAmp(port_num, 30000);  // allen_20170106, FE calibration work around
			
		cnt++;
		if(FECal_flag == 0)
			printf(" FE-%d Tx amp AnaCal wait! (%d)  \r\n", phyaddr, cnt);
	}
	FECal_TxAmp(port_num, 100);		// allen_20170106, FE calibration work around
...
void FECal_R50(uint8 port_num, uint32 delay)
{
	...
	uint8	ad_cal_comp_out_init, ad_cal_comp_out;
	uint16	l3r26_temp, l3r25_temp, l4r22_temp, g7r24_temp; 
		int 	calibration_polarity;
	uint8	cnt=0, phyaddr, zcal_offset;
	...
	{
		//printf("\r\n [7522][%d] R50 ===  \r\n", phyaddr);
		tcPhyWriteGReg(port_num, 7, 24, 0x6000);
		//printf("r0 = 0x%x \r\n", tcPhyReadLReg(port_num, 0, 0));
		//printf("l0r26 = 0x%x \r\n", tcPhyReadLReg(port_num, 0, 26));
		//printf("g2r25 = 0x%x \r\n", tcPhyReadGReg(port_num, 2, 25));
		//printf("l4r21 = 0x%x \r\n", tcPhyReadLReg(port_num, 4, 21));
		//printf("g7r24 = 0x%x \r\n", tcPhyReadGReg(port_num, 7, 24));
	}
	else
	{
	...
	{
		zcal_offset = 6;
		g7r24_temp = tcPhyReadGReg(port_num, 7, 24); 
		tcPhyWriteGReg(port_num, 7, 24, (g7r24_temp | rg_zcal_ctrl<<zcal_offset));
		//printf("g7r24 = 0x%x \r\n", tcPhyReadGReg(port_num, 7, 24));
	}
	else
	{	
	...
	if(isEN7526c)
	{
		ad_cal_comp_out_init = ((tcPhyReadGReg(port_num, 7, 24)>>0) & 0x1);
	}
	else
	{
	...
	while((all_ana_cal_status < ANACAL_ERROR)&&(cnt < 254))
	{
		...
		if(isEN7526c)
		{
			g7r24_temp = tcPhyReadGReg(port_num, 7, 24) & 0xf03f; 
			tcPhyWriteGReg(port_num, 7, 24, (g7r24_temp | rg_zcal_ctrl<<zcal_offset));
		...
		if(isEN7526c)
		{
			ad_cal_comp_out = ((tcPhyReadGReg(port_num, 7, 24)>>0) & 0x1);
		}
		else
		{
			ad_cal_comp_out = ((tcPhyReadLReg(FE_CAL_P0, 4, 23)>>4)&0x1);
		}
		...
		else if(ad_cal_comp_out != ad_cal_comp_out_init) // design bug...it shall be global page register
		{
			all_ana_cal_status = ANACAL_FINISH;	
		}
		else
	...
	if(isEN7526c)
	{
		if(phyaddr == 11)
		{
			rg_zcal_ctrl_tx = ZCAL_TO_R50ohm_TBL_100[(rg_zcal_ctrl+6)];		// p11
			rg_zcal_ctrl_rx = ZCAL_TO_R50ohm_TBL_100[(rg_zcal_ctrl+6)];		// p11
		}
		else if(phyaddr == 10)
		{
			rg_zcal_ctrl_tx = ZCAL_TO_R50ohm_TBL_100[(rg_zcal_ctrl+2)]; 	// p10
			rg_zcal_ctrl_rx = ZCAL_TO_R50ohm_TBL_100[(rg_zcal_ctrl+5)];		// p10
		}
		else
		{
			rg_zcal_ctrl_tx = ZCAL_TO_R50ohm_TBL_100[(rg_zcal_ctrl+3)]; 	// p9
			rg_zcal_ctrl_rx = ZCAL_TO_R50ohm_TBL_100[(rg_zcal_ctrl+6)];		// p9
		}
	}
	else
	{
	...
	if(isEN7526c)
	{
		printf(" 7522 FE zcal tx(%d) rx(%d)  \r\n", rg_zcal_ctrl_tx, rg_zcal_ctrl_rx);
		tcPhyWriteLReg(port_num, 4, 22, (((rg_zcal_ctrl_tx)<<8)));		// l4r22[14:8]:for R50_rsel TX
		l4r22_temp = tcPhyReadLReg(port_num, 4, 22);
		printf(" 7522 FE l4_22(0x%x)  \r\n", l4r22_temp);
		//tcPhyWriteLReg(port_num, 4, 22, ((rg_zcal_ctrl<<0)));		// l4r22[6:0] :for R50_rsel RX	
		tcPhyWriteLReg(port_num, 4, 22, (l4r22_temp|((rg_zcal_ctrl_rx)<<0)));	
		l4r22_temp = tcPhyReadLReg(port_num, 4, 22);
		printf(" 7522 FE l4_22(0x%x)  \r\n", l4r22_temp);
	}
	...
	if(isEN7526c)
	{
		tcPhyWriteGReg(port_num, 7, 24, 0x0000);
	}
	else
	{
		tcPhyWriteLReg(FE_CAL_P0, 3, 25, 0x0000);
...
void FECal_R45(uint8 port_num, uint32 delay)	// allen_20170621
{
	...
	{
		all_ana_cal_status = ANACAL_ERROR;	
		printf(" FE R45 AnaCal ERROR! (init)   \r\n");
	}
	...
	while((all_ana_cal_status < ANACAL_ERROR)&&(cnt < 254))
	{
		...
		else
		{
			...
			{
				all_ana_cal_status = ANACAL_SATURATION;  // need to FT
				printf(" FE R45 AnaCal Saturation! (%d)  \r\n", cnt);
			}
				//else
					//printf(" FE-%d R45 ... (%d)(%d)(0x%x)(0x%x)(0x%x)	\r\n", phyaddr, cnt, ((tcPhyReadLReg(FE_CAL_P0, 4, 23)>>4)&0x1), rg_zcal_ctrl, tcPhyReadLReg(FE_CAL_P0, 3, 26));
		}
	}

		//printf(" FE R45 AnaCal zcal_ctrl (%d)  \r\n", (rg_zcal_ctrl));

	if(isEN7526c)
	{
		rg_zcal_ctrl_tx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl)]; 	// +4
		rg_zcal_ctrl_rx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl)];	// +7
	}
	else
	{
		#if defined(TCSUPPORT_CPU_EN7512)
		if(phyaddr == 10)
		{
			rg_zcal_ctrl_tx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl+3)];	  // p10
			rg_zcal_ctrl_rx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl-7)];   // p10
		}
		if(phyaddr == 11)
		{
			rg_zcal_ctrl_tx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl-4)];	  // p11	
			rg_zcal_ctrl_rx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl-9)];   // p11
		}
		else
		{
			rg_zcal_ctrl_tx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl+3)];	  // p8, 9
			rg_zcal_ctrl_rx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl-4)];   // p8, 9
		}
		#else
		if(phyaddr == 9)
		{
			rg_zcal_ctrl_tx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl-3)];		// p9
			rg_zcal_ctrl_rx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl-7)];		// p9
		}
		else if(phyaddr == 11)
		{
			rg_zcal_ctrl_tx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl-6)];		// p11
			rg_zcal_ctrl_rx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl-10)];	// p11
		}
		else
		{
			rg_zcal_ctrl_tx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl-5)];		// p10
			rg_zcal_ctrl_rx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl-12)];    // p10
		}
		#endif
	}
	
		//printf(" FE R45 AnaCal zcal_ctrl (0x%x)  \r\n", rg_zcal_ctrl);
	...
	{
			printf(" FE-%d R45 AnaCal Done! (%d)(0x%x)	\r\n", phyaddr, cnt, rg_zcal_ctrl);
			FECal_R45_flag = 1;
	}
...
void	FECal_Tx_offset(uint8 port_num, uint32 delay)									// for EN7512 
{
	...
	if(isEN7526c)
	{
		tcPhyWriteGReg(port_num, 7, 24, 0x3000);
		tcPhyWriteLReg(port_num, 3, 25, 0x0400);
	}
	else
	{
	l3r25_temp = tcPhyReadLReg(FE_CAL_P0, 3, 25);
	...
	while((all_ana_cal_status < ANACAL_ERROR)&&(cnt < 254))
	{
		...
		else if(ad_cal_comp_out != ad_cal_comp_out_init) 
		{
			tx_amp_cnt ++;
			if(tx_amp_cnt >= 2)
			{
...
void	FECal_TxAmp(uint8 port_num, uint32 delay)									// for EN7512 
{
	...
	tcPhyWriteLReg(port_num, 0, 0, 0x2100);
	tcPhyWriteLReg(port_num, 0, 26, 0x5203);					// fix mdi  // allen_20170405
	...
	if(isEN7526c)
	{
		tcPhyWriteGReg(port_num, 7, 24, 0x7000);
		tcPhyWriteLReg(port_num, 3, 25, 0x0600);   // 0x0600: 1V  0x0400:1.2V
	}
	else
	{
	...
	tx_amp_temp = 0x12;		// allen_20170106, it is suggested by ACD
	tx_amp_reg_shift = 0;
	...
	if(isEN7526c)
	{
		all_ana_cal_status = allFEAnaCalWait_txamp(delay, port_num);   
	}
	else
	{
	...
	else
	{
		...
		if((port_num == 1)) // allen_20160603 : test for MSTC(7512 LAN2)
			tcPhyWriteLReg(port_num, 2, 23, ((l2r23_temp+2)<<tx_amp_reg_shift));
		else if((port_num == 2))
			tcPhyWriteLReg(port_num, 2, 23, ((l2r23_temp+1)<<tx_amp_reg_shift));  // allen_20170405	// allen_20170502
		else
			...
			//else
		tcPhyWriteLReg(port_num, 2, 23, ((l2r23_temp+1)<<tx_amp_reg_shift));   // allen_20170405
		#endif
		#endif
        ...
        if(isEN7526c)
		{
			l2r23_temp = tcPhyReadLReg(port_num, 2, 23);
			if(port_num == 1)
					tcPhyWriteLReg(port_num, 2, 23, ((l2r23_temp)<<tx_amp_reg_shift));   	// p9
			else if(port_num == 2)
					tcPhyWriteLReg(port_num, 2, 23, ((l2r23_temp-1)<<tx_amp_reg_shift));	// p10
			else
					tcPhyWriteLReg(port_num, 2, 23, ((l2r23_temp-3)<<tx_amp_reg_shift));	// p11
		}
        else
        {
			//tcPhyWriteLReg(port_num, 2, 23, ((l2r23_temp+3)<<tx_amp_reg_shift));
				tcPhyWriteLReg(port_num, 2, 23, ((l2r23_temp+1)<<tx_amp_reg_shift));  // for 7521
		}
...
{
	uint8 	all_ana_cal_status;	
	uint16	l4r23_temp, cnt, g7r24_temp;

	if(isEN7526c)
	{
		g7r24_temp = tcPhyReadGReg(port_num, 7, 24); 
		tcPhyWriteGReg(port_num, 7, 24, (g7r24_temp | 0x0010));
	}
	else
	{
	...
	do{
		...
		if(isEN7526c)
		{
			all_ana_cal_status = ((tcPhyReadGReg(port_num, 7, 24)>>1) & 0x1);
		}
		else
		{
	...
	{
		g7r24_temp = tcPhyReadGReg(port_num, 7, 24); 
		tcPhyWriteGReg(port_num, 7, 24, (g7r24_temp & 0xffef));
	}
	else
	{
	tcPhyWriteLReg(FE_CAL_P0, 4, 23, (0x0000));
...
uint8 allFEAnaCalWait_txamp(uint32 delay, uint8 port_num) // for EN7512 FE // allen_20160616
{
	...
	if(isEN7526c)
	{
		...
		//printf("l0r26 = 0x%x \r\n", tcPhyReadLReg(port_num, 0, 26));
		//printf("g2r25 = 0x%x \r\n", tcPhyReadGReg(port_num, 2, 25));
		//printf("g1r26 = 0x%x \r\n", tcPhyReadGReg(port_num, 1, 26));
		//printf("g4r21 = 0x%x \r\n", tcPhyReadGReg(port_num, 4, 21));
		//printf("g7r24 = 0x%x \r\n", tcPhyReadGReg(port_num, 7, 24));
		//printf("l0r30 = 0x%x \r\n", tcPhyReadLReg(port_num, 0, 30));
		//printf("l3r25 = 0x%x \r\n", tcPhyReadLReg(port_num, 3, 25));
		//printf("=======================================================d \r\n");
	}
	else
	{
	...
	//udelay((cnt*delay));  // workaround for 7512 FE tx amp calibration, it will fix in 7522	// allen_20170106, mark for FH call trace
	//udelay((cnt*delay));  // workaround for 7512 FE tx amp calibration, it will fix in 7522	// allen_20170106, mark for FH call trace
	//udelay((cnt*delay));  // workaround for 7512 FE tx amp calibration, it will fix in 7522	// allen_20170106, mark for FH call trace
	...
	if(isEN7526c)
	{
		...
		g7r24_temp = tcPhyReadGReg(port_num, 7, 24); 
		tcPhyWriteGReg(port_num, 7, 24, (g7r24_temp & 0x7000));
		//printf("g7r24 = 0x%x \r\n", tcPhyReadGReg(port_num, 7, 24));
	}
	else
	{
...
#if debug_flag
#if defined(TCSUPPORT_CPU_EN7516) || defined(TCSUPPORT_AUTOBENCH)
#if 0
int gmac1_dc_offset_test(void)
{
	...
	// When link-down power saving mechanism starts, AD will be disabled.
  	...
  	tcMiiStationWrite(9, 30, 0x0000);

	tcMiiStationWrite(9, 26, 0x5200);
	delay1ms(100);
	dcoff_MDI = tc2102meReadAdcSum(0);
	tcMiiStationWrite(9, 26, 0x9200);
	delay1ms(100);
	dcoff_MDIX=tc2102meReadAdcSum(0);
	printk("dcoff_MDI=%d dcoff_MDIX:%d\r\n", dcoff_MDI, dcoff_MDIX);
	
	// Change back to auto-crossover mode
	tcMiiStationWrite(9, 26, 0x1600);
	
	// Turn on link-down power saving mechanism
	tcMiiStationWrite(9, 30, 0x8000);
	
	if( (dcoff_MDI < 150) && (dcoff_MDI > -150) && (dcoff_MDIX < 150) && (dcoff_MDIX > -150) )
		return 0;

	return 2;
}
EXPORT_SYMBOL(gmac1_dc_offset_test);
#endif
#endif
#endif
