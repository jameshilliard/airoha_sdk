...
#if TCSUPPORT_ETHPHY_5R
  //5R driver code
#define mdi_resister  0
#endif

#if TCSUPPORT_ETHPHY_0R
  //0R driver code
  #define mdi_resister  1
#endif
...
static uint8    inital_finish_flag = 0,command_calbration_enable=1;
...
static int ephy_monitor_all_counter=0;
...
static int downshift_to_force_giga_slave_p9=0,downshift_to_force_giga_slave_p10=0,downshift_to_force_giga_slave_p11=0,downshift_to_force_giga_slave_p12=0;
...
unsigned int slt_not_excute_loop,transformer_status=0; //transformer_status=0-->discrete   ;  transformer_status=0xf-->TXMR
...
void loopback_head_downshift_100M_main(unsigned int phyaddr,unsigned int reg1)
{
	unsigned int reg_temp1,reg_temp,downshift_2pair_limit=24;
		...
		if(reg_temp1==0) {
			...
			if(link_state_polling == 1){ printf("phy=%d loopback header downshift \n\r",phyaddr);}
		}}
...
void downshift_to_force_giga_slave(unsigned int phyaddr,unsigned int dev_1e_reg_a2)
{
	unsigned int dev_1e_reg_a2_temp,page_0_reg_1a_temp,page_0_reg_1a,page_0_reg_1,page_0_reg_1_temp;
		page_0_reg_1a = tcMiiStationRead(phyaddr, 0x1a);
		page_0_reg_1a_temp=page_0_reg_1a&0x0004;
		dev_1e_reg_a2_temp = dev_1e_reg_a2 &0x0004;
        if((phyaddr ==9)&&(page_0_reg_1a_temp !=0)&&(dev_1e_reg_a2_temp !=0)) {	
			if(downshift_to_force_giga_slave_p9==0) {
			tcMiiStationWrite(phyaddr, 0x0 ,0x1840);tcMiiStationWrite(phyaddr, 0x9 ,0x1600);tcMiiStationWrite(phyaddr, 0x0 ,0x1240);if(link_state_polling == 1){ printf("phy=%d force slave set \n\r",phyaddr);}
			downshift_to_force_giga_slave_p9=1;}
		}
        if((phyaddr ==10)&&(page_0_reg_1a_temp !=0)&&(dev_1e_reg_a2_temp !=0)) { 
			if(downshift_to_force_giga_slave_p10==0) {
            tcMiiStationWrite(phyaddr, 0x0 ,0x1840);tcMiiStationWrite(phyaddr, 0x9 ,0x1600);tcMiiStationWrite(phyaddr, 0x0 ,0x1240);if(link_state_polling == 1){ printf("phy=%d force slave set \n\r",phyaddr);}
			downshift_to_force_giga_slave_p10=1;} 
		}
        if((phyaddr ==11)&&(page_0_reg_1a_temp !=0)&&(dev_1e_reg_a2_temp !=0)) { 
			if(downshift_to_force_giga_slave_p11==0) {
			tcMiiStationWrite(phyaddr, 0x0 ,0x1840);tcMiiStationWrite(phyaddr, 0x9 ,0x1600);tcMiiStationWrite(phyaddr, 0x0 ,0x1240);if(link_state_polling == 1){ printf("phy=%d force slave set \n\r",phyaddr);}
			downshift_to_force_giga_slave_p11=1;} 
		}
        if((phyaddr ==12)&&(page_0_reg_1a_temp !=0)&&(dev_1e_reg_a2_temp !=0)) { 		
			if(downshift_to_force_giga_slave_p12==0) {
			tcMiiStationWrite(phyaddr, 0x0 ,0x1840);tcMiiStationWrite(phyaddr, 0x9 ,0x1600);tcMiiStationWrite(phyaddr, 0x0 ,0x1240);if(link_state_polling == 1){ printf("phy=%d force slave set \n\r",phyaddr);}
			downshift_to_force_giga_slave_p12=1;}
		}
	    page_0_reg_1 = tcMiiStationRead(phyaddr, 0x1);
		page_0_reg_1 = tcMiiStationRead(phyaddr, 0x1);
		page_0_reg_1_temp=page_0_reg_1&0x0004;
		if((downshift_to_force_giga_slave_p9==1)&&(page_0_reg_1_temp!=0)) { downshift_to_force_giga_slave_p9=0;tcMiiStationWrite(phyaddr, 0x09, 0x1e00);}
		if((downshift_to_force_giga_slave_p10==1)&&(page_0_reg_1_temp!=0)) { downshift_to_force_giga_slave_p10=0;tcMiiStationWrite(phyaddr, 0x09, 0x1e00);}
		if((downshift_to_force_giga_slave_p11==1)&&(page_0_reg_1_temp!=0)) { downshift_to_force_giga_slave_p11=0;tcMiiStationWrite(phyaddr, 0x09, 0x1e00);}
		if((downshift_to_force_giga_slave_p12==1)&&(page_0_reg_1_temp!=0)) { downshift_to_force_giga_slave_p12=0;tcMiiStationWrite(phyaddr, 0x09, 0x1e00);}
...
{
    uint8 	pn;
	uint32 	reg0, reg1, reg9, reg10,reg_temp,reg_9_temp,reg_10_temp,reg_temp1,master_times=1,auto_times=3;
	uint32 	dev_1e_reg_a2,dev_1e_reg_a2_temp,page_0_reg_1a,page_0_reg_1a_temp;
    ...
    for(phyaddr = gphy_base; phyaddr < (gphy_base+EN75xxGe_PORTNUM); phyaddr++) 
	{
        ...
        else if( !Nmr1[pn].link_status_prev && Nmr1[pn].link_status )
		{
			...
			if(reg_temp ==0x22) {printf("phy=%d link 10M full duplex",phyaddr);}
			if(reg_temp ==0x02) {printf("phy=%d link 10M half duplex",phyaddr);}
			if(reg_temp ==0x24) {printf("phy=%d link 100M full duplex",phyaddr);}
			if(reg_temp ==0x04) {printf("phy=%d link 100M half duplex",phyaddr);}
			if(reg_temp ==0x28) {printf("phy=%d link 1000M full duplex",phyaddr);reg_temp=dev_1e_reg_a2&0x0010;if(reg_temp ==0x10) {printf(" Master");} else {printf(" Slave");}}
			if(reg_temp ==0x08) {printf("phy=%d link 1000M ",phyaddr);reg_temp=dev_1e_reg_a2&0x0010;if(reg_temp ==0x10) {printf(" Master");} else {printf(" Slave");}}
			printf("\n\r");			
			printf("phy=%d link down to up\n\r",phyaddr);}
        }
        else if( Nmr1[pn].link_status_prev && !Nmr1[pn].link_status )
		{
		...
		{ 	//if( Nmr1[pn].link_status_prev && Nmr1[pn].link_status ){
			...
			if( slt_excuting==0 ) {
		    if( phyaddr ==9 ) { force_giga_master_mode_p9=0;force_giga_master_counter_p9=0;}
			if( phyaddr ==10 ) { force_giga_master_mode_p10=0;force_giga_master_counter_p10=0;}
			if( phyaddr ==11 ) { force_giga_master_mode_p11=0;force_giga_master_counter_p11=0;}
			if( phyaddr ==12 ) { force_giga_master_mode_p12=0;force_giga_master_counter_p12=0;}
			...
			if( reg_temp !=0 ) 		    
				...
				if( reg_temp !=0x1800 )
					...
					tcMiiStationWrite(phyaddr, 0x09, reg_temp);	
					}
				}


		
		if( phy_downshift_force_giga_slave_enable==1 ) { downshift_to_force_giga_slave(phyaddr,dev_1e_reg_a2);}
...
static void doPhyGphyTestMode (int argc, char *argv[], void *p);
static int extPhyGphyTestMode (int argc, char *argv[], void *p);
static void settingANSpeed (int argc, char *argv[], void *p);
static void settingloopback (int argc, char *argv[], void *p);
static void settingFlowControl (int argc, char *argv[], void *p);
static void getAllPortStatus (int argc, char *argv[], void *p);
...
static void buck_pbus_read_command(int argc, char *argv[], void *p);
static void buck_pbus_write_command(int argc, char *argv[], void *p);
static int doPhyChkVal (int argc, char *argv[], void *p);
...
int tcPhyVerLookUp(uint16 ephy_addr)
{
    ...
    else if(rval == EPHY_ID_7552Ge)  
	{
        tcPhyVer = tcPhyVer_7552Ge;		
        tcPhyPortNum = 4;
        TCPHYDISP1(" 7552 Ge, \n");
        tcPhyFlag = 1;
        tcPhyInitFlag = 1;
...
{ 
	...
	uint16	start_phy=9,end_phy=12,status_flag=0;
	...
	//TXMR_or_discrete=0-->discrete
	if((TXMR_or_discrete == 0)&& (mdi_resister == phy_5R)) 
	{	
	if(phy == 9) 
	{
		    // test mode TX amp setting
			P9_tx_amp_tst_cha_comp=-5;P9_tx_amp_tst_chb_comp=-7;P9_tx_amp_tst_chc_comp=-3;P9_tx_amp_tst_chd_comp=-8;
			// Giga TX amp setting
			P9_tx_amp_gbe_cha_comp=-15;P9_tx_amp_gbe_chb_comp=-17;P9_tx_amp_gbe_chc_comp=-13;P9_tx_amp_gbe_chd_comp=-18;
			// 100M TX amp setting
			P9_tx_amp_hbt_cha_comp=5;P9_tx_amp_hbt_chb_comp=6;
			// 10M TX amp setting
	...
	{
		    // test mode TX amp setting
			P10_tx_amp_tst_cha_comp=-8;P10_tx_amp_tst_chb_comp=-8;P10_tx_amp_tst_chc_comp=-6;P10_tx_amp_tst_chd_comp=-8;
			// Giga TX amp setting
			P10_tx_amp_gbe_cha_comp=-18;P10_tx_amp_gbe_chb_comp=-18;P10_tx_amp_gbe_chc_comp=-16;P10_tx_amp_gbe_chd_comp=-18;
			// 100M TX amp setting
			P10_tx_amp_hbt_cha_comp=7;P10_tx_amp_hbt_chb_comp=6;
			// 10M TX amp setting
	...
	{
		    // test mode TX amp setting
			P11_tx_amp_tst_cha_comp=-7;P11_tx_amp_tst_chb_comp=-6;P11_tx_amp_tst_chc_comp=-5;P11_tx_amp_tst_chd_comp=-9;
			// Giga TX amp setting
			P11_tx_amp_gbe_cha_comp=-17;P11_tx_amp_gbe_chb_comp=-18;P11_tx_amp_gbe_chc_comp=-18;P11_tx_amp_gbe_chd_comp=-19;
			// 100M TX amp setting
			P11_tx_amp_hbt_cha_comp=7;P11_tx_amp_hbt_chb_comp=6;
			// 10M TX amp setting
	...
	{
		    // test mode TX amp setting
			P12_tx_amp_tst_cha_comp=-5;P12_tx_amp_tst_chb_comp=-6;P12_tx_amp_tst_chc_comp=-2;P12_tx_amp_tst_chd_comp=-8;
			// Giga TX amp setting
			P12_tx_amp_gbe_cha_comp=-18;P12_tx_amp_gbe_chb_comp=-16;P12_tx_amp_gbe_chc_comp=-15;P12_tx_amp_gbe_chd_comp=-18;
			// 100M TX amp setting
			P12_tx_amp_hbt_cha_comp=7;P12_tx_amp_hbt_chb_comp=7;
			// 10M TX amp setting
    ...
    //TXMR_or_discrete=1-->TXMR
	if((TXMR_or_discrete == 1)&& (mdi_resister == phy_5R)) 
	{	
	if(phy == 9) 
	{
		    // test mode TX amp setting
			P9_tx_amp_tst_cha_comp=-11;P9_tx_amp_tst_chb_comp=-9;P9_tx_amp_tst_chc_comp=-10;P9_tx_amp_tst_chd_comp=-12;
			// Giga TX amp setting
			P9_tx_amp_gbe_cha_comp=-21;P9_tx_amp_gbe_chb_comp=-19;P9_tx_amp_gbe_chc_comp=-20;P9_tx_amp_gbe_chd_comp=-22;
			// 100M TX amp setting
			P9_tx_amp_hbt_cha_comp=0;P9_tx_amp_hbt_chb_comp=0;
			// 10M TX amp setting
			P9_tx_amp_tbt_cha_comp=0;P9_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P9_r50_cha_comp=7;P9_r50_chb_comp=8;P9_r50_chc_comp=10;P9_r50_chd_comp=10;
	}
	
	if(phy == 10) 
	{
		    // test mode TX amp setting
			P10_tx_amp_tst_cha_comp=-10;P10_tx_amp_tst_chb_comp=-8;P10_tx_amp_tst_chc_comp=-10;P10_tx_amp_tst_chd_comp=-12;
			// Giga TX amp setting
			P10_tx_amp_gbe_cha_comp=-20;P10_tx_amp_gbe_chb_comp=-18;P10_tx_amp_gbe_chc_comp=-20;P10_tx_amp_gbe_chd_comp=-22;
			// 100M TX amp setting
			P10_tx_amp_hbt_cha_comp=0;P10_tx_amp_hbt_chb_comp=1;
			// 10M TX amp setting
			P10_tx_amp_tbt_cha_comp=0;P10_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P10_r50_cha_comp=8;P10_r50_chb_comp=9;P10_r50_chc_comp=10;P10_r50_chd_comp=10;
	}
	if(phy == 11) 
	{
		    // test mode TX amp setting
			P11_tx_amp_tst_cha_comp=-7;P11_tx_amp_tst_chb_comp=-10;P11_tx_amp_tst_chc_comp=-11;P11_tx_amp_tst_chd_comp=-11;
			// Giga TX amp setting
			P11_tx_amp_gbe_cha_comp=-17;P11_tx_amp_gbe_chb_comp=-20;P11_tx_amp_gbe_chc_comp=-21;P11_tx_amp_gbe_chd_comp=-21;
			// 100M TX amp setting
			P11_tx_amp_hbt_cha_comp=0;P11_tx_amp_hbt_chb_comp=0;
			// 10M TX amp setting
			P11_tx_amp_tbt_cha_comp=0;P11_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P11_r50_cha_comp=9;P11_r50_chb_comp=10;P11_r50_chc_comp=10;P11_r50_chd_comp=10;
	}
	if(phy == 12) 
	{
		    // test mode TX amp setting
			P12_tx_amp_tst_cha_comp=-9;P12_tx_amp_tst_chb_comp=-7;P12_tx_amp_tst_chc_comp=-8;P12_tx_amp_tst_chd_comp=-7;
			// Giga TX amp setting
			P12_tx_amp_gbe_cha_comp=-19;P12_tx_amp_gbe_chb_comp=-17;P12_tx_amp_gbe_chc_comp=-18;P12_tx_amp_gbe_chd_comp=-17;
			// 100M TX amp setting
			P12_tx_amp_hbt_cha_comp=0;P12_tx_amp_hbt_chb_comp=0;
			// 10M TX amp setting
			P12_tx_amp_tbt_cha_comp=0;P12_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P12_r50_cha_comp=11;P12_r50_chb_comp=11;P12_r50_chc_comp=11;P12_r50_chd_comp=10;
	}
	}


	//TXMR_or_discrete=0-->discrete
	if((TXMR_or_discrete == 0)&& (mdi_resister == phy_0R)) 
	{	
	if(phy == 9) 
	{
		    // test mode TX amp setting
			P9_tx_amp_tst_cha_comp=-9;P9_tx_amp_tst_chb_comp=-11;P9_tx_amp_tst_chc_comp=-9;P9_tx_amp_tst_chd_comp=-12;
			// Giga TX amp setting
			P9_tx_amp_gbe_cha_comp=-9;P9_tx_amp_gbe_chb_comp=-11;P9_tx_amp_gbe_chc_comp=-9;P9_tx_amp_gbe_chd_comp=-12;
			// 100M TX amp setting
			P9_tx_amp_hbt_cha_comp=0;P9_tx_amp_hbt_chb_comp=1;
			// 10M TX amp setting
			P9_tx_amp_tbt_cha_comp=0;P9_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P9_r50_cha_comp=11;P9_r50_chb_comp=10;P9_r50_chc_comp=6;P9_r50_chd_comp=8;
	}
	
	if(phy == 10) 
	{
		    // test mode TX amp setting
			P10_tx_amp_tst_cha_comp=-12;P10_tx_amp_tst_chb_comp=-10;P10_tx_amp_tst_chc_comp=-11;P10_tx_amp_tst_chd_comp=-9;
			// Giga TX amp setting
			P10_tx_amp_gbe_cha_comp=-12;P10_tx_amp_gbe_chb_comp=-10;P10_tx_amp_gbe_chc_comp=-11;P10_tx_amp_gbe_chd_comp=-9;
			// 100M TX amp setting
			P10_tx_amp_hbt_cha_comp=0;P10_tx_amp_hbt_chb_comp=2;
			// 10M TX amp setting
			P10_tx_amp_tbt_cha_comp=0;P10_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P10_r50_cha_comp=11;P10_r50_chb_comp=11;P10_r50_chc_comp=6;P10_r50_chd_comp=8;
	}
	if(phy == 11) 
	{
		    // test mode TX amp setting
			P11_tx_amp_tst_cha_comp=-11;P11_tx_amp_tst_chb_comp=-9;P11_tx_amp_tst_chc_comp=-11;P11_tx_amp_tst_chd_comp=-7;
			// Giga TX amp setting
			P11_tx_amp_gbe_cha_comp=-11;P11_tx_amp_gbe_chb_comp=-9;P11_tx_amp_gbe_chc_comp=-11;P11_tx_amp_gbe_chd_comp=-7;
			// 100M TX amp setting
			P11_tx_amp_hbt_cha_comp=0;P11_tx_amp_hbt_chb_comp=2;
			// 10M TX amp setting
			P11_tx_amp_tbt_cha_comp=0;P11_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P11_r50_cha_comp=12;P11_r50_chb_comp=10;P11_r50_chc_comp=8;P11_r50_chd_comp=8;
	}
	if(phy == 12) 
	{
		    // test mode TX amp setting
			P12_tx_amp_tst_cha_comp=-10;P12_tx_amp_tst_chb_comp=-9;P12_tx_amp_tst_chc_comp=-12;P12_tx_amp_tst_chd_comp=-11;
			// Giga TX amp setting
			P12_tx_amp_gbe_cha_comp=-10;P12_tx_amp_gbe_chb_comp=-9;P12_tx_amp_gbe_chc_comp=-12;P12_tx_amp_gbe_chd_comp=-11;
			// 100M TX amp setting
			P12_tx_amp_hbt_cha_comp=1;P12_tx_amp_hbt_chb_comp=2;
			// 10M TX amp setting
			P12_tx_amp_tbt_cha_comp=0;P12_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P12_r50_cha_comp=11;P12_r50_chb_comp=11;P12_r50_chc_comp=8;P12_r50_chd_comp=6;
	}
	}


	//TXMR_or_discrete=1-->TXMR
	if((TXMR_or_discrete == 1)&& (mdi_resister == phy_0R)) 
	{	
	if(phy == 9) 
	{
		    // test mode TX amp setting
...
int my_ephy_monitor(unsigned long data)
{
...
int ePhyInit(uint16 ephy_addr)
{
	...
	switch(tcPhyVer) 
	{			    
		case tcPhyVer_7552Ge: 		// EN7581 Ge
			if(ephy_addr_base == 0)
				ephy_addr_base += EPHY_ADDR_P0;
			break;
	}
    ...
    switch(tcPhyVer) 
	{
		...
		case tcPhyVer_7552Ge: 		// AN7552 Ge
...
{
	uint16 ephy_addr;
	unsigned int mt7530_temp,transformer_temp=0;
	unsigned int TXMR_or_discrete=0,show_temp;
	...
	for(ephy_addr = all_port_start; ephy_addr <= all_port_end; ephy_addr++)
		{
		    ...
		    if(mdi_resister == phy_5R)
            {
					//RX setting for 5R before AN setting
					...
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x01, 0x01c8);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x02, 0x01c2);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x03, 0x0100);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x04, 0x020d);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x05, 0x020a);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x06, 0x038f);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x07, 0x03c8);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x08, 0x03c4);
					...
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x23, 0x0c85);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x24, 0x0c85);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x25, 0x0c85);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x26, 0x0c85);
					//100M sharp
					...
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x03, 0x0100);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x04, 0x020d);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x05, 0x0204);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x06, 0x0390);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x07, 0x03c8);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x08, 0x03c2);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x09, 0x0300);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x0a, 0x000d);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x0b, 0x0004);
			...
			if(mdi_resister == phy_0R)
            {
				
            		//RX setting for 0R before AN setting
					...
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x00, 0x0190);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x01, 0x01c8);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x02, 0x01c6);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x03, 0x0100);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x04, 0x020c);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x05, 0x0206);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x06, 0x0390);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x07, 0x03c8);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x08, 0x03c6);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x09, 0x0300);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x0a, 0x000c);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x0b, 0x0006);
					}
			}
					...
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x269, 0x1111);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x26a, 0x0000);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x26b, 0x0074);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x26e, 0x00f6);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x26f, 0x6666);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x271, 0x2c02);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x272, 0x0c22);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x700, 0x0001);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x701, 0x0803);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x702, 0x01b6);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x703, 0x1111);
					
			
					//Long Loop Reach setting 
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x11, 0x0f00);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x200, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x202, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x204, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x206, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x208, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x20a, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x20c, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x20e, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x210, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x212, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x214, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x216, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x218, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x21a, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x21c, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x21e, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x220, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x222, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x224, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x226, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x228, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x22a, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x22c, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x22e, 0x2010);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x230, 0x2010);
					...
					//tx
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x44, 0x0000);
					...
					//tcMiiStationWrite(ephy_addr, 0x4, 0x0de1);
					...
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x269, 0x3111);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x26a, 0x0000);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x26b, 0x0094);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x26e, 0x00f0);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x26f, 0x0000);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x271, 0x3c04);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x272, 0x0c23);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x700, 0x0003);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x701, 0x14a3);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x702, 0x01ff);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x703, 0x1411);
...
static int doPhySwVer(int argc, char *argv[], void *p)
{
	...
	{    
		case tcPhyVer_7552Ge: 
			printf("%s, \r\n", en75xxGe_cfg[0].name);
		 	printf("tcPhyVer_7552Ge, tcPhyVer=0x%02lX \r\n", tcPhyVer);
			break;	
...
printf(": A7552.05.00_final\n");
if(mdi_resister == phy_5R)
{
transformer_ver=transformer_status&0x01;
if(transformer_ver=transformer_status != 0) { printf("port 9_5R_TXMR \r\n");} else { printf("port 9_5R_discrete_TXMR \r\n");}

transformer_ver=transformer_status&0x02;
if(transformer_ver=transformer_status != 0) { printf("port 10_5R_TXMR \r\n");} else { printf("port 10_5R_discrete_TXMR \r\n");}

transformer_ver=transformer_status&0x04;
if(transformer_ver=transformer_status != 0) { printf("port 11_5R_TXMR \r\n");} else { printf("port 11_5R_discrete_TXMR \r\n");}

transformer_ver=transformer_status&0x08;
if(transformer_ver=transformer_status != 0) { printf("port 12_5R_TXMR \r\n");} else { printf("port 12_5R_discrete_TXMR \r\n");}
}

if(mdi_resister == phy_0R)
{
transformer_ver=transformer_status&0x01;
if(transformer_ver=transformer_status != 0) { printf("port 9_0R_TXMR \r\n");} else { printf("port 9_0R_discrete_TXMR \r\n");}
	
transformer_ver=transformer_status&0x02;
if(transformer_ver=transformer_status != 0) { printf("port 10_0R_TXMR \r\n");} else { printf("port 10_0R_discrete_TXMR \r\n");}
	
transformer_ver=transformer_status&0x04;
if(transformer_ver=transformer_status != 0) { printf("port 11_0R_TXMR \r\n");} else { printf("port 11_0R_discrete_TXMR \r\n");}
	
transformer_ver=transformer_status&0x08;
if(transformer_ver=transformer_status != 0) { printf("port 12_0R_TXMR \r\n");} else { printf("port 12_0R_discrete_TXMR \r\n");}
}
...
void buck_pbus_read_command(int argc, char *argv[], void *p)
{
...
void buck_pbus_write_command(int argc, char *argv[], void *p)
{
...
void doPhyGphyTestMode (int argc, char *argv[], void *p)
{
	...
	else	
	{
		...
		if(speed == 100 )	{
			...
			if(mode == 1 ) 
			{
	            ...
	            if(phyaddr == 9 ) {show_temp=transformer_status&0x01;}
  	            if(phyaddr == 10 ) {show_temp=transformer_status&0x02;}
                if(phyaddr == 11 ) {show_temp=transformer_status&0x04;}
   	            if(phyaddr == 12 ) {show_temp=transformer_status&0x08;}
                if(show_temp == 0 )
				{ 
			       	mtEMiiRegWrite(phyaddr, 0x1f, 0x269, 0x1611);	
					mtEMiiRegWrite(phyaddr, 0x1f, 0x26a, 0x0600);	
					mtEMiiRegWrite(phyaddr, 0x1f, 0x26c, 0x4644);	
					mtEMiiRegWrite(phyaddr, 0x1f, 0x26d, 0x4644);	
	            }
...
int doPhyForceMode(int argc, char *argv[], void *p) 	
{
	...
	else if(argc == 4) // tce forcemode <phyaddr> <an|force> <10|100|1000>
	{
		...
		else if(stricmp(argv[2], "force") == 0)
		{
			...
			else if((stricmp(argv[3], "1000") == 0)&&(reg3==EPHY_ID_7552Ge))
			{
...
int doPhySLTMode(int argc, char *argv[], void *p) 	// allen_20171109
{
	...
	{
		if((stricmp(argv[2], "10") == 0)&&(reg3==EPHY_ID_7552Ge))
		{
			doEphySetSLTMode(phyaddr, 10);
			printf("force 10 half  \r\n");
			}
		else if((stricmp(argv[2], "100") == 0)&&(reg3==EPHY_ID_7552Ge))
		{
			doEphySetSLTMode(phyaddr, 100);
			printf("force 100 half  \r\n");
			}
		else if((stricmp(argv[2], "1000") == 0)&&(reg3==EPHY_ID_7552Ge))
		{
...
int doPhyForceReset(int argc, char *argv[], void *p) 	// allen_20131120
{
	...
	if((reg3 == EPHY_ID_7530Ge_7512Fe)&&(((r15_temp>>13)&0x1) == 1)) // 7530GE
	{
		tcPhyVer = mtPhyVer_7530;
	}
...
void getAllPortStatus (int argc, char *argv[], void *p)
{	
...
void settingFlowControl (int argc, char *argv[], void *p)
{	
...
void settingloopback (int argc, char *argv[], void *p)
{	
...
void settingANSpeed (int argc, char *argv[], void *p)
{	
...
void R50Calibration(uint8 phyaddr,uint8 phyaddr_p0,uint8 resisiter)
{
	uint8	rg_zcal_ctrl, all_ana_cal_status;
	uint16	ad_cal_comp_out_init;
	uint16	dev1e_e0_ana_cal_r5, dev1f_ff_temp,transformer_temp;
	int 	calibration_polarity;
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		...
		else
		{
			...
			if(mdi_resister == phy_5R) {rg_zcal_ctrl = ZCAL_5R_TBL[rg_zcal_ctrl];}
			if(mdi_resister == phy_0R) {rg_zcal_ctrl = ZCAL_0R_TBL[rg_zcal_ctrl];}
