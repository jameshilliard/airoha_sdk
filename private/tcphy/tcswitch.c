...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CT_PON_CY) && !defined(TCSUPPORT_CT_PON_C9)
...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CPU_MT7510) ||  defined(TCSUPPORT_CPU_MT7505)
...
unsigned int queuemark_enable;
unsigned int queuemark_shift;
unsigned int queuemark_width;
...
struct sk_buff *macRT63365STagInsert(struct sk_buff *bp)
{
	...
	if (bp->mark & SKBUF_COPYTOLAN){
		//if the packet is used for wan2lan,set port_mask from masko that from cli
		#ifdef WAN2LAN
		#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CT_2PWIFI)
		if (g_port_reverse) {
			uint8 temp_masko = (uint8)(masko & 0x3);
			if((temp_masko != 0x3) && (temp_masko != 0x0))
			{
...
struct sk_buff *macMT7510STagInsert(struct sk_buff *bp)
{
	...
	if (tag0 == 0x8100) {
		...
		/* add by xyzhu for port reverse */
			if(g_port_reverse){			/* Lan port reverse */
				tag1 -= 1;
				#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CT_2PORTS) && defined(TCSUPPORT_C7)
				if(0<=tag1 && tag1<2)
					tag1 = 1 - tag1;
				#else/*TCSUPPORT_COMPILE*/
				if(0<=tag1 && tag1<4)
	...
	else
	{
		...
		*/
...
#endif
        {
            ...
            if (bp->mark&0x1)
            {
                *(uint16 *)(cp+12) = (vpm << 8) + (1 << 6) + (port_mask);
            }
            else
#endif/*TCSUPPORT_COMPILE*/
            {
...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_XPON_HAL_API_EXT)
                if(g_driver_lan_port_led[i])
#endif/*TCSUPPORT_COMPILE*/
                {
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_XPON_HAL_API_EXT)				
					if (isMT7525) {
						ledTurnOn(LED_ETHER_PORT3_ACT_STATUS + 2*i);
					} else 
#endif/*TCSUPPORT_COMPILE*/									
					{
...
int macMT7520STagRemove(struct sk_buff *bp)
{
	...
	sp_tag = *(uint16 *)(cp+12);
	tci = *(uint16 *)(cp+14);
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_LAN_VLAN) || defined(TCSUPPORT_LAN_VLAN_RANGE) || defined(TCSUPPORT_CDS) || defined(TCSUPPORT_CMCC)
...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_XPON_HAL_API_EXT)
            if(g_driver_lan_port_led[pvid])
#endif/*TCSUPPORT_COMPILE*/
            {
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_XPON_HAL_API_EXT)
				if (isMT7525) {
					ledTurnOn(LED_ETHER_PORT3_ACT_STATUS + 2*(pvid));
				} else 
#endif/*TCSUPPORT_COMPILE*/									
				{
...
void doTxRingRateLimit(int txRing, int speed)
{
	...
	{
		val1&=~(0xffff<<(rem?16:0));
		val2 = ((120*125/(8*4))<<(rem?16:0));    /* 120M bps*/
		val1|=val2;
		write_reg_word(reg, val1);
	}
	else
