...
unsigned int toKenRingRead(unsigned int phy, unsigned int ch, unsigned int node, unsigned int data_addr)
{
	...
	printf("page=0x%x \r\n", tcMiiStationRead(phy, 0x1f));
	data=0xa000+(ch*2048)+(node*128)+(data_addr*2);
		printf("phy=%d data=0x%x \r\n", phy, data);
		mdelay(10);
	tcMiiStationWrite(phy, 0x10, data);

	low_byte=tcMiiStationRead(phy, 0x11);
	high_byte=tcMiiStationRead(phy, 0x12);
		printf("page value=0x%x \r\n", tcMiiStationRead(phy, 0x1f));
		printf("reg10 value=0x%x \r\n", tcMiiStationRead(phy, 0x10));
		printf("reg11 value=0x%x \r\n", tcMiiStationRead(phy, 0x11));
		printf("reg12 value=0x%x \r\n", tcMiiStationRead(phy, 0x12));		
...
void toKenRingWrite(unsigned int phy, unsigned int ch, unsigned int node, unsigned int data_addr,unsigned int value)
{
	...
	data=0x8000|(ch*2048)+(node*128)+(data_addr*2);
	tcMiiStationWrite(phy, 0x10, data);
	tcMiiStationWrite(phy, 0x1f, last_page);
}
...
#define EN75xxGe_PHY_INIT_CL45_LDATA_LEN 	42	 
...
en75xxGe_cfg_cl45data_t en75xxGe_cfg_cl45[EN75xxGe_PHY_INIT_CL45_SET_NUM]=
{
    {	//globle data
     	...
     	//local data
     	{
			{0x1e,0x0000,0x0180},					// 0->1, middle
			{0x1e,0x0001,0x01c3},					// 0->1, ov1
			{0x1e,0x0002,0x01c2},					// 0->1, ov2, z2p_ovs
			{0x1e,0x0003,0x010e},					// 1->0, middle
			{0x1e,0x0004,0x0203},					// 1->0, ov1
			{0x1e,0x0005,0x0204},					// 1->0, ov2, p2z_ovs

			{0x1e,0x0006,0x0384},					// 0->-1, middle
			{0x1e,0x0007,0x03be},					// 0->-1, ov1
			{0x1e,0x0008,0x03c5},					// 0->-1, ov2, z2n_ovs
			{0x1e,0x0009,0x030a},					// -1->0, middle
			{0x1e,0x000a,0x0006},					// -1->0, ov1
			{0x1e,0x000b,0x0003},					// -1->0, ov2, n2z_ovs
...
{ 
	...
	// =============================================================================================
      ...
      P9_tx_amp_tst_chb_comp=-9;P10_tx_amp_tst_chb_comp=-9;P11_tx_amp_tst_chb_comp=-9;P12_tx_amp_tst_chb_comp=-9;
      P9_tx_amp_tst_chc_comp=-9;P10_tx_amp_tst_chc_comp=-9;P11_tx_amp_tst_chc_comp=-9;P12_tx_amp_tst_chc_comp=-9;
      P9_tx_amp_tst_chd_comp=-9;P10_tx_amp_tst_chd_comp=-9;P11_tx_amp_tst_chd_comp=-9;P12_tx_amp_tst_chd_comp=-9;
	// =============================================================================================
	// Giga TX amp setting
	// =============================================================================================
	...
	for(phy_loop = start_phy; phy_loop <= end_phy; phy_loop++)
		{
			...
			//write back reg
				...
				mtEMiiRegWrite(phy_loop, 0x1e, 0x005c, 0x7777);
...
static int doPhySwVer(int argc, char *argv[], void *p)
{
	...
	printf(": A7523.04.04_5R setting_discrete_TXMR  \r\n");  
	return 0;
} 
