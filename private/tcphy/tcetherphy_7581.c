...
#define define_TXMR_or_discrete_TXMR  0x0
#define define_TXMR_or_discrete_resister  0x5
#endif

#if ETHPHY_0R_DIS
  //Discrete 0R driver code
#define define_TXMR_or_discrete_TXMR  0x0
#define define_TXMR_or_discrete_resister  0x0
#endif

#if ETHPHY_5R_TRA
  //Tradition 5R driver code
#define define_TXMR_or_discrete_TXMR  0xf
#define define_TXMR_or_discrete_resister  0x5
...
static uint8	force_giga_master_counter_p9=0,force_giga_master_counter_p10=0,force_giga_master_counter_p11=0,force_giga_master_counter_p12=0;
...
static uint8    inital_finish_flag = 0,command_calbration_enable=0;
...
static uint8    sw_down_shift_en=0,slt_excuting=0,phy_downshift_force_giga_slave_enable=1,loopback_head_downshift_100M=0;
unsigned int slt_not_excute_loop,mdi_resister=5,transformer_status=0x0; //transformer_status=0-->discrete     transformer_status=0xf-->TXMR
//unsigned int slt_not_excute_loop,transformer_status=define_TXMR_or_discrete_TXMR,mdi_resister=define_TXMR_or_discrete_resister;
...
#define tcPhyVer_7581Ge		26	// 94c1
...
*
**************************************************************************/
int phyTxAmpcomp(int show_or_comp,unsigned int phy);
...
#define EN75xxGe_PHY_INIT_TR_LDATA_LEN 		0
    ...
    {	//globle data
     	{
			...
			//{0x1f, 0x0272, 0x7cff},					// ps_op 0x3cff, for PHYD bug, need to workaround 
			//{0x1f, 0x026a, 0x3333},
			//{0x1f, 0x026f, 0xffff},

			//{0x1f, 0x0268, 0x07f4},					
			//{0x1f, 0x0269, 0x444f},					// ps_dri  //0x4344
			//{0x1f, 0x0271, 0x7e14},					// 0x4e13, for PHYD bug, need to workaround
			//{0x1f, 0x0044, 0x00c0},					// pair delay (digital)
     	...
     	//local data
     	{
			...
			//{0x1e,0x0001,0x01cc},					// 0->1, ov1
			//{0x1e,0x0002,0x01c2},					// 0->1, ov2, z2p_ovs
			//{0x1e,0x0003,0x0109},					// 1->0, middle
			//{0x1e,0x0004,0x020b},					// 1->0, ov1
			//{0x1e,0x0005,0x0202},					// 1->0, ov2, p2z_ovs

			//{0x1e,0x0006,0x0387},					// 0->-1, middle
			//{0x1e,0x0007,0x03c5},					// 0->-1, ov1
			//{0x1e,0x0008,0x03c2},					// 0->-1, ov2, z2n_ovs
			//{0x1e,0x0009,0x0309},					// -1->0, middle
			//{0x1e,0x000a,0x000e},					// -1->0, ov1
...
//void Calibration_all(uint8 phyaddr,uint8 resisiter);			 
...
{
    uint8 	pn;
	uint32 	reg0, reg1, reg9, reg10,reg_temp,reg_9_temp,reg_10_temp,reg_temp1,master_times=2,auto_times=3,down_shift_force_slave_time=0;
	uint32 	dev_1e_reg_a2,dev_1e_reg_a2_temp,page_1_reg_1a,page_1_reg_1a_temp,downshift_2pair_limit=32;
    ...
    for(phyaddr = gphy_base; phyaddr < (gphy_base+EN75xxGe_PORTNUM); phyaddr++) 
	{
		...
		if( phyaddr ==9 ) {	
		if(dev_1e_reg_a2_temp !=0) { tcMiiStationWrite(phyaddr, 0x9 ,0x1e00);}
		force_giga_master_counter_p9=0;downshift_2pair_capture_counter_p9_counter=ephy_monitor_all_counter;}
        if( phyaddr ==10 ) { 
		if(dev_1e_reg_a2_temp !=0) { tcMiiStationWrite(phyaddr, 0x9 ,0x1e00);}
		force_giga_master_counter_p10=0;downshift_2pair_capture_counter_p10_counter=ephy_monitor_all_counter;}
        if( phyaddr ==11 ) { 
		if(dev_1e_reg_a2_temp !=0) { tcMiiStationWrite(phyaddr, 0x9 ,0x1e00);}
		force_giga_master_counter_p11=0;downshift_2pair_capture_counter_p11_counter=ephy_monitor_all_counter;}		
        if( phyaddr ==12 ) { 
		if(dev_1e_reg_a2_temp !=0) { tcMiiStationWrite(phyaddr, 0x9 ,0x1e00);}
		force_giga_master_counter_p12=0;downshift_2pair_capture_counter_p12_counter=ephy_monitor_all_counter;}
		...
		{
			if( phy_downshift_force_giga_slave_enable==1 ) { tcMiiStationWrite(phyaddr, 0x0 ,0x1840);tcMiiStationWrite(phyaddr, 0x9 ,0x1600);tcMiiStationWrite(phyaddr, 0x0 ,0x1240);if(link_state_polling == 1){ printf("phy=%d force slave set \n\r",phyaddr);}}
		...
		if((reg_temp != 0)&&(reg_temp1 == 0)) 								// check 1000Mbps capbility
		{
			...
			if((reg_temp1!=0)&&(force_giga_master_mode_p9==0)&&(force_giga_master_counter_p9>=master_times))	// local phy is slave && collision
			{
				...
				force_giga_master_counter_p9=0;
				force_giga_master_mode_p9++;
				if( loopback_head_downshift_100M ==1 ) { downshift_2pair_capture_counter_p9_counter=ephy_monitor_all_counter;}
			...
			if((reg_temp1!=0)&&(force_giga_master_mode_p10==0)&&(force_giga_master_counter_p10>=master_times))	// local phy is slave && collision
			{
				...
				force_giga_master_counter_p10=0;
				force_giga_master_mode_p10++;
				if( loopback_head_downshift_100M ==1 ) { downshift_2pair_capture_counter_p10_counter=ephy_monitor_all_counter;}
			...
			if((reg_temp1!=0)&&(force_giga_master_mode_p11==0)&&(force_giga_master_counter_p11>=master_times))	// local phy is slave && collision
			{
				...
				force_giga_master_counter_p11=0;
				force_giga_master_mode_p11++;
				if( loopback_head_downshift_100M ==1 ) { downshift_2pair_capture_counter_p11_counter=ephy_monitor_all_counter;}
			...
			if((reg_temp1!=0)&&(force_giga_master_mode_p12==0)&&(force_giga_master_counter_p12>=master_times))	// local phy is slave && collision
			{
				...
				force_giga_master_counter_p12=0;
				force_giga_master_mode_p12++;
                if( loopback_head_downshift_100M ==1 ) { downshift_2pair_capture_counter_p12_counter=ephy_monitor_all_counter;}
			}
			}
		...
		//force downshift
		if( loopback_head_downshift_100M ==1 ) { 
		reg_temp1=reg1&0x0004;
		if((phyaddr ==9)&&(force_giga_master_mode_p9==1)&&(reg_temp1==0)) { 
		     reg_temp=ephy_monitor_all_counter-downshift_2pair_capture_counter_p9_counter;
			 if( reg_temp >downshift_2pair_limit ) { tcMiiStationWrite(phyaddr, 0x09, 0x0000);tcMiiStationWrite(phyaddr, 0x00, 0x1240);force_giga_master_mode_p9=2;force_slave_enable_p9 =1;}
		}
		if((phyaddr ==10)&&(force_giga_master_mode_p10==1)&&(reg_temp1==0)) { 
		     reg_temp=ephy_monitor_all_counter-downshift_2pair_capture_counter_p10_counter;
			 if( reg_temp >downshift_2pair_limit ) { tcMiiStationWrite(phyaddr, 0x09, 0x0000);tcMiiStationWrite(phyaddr, 0x00, 0x1240);force_giga_master_mode_p10=2;force_slave_enable_p10 =1;}
		}
		if((phyaddr ==11)&&(force_giga_master_mode_p11==1)&&(reg_temp1==0)) { 
		     reg_temp=ephy_monitor_all_counter-downshift_2pair_capture_counter_p11_counter;
			 if( reg_temp >downshift_2pair_limit ) { tcMiiStationWrite(phyaddr, 0x09, 0x0000);tcMiiStationWrite(phyaddr, 0x00, 0x1240);force_giga_master_mode_p11=2;force_slave_enable_p11 =1;}
		}
		if((phyaddr ==12)&&(force_giga_master_mode_p12==1)&&(reg_temp1==0)) { 
		     reg_temp=ephy_monitor_all_counter-downshift_2pair_capture_counter_p12_counter;
			 if( reg_temp >downshift_2pair_limit ) { tcMiiStationWrite(phyaddr, 0x09, 0x0000);tcMiiStationWrite(phyaddr, 0x00, 0x1240);force_giga_master_mode_p12=2;force_slave_enable_p12 =1;}
		}
		}	
...
static int extPhyGphyTestMode (int argc, char *argv[], void *p);
...
static int recalibration(int argc, char *argv[], void *p);
static int cal_dealy_main(int argc, char *argv[], void *p);
static int doReadAllCalData(int argc, char *argv[], void *p);
#ifdef CONFIG_TP_IMAGE
static int doPhyMiiRead_CL45 (int argc, char *argv[], void *p);
static int doPhyMiiWrite_CL45(int argc, char *argv[], void *p);
#endif
...
static const cmds_t ethertphyprint[] =
{
...
#ifdef CONFIG_TP_IMAGE
    {"miir_cl45",               doPhyMiiRead_CL45,                      0x02, 0, NULL},
    {"miiw_cl45",               doPhyMiiWrite_CL45,                     0x02, 0, NULL},
#endif
	...
	{"cal_delay",  	cal_dealy_main,		0x02, 0, "cal_delay <phyaddr> <p0 phy address> <dealy>"},
...
int tcPhyVerLookUp(uint16 ephy_addr)
{
    ...
    else if(rval == EPHY_ID_7581Ge)  
	{
        tcPhyVer = tcPhyVer_7581Ge;		
        tcPhyPortNum = 4;
        TCPHYDISP1(" 7581 Ge, \n");
        tcPhyFlag = 1;
        tcPhyInitFlag = 1;
...
{ 
	...
	unsigned int TXMR_or_discrete=0,show_temp;
	if(phy == 9 ) {show_temp=transformer_status&0x01;}
  	if(phy == 10 ) {show_temp=transformer_status&0x02;}
    if(phy == 11 ) {show_temp=transformer_status&0x04;}
   	if(phy == 12 ) {show_temp=transformer_status&0x08;}
    if(show_temp != 0 ){ TXMR_or_discrete = 1;} else { TXMR_or_discrete = 0;}

	ICid = GET_PDIDR();    //check IC version
	...
	if(phy == 9) 
	{
			...
			P9_tx_amp_tst_cha_comp=-12;P9_tx_amp_tst_chb_comp=-9;P9_tx_amp_tst_chc_comp=-5;P9_tx_amp_tst_chd_comp=-9;
			// Giga TX amp setting
			P9_tx_amp_gbe_cha_comp=-12;P9_tx_amp_gbe_chb_comp=-9;P9_tx_amp_gbe_chc_comp=-5;P9_tx_amp_gbe_chd_comp=-9;
			// 100M TX amp setting
			P9_tx_amp_hbt_cha_comp=-2;P9_tx_amp_hbt_chb_comp=-2;
			// 10M TX amp setting
			P9_tx_amp_tbt_cha_comp=0;P9_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P9_r50_cha_comp=15;P9_r50_chb_comp=15;P9_r50_chc_comp=-8;P9_r50_chd_comp=-8;
	}
	
	if(phy == 10) 
	{
		    // test mode TX amp setting
			P10_tx_amp_tst_cha_comp=-11;P10_tx_amp_tst_chb_comp=-13;P10_tx_amp_tst_chc_comp=-4;P10_tx_amp_tst_chd_comp=-8;
			// Giga TX amp setting
			P10_tx_amp_gbe_cha_comp=-11;P10_tx_amp_gbe_chb_comp=-13;P10_tx_amp_gbe_chc_comp=-4;P10_tx_amp_gbe_chd_comp=-8;
			// 100M TX amp setting
			P10_tx_amp_hbt_cha_comp=-2;P10_tx_amp_hbt_chb_comp=-2;
			// 10M TX amp setting
			P10_tx_amp_tbt_cha_comp=0;P10_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P10_r50_cha_comp=15;P10_r50_chb_comp=15;P10_r50_chc_comp=-8;P10_r50_chd_comp=-8;
	}
	if(phy == 11) 
	{
		    // test mode TX amp setting
			P11_tx_amp_tst_cha_comp=-9;P11_tx_amp_tst_chb_comp=-9;P11_tx_amp_tst_chc_comp=-3;P11_tx_amp_tst_chd_comp=-7;
			// Giga TX amp setting
			P11_tx_amp_gbe_cha_comp=-9;P11_tx_amp_gbe_chb_comp=-9;P11_tx_amp_gbe_chc_comp=-3;P11_tx_amp_gbe_chd_comp=-7;
			// 100M TX amp setting
			P11_tx_amp_hbt_cha_comp=0;P11_tx_amp_hbt_chb_comp=1;
			// 10M TX amp setting
			P11_tx_amp_tbt_cha_comp=0;P11_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P11_r50_cha_comp=15;P11_r50_chb_comp=18;P11_r50_chc_comp=-8;P11_r50_chd_comp=-8;
	}
	if(phy == 12) 
	{
		    // test mode TX amp setting
			P12_tx_amp_tst_cha_comp=-6;P12_tx_amp_tst_chb_comp=-5;P12_tx_amp_tst_chc_comp=1;P12_tx_amp_tst_chd_comp=-5;
			// Giga TX amp setting
			P12_tx_amp_gbe_cha_comp=-6;P12_tx_amp_gbe_chb_comp=-5;P12_tx_amp_gbe_chc_comp=1;P12_tx_amp_gbe_chd_comp=-5;
			// 100M TX amp setting
			P12_tx_amp_hbt_cha_comp=2;P12_tx_amp_hbt_chb_comp=1;
			// 10M TX amp setting
			P12_tx_amp_tbt_cha_comp=0;P12_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P12_r50_cha_comp=15;P12_r50_chb_comp=15;P12_r50_chc_comp=-8;P12_r50_chd_comp=-8;
	}
	...
	{
		    // test mode TX amp setting
			P9_tx_amp_tst_cha_comp=-10;P9_tx_amp_tst_chb_comp=-10;P9_tx_amp_tst_chc_comp=-5;P9_tx_amp_tst_chd_comp=-8;
			// Giga TX amp setting
			P9_tx_amp_gbe_cha_comp=-12;P9_tx_amp_gbe_chb_comp=-10;P9_tx_amp_gbe_chc_comp=-5;P9_tx_amp_gbe_chd_comp=-8;
			// 100M TX amp setting
			P9_tx_amp_hbt_cha_comp=2;P9_tx_amp_hbt_chb_comp=0;
			// 10M TX amp setting
			P9_tx_amp_tbt_cha_comp=0;P9_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P9_r50_cha_comp=15;P9_r50_chb_comp=15;P9_r50_chc_comp=4;P9_r50_chd_comp=4;
	...
	{
		    // test mode TX amp setting
			P10_tx_amp_tst_cha_comp=-12;P10_tx_amp_tst_chb_comp=-11;P10_tx_amp_tst_chc_comp=-7;P10_tx_amp_tst_chd_comp=-10;
			// Giga TX amp setting
			P10_tx_amp_gbe_cha_comp=-6;P10_tx_amp_gbe_chb_comp=-5;P10_tx_amp_gbe_chc_comp=-3;P10_tx_amp_gbe_chd_comp=-3;
			// 100M TX amp setting
			P10_tx_amp_hbt_cha_comp=1;P10_tx_amp_hbt_chb_comp=0;
			// 10M TX amp setting
			P10_tx_amp_tbt_cha_comp=0;P10_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P10_r50_cha_comp=15;P10_r50_chb_comp=15;P10_r50_chc_comp=4;P10_r50_chd_comp=4;

	}
	if(phy == 11) 
	{
		    // test mode TX amp setting
			P11_tx_amp_tst_cha_comp=-12;P11_tx_amp_tst_chb_comp=-11;P11_tx_amp_tst_chc_comp=-9;P11_tx_amp_tst_chd_comp=-7;
			// Giga TX amp setting
			P11_tx_amp_gbe_cha_comp=-6;P11_tx_amp_gbe_chb_comp=-5;P11_tx_amp_gbe_chc_comp=-4;P11_tx_amp_gbe_chd_comp=-3;
			// 100M TX amp setting
			P11_tx_amp_hbt_cha_comp=0;P11_tx_amp_hbt_chb_comp=0;
			// 10M TX amp setting
			P11_tx_amp_tbt_cha_comp=0;P11_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P11_r50_cha_comp=15;P11_r50_chb_comp=18;P11_r50_chc_comp=4;P11_r50_chd_comp=4;

	}
	if(phy == 12) 
	{
		    // test mode TX amp setting
			P12_tx_amp_tst_cha_comp=-11;P12_tx_amp_tst_chb_comp=-10;P12_tx_amp_tst_chc_comp=-3;P12_tx_amp_tst_chd_comp=-8;
			// Giga TX amp setting
			P12_tx_amp_gbe_cha_comp=4;P12_tx_amp_gbe_chb_comp=1;P12_tx_amp_gbe_chc_comp=11;P12_tx_amp_gbe_chd_comp=4;
			// 100M TX amp setting
			P12_tx_amp_hbt_cha_comp=3;P12_tx_amp_hbt_chb_comp=3;
			// 10M TX amp setting
			P12_tx_amp_tbt_cha_comp=0;P12_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P12_r50_cha_comp=15;P12_r50_chb_comp=15;P12_r50_chc_comp=4;P12_r50_chd_comp=4;
	...
	{
		    // test mode TX amp setting
			P9_tx_amp_tst_cha_comp=-9;P9_tx_amp_tst_chb_comp=-9;P9_tx_amp_tst_chc_comp=-9;P9_tx_amp_tst_chd_comp=-11;
			// Giga TX amp setting
			P9_tx_amp_gbe_cha_comp=-9;P9_tx_amp_gbe_chb_comp=-9;P9_tx_amp_gbe_chc_comp=-9;P9_tx_amp_gbe_chd_comp=-11;
			// 100M TX amp setting
			P9_tx_amp_hbt_cha_comp=4;P9_tx_amp_hbt_chb_comp=4;
			// 10M TX amp setting
			P9_tx_amp_tbt_cha_comp=0;P9_tx_amp_tbt_chb_comp=0;
			// R50 setting
	...
	{
		    // test mode TX amp setting
			P10_tx_amp_tst_cha_comp=-7;P10_tx_amp_tst_chb_comp=-9;P10_tx_amp_tst_chc_comp=-7;P10_tx_amp_tst_chd_comp=-9;
			// Giga TX amp setting
			P10_tx_amp_gbe_cha_comp=-7;P10_tx_amp_gbe_chb_comp=-9;P10_tx_amp_gbe_chc_comp=-7;P10_tx_amp_gbe_chd_comp=-9;
			// 100M TX amp setting
			P10_tx_amp_hbt_cha_comp=3;P10_tx_amp_hbt_chb_comp=4;
			// 10M TX amp setting
			P10_tx_amp_tbt_cha_comp=0;P10_tx_amp_tbt_chb_comp=0;
			// R50 setting
	...
	{
		    // test mode TX amp setting
			P11_tx_amp_tst_cha_comp=-7;P11_tx_amp_tst_chb_comp=-7;P11_tx_amp_tst_chc_comp=-9;P11_tx_amp_tst_chd_comp=-10;
			// Giga TX amp setting
			P11_tx_amp_gbe_cha_comp=-7;P11_tx_amp_gbe_chb_comp=-7;P11_tx_amp_gbe_chc_comp=-9;P11_tx_amp_gbe_chd_comp=-10;
			// 100M TX amp setting
			P11_tx_amp_hbt_cha_comp=4;P11_tx_amp_hbt_chb_comp=5;
			// 10M TX amp setting
			P11_tx_amp_tbt_cha_comp=0;P11_tx_amp_tbt_chb_comp=0;
			// R50 setting
	...
	{
		    // test mode TX amp setting
			P12_tx_amp_tst_cha_comp=-5;P12_tx_amp_tst_chb_comp=-4;P12_tx_amp_tst_chc_comp=-6;P12_tx_amp_tst_chd_comp=-8;
			// Giga TX amp setting
			P12_tx_amp_gbe_cha_comp=-5;P12_tx_amp_gbe_chb_comp=-4;P12_tx_amp_gbe_chc_comp=-6;P12_tx_amp_gbe_chd_comp=-8;
			// 100M TX amp setting
			P12_tx_amp_hbt_cha_comp=4;P12_tx_amp_hbt_chb_comp=4;
			// 10M TX amp setting
			P12_tx_amp_tbt_cha_comp=0;P12_tx_amp_tbt_chb_comp=0;
			// R50 setting
	...
	{
		    // test mode TX amp setting
			P9_tx_amp_tst_cha_comp=-11;P9_tx_amp_tst_chb_comp=-9;P9_tx_amp_tst_chc_comp=-6;P9_tx_amp_tst_chd_comp=-7;
			// Giga TX amp setting
			P9_tx_amp_gbe_cha_comp=-11;P9_tx_amp_gbe_chb_comp=-9;P9_tx_amp_gbe_chc_comp=-6;P9_tx_amp_gbe_chd_comp=-7;
			// 100M TX amp setting
			P9_tx_amp_hbt_cha_comp=0;P9_tx_amp_hbt_chb_comp=5;
			// 10M TX amp setting
			P9_tx_amp_tbt_cha_comp=0;P9_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P9_r50_cha_comp=12;P9_r50_chb_comp=7;P9_r50_chc_comp=2;P9_r50_chd_comp=0;
	}
	
	if(phy == 10) 
	{
		    // test mode TX amp setting
			P10_tx_amp_tst_cha_comp=-11;P10_tx_amp_tst_chb_comp=-10;P10_tx_amp_tst_chc_comp=-3;P10_tx_amp_tst_chd_comp=-8;
			// Giga TX amp setting
			P10_tx_amp_gbe_cha_comp=-11;P10_tx_amp_gbe_chb_comp=-10;P10_tx_amp_gbe_chc_comp=-3;P10_tx_amp_gbe_chd_comp=-8;
			// 100M TX amp setting
			P10_tx_amp_hbt_cha_comp=3;P10_tx_amp_hbt_chb_comp=6;
			// 10M TX amp setting
			P10_tx_amp_tbt_cha_comp=0;P10_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P10_r50_cha_comp=12;P10_r50_chb_comp=7;P10_r50_chc_comp=2;P10_r50_chd_comp=0;
	}
	if(phy == 11) 
	{
		    // test mode TX amp setting
			P11_tx_amp_tst_cha_comp=-12;P11_tx_amp_tst_chb_comp=-9;P11_tx_amp_tst_chc_comp=-7;P11_tx_amp_tst_chd_comp=-10;
			// Giga TX amp setting
			P11_tx_amp_gbe_cha_comp=-12;P11_tx_amp_gbe_chb_comp=-9;P11_tx_amp_gbe_chc_comp=-7;P11_tx_amp_gbe_chd_comp=-10;
			// 100M TX amp setting
			P11_tx_amp_hbt_cha_comp=3;P11_tx_amp_hbt_chb_comp=6;
			// 10M TX amp setting
			P11_tx_amp_tbt_cha_comp=0;P11_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P11_r50_cha_comp=12;P11_r50_chb_comp=7;P11_r50_chc_comp=0;P11_r50_chd_comp=2;
	}
	if(phy == 12) 
	{
		    // test mode TX amp setting
			P12_tx_amp_tst_cha_comp=-7;P12_tx_amp_tst_chb_comp=-8;P12_tx_amp_tst_chc_comp=-4;P12_tx_amp_tst_chd_comp=-7;
			// Giga TX amp setting
			P12_tx_amp_gbe_cha_comp=-7;P12_tx_amp_gbe_chb_comp=-8;P12_tx_amp_gbe_chc_comp=-4;P12_tx_amp_gbe_chd_comp=-7;
			// 100M TX amp setting
			P12_tx_amp_hbt_cha_comp=6;P12_tx_amp_hbt_chb_comp=5; 
			// 10M TX amp setting
			P12_tx_amp_tbt_cha_comp=0;P12_tx_amp_tbt_chb_comp=0;
			// R50 setting
			P12_r50_cha_comp=12;P12_r50_chb_comp=7;P12_r50_chc_comp=0;P12_r50_chd_comp=2;
	}
	...
	if(phy == 10) 
	{
			...
			printf("port10B hbt= %d " , P10_tx_amp_hbt_chb_comp);
	...
	if(phy == 12) 
	{
			...
			printf("port12B hbt= %d " , P12_tx_amp_hbt_chb_comp);
			...
			//write back reg
				...
				//mtEMiiRegWrite(phy, 0x1e, 0x005c, 0x5555);
				mtEMiiRegWrite(phy, 0x1e, 0x0176, 0x4400);
				mtEMiiRegWrite(phy, 0x1e, 0x0177, 0x0044);
...
int auto_select_transformer(unsigned int phy)
{
	...
	printf(" PHY=%d  1 f 10 value=0x%x ", phy , auto_select_transformer_temp);
...
if(phy == 12) { mtEMiiRegWrite(phy, 0x1f, 0x15, 0x7161);}
	...
	for(auto_select_transformer_loop = 0; auto_select_transformer_loop<= 3; auto_select_transformer_loop++)
	{
		...
		printf(" PHY=%d  reg=0x1a value=0x%x ", phy , auto_select_transformer_temp);
	...
	{ 
		if(phy == 9 ) {transformer_status=transformer_status&0xfffe;}
		if(phy == 10 ) {transformer_status=transformer_status&0xfffd;}
		if(phy == 11 ) {transformer_status=transformer_status&0xfffb;}
		if(phy == 12 ) {transformer_status=transformer_status&0xfff7;}
       return 0;
	} 
	else 
	{
	...
	printf("monitor value xxxxxxx \r\n");
		for(slt_not_excute_loop = all_port_start; slt_not_excute_loop <= all_port_end; slt_not_excute_loop++)
	        {			
                    ...
                    mtPhyMiiWrite_TrDbg(slt_not_excute_loop, "PMA", 0x17, 0x2f0671, 0);
...
#ifdef CONFIG_TP_IMAGE
/* CHENMING 20240306 for RTL8261BE phy link status detect for userspace apps */
typedef enum {
	PHY_REG_VAL_LINK_STATUS_BIT = 2,
	PHY_REG_VAL_LINK_DUPLEX_BIT = 3,
	PHY_REG_VAL_SPEED_LSB_1_BIT = 4,
	PHY_REG_VAL_SPEED_LSB_2_BIT = 5,
	PHY_REG_VAL_SPEED_MSB_1_BIT = 9,
	PHY_REG_VAL_SPEED_MSB_2_BIT = 10,
	PHY_REG_VAL_BIT_END
} PHY_REG_VAL_DEF;
#define RTL8261_SPEED_100M		(0x1)
#define RTL8261_SPEED_1000M		(0x2)
#define RTL8261_SPEED_2500M		(0x5)
#define RTL8261_SPEED_5G		(0x6)
#define RTL8261_SPEED_10G		(0x4)
#define REG_VAL_BIT_CHK(val, bit) (((val) & (1 << (bit))) != 0)

#define PHY_NUM				(2)
#define PHY_ADDR_WAN		(0)
#define PHY_ADDR_LAN		(1)

#define RTL8261_MMD_AUTONEG							(0x7)
#define RTL8261_MMD_VENDOR2							(0x1F)

#define RTL8261_AUTONEG_STANDARD_CONTROL1 			(0x0)
#define RTL8261_AUTONEG_ADVERTISEMENT				(0x10)
#define RTL8261_AUTONEG_MULTI_GBASE_TBASE_CONTROL1	(0x20)
#define RTL8261_REG_AUTONEG_ADVERTISEMENT2			(0xA412)
#define RTL8261_CTRL_RESTART_AUTONEGOTIATION		(0x200)
#define RTL8261_CTRL_AUTONEGOTIATION_ENABLE			(0x1000)

#define RTL8261_ADV_10TEX_HD		(0x20)
#define RTL8261_ADV_10TEX_FD		(0x40)
#define RTL8261_ADV_100TX_HD		(0x80)
#define RTL8261_ADV_100TX_FD		(0x100)
#define RTL8261_ADV_1000T			(0x200)
#define RTL8261_ADV_2500T			(0x80)
#define RTL8261_ADV_5000T			(0x100)
#define RTL8261_ADV_10000T			(0x1000)

#define RTL8261_CTRL_AUTORESTART_MAGIC_NUMBER	(0xFFFF)
...
#define FAL_PHY_ADV_1000T_FD		(0x10)
#define FAL_PHY_ADV_2500T_FD		(0x20)
#define FAL_PHY_ADV_5000T_FD		(0x40)
#define FAL_PHY_ADV_10000T_FD		(0x80)

#define CHK_BUF() {pos = begin + index; if (pos < off) { index = 0; begin = pos; }; if (pos > off + count) goto done;}


//phy mode eee enable/disable reg
#define RTL8261_MMD_EEE			(0x1f)
#define RTL8261_REG_EEE_ENABLE		(0xA432)
#define RTL8261_CTL_EEE_ENABLE_FD	(0x20)
#define RTL8261_CTL_EEE_ENABLE		(0x30)
#define RTL8261_CTL_EEE_DISABLE		(0x10)
//phy eee advertisement used for cpa
#define RTL8261_EEE_ADV_REG		(0x3c)
#define RTL8261_EEE_ADV2_REG		(0x3e)
#define RTL8261_EEE_1000BASE_T		(0x2)
#define RTL8261_EEE_100BASE_TX		(0x4)
#define RTL8261_EEE_10GBASE_T		(0x8)
#define RTL8261_EEE_5GBASE_T		(0x2)
#define RTL8261_EEE_2500BASE_T		(0x1)



static u16 phyAddr[PHY_NUM] = {PHY_ADDR_WAN, PHY_ADDR_LAN};

static int ephy_read_status(char *page, char **start, off_t off,
	int count, int *eof, void *data, uint32 phyaddr)

{
	int len = 0;
	int val = 0;
	unsigned int rtl8261_speed = 0;

	const char *speed = "-1";
	const char *duplex = "Half";
	const char *status = "Down";

	val = tcMiiStationRead_CL45(phyaddr, 0x1F, 0xa434);

	if (REG_VAL_BIT_CHK(val, PHY_REG_VAL_LINK_STATUS_BIT))
	{
		status = "Up";
	}

	if (REG_VAL_BIT_CHK(val, PHY_REG_VAL_LINK_DUPLEX_BIT))
	{
		duplex = "Full";
	}

	rtl8261_speed= ((((val	>> 9) & 0x3) << 2) | ((val	>> 4) & 0x3));
	switch (rtl8261_speed)
	{
		case RTL8261_SPEED_100M:
			speed = "100M";
			break;

		case RTL8261_SPEED_1000M:
			speed = "1000M";
			break;

		case RTL8261_SPEED_2500M:
			speed = "2500M";
			break;

		case RTL8261_SPEED_5G:
			speed = "5000M";
			break;

		case RTL8261_SPEED_10G:
			speed = "10G";
			break;
	...
	len = sprintf(page, "Status Link %s %s %s\n", status, speed, duplex);
...
static int ephy_proc_read_status(char *page, char **start, off_t off,
	int count, int *eof, void *data)
{
	return ephy_read_status(page, start, off, count, eof, data, 0);
}

static int ephy_1_proc_read_status(char *page, char **start, off_t off,
	int count, int *eof, void *data)
{
	return ephy_read_status(page, start, off, count, eof, data, 1);
}

static int autoneg_read_proc(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
	u32 phy_data = 0;
	int index = 0;
	off_t begin = 0;
	...
	for (i = 0; i < PHY_NUM; i++)
	{
		if (phyAddr[i] == PHY_ADDR_WAN)
		{
			index += sprintf(buf+index, "10G WAN Advertised:\n");
			CHK_BUF();
		}
		else 
		{
			index += sprintf(buf+index, "10G LAN Advertised:\n");
			CHK_BUF();
		}

		phy_data = tcMiiStationRead_CL45(phyAddr[i], RTL8261_MMD_AUTONEG, RTL8261_AUTONEG_ADVERTISEMENT);
		if (phy_data & RTL8261_ADV_10TEX_HD)
		{
			index += sprintf(buf+index, "\t10baseT/Half\n");
			CHK_BUF();
		}
		if (phy_data & RTL8261_ADV_10TEX_FD)
		{
			index += sprintf(buf+index, "\t10baseT/Full\n");
			CHK_BUF();
		}
		if (phy_data & RTL8261_ADV_100TX_HD)
		{
			index += sprintf(buf+index, "\t100baseT/Half\n");
			CHK_BUF();
		}
		if (phy_data & RTL8261_ADV_100TX_FD)
		{
			index += sprintf(buf+index, "\t100baseT/Full\n");
			CHK_BUF();
		}

		phy_data = tcMiiStationRead_CL45(phyAddr[i], RTL8261_MMD_VENDOR2, RTL8261_REG_AUTONEG_ADVERTISEMENT2);
		if (phy_data & RTL8261_ADV_1000T)
		{
			index += sprintf(buf+index, "\t1000baseT/Full\n");
			CHK_BUF();
		}	

		phy_data = tcMiiStationRead_CL45(phyAddr[i], RTL8261_MMD_AUTONEG, RTL8261_AUTONEG_MULTI_GBASE_TBASE_CONTROL1);
		if (phy_data & RTL8261_ADV_2500T)
		{
			index += sprintf(buf+index, "\t2500baseT/Full\n");
			CHK_BUF();
		}
		if (phy_data & RTL8261_ADV_5000T)
		{
			index += sprintf(buf+index, "\t5000baseT/Full\n");
			CHK_BUF();
		}
		if (phy_data & RTL8261_ADV_10000T)
		{
			index += sprintf(buf+index, "\t10000baseT/Full\n");
			CHK_BUF();
		}
...
static int autoneg_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{
	...
	sscanf(val_string, "%d %x", &phy_addr, &autoneg);
	if (PHY_ADDR_WAN != phy_addr && PHY_ADDR_LAN != phy_addr)
	{
		return -EINVAL ;
	}

	if(RTL8261_CTRL_AUTORESTART_MAGIC_NUMBER != autoneg)
	{
		phy_data = tcMiiStationRead_CL45(phy_addr, RTL8261_MMD_AUTONEG, RTL8261_AUTONEG_ADVERTISEMENT);
		phy_data &= ~(RTL8261_ADV_100TX_HD | RTL8261_ADV_100TX_FD);
		if (autoneg & FAL_PHY_ADV_100TX_HD)
		{
			phy_data |= RTL8261_ADV_100TX_HD;
		}
		if (autoneg & FAL_PHY_ADV_100TX_FD)
		{
			phy_data |= RTL8261_ADV_100TX_FD;
		}
		tcMiiStationWrite_CL45(phy_addr, RTL8261_MMD_AUTONEG, RTL8261_AUTONEG_ADVERTISEMENT, phy_data);

		phy_data = tcMiiStationRead_CL45(phy_addr, RTL8261_MMD_VENDOR2, RTL8261_REG_AUTONEG_ADVERTISEMENT2);
		phy_data &= ~(RTL8261_ADV_1000T);
		if (autoneg & FAL_PHY_ADV_1000T_FD)
		{
			phy_data |= RTL8261_ADV_1000T;
		}
		tcMiiStationWrite_CL45(phy_addr, RTL8261_MMD_VENDOR2, RTL8261_REG_AUTONEG_ADVERTISEMENT2, phy_data);

		phy_data = tcMiiStationRead_CL45(phy_addr, RTL8261_MMD_AUTONEG, RTL8261_AUTONEG_MULTI_GBASE_TBASE_CONTROL1);
		phy_data &= ~(RTL8261_ADV_2500T | RTL8261_ADV_5000T | RTL8261_ADV_10000T);
		if (autoneg & FAL_PHY_ADV_2500T_FD)
		{
			phy_data |= RTL8261_ADV_2500T;
		}
		if (autoneg & FAL_PHY_ADV_5000T_FD)
		{
			phy_data |= RTL8261_ADV_5000T;
		}
		if (autoneg & FAL_PHY_ADV_10000T_FD)
		{
			phy_data |= RTL8261_ADV_10000T;
		}

		tcMiiStationWrite_CL45(phy_addr, RTL8261_MMD_AUTONEG, RTL8261_AUTONEG_MULTI_GBASE_TBASE_CONTROL1, phy_data);
	}
	/* restart Auto neg */
	restartNeg = tcMiiStationRead_CL45(phy_addr, RTL8261_MMD_AUTONEG, RTL8261_AUTONEG_STANDARD_CONTROL1);
	restartNeg |= RTL8261_CTRL_AUTONEGOTIATION_ENABLE | RTL8261_CTRL_RESTART_AUTONEGOTIATION;
	tcMiiStationWrite_CL45(phy_addr, RTL8261_MMD_AUTONEG, RTL8261_AUTONEG_STANDARD_CONTROL1, restartNeg);
	
    return count;
}


static int eee_read_status(char *page, char **start, off_t off,
	int count, int *eof, void *data)

{
	...
	for (i = 0; i < PHY_NUM; i++)
	{
		if (phyAddr[i] == PHY_ADDR_WAN)
		{
			index += sprintf(page+index, "10G WAN EEE Advertised:\n");
			CHK_BUF();
		}
		else 
		{
			index += sprintf(page+index, "10G LAN EEE Advertised:\n");
			CHK_BUF();
		}
	
		phy_data = tcMiiStationRead_CL45(phyAddr[i], RTL8261_MMD_EEE, RTL8261_REG_EEE_ENABLE);
		if (phy_data & RTL8261_CTL_EEE_ENABLE_FD)
		{
			index += sprintf(page+index, "\tEEE STATUS:ON\n");
			CHK_BUF();
		}
		else
		{
			index += sprintf(page+index, "\tEEE STATUS:OFF\n");
			CHK_BUF();			
		}
		
		phy_data = tcMiiStationRead_CL45(phyAddr[i], RTL8261_MMD_AUTONEG, RTL8261_EEE_ADV_REG);
		
		if (phy_data & RTL8261_EEE_10GBASE_T)
		{
			index += sprintf(page+index, "\t10GbaseT EEE:ON\n");
			CHK_BUF();
		}
		
		if (phy_data & RTL8261_EEE_1000BASE_T)
		{
			index += sprintf(page+index, "\t1000baseT EEE:ON\n");
			CHK_BUF();
		}
		
		if (phy_data & RTL8261_EEE_100BASE_TX)
		{
			index += sprintf(page+index, "\t100baseTX EEE:ON\n");
			CHK_BUF();
		}

		phy_data = tcMiiStationRead_CL45(phyAddr[i], RTL8261_MMD_AUTONEG, RTL8261_EEE_ADV2_REG);
		if (phy_data & RTL8261_EEE_5GBASE_T)
		{
			index += sprintf(page+index, "\t5GbaseT EEE:ON\n");
			CHK_BUF();
		}	
		
		if (phy_data & RTL8261_EEE_2500BASE_T)
		{
			index += sprintf(page+index, "\t2500baseT EEE:ON\n");
			CHK_BUF();
		}	
	}
	*eof = 1;
...
static int eee_write_status(struct file *file, const char *buffer, unsigned long count, void *data)
{
	char val_string[128] = {0};
	u32 phy_data = 0;
	u32 eeeOn = 0;
	...
	if (2 != sscanf(val_string, "%d %x", &phy_addr, &eeeOn))
	{
		return -EINVAL;
	}
	...
	if (1 == eeeOn)
	{
		phy_data |= RTL8261_CTL_EEE_ENABLE;
		tcMiiStationWrite_CL45(phy_addr, RTL8261_MMD_EEE, RTL8261_REG_EEE_ENABLE, phy_data);
	}
	else
	{
		phy_data |= RTL8261_CTL_EEE_DISABLE;
		tcMiiStationWrite_CL45(phy_addr, RTL8261_MMD_EEE, RTL8261_REG_EEE_ENABLE, phy_data);
	}
...
static int eee_proc_read_status(char *page, char **start, off_t off,
	int count, int *eof, void *data)
{
	return eee_read_status(page, start, off, count, eof, data);
}

static int eee_proc_write_status(struct file *file, const char *buffer, unsigned long count, void *data)
{
	return eee_write_status(file, buffer, count, data);
}
...
#ifdef CONFIG_TP_IMAGE
	struct proc_dir_entry *autoneg_proc = NULL;
	struct proc_dir_entry *eee_proc = NULL;
#endif /* CONFIG_TP_IMAGE */
		...
		for(ephy_addr = EPHY_ADDR_P0; ephy_addr <= 12; ephy_addr++)
		{
    		...
    		if(tcPhyInitFlag == 1)  // is ECNT's ephy
    		{
    			...
    			if(port_num >= 0)
    			{
					...
					auto_select_transformer(ephy_addr);
					phyTxAmpcomp(1,ephy_addr);
...
#ifdef CONFIG_TP_IMAGE
	/* CHENMING 20240306 for RTL8261BE phy link status detect for userspace apps */
	create_proc_read_entry("tc3162/ephy_status", 0, NULL, ephy_proc_read_status, NULL);
	create_proc_read_entry("tc3162/ephy_1_status", 0, NULL, ephy_1_proc_read_status, NULL);

	autoneg_proc = create_proc_entry("tc3162/autoneg_advertise", 0, NULL);
	if(autoneg_proc) 
	{
		autoneg_proc->read_proc = autoneg_read_proc;
		autoneg_proc->write_proc = autoneg_write_proc;  
	}

	eee_proc = create_proc_entry("tc3162/eee_status", 0, NULL);
	if(eee_proc) 
	{
		eee_proc->read_proc = eee_proc_read_status;
		eee_proc->write_proc = eee_proc_write_status;  
	}
...
#ifdef CONFIG_TP_IMAGE
	/* CHENMING 20240306 for RTL8261BE phy link status detect for userspace apps */
	remove_proc_entry("tc3162/ephy_status", NULL);
	remove_proc_entry("tc3162/ephy_1_status", NULL);
	remove_proc_entry("tc3162/autoneg_advertise", NULL);
	remove_proc_entry("tc3162/eee_status", NULL);
#endif /* CONFIG_TP_IMAGE */
...
int ePhyInit(uint16 ephy_addr)
{
	...
	switch(tcPhyVer) 
	{			    
		case tcPhyVer_7581Ge: 		// EN7581 Ge
			if(ephy_addr_base == 0)
				ephy_addr_base += EPHY_ADDR_P0;
			break;
	}
    ...
    switch(tcPhyVer) 
	{
		...
		case tcPhyVer_7581Ge: 		// EN7523 Ge
...
void normalinit(void)
{
	uint16 ephy_addr,show_temp;
	u32 ICid = 1;
	unsigned int mt7530_temp;
	...
	for(ephy_addr = all_port_start; ephy_addr <= all_port_end; ephy_addr++)
		{
					...
					//RX setting for 0R_TXMR before AN setting
					...
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x202, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x203, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x204, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x205, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x206, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x207, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x208, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x209, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x20a, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x20b, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x20e, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x20f, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x210, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x211, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x212, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x213, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x214, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x215, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x216, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x217, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x21a, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x21b, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x21c, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x21d, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x21e, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x21f, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x220, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x221, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x222, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x223, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x226, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x227, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x228, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x229, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x22a, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x22b, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x22c, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x22d, 0x0025);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x22e, 0x2310);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x22f, 0x0025);
					//led
					...
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x27, 0x003f);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x417, 0x7775);
						//check value and return value 0-->disceret  1-->TXMR
				    if(ephy_addr == 9 ) {show_temp=transformer_status&0x01;}
                    if(ephy_addr == 10 ) {show_temp=transformer_status&0x02;}
                    if(ephy_addr == 11 ) {show_temp=transformer_status&0x04;}
                    if(ephy_addr == 12 ) {show_temp=transformer_status&0x08;}
                  if(show_temp != 0 ) 
				  { 
					//100M sharp
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x00, 0x0187);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x01, 0x01cc);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x02, 0x01c2);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x03, 0x0109);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x04, 0x020b);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x05, 0x0202);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x06, 0x0387);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x07, 0x03c5);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x08, 0x03c2);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x09, 0x0309);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x0a, 0x000e);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x0b, 0x0002);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x23, 0x0880);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x24, 0x0880);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x25, 0x0880);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x26, 0x0880);
				  } 
				  else 
				  { 
					//100M sharp
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x00, 0x0187);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x01, 0x01c3);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x02, 0x01c2);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x03, 0x0109);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x04, 0x020b);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x05, 0x0202);
					...
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x23, 0x0000);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x24, 0x0000);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x25, 0x0000);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x26, 0x0000);
					}
				  }
					...
					//rx
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x41, 0x3333);
			        mtEMiiRegWrite(ephy_addr, 0x1f, 0x44, 0x00c0);
					//10M
					...
					//tcMiiStationWrite(ephy_addr, 0x04, 0x0de1);
				    //tcMiiStationWrite(ephy_addr, 0x00, 0x1240);	
					//toKenRingWrite (ephy_addr,1,0xf,0x12,0x5e4d2a);
					//printf("ch=0x1 node=0xf data_addr=0x12 value=0x%x \r\n", toKenRingRead(ephy_addr,1,0xf,0x12));
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x269, 0x2114);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x26a, 0x1113);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x26f, 0x0000);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x271, 0x2c63);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x272, 0x0c2b);				
					...
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x271, 0x2c63);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x272, 0x0c2b);	
					//mtEMiiRegWrite(ephy_addr, 0x1f, 0x269, 0x444f);
					//mtEMiiRegWrite(ephy_addr, 0x1f, 0x26a, 0x3333);
					//mtEMiiRegWrite(ephy_addr, 0x1f, 0x26f, 0x7777);
					//mtEMiiRegWrite(ephy_addr, 0x1f, 0x271, 0x7e14);
					//mtEMiiRegWrite(ephy_addr, 0x1f, 0x272, 0x7cff);
					...
					mtPhyMiiWrite_TrDbg(ephy_addr, "DEC", 0x15, 0x0055a0, 0);
					mtPhyMiiWrite_TrDbg(ephy_addr, "DEC", 0x17, 0x07ff3f, 0);
					mtPhyMiiWrite_TrDbg(ephy_addr, "PMA", 0x0, 0x00001e, 0);
					mtPhyMiiWrite_TrDbg(ephy_addr, "PMA", 0x1, 0x6fb90a, 0);
					mtPhyMiiWrite_TrDbg(ephy_addr, "PMA", 0x18, 0x0e2f00, 0);
					mtPhyMiiWrite_TrDbg(ephy_addr, "TR", 0x26, 0x444444, 0);
					mtPhyMiiWrite_TrDbg(ephy_addr, "DSPF", 0x3, 0x000004, 0);
					mtPhyMiiWrite_TrDbg(ephy_addr, "DSPF", 0x6, 0x2ebaef, 0);
					mtPhyMiiWrite_TrDbg(ephy_addr, "DSPF", 0x8, 0x00000b, 0);
					mtPhyMiiWrite_TrDbg(ephy_addr, "DSPF", 0xc, 0x00504d, 0);
					mtPhyMiiWrite_TrDbg(ephy_addr, "DSPF", 0xd, 0x02314f, 0);
					mtPhyMiiWrite_TrDbg(ephy_addr, "DSPF", 0xf, 0x003028, 0);
					mtPhyMiiWrite_TrDbg(ephy_addr, "DSPF", 0x10, 0x00000a, 0);
					mtPhyMiiWrite_TrDbg(ephy_addr, "DSPF", 0x11, 0x040001, 0);
					mtPhyMiiWrite_TrDbg(ephy_addr, "DSPF", 0x14, 0x00024a, 0);
					mtPhyMiiWrite_TrDbg(ephy_addr, "DSPF", 0x1c, 0x003210, 0);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x120, 0x8014);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x122, 0xffff);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x144, 0x0200);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x14a, 0xee20);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x19b, 0x0111);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x234, 0x1181);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x238, 0x0120);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x239, 0x0117);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x2d1, 0x0733);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x323, 0x0011);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x324, 0x013f);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x326, 0x0037);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x268, 0x07f4);
					#endif
					
    	}
//MT7530 setting
mt7530_temp=tcMiiStationRead(0, 0x03);
if(mt7530_temp==0xa412)
{
printf(" include MT7530 IC \r\n");
for(ephy_addr = 0; ephy_addr <= 4; ephy_addr++)
	{
			//E2 setting	
					//mtEMiiRegWrite(ephy_addr, 0x1f, 0x103, 0x0020);// invert clock of DFF for phase-mux data input //CML_20130223
					//mtEMiiRegWrite(ephy_addr, 0x1f, 0x104, 0x0608);// disable PLL //CML_20130321
					//mtEMiiRegWrite(ephy_addr, 0x1f, 0x104, 0x2608);// enable PLL //CML_20130321
					//mtEMiiRegWrite(ephy_addr, 0x1f, 0x416, 0x0047);// mcc_acc_ref_th setting //CML_20130327
					//mtEMiiRegWrite(ephy_addr, 0x1f, 0x27c, 0x0c0c);// for 10BASE-T amplitude //CML_20130327
					//mtEMiiRegWrite(ephy_addr, 0x1f, 0x27b, 0x1177);//disable 10BASE-TE
					//mtPhyMiiWrite_TrDbg(phyaddr, "DSPF", 0x010, 0x005000, 0);	
					//mtPhyMiiWrite_TrDbg(phyaddr, "DSPF", 0x00e, 0x800008, 0);
            //E2 end	
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x108, 0x5010);// 5R match
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x417, 0x7775);// allen_20150206
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x27b, 0x1157);// allen_20160803 for ASKEY request
            /* BEGIN: Modified by l18792, 2021-02-02  Des:   PN:HSV7D202102010527   HZTEAM */
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x269, 0x444f);	
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x271, 0x7e14);
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x272, 0x7cff);
            /* END: Modified by l18792, 2021-02-02  Des:   PN:HSV7D202102010527   HZTEAM */
					mtEMiiRegWrite(ephy_addr, 0x1f, 0x044, 0x0000);	//7580 default 6R , maybe 7530 is 5R //{0x1f, 0x0044, 0x0040}	
					
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x045, 0x0706);//5R R offset //0R {0x1e,0x0045,0x0404}
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x046, 0x0707);//5R R offset // 0R {0x1e,0x0046,0x0404}
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x041, 0x3333);// for RX voltage
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x016, 0x0012);// tx i2mpb_tst // allen_20170331 for FH OEM/ODM // allen_20170405  //0R {0x1e,0x0016,0x0018}, // allen_20170405
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x008, 0x02f0);// z2n_ovs_post   // allen_20170405
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x012, 0x71d0);// for 100 baseTX , copy from 7512.c 5R setting
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x013, 0x0000);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x014, 0x0000);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x03e, 0xf000);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x03c, 0xff00);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x03d, 0xfc00);
					mtEMiiRegWrite(ephy_addr, 0x07, 0x03c, 0x0000);
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x05c, 0x7777);//copy from 7512.c 5R setting
					mtEMiiRegWrite(ephy_addr, 0x1e, 0x0a6, 0x0350);

					mtPhyMiiWrite_TrDbg(ephy_addr, "PMA", 0x17, 0x060671, 0);	
					mtPhyMiiWrite_TrDbg(ephy_addr, "PMA", 0x18, 0x0e2e00, 0);	
					mtPhyMiiWrite_TrDbg(ephy_addr, "PMA", 0x00, 0x00002b, 0);
	...
	for(ephy_addr = all_port_start; ephy_addr <= all_port_end; ephy_addr++)
		{
			...
			//mtEMiiRegWrite(ephy_addr, 0x1e, 0x13, 0x0000);
			//mtEMiiRegWrite(ephy_addr, 0x1e, 0x14, 0x0000);

			/*
			mtEMiiRegWrite(ephy_addr, 0x1e, 0x41, 0x3333);
			mtEMiiRegWrite(ephy_addr, 0x1e, 0x201, 0x4000);
			mtEMiiRegWrite(ephy_addr, 0x1e, 0x198, 0x0001);
			mtEMiiRegWrite(ephy_addr, 0x1e, 0x23c, 0x0a20);
			mtEMiiRegWrite(ephy_addr, 0x1e, 0x234, 0x0180);
			mtEMiiRegWrite(ephy_addr, 0x1e, 0x120, 0x9014);
			mtEMiiRegWrite(ephy_addr, 0x1e, 0x236, 0x0020);
			mtEMiiRegWrite(ephy_addr, 0x1e, 0x0a6, 0x0350);
			*/
			//mtEMiiRegWrite(ephy_addr, 0x1e, 0x3c, 0xc000);
			//mtEMiiRegWrite(ephy_addr, 0x1e, 0x3d, 0x0000);
					//mtEMiiRegWrite(ephy_addr, 0x1f, 0x269, 0x2114);
					//mtEMiiRegWrite(ephy_addr, 0x1f, 0x26a, 0x1133);
					//mtEMiiRegWrite(ephy_addr, 0x1f, 0x271, 0x2ca5);
					//mtEMiiRegWrite(ephy_addr, 0x1f, 0x272, 0x0c6b);
					//mtEMiiRegWrite(ephy_addr, 0x1f, 0x26f, 0x3333);
					//mtEMiiRegWrite(ephy_addr, 0x1e, 0xe7, 0x4444);
					//mtEMiiRegWrite(ephy_addr, 0x1e, 0xe9, 0x0002);
...
static int doPhySwVer(int argc, char *argv[], void *p)
{
	...
	{    
		case tcPhyVer_7581Ge: 
			printf("%s, \r\n", en75xxGe_cfg[0].name);
		 	printf("tcPhyVer_7581Ge, tcPhyVer=0x%02lX \r\n", tcPhyVer);
			break;	
...
if(transformer_ver=transformer_status != 0) { printf("port 9_%dR_TXMR \r\n",mdi_resister);} else { printf("port 9_%dR_discrete_TXMR \r\n",mdi_resister);}

transformer_ver=transformer_status&0x02;
if(transformer_ver=transformer_status != 0) { printf("port 10_%dR_TXMR \r\n",mdi_resister);} else { printf("port 10_%dR_discrete_TXMR \r\n",mdi_resister);}

transformer_ver=transformer_status&0x04;
if(transformer_ver=transformer_status != 0) { printf("port 11_%dR_TXMR \r\n",mdi_resister);} else { printf("port 11_%dR_discrete_TXMR \r\n",mdi_resister);}

transformer_ver=transformer_status&0x08;
if(transformer_ver=transformer_status != 0) { printf("port 12_%dR_TXMR \r\n",mdi_resister);} else { printf("port 12_%dR_discrete_TXMR \r\n",mdi_resister);}
...
int doPhyGphyTestMode (int argc, char *argv[], void *p)
{
	...
	if((argc <4)||(argc >=6)) 
	{
	   	printf("tce testmode <speed> <phyaddr> <mode or channel>\r\n");
	}
	else	
	{
    	...
    	if(argc ==5) {pair=checked_atoi(argv[4]);}
		  if(speed == 1000 )	{
            ...
            if(ICid==2)
			{
				mtEMiiRegWrite(phyaddr, 0x1f, 0x269, 0x2114);
				mtEMiiRegWrite(phyaddr, 0x1f, 0x26a, 0x1113);
				mtEMiiRegWrite(phyaddr, 0x1f, 0x26f, 0x0000);
				mtEMiiRegWrite(phyaddr, 0x1f, 0x271, 0x2c63);
				mtEMiiRegWrite(phyaddr, 0x1f, 0x272, 0x0c2b);
			}
		  }
		if(speed == 100 )	{
            ...
            if(ICid==2)
			{
				mtEMiiRegWrite(phyaddr, 0x1f, 0x269, 0x444f);
				mtEMiiRegWrite(phyaddr, 0x1f, 0x26a, 0x3333);
				mtEMiiRegWrite(phyaddr, 0x1f, 0x26f, 0x7777);
				mtEMiiRegWrite(phyaddr, 0x1f, 0x271, 0x7e14);
				mtEMiiRegWrite(phyaddr, 0x1f, 0x272, 0x7cff);
			}
		 }
		if(speed == 10 )	{ 
			//send pair A NLP
	...
	else	
	{
    	...
    	if(argc ==5) {pair=checked_atoi(argv[4]);}
			if(speed == 2500 )	
		  	{
				tcMiiStationWrite(phyaddr, 0x1f, 0x0000);
				tcMiiStationWrite(phyaddr, 0x0, 0x1140);
				buck_pbus_write(phyaddr,0x1e0228,0xf);
		  ...
		  if(speed == 1000 )	
			...
			if( temp1 == 0xa411 ) {buck_pbus_write(phyaddr,0x1e0228,0x0);}
			if(mode == 0 )
			{	//default
				tcMiiStationWrite(phyaddr, 9, 0x600);
				tcMiiStationWrite(phyaddr, 0x0, 0x1040);
...
int sw_ability_setting(int argc, char *argv[], void *p)
{
		...
		if(sw_ability==3) 
	{
		...
		if(sw_ability==4) 
	{	
	if( switch_status ==1 ) { loopback_head_downshift_100M=1;printf("loopback_head_downshift on\r\n");} else { loopback_head_downshift_100M=0;printf("loopback_head_downshift off\r\n");}
	}
		if(sw_ability==5) 
	{	
	if( switch_status ==1 ) { link_state_polling=1;printf("link status print on\r\n");} else { link_state_polling=0;printf("link status print off\r\n");}
	}
		if(sw_ability==6) 
	{	
	if( switch_status ==1 ) { inital_finish_flag=1;printf("phy monitor on\n");} else { inital_finish_flag=0;printf("phy monitor off\r\n");}
	}
		if(sw_ability==7) 
	{	
	if( switch_status ==1 ) { command_calbration_enable=1;printf("command calibration on\n\r");} else { command_calbration_enable=0;printf("command calibration off\r\n");}
	}
	}

}

int cal_dealy_main(int argc, char *argv[], void *p)
{
	unsigned int phy_address,command2,command3,command4,select;
	if((argc <=1)||(argc >=6)) 
	{
	   	printf("tce cal_delay <sel> <phy> <p0_phy> <delay>");
	}
	else
	{
		inital_finish_flag = 0;
		select=checked_atoi(argv[1]);
		phy_address=checked_atoi(argv[2]);
		if(argc==4){ command2=checked_atoi(argv[3]);}
		if(argc==5){ command3=checked_atoi(argv[4]);}	
		if(select==1){	GECal_Rext_delay_measure(phy_address, command2);return 0;}
		if(select==2){	R50Calibration_delay_measure(phy_address, command2, command3);return 0;}
		if(select==3){	TX_OFFSET_Calibration_delay_measure(phy_address, command2, command3);return 0;}
		if(select==4){	TX_AMP_Calibration_delay_measure(phy_address, command2, command3);return 0;}		
inital_finish_flag = 1;
	}
}
...
{
	unsigned int phy_address,ephy_addr=9;
		if((argc <=1)||(argc >=3)) 
	{
	   	printf("tce recal 32 re-calibration75xx\r\n");
	    printf("tce recal phy address re-calibration75xx\r\n");
	}
	else
	{
...
for(do_cal_loop = 9; do_cal_loop<=12; do_cal_loop ++)
	...
	tcMiiStationWrite(do_cal_loop, 0x1f, 0x0);
	    ...
	    if( loopback_head_downshift_100M ==1 ) { printf("loopback_head_downshift on\r\n");} else { printf("loopback_head_downshift off\r\n");}
	    if( link_state_polling ==1 ) { printf("link status print on\r\n");} else { printf("link status print off\r\n");}
	    if( inital_finish_flag ==1 ) { printf("phy monitor on\r\n");} else { printf("phy monitor off\r\n");}
		if( command_calbration_enable ==1 ) { printf("command calibration on\r\n");} else { printf("command calibration off\r\n");}	
        printf("\r\n");
show_temp = GET_PDIDR();
printf("7581 E%d\r\n",show_temp);
...
#ifdef CONFIG_TP_IMAGE
int doPhyMiiRead_CL45(int argc, char *argv[], void *p)
{
        uint32 dev_addr=0;
        uint32 phyaddr=0;
        uint32 reg_addr=0;
        ...
        if (argc!=4 && argc!=6)
        {
                printf("Usage: miir_cl45 all <phyaddr> <devaddr> <s_addr> <e_addr>\r\n");
                printf("           miir_cl45 <phyaddr> <devaddr> <reg>\r\n");
                return 0;
        }
        ...
        {       /*Post Read*/
                ...
                {
                        value = tcMiiStationRead_CL45(phyaddr, dev_addr, i);
                        printf("* doPhyMiiRead_CL45 =>phyaddr=%d,  dev_addr=%d, data_addr=0x%04lX , value=0x%04lX\r\n", phyaddr, dev_addr,  i, value);
        ...
        else
        {
                ...
                value = tcMiiStationRead_CL45(phyaddr, dev_addr, reg_addr);
...
int doPhyMiiWrite_CL45 (int argc, char *argv[], void *p)
{
        ...
        {       // tce emiiw <DevAddr> <PhyAddr> <regAddr> <data>
                ...
                sscanf(argv[2], "%x", &dev_addr);//hex
                sscanf(argv[3], "%x", &reg_addr);//hex
                sscanf(argv[4], "%x", &mask);//hex
                sscanf(argv[5], "%x", &value);//hex
        }

        if(argc == 5)
        {
                tcMiiStationWrite_CL45(phyaddr, dev_addr,reg_addr,value);
                printf("* doPhyMiiWrite_CL45 => phyaddr=%d, dev_addr=%d, data_addr=0x%04lX , value=0x%04lX\r\n", phyaddr, dev_addr, reg_addr, value);
        }
        else if (argc == 6 && (1 == value || 0 == value))
        {
                read_value = tcMiiStationRead_CL45(phyaddr, dev_addr, reg_addr);
                if (value)
                {
                    read_value |= mask;
                }
                ...
                tcMiiStationWrite_CL45(phyaddr, dev_addr, reg_addr, read_value);
                printf("* doPhyMiiWrite_CL45 => phyaddr=%d, dev_addr=%d, data_addr=0x%04lX , mask=0x%04lX, value=0x%04lX\r\n", phyaddr, dev_addr, reg_addr, mask, read_value);
        }
        else
        {
                printf("           CMD Error : miiw_cl45 <PhyAddr> <DevAddr> <regAddr> <data>\r\n");
                printf("                       miiw_cl45 <PhyAddr> <DevAddr> <regAddr> <mask> <data(0/1)>\r\n");
                return 0;
        }
...
static int doPhyRegCheck (int argc, char *argv[], void *p)
{
    ...
    for(i=0; i<loop; i++ )
    {
        ...
        if(data != val )
        {
            printf("\r\nError!! Phyaddr=%d reg=%d loop=%d Expected=0x%x Read=0x%x\r\n", 
...
{       
	...
	ping_start:
    TRCLOG3216( LOG_IP, 5, "doping %x %x %x", ping_p->target,ping_p->len,ping_p->interval);
	...
	if(((argc == 4) ||(argc == 5)) && (PortSel <= 31))
	{
		if(argc == 4)
		{
			if(stricmp(argv[1], "all") == 0)
			{
			   ...
			   printf("Dbg Flag Register Set Value=%x\r\n", value);
  	...
  	if(argc==3)
  	{
        ...
        else if((stricmp(argv[2], "tr") == 0) ||(stricmp(argv[2], "TR") == 0))
        {
        	...
        	for(i=0; i<16; i++)
        	{
                ...
                printf("* NC[%02d], CH%s value=0x%04lX\r\n", i, ch_tbl[0], value);  //coverity #140173
            }
        }
...
int doPhyForceMode(int argc, char *argv[], void *p) 	
{
	...
	else if(argc == 4) // tce forcemode <phyaddr> <an|force> <10|100|1000>
	{
		...
		else if(stricmp(argv[2], "force") == 0)
		{
			...
			else if((stricmp(argv[3], "1000") == 0)&&(reg3==EPHY_ID_7581Ge))
			{
...
int doPhySLTMode(int argc, char *argv[], void *p) 	// allen_20171109
{
	...
	{
		if((stricmp(argv[2], "10") == 0)&&(reg3==EPHY_ID_7581Ge))
		{
			doEphySetSLTMode(phyaddr, 10);
			printf("force 10 half  \r\n");
			}
		else if((stricmp(argv[2], "100") == 0)&&(reg3==EPHY_ID_7581Ge))
		{
			doEphySetSLTMode(phyaddr, 100);
			printf("force 100 half  \r\n");
			}
		else if((stricmp(argv[2], "1000") == 0)&&(reg3==EPHY_ID_7581Ge))
		{
...
void doEphySetSLTMode(uint8 phyaddr, uint16 speed_mode)
{
    ...
    sw_down_shift_en=0;slt_excuting=1;inital_finish_flag = 1,phy_downshift_force_giga_slave_enable=0;
...
{	
	...
	{			    
		...
		if(control==1) { printf("all flowControl on\r\n");}		
		if(control==0) { printf("all flowControl off\r\n");}	   
	}	
	else	 
	{ 		
		...
		if(mode==1) 
			{
				...
				if(control==1) {page1_reg1d=page1_reg1d|0x8000;printf("port %d MDI Loopback  on\r\n",loop);} 		    
				if(control==0) {page1_reg1d=page1_reg1d&0x7fff;printf("port %d MDI Loopback  off\r\n",loop);}		        
		...
		if(mode==0)		
			{
				...
				if(control==1) {reg_00=reg_00|0x4000;printf("port %d Loopback on\r\n",loop);}		
				if(control==0) {reg_00=reg_00&0xbfff;printf("port %d Loopback off\r\n",loop);} 
	...
	{	    
		...
		for(loop=all_port_start;loop <= all_port_end;loop++)
		{
			...
			{			 
				...
				if(duplex == 1 ) 
				{ 
					reg_04=reg_04&0xff1f;reg_04=reg_04|0x0100;printf("port%d 100M full duplex set\r\n",loop);
				} 
				else 
				{ 
					reg_04=reg_04&0xfe9f;reg_04=reg_04|0x0080;printf("port%d 100M Half duplex set\r\n",loop);
				} 	   
				tcMiiStationWrite(loop, 4, reg_04);		   
				tcMiiStationWrite(loop, 0, 0x1340);			  
			} 	
			...
			{ 		   
				...
				if(duplex == 1 ) 
				{ 
					reg_04=reg_04&0xfe5f;reg_04=reg_04|0x0040;printf("port%d 10M full duplex set\r\n",loop);
				} 
				else 
				{ 
					reg_04=reg_04&0xfe3f;reg_04=reg_04|0x0020;printf("port%d 10M Half duplex set\r\n",loop);
				}		   
				tcMiiStationWrite(loop, 4, reg_04);		   
				tcMiiStationWrite(loop, 0, 0x1340);			  
			} 		   
		...
		{			  
			...
			{					 
				...
				{ 
					...
					printf("port%d 100M full duplex set\r\n",phyaddr);
				} 
				else 
				{ 
					reg_04=reg_04&0xfe9f;reg_04=reg_04|0x0080;
					printf("port%d 100M half duplex set\r\n",phyaddr);
				}			   
					tcMiiStationWrite(phyaddr, 4, reg_04);			   
					tcMiiStationWrite(phyaddr, 0, 0x1340);						
				break;
				...
				{ 
					...
					printf("port%d 10M full duplex set\r\n",phyaddr);
				} 
				else 
				{ 
					reg_04=reg_04&0xfe3f;
					reg_04=reg_04|0x0020;
					printf("port%d 10M Half duplex set\r\n",phyaddr);
				}			   
					tcMiiStationWrite(phyaddr, 4, reg_04);				   
					tcMiiStationWrite(phyaddr, 0, 0x1340);					   
				break;
...
int doGbeTxShaper(int argc, char *argv[], void *p)
{
  	...
  	else //if ((strcmpi("norm",argv[1]) == 0) || (strcmpi("test",argv[1]) == 0)) //GbE TX Shaper
  	{
    	...
    	{
      		sscanf(argv[4], "%x", &wTmp1);
      		sscanf(argv[5], "%x", &wTmp2);
      		sscanf(argv[6], "%x", &wTmp3);
...
#if 0
   127,127,127,127,127,127,127,127,127,127,127,127,127,127,127,127,
   127,126,122,117,112,109,104,101,97,94,90, 88, 84, 80, 78, 74,
   72, 68, 66, 64, 61, 58, 56, 53, 51, 48, 47, 44, 42, 40, 38, 36,
   34, 32, 31, 28, 27, 24, 24, 22, 20, 18, 16, 16, 14, 12, 11, 9
#endif
...
void GECal_Rext_delay_measure(uint8 phyaddr, uint32 delay)				
{
	uint8	rg_zcal_ctrl, all_ana_cal_status;
	uint16	dev1e_e0_ana_cal_r5, dev1f_ff_temp; 
	uint16	reg_temp,calibration_reg_temp;
	...
	rg_zcal_ctrl = 0x00;  								// start with 0 dB
    ...
    reg_temp=mtEMiiRegRead(9, 0x1e, 0x017a);
    printf("  start Rext reg17a=0x%x \n\r", reg_temp);

	for(rg_zcal_ctrl=0;rg_zcal_ctrl<=0x3f;rg_zcal_ctrl++)
	{
		mtEMiiRegWrite(phyaddr, 0x1e, 0x00e0, rg_zcal_ctrl);
		all_ana_cal_status = allGeAnaCalWait(delay, phyaddr);
		reg_temp=mtEMiiRegRead(9, 0x1e, 0x017a);
	if(calibration_reg_temp != reg_temp)
	{
		printf("  GE Rext turn reg17a=0x%x    ", reg_temp);
		printf("  GE Rext turn rg_zcal_ctrl=0x%x  \n\r", rg_zcal_ctrl);
	}
	calibration_reg_temp=reg_temp;
	}

reg_temp=mtEMiiRegRead(9, 0x1e, 0x017a); 
printf("  end Rext reg17a=0x%x    ", reg_temp);
printf("  end Rext rg_zcal_ctrl=0x%x  \n\r", rg_zcal_ctrl);
...
void R50Calibration_delay_measure(uint8 phyaddr,uint8 phyaddr_p0, uint32 delay) 
{
	...
	unsigned int calibration_reg_temp,reg_temp,get_calibration_data;
	uint16  cnt=0;
	// *** R45 Cal start *******************************************************************************
	//tcMiiStationWrite(phyaddr, 0x1f, 0x0000);		// page0
	//tcMiiStationWrite(phyaddr, 0x0,  0x0140);		// set [12]AN disable, [8]full duplex, [13/6]1000Mbps
	//mtEMiiRegWrite(phyaddr, 0x1e, 0x0145, 0x1010);			// 
	//mtEMiiRegWrite(phyaddr, 0x1e, 0x0185, 0x0000);			//
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		...
		all_ana_cal_status = allGeAnaCalWait_R45(delay, phyaddr); // delay 20 usec
        calibration_reg_temp=mtEMiiRegRead(9, 0x1e, 0x017a); 
        printf(" calibration_pair=%d start R50 reg17a=0x%x   \n\r ",calibration_pair, calibration_reg_temp);	

	for(rg_zcal_ctrl=0;rg_zcal_ctrl<=0x3f;rg_zcal_ctrl++)
	{
		//rg_zcal_ctrl = rg_zcal_ctrl+1;
		mtEMiiRegWrite( phyaddr_p0, 0x1e, 0x00e0, (dev1e_e0_ana_cal_r5 | rg_zcal_ctrl));	// 1e_e0[5:0]:rg_zcal_ctrl
	...
	if(calibration_reg_temp != reg_temp)
	{
		get_calibration_data=rg_zcal_ctrl;
		printf("calibration_pair=%d  GE R50 turn reg17a=0x%x    ",calibration_pair, reg_temp );
		printf("  GE R50 turn rg_zcal_ctrl=0x%x  \n\r", rg_zcal_ctrl);
	}
	calibration_reg_temp=reg_temp;
	}

reg_temp=mtEMiiRegRead(9, 0x1e, 0x017a); 
printf(" calibration_pair=%d end R50 reg17a=0x%x    ",calibration_pair, reg_temp);
printf("  end R50 rg_zcal_ctrl=0x%x  \n\r", rg_zcal_ctrl);
...
// return default
	tcMiiStationWrite(phyaddr, 0x1f, 0x0000);		// page0
	tcMiiStationWrite(phyaddr, 0x0,  0x1040);		// set [12]AN disable, [8]full duplex, [13/6]1000Mbps
...
void TX_OFFSET_Calibration_delay_measure(uint8 phyaddr,uint8 phyaddr_p0, uint32 delay)
{
	uint8	rg_zcal_ctrl, all_ana_cal_status;
	uint16	ad_cal_comp_out_init;
	int 	calibration_polarity, tx_offset_temp;
	uint16	calibration_pair, cal_temp;
	uint16	tx_offset_reg_shift; 
	uint16	tx_offset_reg;
	uint16	cnt=0;
	unsigned int calibration_reg_temp,reg_temp;
	// *** Tx offset Cal start *************************************************************************
   ...
   //udelay(1);
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		tx_offset_temp = 0x00; //TX_AMP_OFFSET_0mV;
		...
		all_ana_cal_status = allGeAnaCalWait_R45(delay, phyaddr); // delay 20 usec
		reg_temp=mtEMiiRegRead(9, 0x1e, 0x017a); 
        printf("calibration_pair=%d   start TXoffset reg17a=0x%x \n\r",calibration_pair, reg_temp);	

	for(rg_zcal_ctrl=0;rg_zcal_ctrl<=0x3f;rg_zcal_ctrl++)
	{
		//rg_zcal_ctrl = rg_zcal_ctrl+1;
		mtEMiiRegWrite(phyaddr, 0x1e, tx_offset_reg, (reg_temp|(rg_zcal_ctrl<<tx_offset_reg_shift)));
		all_ana_cal_status = allGeAnaCalWait(delay, phyaddr); // delay 20 usec
	...
	if(calibration_reg_temp != reg_temp)
	{
		printf("calibration_pair=%d   GE TXoffset turn reg17a=0x%x    ",calibration_pair, reg_temp);
		printf("  GE TXoffset turn rg_zcal_ctrl=0x%x  \n\r", rg_zcal_ctrl);
	}
	calibration_reg_temp=reg_temp;
	}
	reg_temp=mtEMiiRegRead(9, 0x1e, 0x017a); 
    printf("calibration_pair=%d   end TXoffset reg17a=0x%x    ",calibration_pair, reg_temp);
	printf("  end TXoffset rg_zcal_ctrl=0x%x  \n\r", rg_zcal_ctrl);
...
void TX_AMP_Calibration_delay_measure(uint8 phyaddr,uint8 phyaddr_p0, uint32 delay)
{
	uint8	rg_zcal_ctrl, all_ana_cal_status;
	uint16	ad_cal_comp_out_init;
	int 	calibration_polarity, tx_offset_temp;
	uint16	calibration_pair, cal_temp;
	uint16	tx_amp_reg_shift; 
	uint16	reg_temp_eco;
	uint16	tx_amp_temp, tx_amp_reg, cnt=0, tx_amp_reg_100;
	unsigned int calibration_reg_temp,reg_temp;
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		tx_amp_temp = 0x00;	// start with 0 dB
		...
		else if(calibration_pair == ANACAL_PAIR_B)
		{
			...
			tx_amp_reg_100 = 0x18;
		    mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (tx_amp_temp<<tx_amp_reg_shift));	
			mtEMiiRegWrite(phyaddr, 0x1e, 0x00dd, 0x0100);
			//mtEMiiRegWrite(phyaddr, 0x1e, 0x00db, 0x1100);	// 1e_db[12]:rg_cal_ckinv, [8]:rg_ana_calen, [4]:rg_rext_calen, [0]:rg_zcalen_a
			//mtEMiiRegWrite(phyaddr, 0x1e, 0x00dc, 0x1001);	// 1e_dc[12]:rg_zcalen_b
		}
		else if(calibration_pair == ANACAL_PAIR_C)
		{
			...
			tx_amp_reg_100 = 0x20;
		    mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (tx_amp_temp<<tx_amp_reg_shift));
			mtEMiiRegWrite(phyaddr, 0x1e, 0x00dd, 0x0010);
			//mtEMiiRegWrite(phyaddr, 0x1e, 0x00db, 0x1100);	// 1e_db[12]:rg_cal_ckinv, [8]:rg_ana_calen, [4]:rg_rext_calen, [0]:rg_zcalen_a
			//mtEMiiRegWrite(phyaddr, 0x1e, 0x00dc, 0x0101);	// 1e_dc[8]:rg_zcalen_c
		}
		else //if(calibration_pair == ANACAL_PAIR_D)
		{
			...
			tx_amp_reg_100 = 0x22;
			mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (tx_amp_temp<<tx_amp_reg_shift));
			mtEMiiRegWrite(phyaddr, 0x1e, 0x00dd, 0x0001);
			//mtEMiiRegWrite(phyaddr, 0x1e, 0x00db, 0x1100);	// 1e_db[12]:rg_cal_ckinv, [8]:rg_ana_calen, [4]:rg_rext_calen, [0]:rg_zcalen_a
			//mtEMiiRegWrite(phyaddr, 0x1e, 0x00dc, 0x0011);	// 1e_dc[4]:rg_zcalen_d	
		}
		    all_ana_cal_status = allGeAnaCalWait_R45(delay, phyaddr); // delay 20 usec
			reg_temp=mtEMiiRegRead(9, 0x1e, 0x017a); 
            printf("calibration_pair=%d   start TXAMP reg17a=0x%x    \n\r",calibration_pair, reg_temp);	

	for(rg_zcal_ctrl=0;rg_zcal_ctrl<=0x3f;rg_zcal_ctrl++)
	{
		//rg_zcal_ctrl = rg_zcal_ctrl+1;
        mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (rg_zcal_ctrl<<tx_amp_reg_shift));
		all_ana_cal_status = allGeAnaCalWait(delay, phyaddr); // delay 20 usec
	...
	if(calibration_reg_temp != reg_temp)
	{
		printf("calibration_pair=%d   TXAMP turn reg17a=0x%x    ",calibration_pair, reg_temp);
		printf("  TXAMP turn rg_zcal_ctrl=0x%x  \r\n", rg_zcal_ctrl);
	}
	calibration_reg_temp=reg_temp;
	}
		
	reg_temp=mtEMiiRegRead(9, 0x1e, 0x017a); 
    printf("calibration_pair=%d   end TXamp reg17a=0x%x    ",calibration_pair, reg_temp);
	printf("  end TXAMP rg_zcal_ctrl=0x%x  \r\n", rg_zcal_ctrl);
	}
	//return default
	mtEMiiRegWrite(phyaddr, 0x1e, 0x017d, 0x0000);
	mtEMiiRegWrite(phyaddr, 0x1e, 0x017e, 0x0000);
...
/*	
	//IRSET debug
			printf("  phy=9 devid=0x1e register=0xe0 value=0x%x ",mtEMiiRegRead(9, 0x1e, 0xe0));
			printf("  phy=10 devid=0x1e register=0xe0 value=0x%x ",mtEMiiRegRead(10, 0x1e, 0xe0));
			printf("  phy=11 devid=0x1e register=0xe0 value=0x%x ",mtEMiiRegRead(11, 0x1e, 0xe0));
			printf("  phy=12 devid=0x1e register=0xe0 value=0x%x \n\r",mtEMiiRegRead(12, 0x1e, 0xe0));
*/
}

void R50Calibration(uint8 phyaddr,uint8 phyaddr_p0,uint8 resisiter)
{
	uint8	rg_zcal_ctrl, all_ana_cal_status;
	uint16	ad_cal_comp_out_init;
	uint16	dev1e_e0_ana_cal_r5, dev1f_ff_temp;
...
// return default
	...
	mtEMiiRegWrite( phyaddr_p0, 0x1e, 0x00db, 0x0000);	
	mtEMiiRegWrite( phyaddr_p0, 0x1e, 0x00dc, 0x0000);
    mtEMiiRegWrite( phyaddr_p0, 0x1e, 0x00e1, 0x0000);	
	mtEMiiRegWrite( phyaddr_p0, 0x1f, 0x0100, 0x0000);			
...
/*
	//IRSET
			...
			printf("  phy=12 devid=0x1e register=0xe0 value=0x%x \n\r",mtEMiiRegRead(12, 0x1e, 0xe0));
	//r50
	        printf("  phy=9 devid=0x1e register=0x174 value=0x%x ",mtEMiiRegRead(9, 0x1e, 0x174));
	        printf("  phy=9 devid=0x1e register=0x175 value=0x%x \n\r",mtEMiiRegRead(9, 0x1e, 0x175));
	        printf("  phy=10 devid=0x1e register=0x174 value=0x%x ",mtEMiiRegRead(10, 0x1e, 0x174));
	        printf("  phy=10 devid=0x1e register=0x175 value=0x%x \n\r",mtEMiiRegRead(10, 0x1e, 0x175));
	        printf("  phy=11 devid=0x1e register=0x174 value=0x%x ",mtEMiiRegRead(11, 0x1e, 0x174));
	        printf("  phy=11 devid=0x1e register=0x175 value=0x%x \n\r",mtEMiiRegRead(11, 0x1e, 0x175));
	        printf("  phy=12 devid=0x1e register=0x174 value=0x%x ",mtEMiiRegRead(12, 0x1e, 0x174));
	        printf("  phy=12 devid=0x1e register=0x175 value=0x%x \n\r",mtEMiiRegRead(12, 0x1e, 0x175));
*/
	// *** R50 Cal end ***
}

void TX_OFFSET_Calibration(uint8 phyaddr,uint8 phyaddr_p0)
{
	uint8	rg_zcal_ctrl, all_ana_cal_status;
	uint16	ad_cal_comp_out_init;
	int 	calibration_polarity, tx_offset_temp;
	...
	//r50
	        ...
	        printf("  phy=12 devid=0x1e register=0x175 value=0x%x \n\r",mtEMiiRegRead(12, 0x1e, 0x175));
	//tx offset
	        printf("  phy=9 devid=0x1e register=0x172 value=0x%x ",mtEMiiRegRead(9, 0x1e, 0x172));
	        printf("  phy=9 devid=0x1e register=0x173 value=0x%x \n\r",mtEMiiRegRead(9, 0x1e, 0x173));
	        printf("  phy=10 devid=0x1e register=0x172 value=0x%x ",mtEMiiRegRead(10, 0x1e, 0x172));
	        printf("  phy=10 devid=0x1e register=0x173 value=0x%x \n\r",mtEMiiRegRead(10, 0x1e, 0x173));
	        printf("  phy=11 devid=0x1e register=0x172 value=0x%x ",mtEMiiRegRead(11, 0x1e, 0x172));
	        printf("  phy=11 devid=0x1e register=0x173 value=0x%x \n\r",mtEMiiRegRead(11, 0x1e, 0x173));
	        printf("  phy=12 devid=0x1e register=0x172 value=0x%x ",mtEMiiRegRead(12, 0x1e, 0x172));
	        printf("  phy=12 devid=0x1e register=0x173 value=0x%x \n\r",mtEMiiRegRead(12, 0x1e, 0x173));
			*/
	// *** Tx offset Cal end ***
}

void TX_AMP_Calibration(uint8 phyaddr,uint8 phyaddr_p0)
{
	uint8	rg_zcal_ctrl, all_ana_cal_status;
	uint16	ad_cal_comp_out_init;
	int 	calibration_polarity, tx_offset_temp;
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		...
		else
		{
			...
			GECal_status = 1;
			if(all_ana_cal_status == ANACAL_SATURATION){printf(" GE Tx amp SATURATION! (%d) (0x%x)\r\n", cnt, tx_amp_temp);}
...
void Calibration_all(uint8 phyaddr,uint8 resisiter) 
{
	...
	//printf(" 7580 GbE eco_rev (0x%x) &0xff(0x%x)\r\n", eco_rev, (eco_rev&0xff));
...
{
    uint8 phyaddr_p0=9;
	uint16	reg0_temp, dev1e_145_temp, reg_temp; 
...
TX_AMP_Calibration(phyaddr,phyaddr_p0);	
    ...
    mtEMiiRegWrite(phyaddr, 0x1e, 0x00e1, 0x0001);
	//tcMiiStationWrite(phyaddr, 0x0,  reg0_temp);
	mtEMiiRegWrite(phyaddr, 0x1f, 0x0100, 0x0000);
	mtEMiiRegWrite(phyaddr, 0x1e, 0x0145, 0x1000);
	tcMiiStationWrite(phyaddr, 0x0,  0x1040);
