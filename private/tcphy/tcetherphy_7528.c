/* //tclinux_phoenix/modules/private/tcphy/tcetherphy_7528.c */
/************************************************************************
...
#if defined(TCSUPPORT_CPU_EN7528) || defined(TCSUPPORT_AUTOBENCH) 
...
static uint8  mt7528_0R_flag = 0;		// allen_20190709
...
/************************************************************************
*
*            Variables and Functions for 7528 GE PHY (ID=0x9491)
*
**************************************************************************/
#ifdef LINUX_OS 
#define MT75xxGe_PORTNUM 				5
#define MT75xxGe_PHY_INIT_LDATA_LEN 	3
#define MT75xxGe_PHY_INIT_PERPDATA_LEN 	0
#define MT75xxGe_INIT_SET_NUM 			1
#define MT75xxGe_PHY_INIT_SET_NUM 		1

#define MT75xxGe_PHY_INIT_CL45_GDATA_LEN 	15 	
#define MT75xxGe_PHY_INIT_CL45_LDATA_LEN 	25 	 
#define MT75xxGe_PHY_INIT_CL45_PERPDATA_LEN 0
#define MT75xxGe_PHY_INIT_CL45_SET_NUM 		1

#define MT75xxGe_PHY_INIT_TR_LDATA_LEN 		5
#define MT75xxGe_PHY_INIT_TR_PERPDATA_LEN 	1
#define MT75xxGe_PHY_INIT_TR_SET_NUM 		1

typedef struct mt75xxGe_cfg_data_s
{
	char 		name[10];
	cfg_data_t 	ldata[MT75xxGe_PHY_INIT_LDATA_LEN];
	cfg_data_t 	perpdata[MT75xxGe_PHY_INIT_PERPDATA_LEN];			// per port register setting	
}mt75xxGe_cfg_data_t;

typedef struct mt75xxGe_cfg_cl45data_s
{
	cfg_cl45data_t gdata[MT75xxGe_PHY_INIT_CL45_GDATA_LEN];
	cfg_cl45data_t ldata[MT75xxGe_PHY_INIT_CL45_LDATA_LEN];
	cfg_cl45data_t perpdata[MT75xxGe_PHY_INIT_CL45_PERPDATA_LEN];	// per port register setting	
}mt75xxGe_cfg_cl45data_t;

typedef struct mt75xxGe_cfg_trdata_s
{
	cfg_trdata_t ldata[MT75xxGe_PHY_INIT_TR_LDATA_LEN];
	cfg_trdata_t perpdata[MT75xxGe_PHY_INIT_PERPDATA_LEN];			// per port register setting	
}mt75xxGe_cfg_trdata_t;

const uint32 mt75xxGe_page_sel_addr = 31;

mt75xxGe_cfg_data_t mt75xxGe_cfg[MT75xxGe_PHY_INIT_SET_NUM]=
{
	{
		{"Ge_1.0"}, 
    	//local data	
...
mt75xxGe_cfg_cl45data_t mt75xxGe_cfg_cl45[MT75xxGe_PHY_INIT_CL45_SET_NUM]=
{
    {	//globle data
     	{
			...
			{0x1f, 0x0272, 0x7fff},					// ps_op 0x3cff, for PHYD bug, need to workaround 

			{0x1f, 0x0268, 0x0728},					
			{0x1f, 0x0269, 0x111f},					// allen_20190902 ps_dri  //0x244f
			{0x1f, 0x0271, 0x7ca8},					// allen_20190902 0x4e13, for PHYD bug, need to workaround
			{0x1f, 0x0272, 0x14ff},					// allen_20190902
			{0x1f, 0x0044, 0x0030},					// pair delay (digital)

			{0x1f, 0x027c, 0x0808},					// 10 base Tx
			{0x1f, 0x027b, 0x0177},					// disable 10 base-Te
     	...
     	//local data
     	{
			...
			{0x1e,0x0006,0x03a7},					// 0->-1, middle
			{0x1e,0x0007,0x03c7},					// 0->-1, ov1
			{0x1e,0x0008,0x03bc},					// 0->-1, ov2, z2n_ovs
			{0x1e,0x0009,0x0290},					// -1->0, middle
			{0x1e,0x000a,0x0005},					// -1->0, ov1
			{0x1e,0x000b,0x0006},					// -1->0, ov2, n2z_ovs
			
			{0x1e,0x0013,0x0000},					// allen_20190902
			{0x1e,0x0014,0x0000},					// allen_20190902 // pair delay (afe)
			{0x1e,0x003e,0x0000},					// allen_20190902
			{0x1e,0x0044,0x0060},
			{0x1e,0x0176,0x5500},
			{0x1e,0x0177,0x0000},					// for GbE test mode 4 CM voltage
...
mt75xxGe_cfg_trdata_t mt75xxGe_cfg_tr[MT75xxGe_PHY_INIT_TR_SET_NUM]=
{
    {	//local data
     	{ 
			...
			{"PMA",0x12,0x3c4d2a},					// vgathresh100
			{"PMA",0x03,0x082422}
     	},
...
static int32 MT75xxGePhy_erroversum_A = 0;
static int32 MT75xxGePhy_erroversum_B = 0;
static int32 MT75xxGePhy_erroversum_C = 0;
static int32 MT75xxGePhy_erroversum_D = 0;

//static void mt75xxGePhyConfig(int argc, char *argv[], void *p);
void 		mt75xxGePhyCfgLoad(uint8 idx);
//static void mt75xxGePhyCfgTx10AmpSave(void);
void 		mt75xxGePhyGRCfgCheck(void);
void 		mt75xxGePhyLRCfgCheck(uint8 port_num);
void 		mt75xxGePhyCfgCheck(void);

#ifdef MTPHY_DEBUG
//ESD detect
static int16 mt75xxGePhy_Up2Down_cnt_gbt[5] = {0,0,0,0,0};
static int16 mt75xxGePhy_Up2Down_cnt_hbt[5] = {0,0,0,0,0};
static uint8 mt75xxGePhy_Up2DownFastHbt_detect[5] = {0,0,0,0,0};
static uint8 mt75xxGePhy_Up2DownFastgbt_detect[5] = {0,0,0,0,0};

static bool 	mt75xxGePhy_link_fail_detect_flag = 0;
//static uint8 	mt75xxGePhy_reset_needed_perport[5] = {0,0,0,0,0};
//static uint8 	mt75xxGePhy_reset_needed_flag = 0;
//static uint8 	mt75xxGePhy_esdphy_init_flag = 1; 		// default: enable

//Disable EEE if link fail
static int16 	mt75xxGePhy_Up2Down_cnt_tbt[5] = {0,0,0,0,0};
static uint8 	mt75xxGePhy_Up2DownFastTbt_detect[5] = {0,0,0,0,0};
static uint8 	mt75xxGePhy_linkup_check_timer[5] = {0,0,0,0,0};
static bool 	mt75xxGePhyDisEEEHappened[5] = {0,0,0,0,0};
static uint8 	mt75xxGePhy_linkup_check_cnt[5] = {0,0,0,0,0};
static uint8 	mt75xxGePhyDisNextPage_cnt[5] = {0,0,0,0,0};
static bool 	mt75xxGePhyDisNextPage_flag[5] = {0,0,0,0,0};
static uint8 	mt75xxGePhyDisSlaveMode_cnt[5] = {0,0,0,0,0};  
static bool 	mt75xxGePhyDisSlaveMode_flag[5] = {0,0,0,0,0};
int32 			mt75xxGePhyReadProbe(uint8 port_num, uint8 mode, uint8 ch_num);
int32 			mt75xxGePhyReadCoeff(uint8 port_num, uint8 mode, uint8 ch_num);
void 			mt75xxGePhyDbgPorbeSet(uint8 prb_num, uint8  port_num, uint16 flg_num, uint16 clk_num );
int32 			mt75xxGePhyDbgPorbeRead(uint8 prb_num,uint8  port_num);
#endif

#ifdef MTPHY_DEBUG 
int32 	mt75xxGePhyReadErrOverSum(uint8 port_num);
int32 	mt75xxGePhyReadAdcSum(uint8 port_num);
void 	mt75xxGePhyDispProbe100(uint8 port_num, bool long_msg);
void 	mt75xxGePhySwDispMessage(uint8 port_num);
uint16 	mt75xxGePhyReadSnrSum(uint8 port_num, uint16 cnt);
void 	mt75xxGePhyUpdateErrOverSum(uint8 port_num);
void 	mt75xxGePhySwErrOverMonitor(uint8 port_num);
void 	mt75xxGePhyUp2DownFastHbt(uint8 port_num);
void 	mt75xxGePhyUp2DownFastTbt(uint8 port_num);
bool 	mt75xxGePhyLinkFailDetect (uint8 port_num, uint8 mode);		//0:10BT or 100BT, 1:100BT only, 2:10BT only,3:10BT or 100BT or 1000BT
void 	mt75xxGePhyDisEEENeeded(uint8 port_num);
void 	mt75xxGePhyDisNextPageNeeded(uint8 port_num);
#endif //TCPHY_DEBUG

void 	mt75xxGePhySwPatch(void);
...
static uint8 	GECal_R50_status = 0;   // allen_20190709
...
void mt75xxGePhySwDispMessage(uint8 port_num)
{
    ...
    switch(mtphy_link_state[port_num]) 
    {
        ...
        case ST_LINK_DOWN2UP: 
			...
			{
				mt75xxGePhyReadProbe(port_num, Probe_VgaState, 0);
				mt75xxGePhyReadProbe(port_num, Probe_MSE_ALL, 0);
				mt75xxGePhyReadProbe(port_num, Probe_MSE_Slicer_err_thres, 0);
				mt75xxGePhyReadProbe(port_num, Probe_Slicer_Err_Over_Sum_ALL, 0);
			}
            break;
...
void mt75xxGePhyUpdateErrOverSum(uint8 port_num)
{
        ...
        && (mtphy_link_state[port_num] == ST_LINK_UP)) 
    {
        ...
        err_over_cnt_prev_D = ErovcntD[port_num].err_over_cnt;
      	...
      	// mt75xxGePhyReadProbe(port_num,Probe_MSE_Sum,0);
       	mt75xxGePhyReadProbe(port_num,Probe_Slicer_Err_Over_Sum,0);		//Pair A
       	mt75xxGePhyReadProbe(port_num,Probe_Slicer_Err_Over_Sum,1);		//Pair B
       	mt75xxGePhyReadProbe(port_num,Probe_Slicer_Err_Over_Sum,2);		//Pair C
       	mt75xxGePhyReadProbe(port_num,Probe_Slicer_Err_Over_Sum,3);		//Pair D
        ...
        MT75xxGePhy_erroversum_A = val_A;
        MT75xxGePhy_erroversum_B = val_B;
        MT75xxGePhy_erroversum_C = val_C;
        MT75xxGePhy_erroversum_D = val_D;
    }
    else 
    {
        MT75xxGePhy_erroversum_A = -1;
        MT75xxGePhy_erroversum_B = -1;
        MT75xxGePhy_erroversum_C = -1;
        MT75xxGePhy_erroversum_D = -1;
    }
}
...
void mt75xxGePhySwErrOverMonitor(uint8 port_num)
{
    if (tcPhy_disp_level < 3) 
        return; 			// inactive
	#if debug_flag
    if (MT75xxGePhy_erroversum_A>0)
    {
        TCPHYDISP3("tcphy[%d]: ErrOver A=%ld\r\n",port_num,MT75xxGePhy_erroversum_A);          
    }
    if (MT75xxGePhy_erroversum_B>0)
    {
        TCPHYDISP3("tcphy[%d]: ErrOver B=%ld\r\n",port_num,MT75xxGePhy_erroversum_B);          
    }
    if (MT75xxGePhy_erroversum_C>0)
    {
        TCPHYDISP3("tcphy[%d]: ErrOver C =%ld\r\n",port_num,MT75xxGePhy_erroversum_C);          
    }
    if (MT75xxGePhy_erroversum_D>0)
    {
        TCPHYDISP3("tcphy[%d]: ErrOver D=%ld\r\n",port_num,MT75xxGePhy_erroversum_D);          
    }
    #endif
}

void mt75xxGePhyUp2DownFastgbt(uint8 port_num)
{
	const uint8 mt75xxGePhy_Up2Down_gbt_Thd = 10;
	const uint8 mt75xxGePhy_Up2Down_gbt_ub = 40;

	if(mr1E_A2.final_speed_1000 & !Nmr1[port_num].link_status) 
	{
		if(mt75xxGePhy_Up2Down_cnt_gbt[port_num]<mt75xxGePhy_Up2Down_gbt_ub) 
		{
			mt75xxGePhy_Up2Down_cnt_gbt[port_num]+=3;
		}
    }
	else if(!mr1E_A2.final_speed_1000 & !Nmr1[port_num].link_status)
	{
		if(mt75xxGePhy_Up2Down_cnt_gbt[port_num]>0) 
		{
			mt75xxGePhy_Up2Down_cnt_gbt[port_num]--;
		}
	}
	else if(mr1E_A2.final_speed_1000 & Nmr1[port_num].link_status) 
	{	//adjust link-up time to clear counter
		mt75xxGePhy_Up2Down_cnt_gbt[port_num]=0;
	}
	
	if(mt75xxGePhy_Up2Down_cnt_gbt[port_num] > mt75xxGePhy_Up2Down_gbt_Thd) 
	{
	    mt75xxGePhy_Up2Down_cnt_gbt[port_num]=0;
		mt75xxGePhy_Up2DownFastgbt_detect[port_num]=1;
		#ifdef TCPHY_DEBUG
		TCPHYDISP4("set mt75xxGePhy_Up2DownFastgbt_detect[%d] to 1.\r\n",port_num); 
		#endif
	}
	else 
	{
		mt75xxGePhy_Up2DownFastgbt_detect[port_num]=0;
	}  
}

void mt75xxGePhyUp2DownFastHbt(uint8 port_num)
{
	const uint8 mt75xxGePhy_Up2Down_hbt_Thd = 10;
	const uint8 mt75xxGePhy_Up2Down_hbt_ub = 40;

	if(mr1E_A2.final_speed_100 & !Nmr1[port_num].link_status) 
	{
		if(mt75xxGePhy_Up2Down_cnt_hbt[port_num] < mt75xxGePhy_Up2Down_hbt_ub) 
		{
			mt75xxGePhy_Up2Down_cnt_hbt[port_num] += 3;
		}
    }
	else if(!mr1E_A2.final_speed_100 & !Nmr1[port_num].link_status) 
	{
		if(mt75xxGePhy_Up2Down_cnt_hbt[port_num]>0) 
		{
			mt75xxGePhy_Up2Down_cnt_hbt[port_num]--;
		}
	}
	else if(mr1E_A2.final_speed_100 & Nmr1[port_num].link_status) 
	{	//adjust link-up time to clear counter
		mt75xxGePhy_Up2Down_cnt_hbt[port_num]=0;
	}
	
	if(mt75xxGePhy_Up2Down_cnt_hbt[port_num]> mt75xxGePhy_Up2Down_hbt_Thd) 
	{
	    mt75xxGePhy_Up2Down_cnt_hbt[port_num]=0;
		mt75xxGePhy_Up2DownFastHbt_detect[port_num]=1;
		#ifdef TCPHY_DEBUG
		TCPHYDISP4("set mt75xxGePhy_Up2DownFastHbt_detect[%d] to 1.\r\n",port_num); 
		#endif
	}
	else 
	{
		mt75xxGePhy_Up2DownFastHbt_detect[port_num]=0;
	}  
}

void mt75xxGePhyUp2DownFastTbt(uint8 port_num)
{
	const uint8 mt75xxGePhy_Up2Down_Tbt_Thd = 10;
	const uint8 mt75xxGePhy_Up2Down_Tbt_ub = 40;
	const uint8 linkup_check_timer_done = 1;
	
    if(mr1E_A2.final_speed_10 & !Nmr1[port_num].link_status) 
	{
		mt75xxGePhy_linkup_check_timer[port_num]=0;
		if(mt75xxGePhy_Up2Down_cnt_tbt[port_num] < mt75xxGePhy_Up2Down_Tbt_ub) 
		{
			mt75xxGePhy_Up2Down_cnt_tbt[port_num]+=5;
		}
    }
	else if(!mr1E_A2.final_speed_10 & !Nmr1[port_num].link_status) 
	{
		mt75xxGePhy_linkup_check_timer[port_num]=0;
		if(mt75xxGePhy_Up2Down_cnt_tbt[port_num]>0) 
		{
			mt75xxGePhy_Up2Down_cnt_tbt[port_num]--;
		}
	}
	else if(mr1E_A2.final_speed_10 & Nmr1[port_num].link_status ) 
	{
		if((mt75xxGePhy_linkup_check_timer[port_num] == linkup_check_timer_done)) 
		{
			mt75xxGePhy_Up2Down_cnt_tbt[port_num]=0;
			mt75xxGePhy_linkup_check_timer[port_num]=0;
		}
		else if((mt75xxGePhy_linkup_check_timer[port_num] < linkup_check_timer_done))
		{
			mt75xxGePhy_linkup_check_timer[port_num]++;
		}
	}
	
	if(mt75xxGePhy_Up2Down_cnt_tbt[port_num]> mt75xxGePhy_Up2Down_Tbt_Thd) 
	{
	    mt75xxGePhy_Up2Down_cnt_tbt[port_num]=0; 
		mt75xxGePhy_Up2DownFastTbt_detect[port_num]=1;
		#ifdef TCPHY_DEBUG
		TCPHYDISP4("set mt75xxGePhy_Up2DownFastTbt_detect[%d] to 1.\r\n",port_num); 
		#endif
	}
	else 
	{
		mt75xxGePhy_Up2DownFastTbt_detect[port_num]=0;
	}	
}

bool mt75xxGePhyLinkFailDetect(uint8 port_num, uint8 mode)
{
	switch(mode)
	{
		case TbtOrHbtOrGbt:
			if(sw_FixUp2DownFast_flag 
				&(mt75xxGePhy_Up2DownFastTbt_detect[port_num]
				||mt75xxGePhy_Up2DownFastHbt_detect[port_num]||mt75xxGePhy_Up2DownFastgbt_detect[port_num])) 
			{
				mt75xxGePhy_link_fail_detect_flag =1;
		        #ifdef TCPHY_DEBUG
		        TCPHYDISP3("mt75xxGePhy_link_fail_detect_flag(%d)=%d\r\n", port_num, mt75xxGePhy_link_fail_detect_flag); 
		        #endif
			}
			else 
			{
				mt75xxGePhy_link_fail_detect_flag =0;
			}	
			break;

		case HbtOnly:
			if(sw_FixUp2DownFast_flag & mt75xxGePhy_Up2DownFastHbt_detect[port_num]) 
			{
				mt75xxGePhy_link_fail_detect_flag =1;
		        #ifdef TCPHY_DEBUG
		        TCPHYDISP3("Hbt: mt75xxGePhy_link_fail_detect_flag(%d)=%d\r\n", port_num, mt75xxGePhy_link_fail_detect_flag); 
		        #endif
			}
			else 
			{
		...
		case TbtOnly:
			if(sw_FixUp2DownFast_flag & mt75xxGePhy_Up2DownFastTbt_detect[port_num]) 
			{
				mt75xxGePhy_link_fail_detect_flag =1;
		       	#ifdef TCPHY_DEBUG
		        TCPHYDISP3("Tbt: mt75xxGePhy_link_fail_detect_flag(%d)=%d\r\n", port_num, mt75xxGePhy_link_fail_detect_flag); 
		        #endif
			}
			else 
			{
	...
	return mt75xxGePhy_link_fail_detect_flag;
}

void mt75xxGePhyDisNextPageNeeded(uint8 port_num)
{
	const uint8 mt75xxGePhyDisNextPage_cntdone = 5;
	const uint8 mt75xxGePhyDisSlaveMode_cntdone = 3; 
   
   	if((mtphy_link_state[port_num] == ST_LINK_UP)||(mt75xxGePhyDisEEEHappened[port_num])) 
 	{
   		mt75xxGePhyDisNextPage_cnt[port_num] = 0;
   	}
   	else if((mr5.LPNextAble) && !(mr7_3D.lp_eee_100||mr7_3D.lp_eee_1000||mr7_3D.lp_eee_10g)
   		&& (mt75xxGePhyDisNextPage_cnt[port_num] <= mt75xxGePhyDisNextPage_cntdone)) 
   	{
   		mt75xxGePhyDisNextPage_cnt[port_num]++;
   	}

   	if(mt75xxGePhyDisNextPage_cnt[port_num] > mt75xxGePhyDisNextPage_cntdone) 
	{
   		mt75xxGePhyDisNextPage_flag[port_num] = 1;
		mt75xxGePhyDisNextPage_cnt[port_num] = 0;
   	}
   	else 
	{
   		mt75xxGePhyDisNextPage_flag[port_num]=0;
   	}
	...
	if((mtphy_link_state[port_num] != ST_LINK_UP) && (mtphy_link_state[port_num] != ST_LINK_DOWN) &&
		(mt75xxGePhyDisSlaveMode_cnt[port_num] <= mt75xxGePhyDisSlaveMode_cntdone))
	{
		mt75xxGePhyDisSlaveMode_cnt[port_num]++;
	}

	if(mt75xxGePhyDisSlaveMode_cnt[port_num] > mt75xxGePhyDisSlaveMode_cntdone)
	{
		mt75xxGePhyDisSlaveMode_flag[port_num] = 1;
		mt75xxGePhyDisSlaveMode_cnt[port_num] = 0;
	}
	else
	{
		mt75xxGePhyDisSlaveMode_flag[port_num] = 0;
	}
}

void mt75xxGePhyDisEEENeeded(uint8 port_num)
{
    const uint8 mt75xxGePhy_linkup_check_done = 1;
	uint8 phy_base = 8, phyaddr;
	
	phyaddr = phy_base + port_num;
	
    if((mtphy_link_state[port_num] == ST_LINK_UP)
		&& (mt75xxGePhy_linkup_check_cnt[port_num] < mt75xxGePhy_linkup_check_done)) 
	{
		mt75xxGePhy_linkup_check_cnt[port_num]++;
    }
	else if(mtphy_link_state[port_num] != ST_LINK_UP)
	{
		mt75xxGePhy_linkup_check_cnt[port_num] = 0;
    }
	
    if(mt75xxGePhyDisEEEHappened[port_num] && mt75xxGePhy_linkup_check_cnt[port_num]==mt75xxGePhy_linkup_check_done) 
	{	//extend link-up time
		mtEMiiRegWrite(phyaddr, 0x07, 0x3c, 0x06);
		mt75xxGePhyDisEEEHappened[port_num]=0;
    }
	else if(mt75xxGePhyLinkFailDetect(port_num,3) || mt75xxGePhyDisNextPage_flag[port_num]) 
	{
		mtEMiiRegWrite(phyaddr, 0x07, 0x3c, 0x0000);	// disable next page
		tcMiiStationWrite(0,0x00,0x1200);				// re-start AN
		mt75xxGePhyDisEEEHappened[port_num]=1;
    }
	//TCPHYDISP3("mt75xxGePhy_linkup_check_cnt[%d]=%d\r\n", port_num, mt75xxGePhy_linkup_check_cnt[port_num]);
	//TCPHYDISP3("mt75xxGePhyDisEEEHappened[%d]=%d\r\n", port_num, mt75xxGePhyDisEEEHappened[port_num]);
}

void mt75xxGePhySwPatch(void)
{
    uint8 	pn;
	uint8 	gphy_base=9, phyaddr, phy_base=9;
	uint32 	reg0, reg1, reg9, reg10;
	uint32	reg_vga=0, vga_a=0, vga_b=0, vga_c=0, vga_d=0;

    for(phyaddr = gphy_base; phyaddr < (gphy_base+MT75xxGe_PORTNUM); phyaddr++) 
	{
		pn = phyaddr - phy_base;
        mtPhyReadReg(pn,1);
		...
		if(((reg9>>9)&0x1) == 1) 								// check 1000Mbps capbility
		{
			...
			//mt75xxGePhyDisNextPageNeeded(pn); 
			//mt75xxGePhyUp2DownFastgbt(pn);
			//mt75xxGePhyUp2DownFastHbt(pn);
			//mt75xxGePhyUp2DownFastTbt(pn);
			//mt75xxGePhyDisEEENeeded(pn);
 
			if((mtphy_link_state[pn] != ST_LINK_UP) && (mt75xxGePhyDisSlaveMode_flag[pn] == 1) && (((reg9>>12)&0x1)==1)) 
			{
				tcMiiStationWrite(phyaddr, 0x09, 0x0600); 		// disable manual  
				tcMiiStationWrite(phyaddr, 0x00, 0x1200); 		// re-start AN
				//mt75xxGePhyDisSlaveMode_cnt[pn] = 0;
				//mt75xxGePhyDisSlaveMode_flag[pn] = 0;
			}
		...
		#ifdef MTPHY_DEBUG
		// display message
		mt75xxGePhySwDispMessage(pn);
		#endif

		#ifdef MTPHY_DEBUG
		if(sw_ErrOverMonitor_flag)
			mt75xxGePhyUpdateErrOverSum(pn);
		if(sw_ErrOverMonitor_flag) 
			mt75xxGePhySwErrOverMonitor(pn); // call after mt75xxFEUpdateErrOverSum()
		#endif
		#endif

		#if 0	// backup solution for down speed
		reg_vga = mtPhyMiiRead_TrDbg(phyaddr,"PMA",0x38,0);
    	vga_a = ((reg_vga>>4) & 0x1F);
    	vga_b = ((reg_vga>>9) & 0x1F);
    	vga_c = ((reg_vga>>14) & 0x1F);
    	vga_d = ((reg_vga>>19) & 0x1F);

		if(mtphy_link_state[pn] == ST_LINK_UP2DOWN)
		{
			//tcMiiStationWrite(phyaddr, 0x09, 0x1e00);
			//mtEMiiRegWrite(phyaddr, 0x1e, 0x3c, 0xff00);
			//mtEMiiRegWrite(phyaddr, 0x1e, 0x3d, 0xfc00);
			//mtEMiiRegWrite(phyaddr, 0x1e, 0x3e, 0xf000);
			//tcMiiStationWrite(phyaddr, 0x00, 0x1200);
			if((((reg10>>11)&0x1) == 1) && ((vga_a<0x10)|(vga_b<0x10)|(vga_c<0x10)|(vga_d<0x10) ))	// LP 1000 && vga<0x10(long)
			{
				cnt_up2down ++;
			}
			printf("\n [Alarm] SW down speed (cnt=%d) \n", cnt_up2down);
			
			if(cnt_up2down > 5)
			{
				tcMiiStationWrite(phyaddr, 0x09, 0x0000);
				tcMiiStationWrite(phyaddr, 0x00, 0x1200);
				printf("\n [Alarm] SW down speed (cnt=%d)(reg9=0x%x) \n", cnt_up2down, tcMiiStationRead(phyaddr, 0x09));
				cnt_up2down = 0;	
				down_speed_flag = 1;
			}
		}
		...
		{
			if((down_speed_flag == 1)||(cnt_up2down != 0))
				cnt_up ++;
				
			if(cnt_up > 10)
			{
				tcMiiStationWrite(phyaddr, 0x09, 0x0600); // keep default master 
				down_speed_flag = 0;
			}	
		}
		#endif
...
void mt75xxGePhyCfgLoad(uint8 idx)
{
    ...
    for( pn=0; pn < MT75xxGe_PORTNUM; pn++)		
	{
		phyAddr = phyAddr_base + pn;
	
		TCPHYDISP3("\n 75xxGe, phyaddr= (%d,%d) \n", phyAddr_base, phyAddr);
		for( i=0; i<MT75xxGe_PHY_INIT_CL45_GDATA_LEN; i++ )
		{         
			mtEMiiRegWrite(phyAddr, mt75xxGe_cfg_cl45[current_idx].gdata[i].dev_num, mt75xxGe_cfg_cl45[current_idx].gdata[i].reg_num, mt75xxGe_cfg_cl45[current_idx].gdata[i].val);
		}
		for( i=0; i<MT75xxGe_PHY_INIT_CL45_LDATA_LEN; i++ )
		{         
			mtEMiiRegWrite(phyAddr, mt75xxGe_cfg_cl45[current_idx].ldata[i].dev_num, mt75xxGe_cfg_cl45[current_idx].ldata[i].reg_num, mt75xxGe_cfg_cl45[current_idx].ldata[i].val);
		}
		for( i=0; i<MT75xxGe_PHY_INIT_LDATA_LEN; i++ )
		{         
			tcMiiStationWrite(phyAddr, mt75xxGe_cfg[current_idx].ldata[i].reg_num, mt75xxGe_cfg[current_idx].ldata[i].val);
		}
		for( i=0; i<MT75xxGe_PHY_INIT_TR_LDATA_LEN; i++ )
		{         
			mtPhyMiiWrite_TrDbg( phyAddr, mt75xxGe_cfg_tr[current_idx].ldata[i].reg_typ, mt75xxGe_cfg_tr[current_idx].ldata[i].reg_num, mt75xxGe_cfg_tr[current_idx].ldata[i].val,0);
		}
	}
...
static int doPhyset7528_0R (int argc, char *argv[], void *p);       // allen_20190709	
static int doPhysetLED (int argc, char *argv[], void *p);           // allen_20190709			
...
static const cmds_t ethertphyprint[] =
{
	...
	{"set7528_0R", 		doPhyset7528_0R,		0x02, 1, "set7528_0R <on|off>"},                  // allen_20190709
	{"setLED", 		doPhysetLED,			0x02, 1, "setLED <0|1|2>"},                       // allen_20190709
...
int tcPhyVerLookUp(uint16 ephy_addr)
{
    ...
    else if(rval == EPHY_ID_7528Ge)  
	{
        tcPhyVer = tcPhyVer_7528Ge;		
        tcPhyPortNum = 4;
        TCPHYDISP1(" 7528 Ge, \n");
        tcPhyFlag = 1;
        tcPhyInitFlag = 1;
...
int tcPhyInit(void)
{
	uint16	ephy_addr, port_num, gphy_base=9, xtal=0;
	...
	else
	{
    	...
    	{
    		xtal = (VPint(0xbfa20174)>>18) & 0x3;

			if(xtal == 0x3)	// 40MHz co-clk 
			{
				// === PLL setting
				mtEMiiRegWrite(gphy_base, 0x1f, 0x10a, 0x1000);
				mtEMiiRegWrite(gphy_base, 0x1f, 0x503, 0x0003);
				mtEMiiRegWrite(gphy_base, 0x1f, 0x502, 0x0000);
				mtEMiiRegWrite(gphy_base, 0x1f, 0x501, 0x017c);
				mtEMiiRegWrite(gphy_base, 0x1f, 0x505, 0x0202);
				mtEMiiRegWrite(gphy_base, 0x1f, 0x504, 0x0d01);
				mtEMiiRegWrite(gphy_base, 0x1f, 0x506, 0x0c01);
				mtEMiiRegWrite(gphy_base, 0x1f, 0x100, 0x0d80);
				mtEMiiRegWrite(gphy_base, 0x1f, 0x103, 0x0002);
				mtEMiiRegWrite(gphy_base, 0x1f, 0x111, 0x0001);

				// === PLL pwd
				mtEMiiRegWrite(gphy_base, 0x1f, 0x50e, 0x0001);
				mtEMiiRegWrite(gphy_base, 0x1f, 0x500, 0x0001);
				mtEMiiRegWrite(gphy_base, 0x1f, 0x50f, 0x0100);
				mtEMiiRegWrite(gphy_base, 0x1f, 0x104, 0x0608);
		
				// === PLL power on
				mtEMiiRegWrite(gphy_base, 0x1f, 0x50f, 0x0000);
				mtEMiiRegWrite(gphy_base, 0x1f, 0x500, 0x0000);
				mtEMiiRegWrite(gphy_base, 0x1f, 0x50e, 0x0000);
				mtEMiiRegWrite(gphy_base, 0x1f, 0x104, 0x2608);

				//TCPHYDISP3("  40MHz co-clock \r\n");
				printf("  40MHz co-clock \r\n");
			}
...
int ePhyInit(uint16 ephy_addr)
{
   ...
   // uint16 start_addr = ephy_addr;
	...
	{			    
		...
		case tcPhyVer_7528Ge:		// EN7528 Ge
			if(ephy_addr_base == 0)
				ephy_addr_base += 9;
			break;
	}
...
/************************************************************************
*         API functions body for EPHY (TCPHY_SUPPORT) (8)
**************************************************************************/
#ifdef TCPHY_SUPPORT
static int doPhySwVer(int argc, char *argv[], void *p)
{
	...
	{    
		case tcPhyVer_7528Ge: 
			printf("%s, \r\n", mt75xxGe_cfg[0].name);
		 	printf("tcPhyVer_7528Ge, tcPhyVer=0x%02lX \r\n", tcPhyVer);
			break;	
...
int doPhyGphyTestMode (int argc, char *argv[], void *p)
{
	...
	else	
	{
		...
		if((reg3 == EPHY_ID_7528Ge)||(reg3 == EPHY_ID_7512Ge)||(reg3==EPHY_ID_7522Ge))
		{
...
{ 	// for 7528 (linux)
   	...
   	if(phychkval_flag)
	{
		...
		else if((reg3 == EPHY_ID_7528Ge))
		{ 	// Force-speed
			//mr_dplx = 0; // mr_anen=0, should be half duplex : coverity issue
			if(mr_speed==0x00)
	 		{
		 		printf(" Force-%d%s\r\n",10,(mr_dplx?"F":"H"));
			}
		...
		{ 	// allen_20130926
			...
			if((tcPhyVer == tcPhyVer_7528Ge))
			{
				...
				val=mt75xxGePhyReadProbe(phyAddr, Probe_VgaState, 0);
				printf("75xx VgaStateA, B, C, D = 0x%x, 0x%x, 0x%x, 0x%x\r\n",((val>>4) & 0x1F),((val>>9) & 0x1F),((val>>14) & 0x1F),((val>>19) & 0x1F));
				
				val=mt75xxGePhyReadProbe(phyAddr,Probe_MSE,0);
				printf("75xx lch_mse_mdcA =0x%x\r\n",val);
				val=mt75xxGePhyReadProbe(phyAddr,Probe_MSE,1);
				printf("75xx lch_mse_mdcB =0x%x\r\n",val);
				val=mt75xxGePhyReadProbe(phyAddr,Probe_MSE,2);
				printf("75xx lch_mse_mdcC =0x%x\r\n",val);
				val=mt75xxGePhyReadProbe(phyAddr,Probe_MSE,3);
				printf("75xx lch_mse_mdcD =0x%x\r\n",val);
			
				val=mt75xxGePhyReadProbe(phyAddr,Probe_MSE_Slicer_err_thres,0);
				printf("slicer_err_thres =0x%x\r\n",val);
				val=mt75xxGePhyReadProbe(phyAddr,Probe_Slicer_Err_Over_Sum,0);
				printf("ProbeSlicerErrOverSumA =0x%x\r\n",val);
				val=mt75xxGePhyReadProbe(phyAddr,Probe_Slicer_Err_Over_Sum,1);
				printf("ProbeSlicerErrOverSumB =0x%x\r\n",val);
				val=mt75xxGePhyReadProbe(phyAddr,Probe_Slicer_Err_Over_Sum,2);
				printf("ProbeSlicerErrOverSumC =0x%x\r\n",val);
				val=mt75xxGePhyReadProbe(phyAddr,Probe_Slicer_Err_Over_Sum,3);
		...
		{ 	// link-up
			...
			{	// 100Mbps/1000Mbps
				...
				if((reg3 == EPHY_ID_7530Ge_7512Fe)&&(((r15_temp>>13)&0x1) == 1))
				{
					...
					val=mt75xxGePhyReadProbe(port_num, Probe_VgaState, 0);
					printf("VgaStateA, B, C, D = 0x%x, 0x%x, 0x%x, 0x%x\r\n",((val>>4) & 0x1F),((val>>9) & 0x1F),((val>>14) & 0x1F),((val>>19) & 0x1F));
					//printf("VgaStateB =0x%x,\r\n",((val>>9) & 0x1F));
					//printf("VgaStateC =0x%x,\r\n",((val>>14) & 0x1F));
					//printf("VgaStateD =0x%x\r\n",((val>>19) & 0x1F));
				
					val=mt75xxGePhyReadProbe(port_num,Probe_MSE,0);
					printf("lch_mse_mdcA =0x%x\r\n",val);
					val=mt75xxGePhyReadProbe(port_num,Probe_MSE,1);
					printf("lch_mse_mdcB =0x%x\r\n",val);
					val=mt75xxGePhyReadProbe(port_num,Probe_MSE,2);
					printf("lch_mse_mdcC =0x%x\r\n",val);
					val=mt75xxGePhyReadProbe(port_num,Probe_MSE,3);
					printf("lch_mse_mdcD =0x%x\r\n",val);
			
					val=mt75xxGePhyReadProbe(port_num,Probe_MSE_Slicer_err_thres,0);
					printf("slicer_err_thres =0x%x\r\n",val);
					val=mt75xxGePhyReadProbe(port_num,Probe_Slicer_Err_Over_Sum,0);
					printf("ProbeSlicerErrOverSumA =0x%x\r\n",val);
					val=mt75xxGePhyReadProbe(port_num,Probe_Slicer_Err_Over_Sum,1);
					printf("ProbeSlicerErrOverSumB =0x%x\r\n",val);
					val=mt75xxGePhyReadProbe(port_num,Probe_Slicer_Err_Over_Sum,2);
					printf("ProbeSlicerErrOverSumC =0x%x\r\n",val);
					val=mt75xxGePhyReadProbe(port_num,Probe_Slicer_Err_Over_Sum,3);
					printf("ProbeSlicerErrOverSumD =0x%x\r\n",val);
				}
				#endif
...
/************************************************************************
*            debug functions body for EPHY (TCPHY_TESTDBG CI commands) (9)
**************************************************************************/
#ifdef TCPHY_FIELD_DBGTEST
static int doFieldDebugTest(int argc, char *argv[], void *p)
{
...
int doPhyForceMode(int argc, char *argv[], void *p) 	
{
	...
	else if(argc == 4) // tce forcemode <phyaddr> <an|force> <10|100|1000>
	{
		...
		else if(stricmp(argv[2], "force") == 0)
		{
			...
			else if((stricmp(argv[3], "1000") == 0)&&(reg3==EPHY_ID_7528Ge))
			{
...
int doPhySLTMode(int argc, char *argv[], void *p) 	// allen_20171109
{
	...
	if(argc == 3) // tce sltmode <phyaddr> <10|100|1000> 
	{
		...
		else if((stricmp(argv[2], "1000") == 0)&&(reg3==EPHY_ID_7528Ge))
		{
...
int doPhyForceReset(int argc, char *argv[], void *p) 	// allen_20131120
{
	...
	if (argc == 2) 
	{
		...
		{
			// === PLL setting
			mtEMiiRegWrite(phyaddr, 0x1f, 0x10a, 0x1000);
			mtEMiiRegWrite(phyaddr, 0x1f, 0x503, 0x0003);
			mtEMiiRegWrite(phyaddr, 0x1f, 0x502, 0x0000);
			mtEMiiRegWrite(phyaddr, 0x1f, 0x501, 0x017c);
			mtEMiiRegWrite(phyaddr, 0x1f, 0x505, 0x0202);
			mtEMiiRegWrite(phyaddr, 0x1f, 0x504, 0x0d01);
			mtEMiiRegWrite(phyaddr, 0x1f, 0x506, 0x0c01);
			mtEMiiRegWrite(phyaddr, 0x1f, 0x100, 0x0d80);
			mtEMiiRegWrite(phyaddr, 0x1f, 0x103, 0x0002);
			mtEMiiRegWrite(phyaddr, 0x1f, 0x111, 0x0001);

			// === PLL pwd
			mtEMiiRegWrite(phyaddr, 0x1f, 0x50e, 0x0001);
			mtEMiiRegWrite(phyaddr, 0x1f, 0x500, 0x0001);
			mtEMiiRegWrite(phyaddr, 0x1f, 0x50f, 0x0100);
			mtEMiiRegWrite(phyaddr, 0x1f, 0x104, 0x0608);
		
			// === PLL power on
			mtEMiiRegWrite(phyaddr, 0x1f, 0x50f, 0x0000);
			mtEMiiRegWrite(phyaddr, 0x1f, 0x500, 0x0000);
			mtEMiiRegWrite(phyaddr, 0x1f, 0x50e, 0x0000);
			mtEMiiRegWrite(phyaddr, 0x1f, 0x104, 0x2608);
...
{				
	...
	if((stricmp(argv[1], "on") == 0))
	{
	   	for(phyaddr = 0; phyaddr<=31; phyaddr ++)   // allen_20190709
   		{
...
int doPhyset7528_0R(int argc, char *argv[], void *p)   // allen_20190709	
{				
	uint16 	phyaddr = 0, port_num = 0;
	uint16 	reg0_temp = 0, dev1e_145_temp = 0;
		...
		for(phyaddr = 9; phyaddr<=12; phyaddr ++)
   		{
			...
			mtEMiiRegWrite(phyaddr, 0x1e, 0x003d, 0x0000);	
			mtEMiiRegWrite(phyaddr, 0x1e, 0x003e, 0xf000);
			//udelay(100);
			tcMiiStationWrite(9, 0x0,  0x0800);
			tcMiiStationWrite(10, 0x0,  0x0800);
			tcMiiStationWrite(11, 0x0,  0x0800);
			tcMiiStationWrite(12, 0x0,  0x0800);
	
			ePhyInit(phyaddr);
	
			doGePhyALLAnalogCal(phyaddr);
			if((GECal_status == 0)||(GECal_R50_status ==0))
				doGePhyALLAnalogCal(phyaddr);
			...
			tcMiiStationWrite(phyaddr, 0x0, 0x1200);
			//printf("  set 0R (3) \r\n");
			//tcPhyVerLookUp(phyaddr);
			//ePhyInit(phyaddr);
			tcMiiStationWrite(9, 0x0,  0x1200);
			tcMiiStationWrite(10, 0x0,  0x1200);
			tcMiiStationWrite(11, 0x0,  0x1200);
			tcMiiStationWrite(12, 0x0,  0x1200);
   		}
		...
		for(phyaddr = 9; phyaddr<=12; phyaddr ++)
   		{
			...
			doGePhyALLAnalogCal_R45(phyaddr);
			if(GECal_status == 0)
				doGePhyALLAnalogCal_R45(phyaddr);
...
int doPhysetPowerSaving(int argc, char *argv[], void *p)    // allen_20190709	
{
	...
	{
		mtEMiiRegWrite(9, 0x1f, 0x271, 0x7ca8); // allen_20190902
		mtEMiiRegWrite(9, 0x1f, 0x272, 0x14ff);
		mtEMiiRegWrite(9, 0x1f, 0x269, 0x111f);
	}
...
int doPhysetLED(int argc, char *argv[], void *p) 	 
{
	uint16  phyaddr=0, led_mode=2;
	uint16	dev_1f_21=0, dev_1f_24=0, dev_1f_25=0, dev_1f_26=0, dev_1f_27=0;
	
	led_mode = checked_atoi(argv[1]);

	dev_1f_21 = mtEMiiRegRead(12, 0x1f, 0x21);
	dev_1f_24 = mtEMiiRegRead(12, 0x1f, 0x24);
	dev_1f_25 = mtEMiiRegRead(12, 0x1f, 0x25);
	dev_1f_26 = mtEMiiRegRead(12, 0x1f, 0x26);
	dev_1f_27 = mtEMiiRegRead(12, 0x1f, 0x27);
	...
	{
		printf("Usage: tce setLED <0(led0 only)|1(led1 only)|2(default)> \r\n");
	}
...
void doGePhyALLAnalogCal(uint8 phyaddr) // for EN7528 GBE    // allen_20190709
{
	...
	uint8	tx_amp_temp, tx_amp_reg, cnt=0, tx_amp_reg_100, tst_offset=0, tst_offset_p9d=0, hbt_offset=0, gbe_offset=0, tbt_offset=0;
	...
	mtEMiiRegWrite(phyaddr, 0x1e, 0x00fb, 0x0100);	// [9:8] ldo from 00 to 01	
	...
	mtEMiiRegWrite( 9, 0x1e, 0x00db, 0x1100);	// 1e_db[12]:rg_cal_ckinv, [8]:rg_ana_calen, [4]:rg_rext_calen, [0]:rg_zcalen_a
	mtEMiiRegWrite( 9, 0x1e, 0x00dc, 0x0000);	// 1e_dc[0]:rg_txvos_calen
	mtEMiiRegWrite( 9, 0x1e, 0x00e1, 0x0000);	// 1e_e1[4]:select 1V
	
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		rg_zcal_ctrl = 0x20;  						// start with 0 dB
		dev1e_e0_ana_cal_r5 = (mtEMiiRegRead(9, 0x1e, 0x00e0) & (~0x003f));
		mtEMiiRegWrite(9, 0x1e, 0x00e0, (dev1e_e0_ana_cal_r5 | rg_zcal_ctrl));	// 1e_e0[5:0]:rg_zcal_ctrl
		...
		ad_cal_comp_out_init = (mtEMiiRegRead(9, 0x1e, 0x017a)>>8) & 0x1;		// 1e_17a[8]:ad_cal_comp_out	
		...
		while(all_ana_cal_status < ANACAL_ERROR)
		{
			...
			mtEMiiRegWrite(9, 0x1e, 0x00e0, (dev1e_e0_ana_cal_r5 | rg_zcal_ctrl));
			all_ana_cal_status = allGeAnaCalWait(20, phyaddr); // delay 20 usec
		...
		{	
			rg_zcal_ctrl = 0x20;  // 0 dB
			rg_zcal_ctrl = ZCAL_TO_R50ohm_TBL_100[rg_zcal_ctrl];
			GECal_R50_status = 0;
			break;
		}
		else
		{
			rg_zcal_ctrl = ZCAL_TO_R50ohm_TBL_100[rg_zcal_ctrl];	// wait Mog zcal/r50 mapping table
	...
	mtEMiiRegWrite( 9, 0x1e, 0x00db, 0x0000);
	// *** R50 Cal end ***
	...
	mtEMiiRegWrite( 9, 0x1e, 0x00db, 0x0100);	// 1e_db[12]:rg_cal_ckinv, [8]:rg_ana_calen, [4]:rg_rext_calen, [0]:rg_zcalen_a
	mtEMiiRegWrite( 9, 0x1e, 0x00dc, 0x0001);	// 1e_dc[0]:rg_txvos_calen
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		...
		cnt =0;
		tst_offset = 0x6;        // allen_20190709
		tst_offset_p9d = 0x3;    // allen_20190709
		hbt_offset = 0x4;        // allen_20190709
		gbe_offset = 0x5;        // allen_20190709 
		tbt_offset = 0x6;        // allen_20190709
		...
		while(all_ana_cal_status < ANACAL_ERROR)
		{
			...
			else if(((mtEMiiRegRead(9, 0x1e, 0x017a)>>8)&0x1) != ad_cal_comp_out_init) 
			{
				...
				if((phyaddr== 9) && (calibration_pair == ANACAL_PAIR_D))
						{
					mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|((tx_amp_temp+tst_offset_p9d)<<tx_amp_reg_shift)));
						}
		...
		else
		{
			...
			else
			{
				if((calibration_pair == ANACAL_PAIR_A))
				{
					if((phyaddr== 10))
					{
						mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|((tx_amp_temp-hbt_offset-1)<<tx_amp_reg_shift)));	// for 100
					}
					else
					{
			...
			reg_temp = mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg)& (~0x003f);
			if((all_ana_cal_status == ANACAL_SATURATION)||(tx_amp_temp==0))
				mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|(0x0010)));  // for tbt	
			else
				mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|(tx_amp_temp-tbt_offset)));  					// for tbt
		}
	}
	mtEMiiRegWrite(phyaddr, 0x1e, 0x017d, 0x0000);
	mtEMiiRegWrite(phyaddr, 0x1e, 0x017e, 0x0000);
...
// note : GbE calibration, Mog's register change to global(not per port), but Yiron's register keep local(per port)
//        for SA, keep local(per port)
// register:
//          ad_cal_comp_out		1e_17a[8]
//			da_calin_flag		1e_17c[0]
//			ad_cal_clk			1e_17b[0]
//			rg_zcal_ctrl[5:0]	1e_e0[5:0]
//			rg_rext_calen		1e_db[4]
//			rg_ana_calen		1e_db[8]
//			rg_cal_ckinv		1e_db[12]
//			rg_txvos_calen		1e_dc[0]
//			rg_rext_trim[5:0]	1e_e0[13:8]
//			rg_cal_refsel		1e_e1[4]
//			rg_cal_comp_pwd		1e_e1[0]
...
void doGePhyALLAnalogCal_R45(uint8 phyaddr) 
{
	...
	uint16	tx_amp_offset_tst_p12, tx_amp_offset_tst_p9, tx_amp_offset_tst_p10, tx_amp_offset_tst_p11, tx_amp_offset_gbe=0;
	...
	printf(" 7528 GbE eco_rev (0x%x) &0xff(0x%x)\r\n", eco_rev, (eco_rev&0xff));
	...
	mtEMiiRegWrite(phyaddr, 0x1e, 0x00dc, 0x0000);	// 1e_dc[0]:rg_txvos_calen
	mtEMiiRegWrite(phyaddr, 0x1e, 0x00e1, 0x0000);	// 1e_e1[4]:select 1V
	mtEMiiRegWrite( phyaddr_common, 0x1e, 0x00db, 0x1100);	// 1e_db[12]:rg_cal_ckinv, [8]:rg_ana_calen, [4]:rg_rext_calen, [0]:rg_zcalen_a
	mtEMiiRegWrite( phyaddr_common, 0x1e, 0x00dc, 0x0000);	// 1e_dc[0]:rg_txvos_calen
	mtEMiiRegWrite( phyaddr_common, 0x1e, 0x00e1, 0x0000);	// 1e_e1[4]:select 1V

	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		rg_zcal_ctrl = 0x20;  						// start with 0 dB
		dev1e_e0_ana_cal_r5 = (mtEMiiRegRead( phyaddr_common, 0x1e, 0x00e0) & (~0x003f));
		mtEMiiRegWrite( phyaddr_common, 0x1e, 0x00e0, (dev1e_e0_ana_cal_r5 | rg_zcal_ctrl));	// 1e_e0[5:0]:rg_zcal_ctrl
		...
		ad_cal_comp_out_init = (mtEMiiRegRead(phyaddr_common, 0x1e, 0x017a)>>8) & 0x1;		// 1e_17a[8]:ad_cal_comp_out	
		if(ad_cal_comp_out_init == 1)
		{
			calibration_polarity = -1;
		}
		...
		while(all_ana_cal_status < ANACAL_ERROR)
		{
			cnt ++;
			rg_zcal_ctrl += calibration_polarity;
			mtEMiiRegWrite( phyaddr_common, 0x1e, 0x00e0, (dev1e_e0_ana_cal_r5 | rg_zcal_ctrl));
			all_ana_cal_status = allGeAnaCalWait_R45(20, phyaddr); // delay 20 usec
			...
			else if(((mtEMiiRegRead( phyaddr_common, 0x1e, 0x017a)>>8)&0x1) != ad_cal_comp_out_init) 
			{
				all_ana_cal_status = ANACAL_FINISH;	
			}
			else
	...
	mtEMiiRegWrite(phyaddr, 0x1e, 0x00db, 0x0000);
	mtEMiiRegWrite( phyaddr_common, 0x1e, 0x00db, 0x0000);
	// *** R50 Cal end ***
	...
	mtEMiiRegWrite(phyaddr, 0x1e, 0x00db, 0x0100);	// 1e_db[12]:rg_cal_ckinv, [8]:rg_ana_calen, [4]:rg_rext_calen, [0]:rg_zcalen_a
	mtEMiiRegWrite(phyaddr, 0x1e, 0x00dc, 0x0001);	// 1e_dc[0]:rg_txvos_calen
	mtEMiiRegWrite( phyaddr_common, 0x1e, 0x00db, 0x0100);	// 1e_db[12]:rg_cal_ckinv, [8]:rg_ana_calen, [4]:rg_rext_calen, [0]:rg_zcalen_a
	mtEMiiRegWrite( phyaddr_common, 0x1e, 0x00dc, 0x0001);	// 1e_dc[0]:rg_txvos_calen
	mtEMiiRegWrite(phyaddr, 0x1e, 0x0096, 0x8000);	// 1e_96[15]:bypass_tx_offset_cal, Hw bypass, Fw cal
	mtEMiiRegWrite(phyaddr, 0x1e, 0x003e, 0xf808);	// 1e_3e
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		...
		while(all_ana_cal_status < ANACAL_ERROR)
		{
			...
			else if(((mtEMiiRegRead(phyaddr_common, 0x1e, 0x017a)>>8)&0x1) != ad_cal_comp_out_init) 
			{
				...
				tx_amp_offset_gbe = 14;
				
				if(calibration_pair == ANACAL_PAIR_A)
				{
					mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|((tx_amp_temp + tx_amp_offset_gbe)<<tx_amp_reg_shift)));
				}
				else //if(calibration_pair == ANACAL_PAIR_B)
				{
					mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|((tx_amp_temp + tx_amp_offset_gbe)<<tx_amp_reg_shift)));
		...
		else
		{
			...
			tx_amp_offset_tst_p9  = 14;
			tx_amp_offset_tst_p10 = 14;
			tx_amp_offset_tst_p11 = 14;
			tx_amp_offset_tst_p12 = 14;

			if((phyaddr == 9))
				tx_amp_temp_tst = tx_amp_temp + tx_amp_offset_tst_p9;
			else if(phyaddr == 10)
				tx_amp_temp_tst = tx_amp_temp + tx_amp_offset_tst_p10;
			else if(phyaddr == 11)
				tx_amp_temp_tst = tx_amp_temp + tx_amp_offset_tst_p11;	
			else
				tx_amp_temp_tst = tx_amp_temp + tx_amp_offset_tst_p12;
			...
			#if 0
			tx_amp_offset_hbt = 15;
			tx_amp_offset_hbt_p0_a = 11;
			tx_amp_offset_hbt_p0_b = 12;
			tx_amp_offset_hbt_p1_a = 18;
			tx_amp_offset_hbt_p1_b = 14;
			tx_amp_offset_hbt_p2_a = 15;
			tx_amp_offset_hbt_p2_b = 5;
			tx_amp_offset_hbt_p3_a = 14;
			tx_amp_offset_hbt_p3_b = 9;
			#endif
			
			tx_amp_offset_hbt = 2;
			tx_amp_offset_hbt_p0_a = 3;
			tx_amp_offset_hbt_p0_b = 3;
			tx_amp_offset_hbt_p1_a = 3;
			tx_amp_offset_hbt_p1_b = 3;
			tx_amp_offset_hbt_p2_a = 2;
			tx_amp_offset_hbt_p2_b = 2;
			tx_amp_offset_hbt_p3_a = 2;
			tx_amp_offset_hbt_p3_b = 2;
			
			
			if(((phyaddr == 9))&&(calibration_pair == ANACAL_PAIR_A))
	...
	while(all_ana_cal_status < ANACAL_ERROR)
	{
		...
		else
		{
			if((rg_zcal_ctrl == 0x3F)||(rg_zcal_ctrl == 0x00))	
			{
				...
				printf(" GE Rxet cal (%d)(%d)(%d)(0x%x)  \r\n", cnt, ad_cal_comp_out_init, ((mtEMiiRegRead(phyaddr_common, 0x1e, 0x017a)>>8)&0x1), mtEMiiRegRead(phyaddr, 0x1e, 0xe0));
		}
	}
...
uint8 allGeAnaCalWait(uint32 delay, uint8 phyaddr) 
{
	...
	mtEMiiRegWrite(phyaddr_common, 0x1e, 0x017c, 0x0001);	// da_calin_flag pull high
	
	udelay(delay);

	all_ana_cal_status = mtEMiiRegRead(phyaddr_common, 0x1e, 0x017b) & 0x1;
	mtEMiiRegWrite(phyaddr_common, 0x1e, 0x017c, 0x0000);	// da_calin_flag pull low
