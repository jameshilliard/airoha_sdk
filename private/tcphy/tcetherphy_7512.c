...
void 		FECal_TxAmp_R45(uint8 port_num, uint32 delay);		// for EN7522  // allen_20171012
...
mt7530_cfg_trdata_t mt7530_cfg_tr[MT7530_PHY_INIT_TR_SET_NUM]=
{
    {	//local data
     	{ 
			...
			{"PMA",0x0017,0x060671},  // allen_20190902
        	{"PMA",0x0018,0x0e2e00},  // allen_20190902
        	{"PMA",0x0000,0x00002b}   // allen_20190902
       		#if 0  //E2 setting
...
#define MT7512Ge_PHY_INIT_TR_LDATA_LEN 		4
    ...
    {	//globle data
     	{
			...
			{0x1f, 0x0269, 0x111f},		// allen_20190902 ps_dri_gbe
			{0x1f, 0x0271, 0x7ca8},		// allen_20190902 ps_op
     	...
     	//local data
     	{
			...
			{0x1e,0x0013,0x0000},	// allen_20190902 pair delay (afe)
...
int tcPhyInit(void)
{
	...
	else
	{
		...
		for(ephy_addr = 0; ephy_addr <= 12; ephy_addr++)
		{
    		...
    		if(tcPhyInitFlag == 1)
    		{
    			...
    			if(port_num >= 0)		
    			#endif
    			#if defined(TCSUPPORT_CPU_EN7521)	// allen_20170106, reduce port 0 for 7521
				if(port_num >= 1)
				#endif
    			{
    				if((tcPhyVer==tcPhyVer_7512Fe)||(tcPhyVer==tcPhyVer_7522Fe))
    				{
						FECal_flag = 0;
    					...
    					tcPhyWriteLReg(port_num, 0, 30, 0x0d08); // for Customer bad CAT.5, AN bypass pd wakeup // allen_20170425
    				}	
    			}
    		}	
    	}	
	...
	doTrgmiiPhyInit();    //YMC add_20191107
...
int doPhyForceMode(int argc, char *argv[], void *p) 	//biker_20130814, add command for QA auto test
{
	...
	else if(argc == 4)
	{
		...
		else if(stricmp(argv[2], "force") == 0)
		{
			...
			else if((stricmp(argv[3], "1000") == 0)&&((reg3==EPHY_ID_7512Ge)||(reg3==EPHY_ID_7522Ge)|| ((reg3 == EPHY_ID_7530Ge_7512Fe)&&(((r15_temp>>13)&0x1) == 1))))
			{
				//tcMiiStationWrite(phyaddr, 0x00, 0x40);
				printf("not support 1000 half  \r\n");
			}
...
{				
	uint16 	phyaddr = 0, port_num = 0;
	uint16 	reg0_temp = 0, dev1e_145_temp = 0, l0r26_temp = 0;
	...
	tcMiiStationWrite(12, 0x1f, 0x0000);		// g0 
	reg0_temp = tcMiiStationRead(12, 0x0);		// keep the default value
	tcMiiStationWrite(12, 0x0,  0x0140);		// set [12]AN disable, [8]full duplex, [13/6]1000Mbps

	mtEMiiRegWrite(12, 0x1f, 0x0100, 0xc000);			// BG voltage output
	//dev1e_145_temp = mtEMiiRegRead(12, 0x1e, 0x0145);
	//mtEMiiRegWrite(12, 0x1e, 0x0145, 0x1010);			// fix mdi
	mtEMiiRegWrite(12, 0x1e, 0x0185, 0x0000);			// disable tx slew control
	#endif
...
void doGePhyALLAnalogCal(uint8 phyaddr) // for EN7512 GBE 
{
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		...
		else
		{
			...
			printf("  GE R50 AnaCal Done! (%d) (0x%x)(0x%x) \r\n", cnt, rg_zcal_ctrl, (rg_zcal_ctrl|0x80));
                ...
                // allen_20180706 APN for optimum R50 setting
		if(calibration_pair == ANACAL_PAIR_A)  
		{
		...
		else // if(calibration_pair == ANACAL_PAIR_D)
		{
			...
			{
				printf(" GE-d zcal(%d) \r\n", rg_zcal_ctrl);
				mtEMiiRegWrite(phyaddr, 0x1e, 0x0175, (ad_cal_comp_out_init | ((((rg_zcal_ctrl-4)<<0)&0x00ff) | 0x0080)));		
			}
		}
		...
		else
		{
			...
			if(isEN7526c)
			{
				if((calibration_pair == ANACAL_PAIR_A)||(calibration_pair == ANACAL_PAIR_B))
				{
					...
					if(((VPint(0xbfbf8218)>>9)&0x01) == 1)
					{
						reg_temp = mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg);
						mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, ((reg_temp & 0xff00)|0x10));	// allen_20180706 for 10 (7526FCT, BGA) 
					}
				}		
			}
		}
	...
	mtEMiiRegWrite(phyaddr, 0x1e, 0x0145, dev1e_145_temp);
        mtEMiiRegWrite(phyaddr, 0x1e, 0x0145, 0x1000);		// reset to default
	// *** Rx offset Cal end ***

	tcMiiStationWrite(phyaddr, 0x0,  0x1200);
}

void doGePhyALLAnalogCal_R45(uint8 phyaddr) // for EN7512 GBE 
{
	uint8	rg_zcal_ctrl, all_ana_cal_status;
	uint16	ad_cal_comp_out_init;
	uint16	dev1e_e0_ana_cal_r5, reg0_temp, dev1e_145_temp; 
	...
	uint8	tx_offset_reg_shift, tx_amp_reg_shift; 
	uint16	tx_offset_reg, reg_temp;
	uint8	tx_amp_temp, tx_amp_reg, cnt=0, tx_amp_reg_100, tx_amp_tst_offset = 0;
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		...
		else
		{
			...
			// 1e_16/18/20/22 (hbt/tst)
			
			reg_temp = mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg_100)& (~0x003f);				// for tst
			if((all_ana_cal_status == ANACAL_SATURATION)||(tx_amp_temp==0))
				mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|(0x0010)));			
			else
				mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|(tx_amp_temp+14)));  // for tst

			reg_temp = mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg_100)&(~0xff00);							// for hbt
			if((all_ana_cal_status == ANACAL_SATURATION)||(tx_amp_temp==0))
				mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|((0x4)<<tx_amp_reg_shift)));	
			else
			{
				#if defined(TCSUPPORT_CPU_EN7521)
					mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|((tx_amp_temp+0x4)<<tx_amp_reg_shift)));	// for hbt, 0xc
				#else //if(calibration_pair == ANACAL_PAIR_B)
					mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|((tx_amp_temp+4)<<tx_amp_reg_shift)));
					reg_temp = mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg_100);
					mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp+3)); 
				#endif	
			}

			if(isEN7526c) // allen_20171012 // allen_20171228
			{
				tx_amp_tst_offset = 6;
				if((calibration_pair == ANACAL_PAIR_A)||(calibration_pair == ANACAL_PAIR_B))
				{
					reg_temp = mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg_100);
					mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp-0x7));  // for tst

					reg_temp = mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg_100)&(~0xff00);
					mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|((tx_amp_temp +0x4)<<tx_amp_reg_shift))); // hbt	// modify -0x2 to +0x4
					printf(" GE Tx amp hbt  (0x%x)(0x%x)\r\n",  tx_amp_temp, mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg_100));

					if(((VPint(0xbfbf8218)>>9)&0x01) == 1) // allen_20180810 (7526FCT, BGA)
					{
						reg_temp = mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg_100);
						if(calibration_pair == ANACAL_PAIR_A)
							mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp+0xe));  // for tst
						else
							mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp+0xc));  // for tst

						reg_temp = mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg_100)&(~0xff00);	// for hbt
						if(calibration_pair == ANACAL_PAIR_A)
							mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|((tx_amp_temp + 0x7)<<tx_amp_reg_shift)));	
						else
							mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp|((tx_amp_temp + 0x8)<<tx_amp_reg_shift)));
							
						reg_temp = mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg);
						mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, ((reg_temp & 0xff00)|0x10));	// allen_20180706 for 10 (7526FCT, BGA) 

						reg_temp = mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg)&(~0xff00);	// for gbe
						if(calibration_pair == ANACAL_PAIR_A)
							mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|((tx_amp_temp + 0x7)<<tx_amp_reg_shift)));	
						else
							mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|((tx_amp_temp + 0x8)<<tx_amp_reg_shift)));
					}
				...
				{
					reg_temp = mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg_100);
					mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp-0x4));  // for tst

					if(((VPint(0xbfbf8218)>>9)&0x01) == 1) // allen_20180810 (7526FCT, BGA)
					{
						reg_temp = mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg_100);
						mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp + 0x3));  // for tst
						
						reg_temp = mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg)&(~0xff00);	// for gbe
						mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg, (reg_temp|((tx_amp_temp + 0x3)<<tx_amp_reg_shift)));
					}
				}		

				reg_temp = mtEMiiRegRead(phyaddr, 0x1e, tx_amp_reg_100);
				mtEMiiRegWrite(phyaddr, 0x1e, tx_amp_reg_100, (reg_temp + tx_amp_tst_offset));  // for tst  // 20190704 fine tune for 7522 1000M tx peak : 725~775mV
...
void	GECal_R45(uint8 phyaddr, uint32 delay)		// for EN7512 // allen_20170524 
{
	...
	for(calibration_pair = ANACAL_PAIR_A; calibration_pair <= ANACAL_PAIR_D; calibration_pair ++)
	{
		...
		else // if(calibration_pair == ANACAL_PAIR_D)
		{
			...
			if(isEN7526c)
			{
				mtEMiiRegWrite(phyaddr, 0x1e, 0x0175, (ad_cal_comp_out_init | ((((rg_zcal_ctrl-4)<<0)&0x00ff) | 0x8000)));	// for CID:185980	
			}
		}
		//mtEMiiRegWrite(phyaddr, 0x1e, 0x00e0, ((rg_zcal_ctrl<<8)|rg_zcal_ctrl));
	}
...
{
	uint8	rg_zcal_ctrl, all_ana_cal_status, rg_zcal_ctrl_tx=0, rg_zcal_ctrl_rx=0;  // for CID:186326/186327
	uint8	ad_cal_comp_out_init, ad_cal_comp_out;
	uint16	l3r26_temp, l3r25_temp, l4r22_temp, g7r24_temp; 
	int 	calibration_polarity;
	uint8	cnt=0, phyaddr, zcal_offset;
	...
	if(isEN7526c) // allen_20171012
	{
		if(phyaddr == 9)
		{
			rg_zcal_ctrl_tx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl-6)]; 	
			rg_zcal_ctrl_rx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl-5)];	
		}
		if(phyaddr == 10)
		{
			rg_zcal_ctrl_tx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl-2)]; 	
			rg_zcal_ctrl_rx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl-2)];	
		}
		if(phyaddr == 11)
		{
			rg_zcal_ctrl_tx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl-4)]; 	
			rg_zcal_ctrl_rx = ZCAL_TO_R45ohm_TBL[(rg_zcal_ctrl-4)];	
		}
	}
	else
	{
		...
		tcPhyWriteLReg(port_num, 2, 23, ((l2r23_temp+1)<<tx_amp_reg_shift));   // allen_20170405
                        ...
                        if(isEN7526c)
			{
				...
				if(port_num == 1)
					...
					tcPhyWriteLReg(port_num, 2, 23, ((l2r23_temp-1)<<tx_amp_reg_shift));	// p11
			}
                        else
                        {
...
void	FECal_TxAmp_R45(uint8 port_num, uint32 delay)		// for EN7522  // allen_20171012
{
	...
	{	
        ...
        if(isEN7526c)
		{
			l2r23_temp = tcPhyReadLReg(port_num, 2, 23);
			if(port_num == 1)
				tcPhyWriteLReg(port_num, 2, 23, ((l2r23_temp+1)<<tx_amp_reg_shift));   	// p9
			else if(port_num == 2)
				tcPhyWriteLReg(port_num, 2, 23, ((l2r23_temp+1)<<tx_amp_reg_shift));	// p10
			else
				tcPhyWriteLReg(port_num, 2, 23, ((l2r23_temp)<<tx_amp_reg_shift));		// p11
        ...
        else
        {
			...
			#if defined(TCSUPPORT_CPU_EN7512)  // allen_20160608
			if(port_num == 0) 
				tcPhyWriteLReg(port_num, 2, 23, ((l2r23_temp+2)<<tx_amp_reg_shift));	// allen_20190530: p0+2
			else if(port_num == 1) 														// allen_20160603 : test for MSTC(7512 LAN2)
				tcPhyWriteLReg(port_num, 2, 23, ((l2r23_temp+7)<<tx_amp_reg_shift));	// allen_20190530: p1+5
			else if(port_num == 2)
				tcPhyWriteLReg(port_num, 2, 23, ((l2r23_temp+6)<<tx_amp_reg_shift));  	// allen_20190530: p2+5	// allen_20170405	// allen_20170502
			else
				tcPhyWriteLReg(port_num, 2, 23, ((l2r23_temp+2)<<tx_amp_reg_shift));	// allen_20190530: p3+1
			#endif
		}       
		FECal_flag = 1;
	}
