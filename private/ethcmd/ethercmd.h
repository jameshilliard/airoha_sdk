...
#define HELP_VLAN_WHITE_LIST_ACTIVE		"vlan white-list active <0|1>"
#define HELP_VLAN_WHITE_LIST_PORT_ACTIVE "vlan white-list port-active <port> <0|1>"
#define HELP_VLAN_WHITE_LIST_ADD		"vlan white-list add <port> <vid>"
#define HELP_VLAN_WHITE_LIST_DELETE		"vlan white-list delete <port> <vid>"
...
#define HELP_ARL_DIP_TBL_ADD	"arl diptbl-add <DIP> <DestPortMap> <status:0~3> <leaky_en:0|1> <eg_tag:0~7> <usr_pri:0~7>\n"

#define HELP_ARL_SIP_TBL_ADD	"arl siptbl-add <DIP> <SIP> <DestPortMap> <status:0~3>\n"

#define HELP_ARL_DIP_TBL_CLEAN_BY_DIP "arl diptblcbydip <DIP> <is_extend_gsw>"

#define HELP_ARL_SIP_TBL_CLEAN_BY_DIP "arl siptblcbysip <DIP> <is_extend_gsw>"

#define HELP_ARL_SIP_TBL_CLEAN_BY_SIP "arl siptblcbysip <SIP> <is_extend_gsw>"

#define HELP_ARL_SIP_TBL_CLEAN_BY_DIP_SIP "arl siptblcbydipsip <DIP> <SIP> <is_extend_gsw>"
...
#define HELP_ACL_VIP_SET	"arl vip <arp/ppoe/igmp/mld/dhcp> <1/0> \n"
...
#define HELP_DSTQ		 "dstq [mode][fc][ds_qos]\n" \
						 "mode <state>:   0: Disable\n" \
						 "                1: down stream to gdm\n" \
						 "                2: down stream to qdma\n"\
						 "                3: For TCT stress test\n"\
						 "                   wifi->lan: down stream to qdma\n"\
						 "                   wan->lan: down stream to gdm\n"\
						 "                4: For avalanche test: \n"\
						 "                   multicast: down stream to qdma\n"\
						 "                   unicast: down stream to gdm\n"\
						 "fc <state>:     0: Disable flow control \n" \
						 "                1: Enable flow control\n" \
						 "qos <state>: 0: SWITCH \n" \
						 "             1: QDMA_LAN (qos mode,set qdma_lan total min low threshold to make sure qos is working)\n"\
						 "             2: QDMA_LAN (buffer mode,change the qdma_lan low threshold to MAX value,qos will not work)\n"\
						 "pcie0_mode <state>:   0: default, link rate=10G-down stream to gdm\n" \
						 "                                  link rate<10G-down stream to qdma\n"\
						 "                      1: pcie0 serdes down stream to gdm\n" \
						 "                      2: pcie0 serdes down stream to qdma\n" \
						 "pcie1_mode <state>:   0: default, link rate=10G-down stream to gdm\n" \
						 "                                  link rate<10G-down stream to qdma\n"\
						 "                      1: pcie1 serdes down stream to gdm\n" \
						 "                      2: pcie1 serdes down stream to qdma\n" \
						 "usb_mode <state>:     0: default, link rate=10G-down stream to gdm\n" \
						 "                                  link rate<10G-down stream to qdma\n"\
						 "                      1: usb serdes down stream to gdm\n" \
						 "                      2: usb serdes down stream to qdma\n" \
						 "eth_mode <state>:     0: default, link rate=10G-down stream to gdm\n" \
						 "                                  link rate<10G-down stream to qdma\n"\
						 "                      1: eth serdes down stream to gdm\n" \
						 "                      2: eth serdes down stream to qdma\n" \

#define HELP_PORT_MATRIX "portmatrix <port_id0> <port_id1> <port_id2> <port_id3> <port_id4> <port_id5>\n"\
                         "port_id0~5 is group id for eth0.1~eth0.6\n"\
                         "if eth0.x is not exist, set -1\n"\
                         "if no need to group, set 0\n"
//#endif //SQA_VERIFY
