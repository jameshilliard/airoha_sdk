...
static int doBewan(int argc, char *argv[], void *p);
...
static int findNegoPortID(void);
...
static int doVipPktSet(int argc, char *argv[], void *p);
...
static int doMibPort(int argc, char *argv[], void *p);
//#endif //SQA_VERIFY

#ifdef TCSUPPORT_MT7530_SWITCH_API
static int doArlMACTblDispByPort(int argc, char *argv[], void *p);
static int doArlGetPortByMac(int argc, char *argv[], void *p);
static int doArlDIPTblCleanByDIP(int argc, char *argv[], void *p);
static int doArlSIPTblCleanBySIP(int argc, char *argv[], void *p);
static int doArlSIPTblCleanByDIP(int argc, char *argv[], void *p);
static int doArlSIPTblCleanByDIPSIP(int argc, char *argv[], void *p);
static int doArlMulIpTblSet(int argc, char *argv[], void *p);
...
static int doVlanWhiteList(int argc, char *argv[], void *p);
static int dovlanWhiteListSetActive(int argc, char *argv[], void *p);
static int dovlanWhiteListSetPortActive(int argc,char * argv[],void * p);
static int dovlanWhiteListAdd(int argc, char *argv[], void *p);
static int dovlanWhiteListDelete(int argc, char *argv[], void *p);

#endif

static int doDstq(int argc, char *argv[], void *p);
static int doLinkAggregation(int argc, char *argv[], void *p);
...
extern int lan_portmap[ETH_MAX_IF_NUM];

cmds_t Cmds[] = {
	{LAN_IF,		doEth,			0x12,	0,	NULL},
	{"bewan",		doBewan,  		0x12,   1,  "bewan devname"},
...
//	{"pmiiw",		doGSWPhyWrite,		0x12,	3,	"pmiiw <phy_addr> <phy_reg> <value>"},
	...
	{"portmatrix",		doPortMatrix,		0x12,	6,	HELP_PORT_MATRIX},
	{"lan2lan",     doLan2Lan,  0x12,   0,  "lan2lan port(0~5) will only through switch"},
	#endif
	{"dstq",		doDstq,		0x12,	2,	HELP_DSTQ},
	{"linkAGG",		doLinkAggregation,		0x12,	1,	"ethphxcmd linkAGG enable/disable/status"},
...
#ifdef TCSUPPORT_MT7530_SWITCH_API
	{"white-list",		doVlanWhiteList,			0x12,	0,	NULL},
#endif
	{NULL,			NULL,			0x10,	0,	NULL},
};
//#if defined(SQA_VERIFY)
...
#ifdef TCSUPPORT_MT7530_SWITCH_API
    {"mactbl-by-port", doArlMACTblDispByPort,  0x12,   1,  "arl mactbl-by-port port\n"},
    {"port-by-mac",     doArlGetPortByMac,  0x12,   1,  "arl port-by-mac mac\n"},   
    {"diptblcbydip",  doArlDIPTblCleanByDIP, 0x12,   2,  HELP_ARL_DIP_TBL_CLEAN_BY_DIP},
    {"siptblcbysip",  doArlSIPTblCleanBySIP, 0x12,   2,  HELP_ARL_SIP_TBL_CLEAN_BY_SIP},
    {"siptblcbydip",  doArlSIPTblCleanByDIP, 0x12,   2,  HELP_ARL_SIP_TBL_CLEAN_BY_DIP},
    {"siptblcbydipsip",  doArlSIPTblCleanByDIPSIP, 0x12,   3,  HELP_ARL_SIP_TBL_CLEAN_BY_DIP_SIP},
	{"muliptbl-set",    doArlMulIpTblSet,   0x12,   4,  "arl muliptbl-set <dip> <sip> <sw_mask> <is_extend_gsw>\n"},
#endif
	{"vip",		doVipPktSet,	0x12,	2,	HELP_ACL_VIP_SET},
	{NULL,			NULL,			0x10,	0,	NULL},
};
...
static const cmds_t mibCmds[] = {
    ...
    {"port",        doMibPort,      0x12,   1,  "get one port mib"},   
	{NULL,			NULL,			0x10,	0,	NULL},
};
//#endif //SQA_VERIFY
...
static int doAddwan(int argc, char *argv[], void *p){
	...
	strncpy(szBaseDev,ETHUP_WAN,sizeof(szBaseDev) - 1);
	strncpy(szVDev, argv[2], sizeof(szVDev) - 1);
	iVid = atoi(argv[1]);
	...
	if(((argc==4) && (strcmp(argv[3],"swap")))||
		...
		if(val==ERR_MEDIAY_TYPE){
			...
			if((strcmp(argv[2],ETHCMD_PORT)==0) && ((portid==0) ||
				...
				sleep(1); /*sleep(ETHUP_WAIT_PHY_READY);*/
			}
			else{
				#if defined(TCSUPPORT_2PORTS) || defined(TCSUPPORT_CT_2PWIFI)
				printf(HELP_MEDIA_TYPE, argv[0], vlanCap->max_port_no-1,vlanCap->max_port_no-1);
...
int get_hsgmii_itf_idx()
{
	...
	fp = fopen("/tmp/hsgmii_itf", "r");
	if (fp == NULL)
		return -1;

	memset(itf, 0, sizeof(itf));
	res = fread(itf, sizeof(char), sizeof(itf)-1, fp);
	fclose(fp);

	sscanf(itf,"eth0.%d", &idx);
	
	return idx;
}

int get_itf_start_idx()
{
	FILE *fp = NULL;
	char buf[32] = {0};
	int line = 0;
	...
	fp = fopen("/proc/tc3162/itf_start_idx", "r");
	if ( NULL == fp) 
	{
		return -1;
	}
...
#endif
unsigned int hsgmii_lan[8] = {0xff,0xff,0xff,0xff,0x00,0x00,0x00,0x00};
...
int get_hsgmii_lan_idex(void){
	FILE * file = NULL;
	char buf[64] = {0};
	char tmp[32] = {0};

	file = fopen ("/proc/tc3162/board_args_hsgmii","r");
	if(!file){
		printf("Failed to open /proc/tc3162/board_args_hsgmii\r\n");
		return -1;
	}
	fgets(buf, 64, file);
	sscanf(buf,"%02x %02x %02x %02x %02x %02x %02x %02x %s",
											&hsgmii_lan[0],
											&hsgmii_lan[1],
											&hsgmii_lan[2],
											&hsgmii_lan[3],
...
int check_hsgmii_lan(int idx)
{
	switch(hsgmii_lan[idx])
	{
		case HSGMII_USB:
		case HSGMII_PCIE0:
		case HSGMII_PCIE1:
...
void cmd_for_eth_portmap (int port){
	char command[64];
	snprintf(command,sizeof(command),"echo %d %s %s %s %s -1 -1 > /proc/tc3162/eth_portmap",
		port,
		port == 1 ? "-1" : "1",
		port == 2 ? "-1" : "2",
		port == 3 ? "-1" : "3",
...
static int doBewan(int argc, char *argv[], void *p){
	...
	char cmd[ETHUP_CMDSIZE];
	int retval=0;
	etherChannVlanCap_t*  vlanCap=getEtherVlanCap();
	struct swconfig *swcfg=NULL;
	swcfg=getSwcfg();
	const char *eth_perfix = "eth0.";
	const size_t perfix_length = strlen(eth_perfix);
	...
	if(strlen(argv[1])== perfix_length + 1 && strncmp(argv[1],eth_perfix,perfix_length) == 0){
		int port = argv[1][perfix_length] - '0';
		if(port >= 1 && port <= 4){
			sprintf(cmd, "ifconfig eth0.%d down",port);
            system(cmd);
			sprintf(cmd, "brctl delif br-lan eth0.%d",port);
            system(cmd);
			cmd_for_eth_portmap(port);
		/*Active the switch vlan function*/
			if((swic.vlan_active!=NULL) && (swic.vlan_pvid!=NULL)&&(swic.vlan_vid!=NULL)){
				swic.vlan_active(ACTIVED);
				
#if defined(TCSUPPORT_CPU_MT7520) /* TCSUPPORT_CPU_MT7520 */
				...
				if (-1 != wan_port)
				{
					...
					/* bind etherwan port to cpu port. wan port vid is wan port id +1 */
					swic.vlan_vid(1, ACTIVED, wan_port+1, (1 << wan_port), 0x0, 0, 0, 0);
				#else

					/* bind etherwan port to cpu port. wan port vid is wan port id +1 */
					swic.vlan_vid(1, ACTIVED, wan_port+1, (1 << wan_port), 0x0);
				#endif
	
					dis_flow_ctrl(swcfg);
				}
#endif // def TCSUPPORT_CPU_MT7520
			
			}
		}
		printf("error interface: it only support eth0.1 ~ eth0.4\n");
	}
	else{
		printf("input error: the devname must be eth0.1~eth0.4\n");
		retval=-1;
	}
	return retval;
}
...
static int doVlanpt(int argc, char *argv[], void *p){
	...
	if(!strcmp(argv[1], ETHCMD_ENABLE)){
		...
		for(i=START_VLAN_VID;i<(vlanCap->max_port_no+START_VLAN_VID);i++){
...
#if defined(TCSUPPORT_INIC_HOST) || defined(TCSUPPORT_INIC_CLIENT)
			max_port = 2;
#else if defined(TCSUPPORT_MULTI_SWITCH_EXT)
			max_port = ETH_MAX_IF_NUM;
#endif
			...
			if (-1 == port_id && !check_hsgmii_lan(i-1)){
#if defined(TCSUPPORT_HSGMII_LAN)
				if(i == get_hsgmii_itf_idx()){
					continue;
				}
#endif
                /*Let virtual vlan ethernet interface down*/
                sprintf(cmd, "ifconfig eth0.%d down", i+start_idx);
                system(cmd);
                /*Remove the virtual vlan ethernet interface from bridge interface*/
                sprintf(cmd, "brctl delif br0 eth0.%d", i+start_idx);
                system(cmd);

#ifndef TCSUPPORT_SEPERATED_ETH_ITF
                /*Delete 4 virtual vlan interface*/
                sprintf(cmd, "vconfig rem eth0.%d", i+start_idx);
                system(cmd);
#endif
                continue;
            }
			...
			sprintf(cmd, "/userfs/bin/vconfig add eth0 %d", i+start_idx);
			system(cmd);
#endif
			/*Let virtual vlan ethernet interface up*/
			sprintf(cmd, "ifconfig eth0.%d 0.0.0.0 up", i+start_idx);
			system(cmd);
			/*Binding virtual vlan ethernet interface into br0 interface*/
			sprintf(cmd, "brctl addif br0 eth0.%d", i+start_idx);
			system(cmd);
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_ETH4_WAN_PORT)
			if(i== 4)
				sprintf(cmd,"echo up > %s",ETH04_STATUS);
				system(cmd);
    ...
    // there is no ether wan port, and no need to set VLAN ID
		...
		for(i=START_VLAN_VID;i<(vlanCap->max_port_no + START_VLAN_VID);i++){
#if defined(TCSUPPORT_MULTI_SWITCH_EXT)/* extend to 8 eth dev */
			if(i>8)
				continue;
#else		
#if defined(TCSUPPORT_CPU_MT7520)
		...
		printf("\nVport Number is:%d\n", vlanCap->max_port_no);
		printf("\nVitf Number is:%d", ETH_MAX_IF_NUM);
	}
	else{		
		retval=-1;
	}
...
static int doLanchip(int argc, char *argv[], void *p){
	...
	if(argc==2){/*LAN Chip 4ports-switch Enable/Disable*/
		if(vlanCap->max_port_no == SWIC_PORTS_NUM){
			...
			if(setLanchip(argv[1], singlePID))
            {
                goto err_exit;
            }
			i = 1;
...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_ETH4_WAN_PORT)
				if(portid == 3){
					if(((val & BMSR_LSTATUS) ?1:0) == 1){
						sprintf(cmd,"echo 1 > %s",PORT4_LINK_STATUS);
						system(cmd);
					}
					else{
						sprintf(cmd,"echo 0 > %s",PORT4_LINK_STATUS);
						system(cmd);
					}
				}
#endif/*TCSUPPORT_COMPILE*/
...
static int doQosSetPrioQmap(int argc, char *argv[], void *p){
	...
	if ((swcfg->swic_id == MT7530) || (swcfg->swic_id == RT62806)) {
		if ((prio>7) || (queue>7)) {	/* MT7530 total 7 port */
...
static int doQosSetPrioTagmap(int argc, char *argv[], void *p){
	...
	if ((swcfg->swic_id == MT7530) || (swcfg->swic_id == RT62806)) {
		if ((prio>7) || (tag>7)) {	/* MT7530 total 7 port */
			printf(HELP_QOS_PRIO_QMAP);
			return -1;
		}
...
static int doQosSetPortPrio(int argc, char *argv[], void *p){
	...
	if ((swcfg->swic_id == MT7530) || (swcfg->swic_id == RT62806)) {
		if ((port>=7) || (prio>7)) {	/* MT7530 total 7 port */
...
static int doQosSetDSCPPrio(int argc, char *argv[], void *p){
	...
	if ((swcfg->swic_id == MT7530) || (swcfg->swic_id == RT62806)) {
		if ((dscp>63) || (prio>7)) {	/* MT7530 total 7 port */
...
#ifdef TCSUPPORT_MT7530_SWITCH_API
/*___________________________________________________________________
**      function name: doArlMACTblDispByPort
**      descriptions:
**         Display Entries in MAC Address Table by Port.
**
**      parameters:
**      call:
**           subcmd
...
static int doArlMACTblDispByPort(int argc, char *argv[], void *p){
    ...
    mt7530_switch_brtable *out_pBrtable = NULL;
    
	out_pBrtable = (mt7530_switch_brtable *)malloc(sizeof(mt7530_switch_brtable));
	if ( !out_pBrtable ){
		return -ENOMEM;
	}
    ret = macMT7530GetBRTable(port, out_pBrtable);

    if (0 == ret)
    {
        printf("port-no\tmac-addr\t\tageing-timer\tvid\n");
        for (i = 0; i < out_pBrtable->count; i++)
        {
            printf("%3i\t%.2x:%.2x:%.2x:%.2x:%.2x:%.2x\t%d",
                  out_pBrtable->macinfo[i].port, out_pBrtable->macinfo[i].mac[0], out_pBrtable->macinfo[i].mac[1], 
                  out_pBrtable->macinfo[i].mac[2], out_pBrtable->macinfo[i].mac[3],
                  out_pBrtable->macinfo[i].mac[4], out_pBrtable->macinfo[i].mac[5],
                  out_pBrtable->macinfo[i].timer);
            if(out_pBrtable->macinfo[i].vid != 0xFFFF)
            {
                printf("\t\t%d\n", out_pBrtable->macinfo[i].vid);
            }
            else
            {
                printf("\t\tuntag\n");
...
}/*end doArlMACTblDispByPort*/


/*___________________________________________________________________
**      function name: doArlGetPortByMac
**      descriptions:
**         Get Port by mac.
**      call:
**           subcmd
...
static int doArlGetPortByMac(int argc, char *argv[], void *p){ 
	u32 i, j;
	char temp_str[3] = {0,0,0};
    mt7530_switch_macinfo inptr_PortMac = {0};
	
    for (i = 0, j = 0; i < 6; i ++){
        temp_str[0] = ((const char*)(argv[1]))[j++];
        temp_str[1] = ((const char*)(argv[1]))[j++];
        inptr_PortMac.mac[i] = strtoul(temp_str, (char **)NULL, 16);
    }
    
    inptr_PortMac.port = 255;
    
    macMT7530GetPortByMac(&inptr_PortMac);

    if(inptr_PortMac.port != 255)
        printf("mac %02x:%02x:%02x:%02x:%02x:%02x is found, port is %d.\n", 
        inptr_PortMac.mac[0],inptr_PortMac.mac[1],inptr_PortMac.mac[2],
        inptr_PortMac.mac[3],inptr_PortMac.mac[4],inptr_PortMac.mac[5],
        inptr_PortMac.port);
    else
        printf("mac %02x:%02x:%02x:%02x:%02x:%02x cannot be found!\n", 
        inptr_PortMac.mac[0],inptr_PortMac.mac[1],inptr_PortMac.mac[2],
        inptr_PortMac.mac[3],inptr_PortMac.mac[4],inptr_PortMac.mac[5]);
    
	return 0;    
    
}/*end doArlGetPortByMac*/
#endif
...
static int doArlDIPTblDisp(int argc, char *argv[], void *p){
	...
	if ((swcfg->swic_id == MT7530) && (argc >= 2)){	// optional argv[1] : ext_switch
		if (atoi(argv[1]) == 255){
			doArlAddrTblDisp2(0, 1);	// 2'b01 : DIP Address Entry
			doArlAddrTblDisp2(1, 1);	// 2'b01 : DIP Address Entry
		}
		else{
			doArlAddrTblDisp2(atoi(argv[1]), 1);	// 2'b01 : DIP Address Entry
		}
	}
	else{
...
static int doArlDIPTblClr(int argc, char *argv[], void *p){
	...
	if ((swcfg->swic_id == MT7530) && (argc >= 2)){	// optional argv[1] : ext_switch
		if (atoi(argv[1]) == 255){
			doArlAddrTblClr2(0, 1);		// 2'b01 : DIP Address Entry
			doArlAddrTblClr2(1, 1);		// 2'b01 : DIP Address Entry
		}
		else{
			doArlAddrTblClr2(atoi(argv[1]), 1);	// 2'b01 : DIP Address Entry
		}
	}
	else{
...
static int doArlSIPTblDisp(int argc, char *argv[], void *p){
	...
	if ((swcfg->swic_id == MT7530) && (argc >= 2)){	// optional argv[1] : ext_switch
		if (atoi(argv[1]) == 255){
			doArlAddrTblDisp2(0, 2);	// 2'b10 : SIP Address Entry
			doArlAddrTblDisp2(1, 2);	// 2'b10 : SIP Address Entry
		}
		else{
			doArlAddrTblDisp2(atoi(argv[1]), 2);	// 2'b10 : SIP Address Entry
		}
	}
	else{
...
static int doArlSIPTblClr(int argc, char *argv[], void *p){
	...
	if ((swcfg->swic_id == MT7530) && (argc >= 2)){	// optional argv[1] : ext_switch
		if (atoi(argv[1]) == 255){
			doArlAddrTblClr2(0, 2);		// 2'b10 : SIP Address Entry
			doArlAddrTblClr2(1, 2);		// 2'b10 : SIP Address Entry
		}
		else{
			doArlAddrTblClr2(atoi(argv[1]), 2);	// 2'b10 : SIP Address Entry
		}
	}
	else{
...
static int doArlDIPTblCleanByDIP(int argc, char *argv[], void *p){
    ...
    is_extend_gsw = atoi(argv[2]);
    
    macMT7530CleanSpecifiedArlDipTblByDip(is_extend_gsw, dip);
...
static int doArlSIPTblCleanBySIP(int argc, char *argv[], void *p){
    ...
    macMT7530CleanMultipleArlSipTblBySip(is_extend_gsw, sip);
...
static int doArlSIPTblCleanByDIP(int argc, char *argv[], void *p){
    ...
    macMT7530CleanMultipleArlSipTblByDip(is_extend_gsw, dip);
...
static int doArlSIPTblCleanByDIPSIP(int argc, char *argv[], void *p){
    ...
    is_extend_gsw = atoi(argv[3]);

    macMT7530CleanSpecifiedArlSipTblByDipSip(is_extend_gsw, dip, sip);
...
static int doArlMulIpTblSet(int argc, char *argv[], void *p){
    mt7530_switch_arl_mul_iptbladd_t mul_iptbl_info;
    ...
    mul_iptbl_info.dip = dip;
    mul_iptbl_info.sip = sip;
    mul_iptbl_info.sw_mask = atoi(argv[3]);
    mul_iptbl_info.is_extend_gsw = atoi(argv[4]);
    
    macMT7530SetMulArlIpTblAdd(&mul_iptbl_info);
...
#endif
/*___________________________________________________________________
**      function name: doVipPktSet
**      descriptions:
**         set switch manage frame packet.
**
**      parameters:
**         argc: The arguments number.
**         p:function pointer
...
{
#ifdef TCSUPPORT_MT7530_SWITCH_API
	mt7530_switch_vip_info vip_info;
	...
	if(!strcmp(argv[1], "arp"))
		vip_info.type = ARP;	
	else if(!strcmp(argv[1], "ppoe"))
		vip_info.type = PPOE;
	else if(!strcmp(argv[1], "igmp"))
		vip_info.type = IGMP;
	else if(!strcmp(argv[1], "mld"))
		vip_info.type = MLD;
	else if(!strcmp(argv[1], "dhcp"))
		vip_info.type = DHCP;
	else 
		vip_info.type = DEFAULT;

	vip_info.isvip = atoi(argv[2]);	
	macMT7530SetVipPkt(&vip_info);
#endif 
	return 0;
}/*end doVipPktSet*/
...
static int doVlanSetPortAttr(int argc, char *argv[], void *p){
	...
	/*Check the input parameters is right or not.*/
	if((port>vlanCap->max_port_no) || (attr>3)){
...
//	portmap=atoi(argv[3]);
	...
	/*Check the input parameters is right or not.*/
	if((port>vlanCap->max_port_no) || (mode>3)){
...
*/
		val = ((rate/32)<<16) + (1<<15) + (7<<8) + (1<<7) + 0xf; /* modify rate measurement period to 7:1ms, change ticker source from sw_2x_clk to sw_ticker_gen */
...
static int doMibPort(int argc, char *argv[], void *p){
	...
	unsigned char port = atoi(argv[1]);
    macMT7530GetPortCnt(port, &port_cnt);
    printf("port %d, Tx %d: Rx %d \n", port, port_cnt.TxPktsCnt, port_cnt.RxPktsCnt);
    return 0;
}
...
static int doLinkAggregation(int argc, char *argv[], void *p)
{
    ...
    if(gsw_link_aggregation_write(swcfg->skfd, swcfg->ifr, argc, argv )<0)
    {
		printf("Usage:ethphxcmd linkAGG enable/disable/status\n");
	}
...
static int doPortMatrix(int argc, char *argv[], void *p)
{
	...
	ret_value = macMT7530SetPortMatrix(port_group, 0);
	return 0;
}

static int doLan2Lan(int argc, char *argv[], void *p)
{
	macMT7530SetLan2Lanswitch();
	return 0;
}
...
static const cmds_t vlanWhiteListCmds[] = {
	{"active",		dovlanWhiteListSetActive,	0x12,	1,	HELP_VLAN_WHITE_LIST_ACTIVE},
    {"port-active", dovlanWhiteListSetPortActive,   0x12,   2,  HELP_VLAN_WHITE_LIST_PORT_ACTIVE},
	{"add",			dovlanWhiteListAdd,		0x12,	2,	HELP_VLAN_WHITE_LIST_ADD},
	{"del",			dovlanWhiteListDelete,		0x12,	2,	HELP_VLAN_WHITE_LIST_DELETE},
	{NULL,			NULL,			0x10,	0,	NULL},
};


static int doVlanWhiteList(int argc, char *argv[], void *p){
    return subcmd(vlanWhiteListCmds, argc, argv, p);
}
...
static int dovlanWhiteListSetActive(int argc, char *argv[], void *p)
{
    u8 mode=0;
    mode=atoi(argv[1]);
    macMT7530APIctagWhiteListMode(mode);
    return 0;
...
static int dovlanWhiteListSetPortActive(int argc, char *argv[], void *p){
    ...
    macMT7530APIctagWhiteListPerPortMode(port,mode);
	return 0;
}
static int dovlanWhiteListAdd(int argc, char *argv[], void *p)
{
    ...
    port=atoi(argv[1]);
    vid=atoi(argv[2]);
    macMT7530APIctagWhiteListAdd(port,vid);
    return 0;
}
static int dovlanWhiteListDelete(int argc, char *argv[], void *p)
{
    ...
    port=atoi(argv[1]);
    vid=atoi(argv[2]);
    macMT7530APIctagWhiteListDel(port,vid);
    return 0;
}
