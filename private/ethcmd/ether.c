...
macTC2206VlanGetVtbl(u16 index){	// MT7530 need 16bit index (4K)
	...
	fprintf(stderr,"%02d.%s vid:%02d fid:%02d portMap:0x%02x tagMap:0x%02x\r\n" \
...
#if defined(SQA_VERIFY)
	fprintf(stderr,"%02d.%s vid:%02d fid:%02d portMap:0x%02x tagMap:0x%04x stag:0x%02x ivl_en:0x%02x\r\n",
...
macRT7530VlanActive(u8 type)
{
	...
	else
	{
        ...
        {
            reg = 0x200c + i * 0x100;
            gsw_reg_write2(swcfg->skfd, swcfg->ifr, ext_switch, reg, 0xfff00);
        }	
		if (ext_switch == 1)
		{
			for (i = 0; i <= 6; i++)
...
macMT7530VlanCap(void){
	...
	etherVlanCap.max_port_no=11;
#elif defined(SQA_VERIFY)
...
macEN7512VlanActive(u8 type)
{
  ...
  //  use_extern_switch = detect_switch(swcfg);
	...
	if(type==ACTIVED)
	{
		...
		for (i = 0; i <= 5; i++)
		{
			reg = 0x2010 + i*0x100;
			    ...
			    gsw_reg_write2(swcfg->skfd, swcfg->ifr, ext_switch, reg, 0x810081C0); // EG_TAG consistent, transparent port for insert mode
			#else
			    gsw_reg_write2(swcfg->skfd, swcfg->ifr, ext_switch, reg, 0x81008100); // EG_TAG consistent, user port for replace mode
			#endif
		}
		reg = 0x2610;
                value = 0x81008120;
                if(use_soc_lanport & ext_switch){   // only ext switch support soc lan/wan port
                    value |= 0x800;// PT option for soc lan or wan port 
                }
                gsw_reg_write2(swcfg->skfd, swcfg->ifr, ext_switch, reg, value);// user port & enable stag		
		for (i = 0; i <= 6; i++) 
		{	// set vlan group
			reg = 0x2014 + i*0x100;
			value = 0x10000 + (i+1);
		...
		if (ext_switch == 1)
		{
            ...
            if(use_soc_lanport){
                reg = 0x2510;
                gsw_reg_write2(swcfg->skfd, swcfg->ifr, 0, reg, 0x81008120);//port5 user port & enable stag & consistent mode

                reg = 0x2010;
                gsw_reg_write2(swcfg->skfd, swcfg->ifr, 0, reg, 0x81008100);//port0 user port
                
                reg = 0x2610;
                gsw_reg_write2(swcfg->skfd, swcfg->ifr, 0, reg, 0x81008120);//port6 user port & enable stag & consistent mode

                reg = 0x2604;
                gsw_reg_write2(swcfg->skfd, swcfg->ifr, 0, reg, 0xff0400);//port6 ACL_EN
                
                reg = 0x2504;
                gsw_reg_write2(swcfg->skfd, swcfg->ifr, 0, reg, 0x400000);//port5 force to cpu port6
                
                reg = 0x2004;
                gsw_reg_write2(swcfg->skfd, swcfg->ifr, 0, reg, 0x400000);//port0 force to cpu port6
            }
			for (i = 0; i <= 6; i++)
			{
				//disable mac learning
        ...
        if (ext_switch == 1)
        {
            for (i = 0; i <= 4; i++)
            {
                //disable mac learning
                reg = 0x200c + i*0x100;
                gsw_reg_write2(swcfg->skfd, swcfg->ifr, 1, reg, 0xfff10);
            }
        }
#endif
	...
	else
	{
		...
		gsw_reg_write2(swcfg->skfd, swcfg->ifr, ext_switch, 0x2604, 0xff0003);
#endif
		/*for (i = 0; i <= 5; i++)
		{
			reg = 0x2010 + i*0x100;
