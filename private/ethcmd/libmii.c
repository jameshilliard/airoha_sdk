...
extern int use_soc_lanport;

static struct swconfig swcfg;
swic_api_t swic;
int lan_portmap[ETH_MAX_IF_NUM];
...
gsw_reg_read2(int skfd, struct ifreq ifr, u32 ext_switch, u32 offset)
    {
        ...
        else if (ext_switch == 2)
        {
            if (ioctl(skfd, RAETH_GSWEXT2_REG_READ, &ifr) < 0) {
                fprintf(stderr, "RAETH_GSWEXT2_REG_READ on %s failed: %s\n", ifr.ifr_name, strerror(errno));
                return (-1);
            }
        }
        else if (ext_switch == 3)
        {
            if (ioctl(skfd, RAETH_GSWEXT3_REG_READ, &ifr) < 0) {
                fprintf(stderr, "RAETH_GSWEXT3_REG_READ on %s failed: %s\n", ifr.ifr_name, strerror(errno));
                return (-1);
            }
        }
        else
...
gsw_dstq_write(int skfd, struct ifreq ifr, int argc, char *argv[])
{
	...
	if(!strcmp(argv[DSTQ_ARG_TYPE],"mode")){
		...
		if( (argv[DSTQ_ARG_VAL][0]!='0') && (argv[DSTQ_ARG_VAL][0]!='1') && \
            (argv[DSTQ_ARG_VAL][0]!='2') && (argv[DSTQ_ARG_VAL][0]!='3') && \
            (argv[DSTQ_ARG_VAL][0]!='4')){
			return -1;
		}else{
			/* [val] 0: disable, 1: to gdm, 2: to qdma, 3: for tct stress test, wifi->lan to qdma, wan->lan to gdm */
	...
	}else if(!strcmp(argv[DSTQ_ARG_TYPE],"qos")){
		reg.off = 2;
		if( (argv[DSTQ_ARG_VAL][0]!='0') && (argv[DSTQ_ARG_VAL][0]!='1') && \
			(argv[DSTQ_ARG_VAL][0]!='2')){	
			return -1;
		}else{
			/* [val] 0: switch, 1: qdma_lan qos mode, 2: qdma_lan buffer mode*/
			reg.val = argv[DSTQ_ARG_VAL][0]-'0';
		}
	}else if(!strncmp(argv[DSTQ_ARG_TYPE], "pcie0", 5)){
		/* [off] 3: serdes pcie0 mode */
		reg.off = 3;	
		if( (argv[DSTQ_ARG_VAL][0]!='0') && (argv[DSTQ_ARG_VAL][0]!='1') && \
			(argv[DSTQ_ARG_VAL][0]!='2')){
			return -1;
		}else{
			/* [val] 0: default, 1: to gdm, 2: to qdma*/
			reg.val = argv[DSTQ_ARG_VAL][0]-'0';	
		}
	}else if(!strncmp(argv[DSTQ_ARG_TYPE], "pcie1", 5)){
		/* [off] 4: serdes pcie1 mode */
		reg.off = 4;	
		if( (argv[DSTQ_ARG_VAL][0]!='0') && (argv[DSTQ_ARG_VAL][0]!='1') && \
			(argv[DSTQ_ARG_VAL][0]!='2')){
			return -1;
	...
	}else if(!strncmp(argv[DSTQ_ARG_TYPE], "usb", 3)){
		/* [off] 5: serdes usb mode */
		reg.off = 5;	
		if( (argv[DSTQ_ARG_VAL][0]!='0') && (argv[DSTQ_ARG_VAL][0]!='1') && \
			(argv[DSTQ_ARG_VAL][0]!='2')){
			return -1;
	...
	}else if(!strncmp(argv[DSTQ_ARG_TYPE], "eth", 3)){
		/* [off] 6: serdes ethernet mode */
		reg.off = 6;	
		if( (argv[DSTQ_ARG_VAL][0]!='0') && (argv[DSTQ_ARG_VAL][0]!='1') && \
			(argv[DSTQ_ARG_VAL][0]!='2')){
			return -1;
...
int gsw_link_aggregation_write(int skfd, struct ifreq ifr, int argc, char *argv[])
{
	gsw_reg reg;
	ifr.ifr_data = (char *)(&reg);
	...
	if (ioctl(skfd, RAETH_LINK_AGGREGATION, &ifr) < 0) {
		fprintf(stderr, "RAETH_LINK_AGGREGATION on %s failed: %s\n", ifr.ifr_name, strerror(errno));
		return 0;
	}
...
gsw_reg_write2(int skfd, struct ifreq ifr, u32 ext_switch, u32 offset, u32 value)
{
	...
	{
		if (ioctl(skfd, RAETH_GSWEXT2_REG_WRITE, &ifr) < 0) {
			fprintf(stderr, "RAETH_GSWEXT2_REG_WRITE on %s failed: %s\n", ifr.ifr_name, strerror(errno));
			return;
		}
	}
	else
...
void eth_port_init(void){
	char buf[32];
	FILE *fp = NULL;
	int i, index, port, wan_port;
    int lan[ETH_MAX_IF_NUM];
	...
	if(fp != NULL){
		...
		while ( NULL != fgets (buf, sizeof(buf), fp) )
		{
			if ( 0 == strcmp(buf, "lan_port_map\n") )
			{
				for ( i=0; i<ETH_MAX_IF_NUM; i++ )
				{
					...
					{
						if ( 2 == sscanf(buf,"%d %d", &index, &port) )
						{
							if ( 0<=index && index< ETH_MAX_IF_NUM )
							{
		...
		sscanf(TCSUPPORT_ETHERNET_PORTMAP, "%d_%d_%d_%d_%d_%d_%d", &wan_port, 
									&lan[0], &lan[1], &lan[2], &lan[3], &lan[4], &lan[5]);
#endif						
		for (port=0; port<ETH_MAX_IF_NUM; port++){	
	        if(lan[port]/10){
	            use_soc_lanport = 1;
	        }
	}
